...
class AdminManager(val config: KafkaConfig,
  ...
  def createTopics(timeout: Int,
    ...
    val metadata = createInfo.map {
      case (topic, arguments) =>
        try {
          val configs = new Properties()
          arguments.configs.asScala.foreach {
            case (key, value) =>
          ...
          val assignments = {
            if ((arguments.numPartitions != NO_NUM_PARTITIONS || arguments.replicationFactor != NO_REPLICATION_FACTOR)
              ...
              arguments.replicasAssignments.asScala.map {
          ...
          createTopicPolicy match {
            case Some(policy) =>
              ...
              val replicaAssignments =
                if (arguments.replicasAssignments.isEmpty) null else arguments.replicasAssignments

              policy.validate(
                new RequestMetadata(topic, numPartitions, replicationFactor, replicaAssignments, arguments.configs)
              )
   ...
   */
  def deleteTopics(timeout: Int, topics: Set[String], responseCallback: Map[String, Errors] => Unit) {
  ...
  def createPartitions(timeout: Int,
    ...
    val metadata = newPartitions.map {
      case (topic, newPartition) =>
        try {
          ...
          val reassignment = Option(newPartition.assignments).map(_.asScala.map(_.asScala.map(_.toInt))).map {
            assignments =>
  ...
  def describeConfigs(resourceToConfigNames: Map[ConfigResource, Option[Set[String]]],
                      includeSynonyms: Boolean): Map[ConfigResource, DescribeConfigsResponse.Config] =
    resourceToConfigNames.map {
      case (resource, configNames) =>
        def allConfigs(config: AbstractConfig) =
          config.originals.asScala.filter(_._2 != null) ++ config.values.asScala
        def createResponseConfig(
          configs: Map[String, Any],
          createConfigEntry: (String, Any) => DescribeConfigsResponse.ConfigEntry
        ): DescribeConfigsResponse.Config = {
          val filteredConfigPairs = configs.filter {
            case (configName, _) =>
        ...
        try {
          val resourceConfig = resource.`type` match {
            ...
            case ConfigResource.Type.TOPIC =>
              ...
              if (metadataCache.contains(topic)) {
                ...
                new DescribeConfigsResponse.Config(new ApiError(Errors.UNKNOWN_TOPIC_OR_PARTITION, null),
            ...
            case ConfigResource.Type.BROKER =>
              if (resource.name == null || resource.name.isEmpty)
                ...
                throw new InvalidRequestException(
                  s"Unexpected broker id, expected ${config.brokerId} or empty string, but received $resource.name"
                )
          ...
          case e: Throwable =>
            ...
            if (e.isInstanceOf[ApiException])
              ...
              error(message, e)
            resource -> new DescribeConfigsResponse.Config(ApiError.fromThrowable(e),
                                                           Collections.emptyList[DescribeConfigsResponse.ConfigEntry])
        }
    }.toMap
      ...
      case (resource, config) =>
        def validateConfigPolicy(resourceType: ConfigResource.Type): Unit =
          alterConfigPolicy match {
            case Some(policy) =>
              ...
              policy.validate(
                new AlterConfigPolicy.RequestMetadata(new ConfigResource(resourceType, resource.name),
        ...
        try {
          resource.`type` match {
              ...
              val brokerId =
                if (resource.name == null || resource.name.isEmpty)
                  ...
                  if (id != this.config.brokerId)
                    throw new InvalidRequestException(
                      s"Unexpected broker id, expected ${this.config.brokerId}, but received $resource.name"
                    )
                  Some(id)
                }
            ...
            case resourceType =>
              throw new InvalidRequestException(
                s"AlterConfigs is only supported for topics and brokers, but resource type is $resourceType"
              )
          }
        } catch {
  ...
  private def configSynonyms(name: String,
    ...
    synonyms.foreach(
      maybeAddSynonym(dynamicConfig.currentDynamicDefaultConfigs, ConfigSource.DYNAMIC_DEFAULT_BROKER_CONFIG)
    )
  ...
  private def createTopicConfigEntry(logConfig: LogConfig, topicProps: Properties, includeSynonyms: Boolean)(
    ...
    val allSynonyms = {
      val list = LogConfig.TopicConfigSynonyms
        .get(name)
  ...
  private def createBrokerConfigEntry(
