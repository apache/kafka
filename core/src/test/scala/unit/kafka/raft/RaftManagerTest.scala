...
import org.apache.kafka.server.config.{KRaftConfigs, KafkaServerConfigs, ReplicationConfigs, ServerLogConfigs, ZkConfigs}
...
class RaftManagerTest {
  private def createZkBrokerConfig(
    ...
    props.setProperty(KafkaServerConfigs.BROKER_ID_CONFIG, nodeId.toString)
