...
class DynamicConnectionQuotaTest extends BaseRequestTest {
  ...
  def testDynamicListenerConnectionQuota(): Unit = {
    ...
    props.put(KafkaConfig.LISTENER_SECURITY_PROTOCOL_MAP_PROP, "PLAINTEXT:PLAINTEXT,INTERNAL:PLAINTEXT")
  ...
  def testDynamicListenerConnectionCreationRateQuota(): Unit = {
    ...
    val props = new Properties
    props.put(KafkaConfig.LISTENERS_PROP, newListeners)
    props.put(KafkaConfig.LISTENER_SECURITY_PROTOCOL_MAP_PROP, "PLAINTEXT:PLAINTEXT,EXTERNAL:PLAINTEXT")
    reconfigureServers(props, perBrokerConfig = true, (KafkaConfig.LISTENERS_PROP, newListeners))
