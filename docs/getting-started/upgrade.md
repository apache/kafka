...
### Notable changes in 4.3.0
  ...
  * Kafka Streams now supports opt-in headers-aware state stores for DSL operators via the new `dsl.store.format` config (accepts `DEFAULT` or `HEADERS`) and the `DslStoreFormat` enum (`PLAIN`, `TIMESTAMPED`, or `HEADERS`) on `DslKeyValueParams`, `DslWindowParams`, and `DslSessionParams`. Note that the string config values (`DEFAULT`/`HEADERS`) differ from the `DslStoreFormat` enum constants (`PLAIN`, `TIMESTAMPED`, `HEADERS`); `DslStoreFormat.DEFAULT` does not exist. This builds on the headers-aware state store implementations introduced in [KIP-1271](https://cwiki.apache.org/confluence/x/QIM8G). For further details and current DSL result-header limitations, please refer to [KIP-1285](https://cwiki.apache.org/confluence/x/4ow8G).
