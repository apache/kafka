...
class LeaderEpochFileCache(topicPartition: TopicPartition,
                           leo: () => LogOffsetMetadata,
                           checkpoint: LeaderEpochCheckpoint)
    extends LeaderEpochCache
    with Logging {
  ...
  override def assign(epoch: Int, offset: Long): Unit =
    inWriteLock(lock) {
      if (epoch >= 0 && epoch > latestEpoch && offset >= latestOffset) {
        info(
          s"Updated PartitionLeaderEpoch. ${epochChangeMsg(epoch, offset)}. Cache now contains ${epochs.size} entries."
        )
  ...
  override def endOffsetFor(requestedEpoch: Int): (Int, Long) =
    inReadLock(lock) {
      ...
      debug(
        s"Processed offset for epoch request for partition ${topicPartition} epoch:$requestedEpoch and returning epoch ${epochAndOffset._1} and offset ${epochAndOffset._2} from epoch list of size ${epochs.size}"
      )
      epochAndOffset
    }
  ...
  override def clearAndFlushLatest(offset: Long): Unit =
    inWriteLock(lock) {
      ...
      if (offset >= 0 && offset <= latestOffset()) {
        ...
        flush()
        info(
          s"Cleared latest ${before.toSet
            .filterNot(epochs.toSet)} entries from epoch cache based on passed offset $offset leaving ${epochs.size} in EpochFile for partition $topicPartition"
        )
      }
    }
    ...
    inWriteLock(lock) {
      ...
      if (offset >= 0 && earliestOffset() < offset) {
        ...
        if (earliest.nonEmpty) {
          ...
          flush()
          info(
            s"Cleared earliest ${before.toSet.filterNot(epochs.toSet).size} entries from epoch cache based on passed offset $offset leaving ${epochs.size} in EpochFile for partition $topicPartition"
          )
        }
      }
    }
  ...
  def epochChangeMsg(epoch: Int, offset: Long) =
    s"New: {epoch:$epoch, offset:$offset}, Current: {epoch:$latestEpoch, offset:$latestOffset} for Partition: $topicPartition"

  def validateAndMaybeWarn(epoch: Int, offset: Long) = {
    assert(
      epoch >= 0,
      s"Received a PartitionLeaderEpoch assignment for an epoch < 0. This should not happen. ${epochChangeMsg(epoch, offset)}"
    )
    if (epoch < latestEpoch())
      warn(
        s"Received a PartitionLeaderEpoch assignment for an epoch < latestEpoch. " +
          s"This implies messages have arrived out of order. ${epochChangeMsg(epoch, offset)}"
      )
    else if (offset < latestOffset())
      warn(
        s"Received a PartitionLeaderEpoch assignment for an offset < latest offset for the most recent, stored PartitionLeaderEpoch. " +
          s"This implies messages have arrived out of order. ${epochChangeMsg(epoch, offset)}"
      )
  }
}
