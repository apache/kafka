...
fetch_jdk_tgz() {
  ...
  if [ ! -e $path ]; then
    ...
    curl --retry-all-errors --retry 5 -s -L "https://s3-us-west-2.amazonaws.com/kafka-packages/jdk-${jdk_version}.tar.gz" -o $path
  fi
}
...
get_kafka() {
    ...
    if [ ! -d /opt/kafka-$version ]; then
        pushd /tmp
        curl --retry-all-errors --retry 5 -O $url
        curl --retry-all-errors --retry 5 -O $url_streams_test || true
