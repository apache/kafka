...
public class BlockingConnectorTest {
    ...
    public void testBlockInSourceTaskStop() throws Exception {
        ...
        waitForConnectorStart(BLOCKING_CONNECTOR_NAME, NUM_TASKS);
    ...
    private void createConnectorWithBlock(Class<? extends Connector> connectorClass, String block) {
        ...
        props.put(TASKS_MAX_CONFIG, Integer.toString(NUM_TASKS));
    ...
    private void waitForConnectorStart(String connector) throws InterruptedException {
        waitForConnectorStart(connector, 0);
    }

    private void waitForConnectorStart(String connector, int numTasks) throws InterruptedException {
    ...
    private static class Block {
        ...
        public static void waitForBlock() throws InterruptedException {
            synchronized (Block.class) {
                if (blockLatch == null) {
                    log.debug("No blocking latch has been registered yet; awaiting the creation of one");
                    Block.class.wait(CONNECTOR_SETUP_DURATION_MS);

                    if (blockLatch == null) {
                        throw new IllegalArgumentException("No blocking latch was registered in time");
                    }
                }
            }
        ...
        public Block(String block) {
            this.block = block;
            log.debug("Registering blocking latch for {}", block);
            synchronized (Block.class) {
                if (blockLatch != null) {
                    blockLatch.countDown();
                }
                blockLatch = new CountDownLatch(1);
                Block.class.notifyAll();
                log.debug("Finished registering blocking latch for {}", block);
            }
        }
