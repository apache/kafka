...
public abstract class AbstractWorkerSourceTask extends WorkerTask {
    ...
    private final Supplier<Producer<byte[], byte[]>> producer;
    ...
    protected AbstractWorkerSourceTask(ConnectorTaskId id,
                                       ...
                                       Supplier<Producer<byte[], byte[]>> producer,
                                       TopicAdmin admin,
    ...
    boolean sendRecords() {
        ...
        for (final SourceRecord preTransformRecord : toSend) {
            ...
            try {
                ...
                throw interceptProducerSendException(e);
            }
            processed++;
    ...
    /**
     * If any producer failure has been reported asynchronously via callback from
     * {@link Producer#send(ProducerRecord, Callback)}, throw an exception for that failure.
     */
    protected void maybeThrowProducerSendException() {
        if (producerSendException.get() != null) {
            throw interceptProducerSendException(producerSendException.get());
        }
    }

    /**
     * Possibly wrap or otherwise alter an exception that will be thrown due to a (synchronous or asynchronous)
     * producer send failure
     * @param error the exception to alter
     */
    protected RuntimeException interceptProducerSendException(Throwable error) {
        throw new ConnectException("Unrecoverable exception from producer send", error);
    }
