...
object DeleteRecordsCommand {
      ...
      case 1 =>
        js.asJsonObject.get("partitions") match {
          case Some(partitions) =>
            partitions.asJsonArray.iterator
              .map(_.asJsonObject)
              .map { partitionJs =>
  ...
  def execute(args: Array[String], out: PrintStream): Unit = {
    ...
    if (duplicatePartitions.nonEmpty)
      throw new AdminCommandFailedException(
        "Offset json file contains duplicate topic partitions: %s".format(duplicatePartitions.mkString(","))
      )

    val recordsToDelete = offsetSeq
      .map {
    ...
    deleteRecordsResult.lowWatermarks.asScala.foreach {
  ...
  class DeleteRecordsCommandOptions(args: Array[String]) {
    ...
    val offsetJsonFileOpt = parser
      .accepts("offset-json-file", offsetJsonFileDoc)
      .withRequiredArg
