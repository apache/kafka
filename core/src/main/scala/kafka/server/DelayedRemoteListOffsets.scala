...
case class ListOffsetsPartitionStatus(var responseOpt: Option[ListOffsetsPartitionResponse] = None,
                                      taskOpt: Option[RemoteLogManager#AsyncReadTask[Either[Exception, Option[TimestampAndOffset]]]] = None,
                                      lastFetchableOffset: Option[Long] = None,
                                      maybeOffsetsError: Option[ApiException] = None) {
  @volatile var completed = false

  override def toString: String = {
    s"[responseOpt: $responseOpt, taskOpt: $taskOpt, lastFetchableOffset: $lastFetchableOffset, " +
...
class DelayedRemoteListOffsets(delayMs: Long,
  ...
  metadata.statusByPartition.forKeyValue { (topicPartition, status) =>
    status.completed = status.taskOpt.isEmpty
    if (status.taskOpt.isDefined) {
  ...
  override def onExpiration(): Unit = {
    metadata.statusByPartition.forKeyValue { (topicPartition, status) =>
      if (!status.completed) {
        ...
        status.taskOpt.foreach(_.cancel(true))
  ...
  override def tryComplete(): Boolean = {
    ...
    metadata.statusByPartition.forKeyValue { (partition, status) =>
      if (!status.completed) {
        status.taskOpt.foreach { task =>
          if (task.taskFuture.isDone) {
            val response = task.taskFuture.get() match {
              case Left(e) =>
          ...
          completable = completable && task.taskFuture.isDone
        }
      }
    }
    if (completable) {
...
object DelayedRemoteListOffsetsMetrics extends KafkaMetricsGroup {
