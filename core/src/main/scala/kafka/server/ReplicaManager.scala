...
class ReplicaManager(val config: KafkaConfig,
  ...
  def startHighWatermarkCheckPointThread(): Unit = {
    if (highWatermarkCheckPointThreadStarted.compareAndSet(false, true))
      scheduler.schedule("highwatermark-checkpoint", checkpointHighWatermarks _, 0L, config.replicaHighWatermarkCheckpointIntervalMs)
  }
  ...
  def startup(): Unit = {
    ...
    scheduler.schedule("isr-expiration", maybeShrinkIsr _, 0L, config.replicaLagTimeMaxMs / 2)
    scheduler.schedule("shutdown-idle-replica-alter-log-dirs-thread", shutdownIdleReplicaAlterLogDirsThread _, 0L, 10000L)
