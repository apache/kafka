...
class KafkaConfig private(doLog: Boolean, val props: util.Map[_, _])
  ...
  val earlyStartListeners: Set[ListenerName] = {
    val listenersSet = listeners.map(l => ListenerName.normalised(l.listenerName)).toSet
    val controllerListenersSet = controllerListeners.map(l => ListenerName.normalised(l.listenerName)).toSet
  ...
  def controllerListeners: Seq[Endpoint] =
    listeners.filter(l => controllerListenerNames.contains(l.listenerName))
  ...
  def dataPlaneListeners: Seq[Endpoint] = {
    listeners.filterNot { listener =>
      val name = listener.listenerName
  ...
  def effectiveAdvertisedControllerListeners: Seq[Endpoint] = {
    ...
    val controllerAdvertisedListeners = if (advertisedListenersProp != null) {
      CoreUtils.listenerListToEndPoints(advertisedListenersProp, effectiveListenerSecurityProtocolMap, requireDistinctPorts=false)
        .filter(l => controllerListenerNames.contains(l.listenerName))
    } else {
      Seq.empty
    }
    ...
    controllerListenerNames.flatMap { name =>
      controllerAdvertisedListeners
        .find(endpoint => ListenerName.normalised(endpoint.listenerName).equals(ListenerName.normalised(name)))
        .orElse(
          ...
          controllerListenersValue
            ...
            .map(endpoint => if (endpoint.host == "0.0.0.0") {
              new Endpoint(endpoint.listenerName, endpoint.securityProtocol, null, endpoint.port)
            } else {
              endpoint
            })
        )
  ...
  def effectiveAdvertisedBrokerListeners: Seq[Endpoint] = {
    ...
    advertisedListeners.filterNot(l => controllerListenerNames.contains(l.listenerName))
  }
  ...
  private def validateValues(): Unit = {
    ...
    val advertisedBrokerListenerNames = effectiveAdvertisedBrokerListeners.map(l => ListenerName.normalised(l.listenerName)).toSet
    ...
    def validateControllerListenerNamesMustAppearInListenersForKRaftController(): Unit = {
      val listenerNameValues = listeners.map(_.listenerName).toSet
    ...
    val listenerNames = listeners.map(l => ListenerName.normalised(l.listenerName)).toSet
