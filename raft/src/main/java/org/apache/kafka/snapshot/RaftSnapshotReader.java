 ...
 */
public final class RaftSnapshotReader<T> implements SnapshotReader<T> {
    private final OffsetAndEpoch snapshotId;
    private final RecordsIterator<T> iterator;

    private RaftSnapshotReader(
    ...
    public static <T> RaftSnapshotReader<T> of(
        RawSnapshotReader snapshot,
        RecordSerde<T> serde,
        BufferSupplier bufferSupplier,
        int maxBatchSize
    ) {
        return new RaftSnapshotReader<>(
            snapshot.snapshotId(),
