...
/**
 * Leader-side benchmarks. The leader is built once ({@code @Setup(Level.Trial)}) and reused across
 * thousands of fully JIT-warmed invocations, so the steady-state request-handling scenarios here use
 * {@code Mode.AverageTime}.
 */
@State(Scope.Thread)
