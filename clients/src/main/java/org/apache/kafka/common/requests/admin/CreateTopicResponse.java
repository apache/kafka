...
public class CreateTopicResponse extends AbstractRequestResponse {
    private static final Schema CURRENT_SCHEMA = ProtoUtils.currentResponseSchema(ApiKeys.CREATE_TOPIC.id);
    ...
    public CreateTopicResponse(Map<String, Errors> errors) {
    ...
    public static CreateTopicResponse parse(ByteBuffer buffer) {
        return new CreateTopicResponse((Struct) CURRENT_SCHEMA.read(buffer));
    }
}
