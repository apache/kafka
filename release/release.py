...
def command_release_announcement_email():
    ...
    contributors = ", ".join(str(x) for x in filter(None, git.contributors(previous_release_version_num, release_version_num))),
    release_announcement_email = templates.release_announcement_email(release_version, contributors)
...
textfiles.replace(f"{repo_dir}/tests/kafkatest/version.py", "^DEV_VERSION =.*",
    "DEV_VERSION = KafkaVersion(\"{release_version}-SNAPSHOT\")", regex=True)
