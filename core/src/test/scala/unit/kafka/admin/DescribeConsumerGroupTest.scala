...
class DescribeConsumerGroupTest extends ConsumerGroupCommandTest {
  ...
  def testDescribeExistingGroup() {
    ...
    for (describeType <- describeTypes) {
      ...
      }, s"Expected a data row and no error in describe results with describe type ${describeType.mkString(" ")}.", retryExceptions = true)
    }
  }
  ...
  def testDescribeOffsetsOfExistingGroup() {
    ...
    }, s"Expected a 'Stable' group status, rows and valid values for consumer id / client id / host columns in describe results for group $group.", retryExceptions = true)
  }
  ...
  def testDescribeMembersOfExistingGroup() {
    ...
    }, s"Expected a 'Stable' group status, rows and valid member information for group $group.", retryExceptions = true)
  ...
  def testDescribeStateOfExistingGroup() {
    ...
    }, s"Expected a 'Stable' group status, with one member and round robin assignment strategy for group $group.", retryExceptions = true)
  }
  ...
  def testDescribeExistingGroupWithNoMembers() {
    ...
    for (describeType <- describeTypes) {
      ...
      TestUtils.waitUntilTrue(() => {
        val (output, error) = TestUtils.grabConsoleOutputAndError(service.describeGroups())
        output.trim.split("\n").length == 2 && error.isEmpty
      }, s"Expected describe group results with one data row for describe type '${describeType.mkString(" ")}'", retryExceptions = true)

      // stop the consumer so the group has no active member anymore
      executor.shutdown()
      TestUtils.waitUntilTrue(() => {
        TestUtils.grabConsoleError(service.describeGroups()).contains(s"Consumer group '$group' has no active members.")
      }, s"Expected no active member in describe group results with describe type ${describeType.mkString(" ")}", retryExceptions = true)
    }
  }
  ...
  def testDescribeOffsetsOfExistingGroupWithNoMembers() {
    ...
    }, "Expected the group to initially become stable, and to find group in assignments after initial offset commit.", retryExceptions = true)
  ...
  def testDescribeMembersOfExistingGroupWithNoMembers() {
    ...
    }, s"Expected no member in describe group members results for group '$group'", retryExceptions = true)
  }
  ...
  def testDescribeStateOfExistingGroupWithNoMembers() {
    ...
    TestUtils.waitUntilTrue(() => {
      ...
      state.state == "Stable" &&
        ...
        state.coordinator != null &&
        servers.map(_.config.brokerId).toList.contains(state.coordinator.id)
    }, s"Expected the group '$group' to initially become stable, and have a single member.", retryExceptions = true)
    ...
    }, s"Expected the group '$group' to become empty after the only member leaving.", retryExceptions = true)
  }
  ...
  def testDescribeWithConsumersWithoutAssignedPartitions() {
    ...
    for (describeType <- describeTypes) {
      ...
      }, s"Expected a single data row in describe group result with describe type '${describeType.mkString(" ")}'", retryExceptions = true)
    }
  }
  ...
  def testDescribeOffsetsWithConsumersWithoutAssignedPartitions() {
    ...
    }, "Expected rows for consumers with no assigned partitions in describe group results", retryExceptions = true)
  }
  ...
  def testDescribeStateWithConsumersWithoutAssignedPartitions() {
    ...
    }, "Expected two consumers in describe group results", retryExceptions = true)
  }
  ...
  def testDescribeOffsetsWithMultiPartitionTopicAndMultipleConsumers() {
    ...
    }, "Expected two rows (one row per consumer) in describe group results.", retryExceptions = true)
  }
  ...
  def testDescribeMembersWithMultiPartitionTopicAndMultipleConsumers() {
    ...
    }, "Expected two rows (one row per consumer) in describe group members results.", retryExceptions = true)
  ...
  def testDescribeStateWithMultiPartitionTopicAndMultipleConsumers() {
    ...
    }, "Expected a stable group with two members in describe group state result.", retryExceptions = true)
  }
