    ...
    protected Struct toStruct(short version) {
        ...
        for (Map.Entry<String, Map<Integer, EpochEndOffset>> topicToPartitionEpochs : endOffsetsByTopic.entrySet()) {
            ...
            for (Map.Entry<Integer, EpochEndOffset> partitionEndOffset : partitionEpochs.entrySet()) {
                Struct partitionRow = topicStruct.instance(PARTITIONS);
                partitionRow.set(ERROR_CODE, partitionEndOffset.getValue().error().code());
                partitionRow.set(PARTITION_ID, partitionEndOffset.getKey());
                partitionRow.set(END_OFFSET, partitionEndOffset.getValue().endOffset());
