...
public final class RaftClientContextBuilder {
    ...
    /**
     * Builds a {@link RaftClientBenchmarkContext} rather than a plain test context: it registers a
     * no-op listener and skips the network round trip.
     */
    public RaftClientBenchmarkContext buildBenchmark(
        ReplicaKey localKey,
        ...
        MockNetworkChannel channel = new MockNetworkChannel();
        ExternalKRaftMetrics externalKRaftMetrics = Mockito.mock(ExternalKRaftMetrics.class);
        KafkaRaftClient<String> client = buildClient(new NoOpListener(), channel, metrics, externalKRaftMetrics);

        RaftClientBenchmarkContext context = new RaftClientBenchmarkContext(
            clusterId,
            localId,
            localDirectoryId,
            kraftVersion,
    ...
    // A listener that does nothing, registered by benchmarks so leader-change and commit handling —
    // and their correctness assertions — stay out of the measured region. It only closes the readers
    // it is handed so their buffers return to the memory pool. handleLeaderChange and beginShutdown
    // are left to the interface's no-op defaults.
    private static final class NoOpListener implements RaftClient.Listener<String> {
        @Override
