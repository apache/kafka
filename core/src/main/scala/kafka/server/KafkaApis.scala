    ...
    authorizer match {
      ...
      case Some(auth) =>
        ...
        val returnedAcls = auth.getAcls.toSeq.flatMap { case (resource, acls) =>
          acls.flatMap { acl =>
            val fixture = new AclBinding(new AdminResource(resource.resourceType.toJava, resource.name, resource.nameType),
        ...
        if (filters.forall(_.matchesAtMostOne)) {
          ...
          for ((resource, acls) <- aclMap; acl <- acls) {
            val binding = new AclBinding(
              new AdminResource(resource.resourceType.toJava, resource.name, resource.nameType),
