...
class ReassignPartitionsClusterTest extends ZooKeeperTestHarness with Logging {
  ...
  def startBrokers(brokerIds: Seq[Int]) {
    servers = brokerIds
      .map(i => createBrokerConfig(i, zkConnect, enableControlledShutdown = false, logDirCount = 3))
  ...
  def testHwAfterPartitionReassignment(): Unit = {
    ...
    val topicJson: String =
      s"""{"version":1,"partitions":[{"topic":"$topicName","partition":0,"replicas":[101, 102]}]}"""
    ...
    TestUtils.waitUntilTrue(
      ...
      "broker 101 should be the new leader",
      pause = 1L
    )
    ...
    TestUtils.waitUntilTrue(
      () => newFollowerServer.replicaManager.getReplicaOrException(topicPartition).highWatermark.messageOffset == 100,
  ...
  def shouldMoveSinglePartition(): Unit = {
    ...
    val topicJson: String =
      s"""{"version":1,"partitions":[{"topic":"$topicName","partition":0,"replicas":[101],"log_dirs":["$expectedLogDir"]}]}"""
    ReassignPartitionsCommand.executeAssignment(zkClient, Some(adminClient), topicJson, NoThrottle)
    ...
    assertEquals(
      expectedLogDir,
      adminClient.describeReplicaLogDirs(Collections.singleton(replica)).all().get.get(replica).getCurrentReplicaLogDir
    )
  }
  ...
  def shouldMoveSinglePartitionWithinBroker() {
    ...
    val topicJson: String =
      s"""{"version":1,"partitions":[{"topic":"$topicName","partition":0,"replicas":[100],"log_dirs":["$expectedLogDir"]}]}"""
    ReassignPartitionsCommand.executeAssignment(zkClient, Some(adminClient), topicJson, NoThrottle)
    val replica = new TopicPartitionReplica(topicName, 0, 100)
    TestUtils.waitUntilTrue(
      () => {
        expectedLogDir == adminClient
          .describeReplicaLogDirs(Collections.singleton(replica))
          .all()
          .get
          .get(replica)
          .getCurrentReplicaLogDir
      },
      "Partition should have been moved to the expected log directory",
      1000
    )
  }
  ...
  def shouldExpandCluster() {
    ...
    waitUntilTrue(
      () => {
        expectedLogDir1 == adminClient
          .describeReplicaLogDirs(Collections.singleton(replica1))
          .all()
          .get
          .get(replica1)
          .getCurrentReplicaLogDir
      },
      "Partition should have been moved to the expected log directory on broker 102",
      1000
    )
    waitUntilTrue(
      () => {
        expectedLogDir2 == adminClient
          .describeReplicaLogDirs(Collections.singleton(replica2))
          .all()
          .get
          .get(replica2)
          .getCurrentReplicaLogDir
      },
      "Partition should have been moved to the expected log directory on broker 100",
      1000
    )
  }
  ...
  def shouldExecuteThrottledReassignment() {
    ...
    checkThrottleConfigAddedToZK(adminZkClient,
                                 initialThrottle.interBrokerLimit,
                                 servers,
                                 topicName,
                                 Set("0:100", "0:101"),
                                 Set("0:102"))
  ...
  def shouldFailIfProposedHasInvalidLogDir() {
    ...
    val topicJson: String =
      s"""{"version":1,"partitions":[{"topic":"$topicName","partition":0,"replicas":[101],"log_dirs":["invalidDir"]}]}"""
    ReassignPartitionsCommand.executeAssignment(zkClient, Some(adminClient), topicJson, NoThrottle)
  }
  ...
  def shouldFailIfProposedHasInconsistentReplicasAndLogDirs() {
    ...
    val topicJson: String =
      s"""{"version":1,"partitions":[{"topic":"$topicName","partition":0,"replicas":[101],"log_dirs":["$logDir", "$logDir"]}]}"""
    ReassignPartitionsCommand.executeAssignment(zkClient, Some(adminClient), topicJson, NoThrottle)
  }
  ...
  def shouldPerformMultipleReassignmentOperationsOverVariousTopics() {
    ...
    Iterator
      .continually {
        try new ReassignPartitionsCommand(zkClient, None, fourthMove, adminZkClient = adminZkClient)
          .reassignPartitions()
        catch {
  ...
  def waitForReassignmentToComplete(pause: Long = 100L) {
    waitUntilTrue(() => !zkClient.reassignPartitionsInProgress,
                  s"Znode ${ReassignPartitionsZNode.path} wasn't deleted",
                  pause = pause)
  }
  ...
  private def produceMessages(topic: String, numMessages: Int, acks: Int, valueLength: Int): Unit = {
    val records =
      (0 until numMessages).map(_ => new ProducerRecord[Array[Byte], Array[Byte]](topic, new Array[Byte](valueLength)))
