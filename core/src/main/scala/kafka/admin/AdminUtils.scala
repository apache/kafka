...
object AdminUtils extends Logging {
  ...
  private def assignReplicasToBrokersRackUnaware(nPartitions: Int,
    ...
    for (_ <- 0 until nPartitions) {
      if (currentPartitionId > 0 && (currentPartitionId % brokerArray.length == 0))
        nextReplicaShift += 1
      @annotation.nowarn("msg=never used") val firstReplicaIndex = (currentPartitionId + startIndex) % brokerArray.length
  ...
  private def assignReplicasToBrokersRackAware(nPartitions: Int,
    ...
    val brokerRackMap = brokerMetadatas.collect { case bm: BrokerMetadata if bm.rack.isPresent =>
      bm.id -> rackIdMapperForBrokerAssignment.apply(bm.rack.get())
    }.toMap
    ...
    for (_ <- 0 until nPartitions) {
      if (currentPartitionId > 0 && (currentPartitionId % arrangedBrokerList.size == 0))
        nextReplicaShift += 1
      @annotation.nowarn("msg=never used") val firstReplicaIndex = (currentPartitionId + startIndex) % arrangedBrokerList.size
