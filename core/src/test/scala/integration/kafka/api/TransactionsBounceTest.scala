...
class TransactionsBounceTest extends IntegrationTestHarness {
  ...
  overridingProps.put(KafkaServerConfigs.MESSAGE_MAX_BYTES_CONFIG, serverMessageMaxBytes.toString)
  // Set a smaller value for the number of partitions for the offset commit topic (__consumer_offset topic)
  // so that the creation of that topic/partition(s) and subsequent leader assignment doesn't take relatively long
  overridingProps.put(KafkaServerConfigs.CONTROLLED_SHUTDOWN_ENABLE_CONFIG, true.toString)
