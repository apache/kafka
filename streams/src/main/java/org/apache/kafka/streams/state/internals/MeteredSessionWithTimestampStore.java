...
public class MeteredSessionWithTimestampStore<K, V> extends WrappedStateStore.AbstractStateStore implements SessionWithTimestampStore<K, V> {
    ...
    public void init(final ProcessorContext context, final StateStore root) {
        ...
        serdes = new StateSerdes<>(
            ...
            new KeyValueWithTimestampStoreBuilder.ValueAndTimestampSerde(valueSerde == null ? (Serde<V>) context.valueSerde() : valueSerde));
    ...
    @Override
    public KeyValueIterator<Windowed<K>, ValueAndTimestamp<V>> findSessions(final K key,
    ...
    @Override
    public KeyValueIterator<Windowed<K>, ValueAndTimestamp<V>> findSessions(final K keyFrom,
                                                                            final K keyTo,
                                                                            final long earliestSessionEndTime,
                                                                            final long latestSessionStartTime) {
    ...
    public void put(final Windowed<K> sessionKey,
        ...
        put(sessionKey, aggregate == null ? null : new ValueAndTimestampImpl<>(aggregate, timestamp));
    }

    @Override
    public void put(final Windowed<K> sessionKey,
                    final ValueAndTimestamp<V> aggregate) {
        Objects.requireNonNull(sessionKey, "sessionKey can't be null");
        final long startNs = time.nanoseconds();
        try {
            final Bytes key = keyBytes(sessionKey.key());
    ...
    @Override
    public KeyValueIterator<Windowed<K>, ValueAndTimestamp<V>> fetch(final K key) {
        Objects.requireNonNull(key, "key cannot be null");
        return findSessions(key, 0, Long.MAX_VALUE);
    }

    @Override
    public KeyValueIterator<Windowed<K>, ValueAndTimestamp<V>> fetch(final K from, final K to) {
