...
public class KStreamImpl<K, V> extends AbstractStream<K, V> implements KStream<K, V> {
    ...
    private KStream<K, V>[] doBranch(final NamedInternal named,
        ...
        for (int i = 0; i < predicates.length; i++) {
            childNames[i] = named.suffixWithOrElseGet("-predicate-" + i, () -> builder.newProcessorName(BRANCHCHILD_NAME));
        }
    ...
    @Override
    public <KR, VR> KStream<KR, VR> transform(final TransformerSupplier<? super K, ? super V, KeyValue<KR, VR>> transformerSupplier,
                                              final Named processorName,
                                              final String... stateStoreNames) {
        Objects.requireNonNull(transformerSupplier, "transformerSupplier can't be null");
        return flatTransform(new TransformerSupplierAdapter<>(transformerSupplier), processorName, stateStoreNames);
    ...
    public <VR> KStream<K, VR> transformValues(final ValueTransformerSupplier<? super V, ? extends VR> valueTransformerSupplier,
        ...
        return doTransformValues(toValueTransformerWithKeySupplier(valueTransformerSupplier),
                new NamedInternal(processorName), stateStoreNames);
    }
    ...
    public <VR> KStream<K, VR> transformValues(final ValueTransformerWithKeySupplier<? super K, ? super V, ? extends VR> valueTransformerSupplier,
        ...
        Objects.requireNonNull(valueTransformerSupplier, "valueTransformSupplier can't be null");
        Objects.requireNonNull(processorName, "processorName can't be null");
        return doTransformValues(valueTransformerSupplier, new NamedInternal(processorName), stateStoreNames);
    }
    ...
    private <VR> KStream<K, VR> doTransformValues(final ValueTransformerWithKeySupplier<? super K, ? super V, ? extends VR> valueTransformerWithKeySupplier,
        ...
        final String name = processorName.orElseGenerateWithPrefix(builder, TRANSFORMVALUES_NAME);
        final StatefulProcessorNode<? super K, ? super V> transformNode = new StatefulProcessorNode<>(
                name,
    ...
    public void process(final ProcessorSupplier<? super K, ? super V> processorSupplier,
                        ...
                        final String... stateStoreNames) {
        Objects.requireNonNull(processorSupplier, "ProcessSupplier cant' be null");
        Objects.requireNonNull(named, "processorName cant' be null");
    ...
    private <VO, VR> KStream<K, VR> doJoin(final KStream<K, VO> other,
        ...
        final NamedInternal joinedName = new NamedInternal(joinedInternal.internalName());
        if (joinThis.repartitionRequired) {
            final String joinThisName = joinThis.name;
            final String leftJoinRepartitionTopicName = joinedName.suffixWithOrElseGet("-left", () -> joinThisName);
        ...
        if (joinOther.repartitionRequired) {
            ...
            final String rightJoinRepartitionTopicName = joinedName.suffixWithOrElseGet("-right", () -> joinOtherName);
    ...
    public <VO, VR> KStream<K, VR> join(final KTable<K, VO> other,
        ...
        final JoinedInternal<K, V, VO> joinedInternal = new JoinedInternal<>(joined);
        final String internalName = joinedInternal.internalName();
    ...
    private <VO, VR> KStream<K, VR> doStreamTableJoin(final KTable<K, VO> other,
        ...
        final JoinedInternal<K, V, VO> joinedInternal = new JoinedInternal<>(joined);
        final NamedInternal renamed = new NamedInternal(joinedInternal.internalName());
        // we actually do not need to generate processor names for join operation if specified name.
        // But, we still need to burn index for join operation keep topology backward compatibility.
        if (renamed.isDefined()) {
            builder.newProcessorName(leftJoin ? LEFTJOIN_NAME : JOIN_NAME);
        }
    ...
    private class KStreamImplJoin {
        ...
        public <K1, R, V1, V2> KStream<K1, R> join(final KStream<K1, V1> lhs,
            ...
            // we actually do not need to generate processor names for join operations if a name is specified.
            // But, we still need to burn index for all joins operation keep topology backward compatibility.
            if (renamed.isDefined()) {
                builder.newProcessorName(WINDOWED_NAME);
                builder.newProcessorName(WINDOWED_NAME);
                if (rightOuter)
                    builder.newProcessorName(OUTERTHIS_NAME);
                else
                    builder.newProcessorName(JOINTHIS_NAME);
                if (leftOuter)
                    builder.newProcessorName(OUTEROTHER_NAME);
                else
                    builder.newProcessorName(JOINOTHER_NAME);
                builder.newProcessorName(MERGE_NAME);
            }

            final String thisWindowStreamName =  renamed.suffixWithOrElseGet(
                    "-this-windowed", () -> builder.newProcessorName(WINDOWED_NAME));
            final String otherWindowStreamName = renamed.suffixWithOrElseGet(
                    "-other-windowed", () -> builder.newProcessorName(WINDOWED_NAME));

            final String joinThisName = rightOuter ?
                    renamed.suffixWithOrElseGet("-outer-this-join", () -> builder.newProcessorName(OUTERTHIS_NAME))
                    : renamed.suffixWithOrElseGet("-this-join", () -> builder.newProcessorName(JOINTHIS_NAME));
            final String joinOtherName = leftOuter ?
                    renamed.suffixWithOrElseGet("-outer-other-join", () -> builder.newProcessorName(OUTEROTHER_NAME))
                    : renamed.suffixWithOrElseGet("-other-join", () -> builder.newProcessorName(JOINOTHER_NAME));
            final String joinMergeName = renamed.suffixWithOrElseGet(
                    "-merge", () -> builder.newProcessorName(MERGE_NAME));
