...
vagrant_version=$(vagrant --version | grep -E -o '[0-9]+\.[0-9]+\.[0-9]+')
bad_vagrant=false
...
if [ "x$bad_vagrant" = 'xtrue' ] || [ "x$bad_vb" = 'xtrue' ]; then
    exit 1
fi

echo "Checking for necessary Vagrant plugins..."
hostmanager_version=$(vagrant plugin list | grep vagrant-hostmanager | grep -E -o '[0-9]+\.[0-9]+\.[0-9]+')
...
vagrantfile_ok=true
num_brokers=$(grep -E -o 'num_brokers\s*=\s*[0-9]+' Vagrantfile.local | cut -d '=' -f 2 | xargs)
num_zookeepers=$(grep -E -o 'num_zookeepers\s*=\s*[0-9]+' Vagrantfile.local | cut -d '=' -f 2 | xargs)
num_workers=$(grep -E -o 'num_workers\s*=\s*[0-9]+' Vagrantfile.local | cut -d '=' -f 2 | xargs)
if [ "x$num_brokers" = 'x' ] || [ "$num_brokers" -ne 0 ]; then
    echo "Vagrantfile.local: bad num_brokers. Update to: num_brokers = 0"
    vagrantfile_ok=false
fi
if [ "x$num_zookeepers" = 'x' ] || [ "$num_zookeepers" -ne 0 ]; then
    echo "Vagrantfile.local: bad num_zookeepers. Update to: num_zookeepers = 0"
    vagrantfile_ok=false
fi
if [ "x$num_workers" = 'x' ] || [ "$num_workers" -eq 0 ]; then
