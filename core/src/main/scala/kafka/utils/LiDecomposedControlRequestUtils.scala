...
object LiDecomposedControlRequestUtils {
  def decomposeRequest(request: LiCombinedControlRequest, brokerEpoch: Long, config: KafkaConfig): LiDecomposedControlRequest = {
    ...
    val stopReplicaRequestVersion: Short = {
      if (config.interBrokerProtocolVersion.isAtLeast(MetadataVersion.IBP_3_0_IV1)) 4
      else if (config.interBrokerProtocolVersion.isAtLeast(MetadataVersion.IBP_2_4_IV1)) 3
  ...
  private def extractLeaderAndIsrRequest(request: LiCombinedControlRequest, brokerEpoch: Long, config: KafkaConfig): Option[LeaderAndIsrRequest] = {
    ...
    if (effectivePartitionStates.isEmpty) {
      ...
      val leaderAndIsrRequestVersion: Short =
        if (config.interBrokerProtocolVersion.isAtLeast(MetadataVersion.IBP_3_0_IV1)) 6
        else if (config.interBrokerProtocolVersion.isAtLeast(MetadataVersion.IBP_2_4_IV1)) 5
        else throw new IllegalStateException("The inter.broker.protocol.version config should not be smaller than 2.4-IV1")
      ...
      Some(new LeaderAndIsrRequest.Builder(leaderAndIsrRequestVersion, request.controllerId(), request.controllerEpoch(),
        request.brokerEpoch(), effectivePartitionStates, topicIds, leaderNodes
      ).build())
    }
  }
  ...
  private def extractUpdateMetadataRequest(request: LiCombinedControlRequest, config: KafkaConfig): Option[UpdateMetadataRequest] = {
    ...
    if (effectivePartitionStates.isEmpty && liveBrokers.isEmpty) {
      ...
      val updateMetadataRequestVersion: Short =
        if (config.interBrokerProtocolVersion.isAtLeast(MetadataVersion.IBP_3_0_IV1)) 8
        else if (config.interBrokerProtocolVersion.isAtLeast(MetadataVersion.IBP_2_4_IV1)) 7
        else throw new IllegalStateException("The inter.broker.protocol.version config should not be smaller than 2.4-IV1")
      ...
      Some(new UpdateMetadataRequest.Builder(updateMetadataRequestVersion, request.controllerId(), request.controllerEpoch(), request.brokerEpoch(), effectivePartitionStates, liveBrokers, topicIds).build())
    }
  }
  ...
  private def extractStopReplicaRequestWithTopicStates(request: LiCombinedControlRequest, stopReplicaRequestVersion: Short): List[StopReplicaRequest] = {
    ...
    List(new StopReplicaRequest.Builder(stopReplicaRequestVersion, request.controllerId(), request.controllerEpoch(),
      request.brokerEpoch(), defaultDeletePartitions, stopReplicaTopicStates).build())
  }
  ...
  def getStopReplicaRequest(stopReplicaRequestVersion: Short, request: LiCombinedControlRequest,
    ...
    new StopReplicaRequest.Builder(stopReplicaRequestVersion, request.controllerId(), request.controllerEpoch(), request.brokerEpoch(), deletePartitions, topicStatesList).build()
  }
