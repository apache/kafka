...
class MinIsrConfigTest extends KafkaServerTestHarness {
  ...
  overridingProps.put(KafkaConfig.MIN_IN_SYNC_REPLICAS_PROP, "5")
  def generateConfigs: Seq[KafkaConfig] = TestUtils.createBrokerConfigs(1, zkConnectOrNull).map(KafkaConfigProvider.fromProps(_, overridingProps))
