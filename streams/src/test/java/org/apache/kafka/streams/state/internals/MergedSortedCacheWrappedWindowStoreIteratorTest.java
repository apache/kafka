...
public class MergedSortedCacheWrappedWindowStoreIteratorTest {
    ...
    private final WindowKeySchema windowKeySchema  = new WindowKeySchema(10000, 10);
    ...
    public void shouldIterateOverValueFromBothIterators() throws Exception {
        ...
        for (long t = 0; t < 100; t += 20) {
            ...
            cache.put(namespace, windowKeySchema.toCacheKey(keyBytes), new LRUCacheEntry(valBytes));
        }
        ...
        final ThreadCache.MemoryLRUCacheBytesIterator cacheIterator = cache.range(
            namespace, windowKeySchema.toCacheKey(fromBytes), windowKeySchema.toCacheKey(toBytes)
        );
    ...
    public void shouldPeekNextStoreKey() throws Exception {
        ...
        cache.put(namespace, windowKeySchema.toCacheKey(WindowStoreUtils.toBinaryKey("a", 0, 0, stateSerdes)), new LRUCacheEntry("b".getBytes()));
        Bytes fromBytes = WindowStoreUtils.toBinaryKey("a", 0, 0, stateSerdes);
        Bytes toBytes = WindowStoreUtils.toBinaryKey("a", 100, 0, stateSerdes);
        final KeyValueIterator<Long, byte[]> storeIterator = new DelegatingPeekingKeyValueIterator<>("store", new KeyValueIteratorStub<>(windowStoreKvPairs.iterator()));
        final ThreadCache.MemoryLRUCacheBytesIterator cacheIterator = cache.range(
    ...
    public void shouldPeekNextCacheKey() throws Exception {
        ...
        cache.put(namespace, windowKeySchema.toCacheKey(WindowStoreUtils.toBinaryKey("a", 10L, 0, stateSerdes)), new LRUCacheEntry("b".getBytes()));
        Bytes fromBytes = WindowStoreUtils.toBinaryKey("a", 0, 0, stateSerdes);
        Bytes toBytes = WindowStoreUtils.toBinaryKey("a", 100, 0, stateSerdes);
        final KeyValueIterator<Long, byte[]> storeIterator = new DelegatingPeekingKeyValueIterator<>("store", new KeyValueIteratorStub<>(windowStoreKvPairs.iterator()));
        final ThreadCache.MemoryLRUCacheBytesIterator cacheIterator = cache.range(namespace, windowKeySchema.toCacheKey(fromBytes), windowKeySchema.toCacheKey(toBytes));
        final MergedSortedCacheWindowStoreIterator iterator = new MergedSortedCacheWindowStoreIterator(cacheIterator, storeIterator, windowKeySchema);
        assertThat(iterator.peekNextKey(), equalTo(0L));
        iterator.next();
        assertThat(iterator.peekNextKey(), equalTo(10L));
        iterator.close();
