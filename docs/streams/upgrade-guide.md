...
[KIP-770](https://cwiki.apache.org/confluence/pages/viewpage.action?pageId=186878390) is now fully implemented: config `input.buffer.max.bytes` (LONG, default 512 MB, topology-wide) is introduced and `buffered.records.per.partition` is deprecated ([KAFKA-13152](https://issues.apache.org/jira/browse/KAFKA-13152)). The total buffer is split evenly across stream threads (`input.buffer.max.bytes / num.stream.threads`); when a thread's tasks collectively exceed their slice, the main consumer pauses every non-empty input partition on that thread and resumes once the buffered bytes drop back below the cap. When `input.buffer.max.bytes` is set, the new bytes-based topology-wide pause supersedes the legacy per-partition record-count pause. If only the deprecated `buffered.records.per.partition` is set, the bytes cap is still applied at its default (512 MB) so memory stays bounded, while the legacy per-partition record cap continues to apply in parallel; a deprecation warning is logged. Memory note: upgraders who never set `input.buffer.max.bytes` will reserve 512 MB for input buffering by default — set it explicitly on memory-constrained deployments.
