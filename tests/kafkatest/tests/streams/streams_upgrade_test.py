            ...
            with second_other_node.account.monitor_log(second_other_processor.LOG_FILE) as second_other_monitor:
                ...
                with node.account.monitor_log(processor.LOG_FILE) as log_monitor:
                    ...
                    if len(self.old_processors) > 0:
                        log_monitor.wait_until("Sent a version 11 subscription and got version 10 assignment back (successful version probing). Downgrade subscription metadata to commonly supported version 10 and trigger new rebalance..",
                                               timeout_sec=60,
