...
public class InMemoryKeyValueWithTimestampStore<K, V> extends InMemoryKeyValueStore<K, ValueAndTimestamp<V>> {
    ...
    void initStateSerdes(final ProcessorContext context) {
        this.serdes = new StateSerdes<>(
            ...
            ((ValueAndTimestampSerde) valueSerde).initialized() ?
                valueSerde :
                ((ValueAndTimestampSerde<V>) valueSerde).init((Serde<V>) context.valueSerde()));
    }
}
