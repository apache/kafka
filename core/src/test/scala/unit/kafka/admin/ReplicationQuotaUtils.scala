  ...
  def checkThrottleConfigRemovedFromZK(adminZkClient: AdminZkClient, topic: String, servers: Seq[KafkaServer]): Unit = {
    TestUtils.waitUntilTrue(() => {
      val hasRateProp = servers.forall { server =>
        ...
        brokerConfig.contains(ServerQuotaConfigs.LEADER_REPLICATION_THROTTLED_RATE_CONFIG) ||
          brokerConfig.contains(ServerQuotaConfigs.FOLLOWER_REPLICATION_THROTTLED_RATE_CONFIG)
      }
      val topicConfig = adminZkClient.fetchEntityConfig(ConfigType.TOPIC, topic)
      val hasReplicasProp = topicConfig.contains(ServerQuotaConfigs.LEADER_REPLICATION_THROTTLED_REPLICAS_CONFIG) ||
        topicConfig.contains(ServerQuotaConfigs.FOLLOWER_REPLICATION_THROTTLED_REPLICAS_CONFIG)
  ...
  def checkThrottleConfigAddedToZK(adminZkClient: AdminZkClient, expectedThrottleRate: Long, servers: Seq[KafkaServer], topic: String, throttledLeaders: Set[String], throttledFollowers: Set[String]): Unit = {
    TestUtils.waitUntilTrue(() => {
      ...
      val brokerConfigAvailable = servers.forall { server =>
        ...
        val zkLeaderRate = configInZk.getProperty(ServerQuotaConfigs.LEADER_REPLICATION_THROTTLED_RATE_CONFIG)
        val zkFollowerRate = configInZk.getProperty(ServerQuotaConfigs.FOLLOWER_REPLICATION_THROTTLED_RATE_CONFIG)
      ...
      val topicConfig = adminZkClient.fetchEntityConfig(ConfigType.TOPIC, topic)
      val leader = topicConfig.getProperty(ServerQuotaConfigs.LEADER_REPLICATION_THROTTLED_REPLICAS_CONFIG).split(",").toSet
      val follower = topicConfig.getProperty(ServerQuotaConfigs.FOLLOWER_REPLICATION_THROTTLED_REPLICAS_CONFIG).split(",").toSet
