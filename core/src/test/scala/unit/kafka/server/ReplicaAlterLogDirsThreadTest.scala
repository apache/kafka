...
class ReplicaAlterLogDirsThreadTest {
  ...
  def issuesEpochRequestFromLocalReplica(): Unit = {
    ...
    expect(replica.endOffsetFor(leaderEpoch)).andReturn(leo).anyTimes()
  ...
  def shouldTruncateToReplicaOffset(): Unit = {
    ...
    expect(replicaT1p0.endOffsetFor(leaderEpoch)).andReturn(replicaT1p0LEO).anyTimes()
    expect(replicaT1p1.endOffsetFor(leaderEpoch)).andReturn(replicaT1p1LEO).anyTimes()
    expect(futureReplica.endOffsetFor(leaderEpoch)).andReturn(futureReplicaLEO).anyTimes()
  ...
  def shouldTruncateToEndOffsetOfLargestCommonEpoch(): Unit = {
    ...
    expect(replica.endOffsetFor(leaderEpoch)).andReturn(replicaLEO).anyTimes()
    // but future replica does not know about this leader epoch, so returns a smaller leader epoch
    expect(futureReplica.endOffsetFor(leaderEpoch - 1)).andReturn(futureReplicaLEO).anyTimes()
    // finally, the leader replica knows about the leader epoch and returns end offset
    expect(replica.endOffsetFor(leaderEpoch - 2)).andReturn(replicaEpochEndOffset).anyTimes()
    expect(futureReplica.endOffsetFor(leaderEpoch - 2)).andReturn(futureReplicaEpochEndOffset).anyTimes()
  ...
  def shouldTruncateToInitialFetchOffsetIfReplicaReturnsUndefinedOffset(): Unit = {
    ...
    expect(replica.endOffsetFor(UNDEFINED_EPOCH)).andReturn(UNDEFINED_EPOCH_OFFSET).anyTimes()
    stubWithFetchMessages(replica, replica, futureReplica, partition, replicaManager, responseCallback)
  ...
  def shouldPollIndefinitelyIfReplicaNotAvailable(): Unit = {
    ...
    expect(replica.endOffsetFor(futureReplicaLeaderEpoch)).andReturn(replicaLEO).anyTimes()
    expect(futureReplica.endOffsetFor(futureReplicaLeaderEpoch)).andReturn(futureReplicaLEO).anyTimes()
  ...
  def shouldFetchLeaderEpochOnFirstFetchOnly(): Unit = {
    ...
    expect(futureReplica.logEndOffset).andReturn(new LogOffsetMetadata(futureReplicaLEO)).anyTimes()
    expect(futureReplicaLeaderEpochs.latestEpoch).andReturn(leaderEpoch).anyTimes()
    expect(replica.endOffsetFor(leaderEpoch)).andReturn(replicaLEO)
    expect(futureReplica.endOffsetFor(leaderEpoch)).andReturn(futureReplicaLEO)
    expect(replicaManager.logManager).andReturn(logManager).anyTimes()
    stubWithFetchMessages(replica, replica, futureReplica, partition, replicaManager, responseCallback)
