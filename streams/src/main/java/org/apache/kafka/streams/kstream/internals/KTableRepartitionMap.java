...
public class KTableRepartitionMap<K, V, K1, V1> implements
    KTableChangeProcessorSupplier<K, V, KeyValue<K1, V1>, K1, V1> {

    private final KTableImpl<K, ?, V> parent;
    private final KeyValueMapper<? super K, ? super V, KeyValue<K1, V1>> mapper;

    KTableRepartitionMap(final KTableImpl<K, ?, V> parent,
        final KeyValueMapper<? super K, ? super V, KeyValue<K1, V1>> mapper) {
        this.parent = parent;
        this.mapper = mapper;
    }
    ...
    private class KTableMapProcessor extends ContextualProcessor<K, Change<V>, K1, Change<V1>> {
        ...
        public void process(Record<K, Change<V>> record) {
            ...
            final KeyValue<? extends K1, ? extends V1> newPair =
