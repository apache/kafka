...
class ReplicaManager(val config: KafkaConfig,
        ...
        for (topicPartition <- partitions){
          try {
            ...
            case e@ (_: KafkaStorageException | _: IOException) =>
              ...
              getLogDir(topicPartition).foreach(handleLogDirFailure)
          ...
          case Some(log) =>
            try {
              ...
              case e@ (_: IOException | _: KafkaStorageException) =>
                ...
                getLogDir(tp).foreach(handleLogDirFailure)
                throw new KafkaStorageException(s"Error processing fetch operation on partition ${tp}, offset $offset", e)
            }
  ...
  def handleLogDirFailure(dir: String) {
    ...
    replicaStateChangeLock synchronized {
      ...
      stateChangeLogger.warn("Broker %d stopped fetcher for partitions %s because they are in the failed log dir %s"
