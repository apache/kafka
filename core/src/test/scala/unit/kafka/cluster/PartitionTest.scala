...
class PartitionTest {
  ...
  val offsetCheckpoints: LazyOffsetCheckpoints = mock(classOf[LazyOffsetCheckpoints])
  ...
  def testMaybeReplaceCurrentWithFutureReplica(): Unit = {
    ...
    partition.getOrCreateReplica(brokerId, isNew = true, new LazyOffsetCheckpoints(Map.empty))
    logManager.maybeUpdatePreferredLogDir(topicPartition, logDir2.getAbsolutePath)
    partition.maybeCreateFutureReplica(logDir2.getAbsolutePath)
  ...
  def testMaybeReplaceCurrentWithFutureReplicaDifferentBaseOffsets(): Unit = {
    logManager.maybeUpdatePreferredLogDir(topicPartition, logDir1.getAbsolutePath)
    val currentReplica = partition.getOrCreateReplica(brokerId, isNew = true, new LazyOffsetCheckpoints(Map.empty))
    logManager.maybeUpdatePreferredLogDir(topicPartition, logDir2.getAbsolutePath)
    val futureReplica = partition.maybeCreateFutureReplica(logDir2.getAbsolutePath).get
