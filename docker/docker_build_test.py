...
Usage:
    ...
    Example command:-
        docker_build_test.py <image_name> --image-tag <image_tag> --image-type <image_type> --kafka-url <kafka_url> --skip_signing_verification <skip_signing_verification>
        ...
        <skip_signing_verification> controls GPG signature verification, default is false.
...
def build_docker_image(image, tag, kafka_url, image_type, skip_signing_verification="false"):
    image = f'{image}:{tag}'
    build_docker_image_runner(f"docker build -f $DOCKER_FILE -t {image} --build-arg kafka_url={kafka_url} "
                              f"--build-arg build_date={date.today()} --build-arg skip_signing_verification={skip_signing_verification} $DOCKER_DIR", image_type)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--skip_signing_verification", "-s", dest="skip_signing_verification", help="Controls GPG signature verification (default: false)")
    ...
    if args.build_only or not (args.build_only or args.test_only):
        if args.kafka_url:
            build_docker_image(args.image, args.tag, args.kafka_url, args.image_type, args.skip_signing_verification)
        else:
