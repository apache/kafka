...
class ServerGenerateBrokerIdTest extends QuorumTestHarness {
  ...
  def testDisableGeneratedBrokerId(): Unit = {
    ...
    props3.put(KafkaServerConfigs.BROKER_ID_GENERATION_ENABLE_CONFIG, "false")
    // Set reserve broker ids to cause collision and ensure disabling broker id generation ignores the setting
    props3.put(KafkaServerConfigs.RESERVED_BROKER_MAX_ID_CONFIG, "0")
  ...
  def testBrokerMetadataOnIdCollision(): Unit = {
    ...
    propsB.setProperty(KafkaServerConfigs.BROKER_ID_CONFIG, "2")
    ...
    propsB.setProperty(KafkaServerConfigs.BROKER_ID_CONFIG, "3")
