...
object RequestConvertToJson {
  def request(request: AbstractRequest): JsonNode = {
    request match {
      ...
      case req: GetTelemetrySubscriptionRequest => GetTelemetrySubscriptionsRequestDataJsonConverter.write(req.data, request.version)
  ...
  def response(response: AbstractResponse, version: Short): JsonNode = {
    response match {
      ...
      case res: GetTelemetrySubscriptionResponse => GetTelemetrySubscriptionsResponseDataJsonConverter.write(res.data, version)
