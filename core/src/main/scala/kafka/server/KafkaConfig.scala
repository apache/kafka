...
object KafkaConfig {
  ...
  val ListenerSecurityProtocolMapDoc = "Map between listener names and security protocols. This must be defined for " +
    "the same security protocol to be usable in more than one port or IP. For example, users can separate internal and " +
    "external traffic even if SSL is required for both. Concretely, users could define listeners with names INTERNAL " +
    ...
    "Users can provide different security (SSL and SASL) settings for each listener name by adding a normalised " +
    "prefix (the listener name is lowercased) to the config name. For example, if the user wanted to set a different " +
    "keystore for the INTERNAL listener, we would set a config with name `listener.name.internal.ssl.keystore.location`. " +
