...
private[group] class GroupCoordinatorAdapter(
  ...
  override def deleteShareGroupOffsets(
     context: RequestContext,
     request: DeleteShareGroupOffsetsRequestData
  ): CompletableFuture[DeleteShareGroupOffsetsResponseData] = {
    FutureUtils.failedFuture(Errors.UNSUPPORTED_VERSION.exception(
      s"The old group coordinator does not support ${ApiKeys.DELETE_SHARE_GROUP_OFFSETS.name} API."
    ))
  }
