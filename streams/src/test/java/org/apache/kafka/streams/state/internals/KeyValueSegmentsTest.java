...
public class KeyValueSegmentsTest {
    ...
    public void createContext() {
        ...
        segments = new KeyValueSegments(storeName, METRICS_SCOPE, RETENTION_PERIOD, SEGMENT_INTERVAL, txnMechanism);
    ...
    public void shouldBaseSegmentIntervalOnRetentionAndNumSegments() {
        final KeyValueSegments segments = new KeyValueSegments(
            "test",
            METRICS_SCOPE,
            8 * SEGMENT_INTERVAL,
            2 * SEGMENT_INTERVAL,
    ...
    public void shouldGetCorrectSegmentString() {
        ...
        if (txnMechanism == null) {
            segmentString = "KeyValueSegment(id=0, name=test.0)";
        } else {
            segmentString = "TransactionalKeyValueSegment(id=0, name=transactional-test.0)";
        }
    ...
    public void shouldOpenExistingSegments() {
        segments = new KeyValueSegments("test",  METRICS_SCOPE, 4, 1, null);
        segments.openExisting(context, -1L);
    ...
    public void shouldUpdateSegmentFileNameFromOldDateFormatToNewFormat() throws Exception {
        ...
        segments = new KeyValueSegments(storeName,  METRICS_SCOPE, NUM_SEGMENTS * segmentInterval, segmentInterval, null);
