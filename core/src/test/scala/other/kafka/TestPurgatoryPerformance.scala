...
object TestPurgatoryPerformance {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (verbose) {
      ...
      println(
        "# <elapsed time ms>\t<target rate>\t<actual rate>\t<process cpu time ms>\t%s\t%s".format(gcCountHeader,
                                                                                                  gcTimeHeader)
      )
    }
  ...
  private class LatencySamples(sampleSize: Int, pct75: Double, pct50: Double) {
    ...
    def printStats(): Unit = {
      ...
      println(
        "# latency samples: pct75 = %d, pct50 = %d, min = %d, max = %d".format(p75, p50, samples.min, samples.max)
      )
    }
  }
