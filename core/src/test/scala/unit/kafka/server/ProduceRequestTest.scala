...
class ProduceRequestTest extends BaseRequestTest {
  ...
  def testSimpleProduceRequest(): Unit = {
    ...
    sendAndCheck(MemoryRecords.withRecords(CompressionConfig.of(CompressionType.GZIP),
  ...
  def testProduceToNonReplica(): Unit = {
    ...
    val records = MemoryRecords.withRecords(CompressionConfig.none(), new SimpleRecord("key".getBytes, "value".getBytes))
    val topicPartition = new TopicPartition("topic", partition)
  ...
  def testCorruptLz4ProduceRequest(): Unit = {
    ...
    val memoryRecords = MemoryRecords.withRecords(CompressionConfig.of(CompressionType.LZ4),
  ...
  def testZSTDProduceRequest(): Unit = {
    ...
    val partitionToLeader = TestUtils.createTopic(zkClient, topic, 1, 1, servers, topicConfig)
    val leader = partitionToLeader(partition)
    val memoryRecords = MemoryRecords.withRecords(CompressionConfig.of(CompressionType.ZSTD),
