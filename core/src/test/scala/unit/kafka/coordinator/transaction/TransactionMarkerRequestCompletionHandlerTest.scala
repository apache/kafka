...
class TransactionMarkerRequestCompletionHandlerTest {
  ...
  private val txnIdAndMarkers =
    Utils.mkList(
      TxnIdAndMarkerEntry(transactionalId,
                          new WriteTxnMarkersRequest.TxnMarkerEntry(producerId,
                                                                    producerEpoch,
                                                                    coordinatorEpoch,
                                                                    txnResult,
                                                                    Utils.mkList(topicPartition)))
    )
  ...
  private val handler =
    new TransactionMarkerRequestCompletionHandler(brokerId, txnStateManager, markerChannelManager, txnIdAndMarkers)

  private def mockCache(): Unit = {
    EasyMock
      .expect(txnStateManager.partitionFor(transactionalId))
      .andReturn(txnTopicPartition)
      .anyTimes()
    EasyMock
      .expect(txnStateManager.getTransactionState(EasyMock.eq(transactionalId)))
  ...
  def shouldReEnqueuePartitionsWhenBrokerDisconnected(): Unit = {
    ...
    EasyMock.expect(
      markerChannelManager.addTxnMarkersToBrokerQueue(transactionalId,
                                                      producerId,
                                                      producerEpoch,
                                                      txnResult,
                                                      coordinatorEpoch,
    ...
    handler.onComplete(
      new ClientResponse(new RequestHeader(ApiKeys.PRODUCE, 0, "client", 1), null, null, 0, 0, true, null, null, null)
    )
  ...
  def shouldThrowIllegalStateExceptionIfErrorCodeNotAvailableForPid(): Unit = {
    ...
    val response = new WriteTxnMarkersResponse(
      new java.util.HashMap[java.lang.Long, java.util.Map[TopicPartition, Errors]]()
    )
  ...
  private def verifyCompleteDelayedOperationOnError(error: Errors): Unit = {
    ...
    var completed = false
    EasyMock
      .expect(markerChannelManager.completeSendMarkersForTxnId(transactionalId))
  ...
  private def verifyRemoveDelayedOperationOnError(error: Errors): Unit = {
    ...
    var removed = false
    EasyMock
      .expect(markerChannelManager.removeMarkersForTxnId(transactionalId))
      .andAnswer(new IAnswer[Unit] {
        override def answer(): Unit =
          removed = true
      })
