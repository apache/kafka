...
class ReplicaFetcherMockBlockingSend(offsets: java.util.Map[TopicPartition, EpochEndOffset],
    ...
    val response = requestBuilder.apiKey() match {
      ...
      case ApiKeys.FETCH =>
        ...
        new FetchResponse(Errors.NONE,
                          new java.util.LinkedHashMap[TopicPartition, FetchResponse.PartitionData[Records]],
                          0,
  ...
  private def request(requestBuilder: Builder[_ <: AbstractRequest]): ClientRequest =
    client.newClientRequest(destination.id.toString, requestBuilder, time.milliseconds(), true)
