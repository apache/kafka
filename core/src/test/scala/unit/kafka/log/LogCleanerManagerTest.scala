...
class LogCleanerManagerTest extends JUnitSuite with Logging {
  ...
  def testCleanableOffsetsForNone(): Unit = {
    ...
    while (log.numberOfSegments < 8) log.appendAsLeader(
      records(log.logEndOffset.toInt, log.logEndOffset.toInt, time.milliseconds()),
      leaderEpoch = 0
    )
    ...
    assertEquals("The first uncleanable offset begins with the active segment.",
  ...
  def testCleanableOffsetsForTime(): Unit = {
    ...
    while (log.numberOfSegments < 4) log.appendAsLeader(records(log.logEndOffset.toInt, log.logEndOffset.toInt, t0),
                                                        leaderEpoch = 0)
    ...
    while (log.numberOfSegments < 8) log.appendAsLeader(records(log.logEndOffset.toInt, log.logEndOffset.toInt, t1),
                                                        leaderEpoch = 0)
  ...
  def testCleanableOffsetsForShortTime(): Unit = {
    ...
    val t0 = time.milliseconds
    while (log.numberOfSegments < 8) log.appendAsLeader(records(log.logEndOffset.toInt, log.logEndOffset.toInt, t0),
                                                        leaderEpoch = 0)
  ...
  def testUndecidedTransactionalDataNotCleanable(): Unit = {
    ...
    log.appendAsLeader(
      MemoryRecords.withTransactionalRecords(CompressionType.NONE,
                                             ...
                                             producerEpoch,
                                             sequence + 2,
                                             new SimpleRecord(time.milliseconds(), "3".getBytes, "c".getBytes)),
      leaderEpoch = 0
    )
    log.roll()
    ...
    var cleanableOffsets =
      LogCleanerManager.cleanableOffsets(log, topicPartition, Map(topicPartition -> 0L), time.milliseconds())
