...
class DeleteTopicTest extends ZooKeeperTestHarness {
    ...
    TestUtils.waitUntilTrue(
      () =>
        servers
          ...
          .forall(_.getLogManager().getLog(topicPartition).isEmpty),
  ...
  def testPartitionReassignmentDuringDeleteTopic() {
    ...
    val reassignPartitionsCommand =
      new ReassignPartitionsCommand(zkClient, None, Map(topicPartition -> newReplicas), adminZkClient = adminZkClient)
  ...
  private def getAllReplicasFromAssignment(topic: String, assignment: Map[Int, Seq[Int]]): Set[PartitionAndReplica] =
    assignment.flatMap {
      case (partition, replicas) =>
        replicas.map { r =>
          new PartitionAndReplica(new TopicPartition(topic, partition), r)
        }
    }.toSet
  ...
  def testIncreasePartitionCountDuringDeleteTopic() {
    ...
    TestUtils.waitUntilTrue(
      () => {
        val replicasInDeletionSuccessful =
          controller.kafkaController.replicaStateMachine.replicasInState(topic, ReplicaDeletionSuccessful)
    ...
    val topicCommandOptions = new TopicCommand.TopicCommandOptions(
      Array("--zookeeper", zkConnect, "--alter", "--topic", topic, "--partitions", "2")
    )
  ...
  def testDeleteTopicDuringAddPartition() {
    ...
    adminZkClient.addPartitions(topic,
  ...
  private def createTestTopicAndCluster(
    topic: String,
    deleteTopicEnabled: Boolean = true,
    replicaAssignment: Map[Int, List[Int]] = expectedReplicaAssignment
  ): Seq[KafkaServer] = {
  ...
  private def createTestTopicAndCluster(topic: String,
                                        brokerConfigs: Seq[Properties],
                                        replicaAssignment: Map[Int, List[Int]]): Seq[KafkaServer] = {
    val topicPartition = new TopicPartition(topic, 0)
    // create brokers
  ...
  private def writeDups(numKeys: Int, numDups: Int, log: Log): Seq[(Int, Int)] = {
    ...
    for (_ <- 0 until numDups; key <- 0 until numKeys) yield {
      val count = counter
      log.appendAsLeader(TestUtils.singletonRecords(value = counter.toString.getBytes, key = key.toString.getBytes),
                         leaderEpoch = 0)
      counter += 1
      (key, count)
    }
  ...
  def testDisableDeleteTopic() {
    ...
    adminZkClient.deleteTopic("test")
    TestUtils.waitUntilTrue(
      () => !zkClient.isTopicMarkedForDeletion(topic),
