...
class LeaderEpochFileCacheTest {
  ...
  def shouldAddEpochAndMessageOffsetToCache() = {
    ...
    assertEquals((Some(2), Some(leo)), cache.endOffsetFor(2)) //should match leo
  }

  @Test
  def shouldReturnLogEndOffsetIfLatestEpochRequested() = {
    def leoFinder() = new LogOffsetMetadata(14)
    ...
    //Then
    assertEquals((Some(2), Some(14)), cache.endOffsetFor(2))
  }
  ...
  def shouldReturnUnsupportedIfNoEpochRecorded(){
    ...
    //Then
    assertEquals((None, None), cache.endOffsetFor(0))
  }
  ...
  def shouldReturnUnsupportedIfRequestedEpochLessThanFirstEpoch(){
    ...
    //Then
    assertEquals((None, None), epochAndOffset)
  }

  @Test
  def shouldGetFirstOffsetOfSubsequentEpochWhenOffsetRequestedForPreviousEpoch() = {
    var leo = Some(0)
    def leoFinder() = new LogOffsetMetadata(leo.get)
    ...
    assertEquals((Some(2), Some(15L)), cache.endOffsetFor(2))
  }
  ...
  def shouldReturnNextAvailableEpochIfThereIsNoExactEpochForTheOneRequested(){
    ...
    //Then
    assertEquals((Some(0), Some(13L)), cache.endOffsetFor(requestedEpoch = 1))
    assertEquals((Some(2), Some(17L)), cache.endOffsetFor(requestedEpoch = 2))
    assertEquals((Some(2), Some(17L)), cache.endOffsetFor(requestedEpoch = 3))
  }
  ...
  def shouldReturnInvalidOffsetIfEpochIsRequestedWhichIsNotCurrentlyTracked(): Unit = {
    ...
    //Then
    assertEquals((None, None), cache.endOffsetFor(3))
  }

  @Test
  def shouldSupportEpochsThatDoNotStartFromZero(): Unit = {
    val leo = 7
    def leoFinder() = new LogOffsetMetadata(7)
    ...
    //Then
    assertEquals((Some(2), Some(leo)), cache.endOffsetFor(2))
    assertEquals(1, cache.epochEntries.size)
  ...
  def shouldIncreaseAndTrackEpochsAsLeadersChangeManyTimes(): Unit = {
    ...
    assertEquals((Some(1), Some(0)), cache.endOffsetFor(1))
    //offset for epoch 0 should still be 0
    assertEquals((Some(0), Some(0)), cache.endOffsetFor(0))

    //When we write 5 messages as epoch 1
    leo = Some(5)

    //Then end offset for epoch(1) should be leo =>Some( )5
    assertEquals((Some(1), Some(5)), cache.endOffsetFor(1))
    ...
    leo = Some(10 )//write another 5 messages

    //Then end offset for epoch(2) should be leo =>Some( )10
    assertEquals((Some(2), Some(10)), cache.endOffsetFor(2))
  ...
  def shouldIncreaseAndTrackEpochsAsFollowerReceivesManyMessages(): Unit = {
    ...
    cache.assign(epoch = 0, offset = 0); leo = Some(1)
    cache.assign(epoch = 0, offset = 1); leo = Some(2)
    cache.assign(epoch = 0, offset = 2); leo = Some(3)

    //Then epoch should stay, offsets should grow
    assertEquals(Some(0), cache.latestEpoch)
    assertEquals((Some(0), leo), cache.endOffsetFor(0))

    //When messages arrive with greater epoch
    cache.assign(epoch = 1, offset = 3); leo = Some(4)
    cache.assign(epoch = 1, offset = 4); leo = Some(5)
    cache.assign(epoch = 1, offset = 5); leo = Some(6)

    assertEquals(Some(1), cache.latestEpoch)
    assertEquals((Some(1), leo), cache.endOffsetFor(1))

    //When
    cache.assign(epoch = 2, offset = 6); leo = Some(7)
    cache.assign(epoch = 2, offset = 7); leo = Some(8)
    cache.assign(epoch = 2, offset = 8); leo = Some(9)

    assertEquals(Some(2), cache.latestEpoch)
    assertEquals((Some(2), leo), cache.endOffsetFor(2))

    //Older epochs should return the start offset of the first message in the subsequent epoch.
    assertEquals((Some(0), Some(3)), cache.endOffsetFor(0))
    assertEquals((Some(1), Some(6)), cache.endOffsetFor(1))
  }
  ...
  def shouldNotResetEpochHistoryHeadIfUndefinedPassed(): Unit = {
    ...
    //When reset to offset on epoch boundary
    cache.clearAndFlushLatest(offset = -1)
  ...
  def shouldFetchLatestEpochOfEmptyCache(): Unit = {
    ...
    //Then
    assertTrue(cache.latestEpoch.isEmpty)
  }
  ...
  def shouldFetchEndOffsetOfEmptyCache(): Unit = {
    ...
    //Then
    assertEquals((None, None), cache.endOffsetFor(7))
  }
