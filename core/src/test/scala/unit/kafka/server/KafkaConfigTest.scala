...
import org.apache.kafka.common.config.{ConfigException, TopicConfig, ZkConfig}
...
class KafkaConfigTest {
  ...
  def testAdvertiseDefaults(): Unit = {
    ...
    props.setProperty(ZkConfig.ZK_CONNECT_CONFIG, "localhost:2181")
  ...
  def testFromPropsInvalid(): Unit = {
    def baseProperties: Properties = {
      ...
      validRequiredProperties.setProperty(ZkConfig.ZK_CONNECT_CONFIG, "127.0.0.1:2181")
    ...
    KafkaConfig.configNames.foreach { name =>
      name match {
        case ZkConfig.ZK_CONNECT_CONFIG => // ignore string
        case ZkConfig.ZK_SESSION_TIMEOUT_MS_CONFIG => assertPropertyInvalid(baseProperties, name, "not_a_number")
        case ZkConfig.ZK_CONNECTION_TIMEOUT_MS_CONFIG => assertPropertyInvalid(baseProperties, name, "not_a_number")
        case ZkConfig.ZK_ENABLE_SECURE_ACLS_CONFIG => assertPropertyInvalid(baseProperties, name, "not_a_boolean")
        case ZkConfig.ZK_MAX_IN_FLIGHT_REQUESTS_CONFIG => assertPropertyInvalid(baseProperties, name, "not_a_number", "0")
        case ZkConfig.ZK_SSL_CLIENT_ENABLE_CONFIG => assertPropertyInvalid(baseProperties, name, "not_a_boolean")
        case ZkConfig.ZK_CLIENT_CNXN_SOCKET_CONFIG =>  //ignore string
        case ZkConfig.ZK_SSL_KEY_STORE_LOCATION_CONFIG =>  //ignore string
        case ZkConfig.ZK_SSL_KEY_STORE_PASSWORD_CONFIG =>  //ignore string
        case ZkConfig.ZK_SSL_KEY_STORE_TYPE_CONFIG =>  //ignore string
        case ZkConfig.ZK_SSL_TRUST_STORE_LOCATION_CONFIG =>  //ignore string
        case ZkConfig.ZK_SSL_TRUST_STORE_PASSWORD_CONFIG =>  //ignore string
        case ZkConfig.ZK_SSL_TRUST_STORE_TYPE_CONFIG =>  //ignore string
        case ZkConfig.ZK_SSL_PROTOCOL_CONFIG =>  //ignore string
        case ZkConfig.ZK_SSL_ENABLED_PROTOCOLS_CONFIG =>  //ignore string
        case ZkConfig.ZK_SSL_CIPHER_SUITES_CONFIG =>  //ignore string
        case ZkConfig.ZK_SSL_ENDPOINT_IDENTIFICATION_ALGORITHM_CONFIG => //ignore string
        case ZkConfig.ZK_SSL_CRL_ENABLE_CONFIG => assertPropertyInvalid(baseProperties, name, "not_a_boolean")
        case ZkConfig.ZK_SSL_OCSP_ENABLE_CONFIG => assertPropertyInvalid(baseProperties, name, "not_a_boolean")
  ...
  def testSpecificProperties(): Unit = {
    ...
    defaults.setProperty(ZkConfig.ZK_CONNECT_CONFIG, "127.0.0.1:2181")
    // For ZkConnectionTimeoutMs
    defaults.setProperty(ZkConfig.ZK_SESSION_TIMEOUT_MS_CONFIG, "1234")
  ...
  def testNonroutableAdvertisedListeners(): Unit = {
    val props = new Properties()
    props.setProperty(ZkConfig.ZK_CONNECT_CONFIG, "127.0.0.1:2181")
