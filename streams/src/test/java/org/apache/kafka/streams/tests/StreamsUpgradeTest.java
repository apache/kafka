...
public class StreamsUpgradeTest {
    ...
    private static class FutureSubscriptionInfo extends SubscriptionInfo {
        ...
        public ByteBuffer encode() {
            ...
            final ByteBuffer buf = super.encodeFutureVersion(SubscriptionInfo.LATEST_SUPPORTED_VERSION + 1);
            buf.rewind();
            return buf;
        }
        ...
        public static SubscriptionInfo decode(final ByteBuffer data) {
            ...
            if (usedVersion <= SubscriptionInfo.LATEST_SUPPORTED_VERSION) {
                subscriptionInfo =  SubscriptionInfo.decode(data);
            } else {
                if (usedVersion == SubscriptionInfo.LATEST_SUPPORTED_VERSION + 1) {
                    subscriptionInfo = new FutureSubscriptionInfo(
                        usedVersion,
                        usedVersion,
                        null,
                        null,
                        null,
                        null);
                    SubscriptionInfo.decodeLatestVersionData(subscriptionInfo, data);
                } else {
                    subscriptionInfo = new FutureSubscriptionInfo(
                        usedVersion,
                        UNKNOWN,
