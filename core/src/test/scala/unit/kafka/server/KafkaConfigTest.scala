    ...
    ServerTopicConfigSynonyms.TOPIC_CONFIG_SYNONYMS.forEach { case (logConfig, kafkaConfigProp) =>
      logConfig match {
        ...
        case TopicConfig.COMPRESSION_GZIP_BUFFER_CONFIG =>
          assertDynamic(kafkaConfigProp, "16384", () => config.gzipBufferSize)
        case TopicConfig.COMPRESSION_GZIP_STRATEGY_CONFIG =>
          assertDynamic(kafkaConfigProp, "2", () => config.gzipStrategy)
        ...
        case TopicConfig.COMPRESSION_LZ4_BLOCK_CONFIG =>
          assertDynamic(kafkaConfigProp, "65536", () => config.lz4BlockSize)
        ...
        case TopicConfig.COMPRESSION_ZSTD_WINDOW_CONFIG =>
          assertDynamic(kafkaConfigProp, "8", () => config.zstdWindowSize)
        case TopicConfig.COMPRESSION_ZSTD_WORKERS_CONFIG =>
          assertDynamic(kafkaConfigProp, "4", () => config.zstdWorkers)
        case TopicConfig.COMPRESSION_SNAPPY_BLOCK_CONFIG =>
          assertDynamic(kafkaConfigProp, "65536", () => config.snappyBlockSize)
