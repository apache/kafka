...
class ConsumeGroupBenchTest(Test):
    def __init__(self, test_context):
        """:type test_context: ducktape.tests.test.TestContext"""
        super(ConsumeGroupBenchTest, self).__init__(test_context)
        ...
        self.consumer_workload_service_1 = ConsumeBenchWorkloadService(test_context, self.kafka)
        self.consumer_workload_service_2 = ConsumeBenchWorkloadService(test_context, self.kafka)
        self.active_topics = {"consume_group_bench_topic[0-10]": {"numPartitions": 1, "replicationFactor": 3}}
        self.trogdor = TrogdorService(context=self.test_context,
                                      client_services=[self.kafka, self.producer_workload_service,
                                                       self.consumer_workload_service_1,
    ...
    def test_consume_group_bench(self):
        """
        Runs a ProduceBench workload to produce messages to a topic
            and then two ConsumeBench workloads in the same consumer group to read said messages from the topic
        """
        ...
        consume_spec = ConsumeBenchWorkloadSpec(0, TaskSpec.MAX_DURATION_MS,
                                                     self.consumer_workload_service_1.consumer_node,
                                                     self.consumer_workload_service_1.bootstrap_servers,
                                                     target_messages_per_sec=1000,
                                                     max_messages=2000,
                                                     consumer_conf={},
                                                     admin_client_conf={},
                                                     common_client_conf={},
                                                     consumer_group="testGroup",  # passing in consumer_group activates group functionality
                                                     active_topics=self.active_topics)
