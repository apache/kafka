...
public class DescribeTopicPartitionsResponse extends AbstractResponse {
    ...
    public static TopicPartitionInfo partitionToTopicPartitionInfo(
        ...
        return new TopicPartitionInfo(
            ...
            partition.replicaNodes().stream().map(id -> nodes.getOrDefault(id, new Node(id, "", -1))).toList(),
            partition.isrNodes().stream().map(id -> nodes.getOrDefault(id, new Node(id, "", -1))).toList(),
            partition.eligibleLeaderReplicas().stream().map(id -> nodes.getOrDefault(id, new Node(id, "", -1))).toList(),
            partition.lastKnownElr().stream().map(id -> nodes.getOrDefault(id, new Node(id, "", -1))).toList());
    }
}
