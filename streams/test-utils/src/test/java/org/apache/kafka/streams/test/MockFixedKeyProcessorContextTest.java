...
public class MockFixedKeyProcessorContextTest {
    ...
    public void shouldCaptureOutputRecords() {
        final FixedKeyProcessor<String, Long, Long> processor = new FixedKeyProcessor<String, Long, Long>() {
        ...
        final MockFixedKeyProcessorContext<String, Long> context = new MockFixedKeyProcessorContext<>();
        processor.init(context);

        processor.process(InternalFixedKeyRecordFactory.create(new Record<>("foo", 5L, 0L)));
        processor.process(InternalFixedKeyRecordFactory.create(new Record<>("barbaz", 50L, 0L)));

        final List<CapturedForward<? extends String, ? extends Long>> actual = context.forwarded();
        final List<CapturedForward<String, Long>> expected = asList(new CapturedForward<>(new Record<>("foo", 8L, 0L)), new CapturedForward<>(new Record<>("barbaz", 56L, 0L)));
    ...
    public void shouldCaptureRecordsOutputToChildByName() {
        final FixedKeyProcessor<String, Long, Long> processor = new FixedKeyProcessor<String, Long, Long>() {
            ...
            public void process(final FixedKeyRecord<String, Long> record) {
                ...
                final Long value = record.value();
                if (count == 0) {
                    context.forward(InternalFixedKeyRecordFactory.create(new Record<>("start", -1L, 0L))); // broadcast
                }
                final String toChild = count % 2 == 0 ? "george" : "pete";
                context.forward(InternalFixedKeyRecordFactory.create(new Record<>(key + value, key.length() + value, 0L)), toChild);
                count++;
            }
...

        {
            ...
            final List<CapturedForward<? extends String, ? extends Long>> expected = asList(new CapturedForward<>(new Record<>("start", -1L, 0L), Optional.empty()), new CapturedForward<>(new Record<>("foo5", 8L, 0L), Optional.of("george")), new CapturedForward<>(new Record<>("barbaz50", 56L, 0L), Optional.of("pete")));
        ...
        }
        {
            ...
            final List<CapturedForward<? extends String, ? extends Long>> expected = asList(new CapturedForward<>(new Record<>("start", -1L, 0L), Optional.empty()), new CapturedForward<>(new Record<>("foo5", 8L, 0L), Optional.of("george")));

            assertThat(forwarded, is(expected));
        }
        {
            final List<CapturedForward<? extends String, ? extends Long>> forwarded = context.forwarded("pete");
            final List<CapturedForward<? extends String, ? extends Long>> expected = asList(new CapturedForward<>(new Record<>("start", -1L, 0L), Optional.empty()), new CapturedForward<>(new Record<>("barbaz50", 56L, 0L), Optional.of("pete")));

            assertThat(forwarded, is(expected));
        }
        {
            final List<CapturedForward<? extends String, ? extends Long>> forwarded = context.forwarded("steve");
            final List<CapturedForward<? extends String, ? extends Long>> expected = singletonList(new CapturedForward<>(new Record<>("start", -1L, 0L)));
    ...
    public void shouldCaptureCommitsAndAllowReset() {
        final FixedKeyProcessor<String, Long, Void> processor = new FixedKeyProcessor<String, Long, Void>() {
        ...
        final MockFixedKeyProcessorContext<String, Void> context = new MockFixedKeyProcessorContext<>();
        ...
        processor.process(InternalFixedKeyRecordFactory.create(new Record<>("foobar", 500L, 0L)));
    ...
    public void shouldStoreAndReturnStateStores() {
        ...
        processor.process(InternalFixedKeyRecordFactory.create(new Record<>("foo", 5L, 0L)));
        processor.process(InternalFixedKeyRecordFactory.create(new Record<>("bar", 50L, 0L)));
    ...
    public void shouldCaptureApplicationAndRecordMetadata() {
        ...
        final FixedKeyProcessor<String, Object, Object> processor = new FixedKeyProcessor<String, Object, Object>() {
            ...
            public void process(final FixedKeyRecord<String, Object> record) {
                ...
                context.forward(InternalFixedKeyRecordFactory.create(new Record<String, Object>("record", new Record<>(record.key(), record.value(), record.timestamp(), record.headers()), 0L)));
            }
        };

        final MockFixedKeyProcessorContext<String, Object> context = new MockFixedKeyProcessorContext<>(config);
        processor.init(context);
    ...
    public void shouldCapturePunctuator() {
        ...
        final MockFixedKeyProcessorContext.CapturedPunctuator capturedPunctuator = context.scheduledPunctuators().get(0);
    ...
    public void fullConstructorShouldSetAllExpectedAttributes() {
        ...
        final MockFixedKeyProcessorContext<Void, Void> context = new MockFixedKeyProcessorContext<>(config, new TaskId(1, 1), dummyFile);
