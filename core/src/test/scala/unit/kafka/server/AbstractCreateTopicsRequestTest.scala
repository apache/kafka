...
class AbstractCreateTopicsRequestTest extends BaseRequestTest {
  ...
  protected def validateValidCreateTopicsRequests(request: CreateTopicsRequest): Unit = {
    ...
    request.topics.asScala.foreach {
      case (topic, details) =>
        def verifyMetadata(socketServer: SocketServer) = {
          val metadata =
            sendMetadataRequest(new MetadataRequest.Builder(List(topic).asJava, true).build()).topicMetadata.asScala
          val metadataForTopic = metadata.filter(_.topic == topic).head

          val partitions =
            if (!details.replicasAssignments.isEmpty)
          ...
          if (request.validateOnly) {
            ...
            assertEquals("The topic should have the correct number of partitions",
                         partitions,
                         metadataForTopic.partitionMetadata.size)
            assertEquals("The topic should have the correct replication factor",
                         replication,
  ...
  protected def validateErrorCreateTopicsRequests(request: CreateTopicsRequest,
    ...
    val response =
      requestStruct.map(sendCreateTopicRequestStruct(_, request.version)).getOrElse(sendCreateTopicRequest(request))
  ...
  protected def validateTopicExists(topic: String): Unit = {
    TestUtils.waitUntilMetadataIsPropagated(servers, topic, 0)
    val metadata = sendMetadataRequest(new MetadataRequest.Builder(List(topic).asJava, true).build()).topicMetadata.asScala
  ...
  protected def sendCreateTopicRequestStruct(
  ...
  protected def sendMetadataRequest(request: MetadataRequest,
                                    destination: SocketServer = anySocketServer): MetadataResponse = {
