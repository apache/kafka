...
class DynamicConfigChangeUnitTest {
  ...
  def shouldParseReplicationQuotaProperties(): Unit = {
    ...
    //Given
    props.put(LogConfig.LeaderReplicationThrottledReplicasProp, "0:101,0:102,1:101,1:102")

    //When/Then
    assertEquals(Seq(0,1), configHandler.parseThrottledPartitions(props, 102, LogConfig.LeaderReplicationThrottledReplicasProp))
    assertEquals(Seq(), configHandler.parseThrottledPartitions(props, 103, LogConfig.LeaderReplicationThrottledReplicasProp))
  }
  ...
  def shouldParseWildcardReplicationQuotaProperties(): Unit = {
    ...
    //Given
    props.put(LogConfig.LeaderReplicationThrottledReplicasProp, "*")

    //When
    val result = configHandler.parseThrottledPartitions(props, 102, LogConfig.LeaderReplicationThrottledReplicasProp)
  ...
  def shouldParseRegardlessOfWhitespaceAroundValues(): Unit = {
    def parse(configHandler: TopicConfigHandler, value: String): Seq[Int] = {
      configHandler.parseThrottledPartitions(
        CoreUtils.propsWith(LogConfig.LeaderReplicationThrottledReplicasProp, value),
        102, LogConfig.LeaderReplicationThrottledReplicasProp)
    }
    val configHandler: TopicConfigHandler = new TopicConfigHandler(null, null, null, null)
  ...
  def shouldParseReplicationQuotaReset(): Unit = {
    ...
    //Given
    props.put(LogConfig.FollowerReplicationThrottledReplicasProp, "")

    //When
    val result = configHandler.parseThrottledPartitions(props, 102, LogConfig.FollowerReplicationThrottledReplicasProp)
