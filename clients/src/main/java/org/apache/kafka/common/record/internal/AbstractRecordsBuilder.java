...
/**
 * Abstract base for record set builders that accumulate records in a {@link ByteBufferOutputStream}
 * and finalize them into an {@link AbstractRecords}. Contains the shared state and append/compression
 * machinery; subclasses choose how the accumulated bytes are materialized into a concrete records type
 * by implementing {@link #build()} and {@link #close()}.
 */
public abstract class AbstractRecordsBuilder implements AutoCloseable {
    ...
    protected final ByteBufferOutputStream bufferStream;
    ...
    protected AbstractRecords builtRecords;
     ...
     * {@link AbstractRecords} implementation.
     */
    public abstract AbstractRecords build();
    ...
    /**
     * Finalize the batch, write the header, and set {@link #builtRecords}. Subclasses choose how
     * to materialize the accumulated bytes into a concrete records type.
     */
    @Override
    public abstract void close();

    protected void validateProducerState() {
     ...
     */
    protected int writeDefaultBatchHeader() {
     ...
     * @return the written compressed bytes.
     */
    protected int writeLegacyCompressedWrapperHeader() {
        ensureOpenForRecordBatchWrite();
        ByteBuffer buffer = bufferStream.buffer();
        int pos = buffer.position();
        buffer.position(initialPosition);
    ...
    protected void appendDefaultRecord(long offset, long timestamp, ByteBuffer key, ByteBuffer value,
    ...
    protected long appendLegacyRecord(long offset, long timestamp, ByteBuffer key, ByteBuffer value, byte magic) throws IOException {
        ensureOpenForRecordAppend();
    ...
    protected void ensureOpenForRecordBatchWrite() {
        if (isClosed())
