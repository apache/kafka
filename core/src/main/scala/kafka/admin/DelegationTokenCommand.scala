...
object DelegationTokenCommand extends Logging {
  ...
  def createToken(adminClient: JAdminClient, opts: DelegationTokenCommandOptions): DelegationToken = {
    val renewerPrincipals =
      getPrincipals(opts, opts.renewPrincipalsOpt).getOrElse(new util.LinkedList[KafkaPrincipal]())
    val maxLifeTimeMs = opts.options.valueOf(opts.maxLifeTimeOpt).longValue

    println(
      "Calling create token operation with renewers :" + renewerPrincipals + " , max-life-time-period :" + maxLifeTimeMs
    )
    val createDelegationTokenOptions =
      new CreateDelegationTokenOptions().maxlifeTimeMs(maxLifeTimeMs).renewers(renewerPrincipals)
  ...
  def printToken(tokens: List[DelegationToken]): Unit = {
    ...
    print(
      "\n%-15s %-30s %-15s %-25s %-15s %-15s %-15s"
        .format("TOKENID", "HMAC", "OWNER", "RENEWERS", "ISSUEDATE", "EXPIRYDATE", "MAXDATE")
    )
    for (token <- tokens) {
  ...
  private def getPrincipals(
    opts: DelegationTokenCommandOptions,
  ...
  def renewToken(adminClient: JAdminClient, opts: DelegationTokenCommandOptions): Long = {
    ...
    val renewResult = adminClient.renewDelegationToken(
      Base64.getDecoder.decode(hmac),
      new RenewDelegationTokenOptions().renewTimePeriodMs(renewTimePeriodMs)
    )
  ...
  def expireToken(adminClient: JAdminClient, opts: DelegationTokenCommandOptions): Long = {
    ...
    val expireResult = adminClient.expireDelegationToken(
      Base64.getDecoder.decode(hmac),
  ...
  def describeToken(adminClient: JAdminClient, opts: DelegationTokenCommandOptions): List[DelegationToken] = {
    ...
    val describeResult =
      adminClient.describeDelegationToken(new DescribeDelegationTokenOptions().owners(ownerPrincipals.orNull))
  ...
  class DelegationTokenCommandOptions(args: Array[String]) {
    ...
    val createOpt = parser.accepts(
      "create",
      "Create a new delegation token. Use --renewer-principal option to pass renewers principals."
    )
    val renewOpt =
      parser.accepts("renew", "Renew delegation token. Use --renew-time-period option to set renew time period.")
    val expiryOpt =
      parser.accepts("expire", "Expire delegation token. Use --expiry-time-period option to expire the token.")
    val describeOpt = parser.accepts(
      "describe",
      "Describe delegation tokens for the given principals. Use --owner-principal to pass owner/renewer principals." +
    ...
    val ownerPrincipalsOpt = parser
      .accepts("owner-principal", "owner is a kafka principal. It is should be in principalType:name format.")
      .withOptionalArg()
      .ofType(classOf[String])

    val renewPrincipalsOpt = parser
      .accepts("renewer-principal", "renewer is a kafka principal. It is should be in principalType:name format.")
      .withOptionalArg()
      .ofType(classOf[String])

    val maxLifeTimeOpt = parser
      .accepts(
        "max-life-time-period",
        "Max life period for the token in milliseconds. If the value is -1," +
    ...
    val renewTimePeriodOpt = parser
      .accepts(
        "renew-time-period",
        "Renew time period in milliseconds. If the value is -1, then the" +
          " renew time period will default to a server side config value (delegation.token.expiry.time.ms)."
      )
      .withOptionalArg()
      .ofType(classOf[Long])

    val expiryTimePeriodOpt = parser
      .accepts("expiry-time-period",
               "Expiry time period in milliseconds. If the value is -1, then the" +
                 " token will get invalidated immediately.")
      .withOptionalArg()
      .ofType(classOf[Long])

    val hmacOpt = parser
      .accepts("hmac", "HMAC of the delegation token")
      .withOptionalArg
      .ofType(classOf[String])
    ...
    def checkArgs() {
      ...
      CommandLineUtils.checkInvalidArgs(parser,
                                        options,
                                        createOpt,
                                        Set(hmacOpt, renewTimePeriodOpt, expiryTimePeriodOpt, ownerPrincipalsOpt))
      CommandLineUtils.checkInvalidArgs(
        parser,
        options,
        renewOpt,
        Set(renewPrincipalsOpt, maxLifeTimeOpt, expiryTimePeriodOpt, ownerPrincipalsOpt)
      )
      CommandLineUtils.checkInvalidArgs(parser,
                                        options,
                                        expiryOpt,
      ...
      CommandLineUtils.checkInvalidArgs(
        ...
        options,
        describeOpt,
        Set(renewTimePeriodOpt, maxLifeTimeOpt, hmacOpt, renewTimePeriodOpt, expiryTimePeriodOpt)
      )
    }
  }
}
