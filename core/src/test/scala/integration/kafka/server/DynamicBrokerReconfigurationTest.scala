...
import org.apache.kafka.storage.internals.log.{CleanerConfig, LogCleanerConfig, LogConfig}
...
class DynamicBrokerReconfigurationTest extends QuorumTestHarness with SaslSetup {
  ...
  def testConfigDescribeUsingAdminClient(quorum: String): Unit = {
    ...
    expectedProps.setProperty(LogCleanerConfig.LOG_CLEANER_THREADS_PROP, "1")
    ...
    verifyConfig(KafkaConfig.LogRollTimeHoursProp, logRollHours,
      isSensitive = false, isReadOnly = true, expectedProps)
    val logCleanerThreads = configEntry(configDesc, LogCleanerConfig.LOG_CLEANER_THREADS_PROP)
    verifyConfig(LogCleanerConfig.LOG_CLEANER_THREADS_PROP, logCleanerThreads,
      isSensitive = false, isReadOnly = false, expectedProps)
    ...
    assertEquals(List((LogCleanerConfig.LOG_CLEANER_THREADS_PROP, ConfigSource.DEFAULT_CONFIG)), synonymsList(logCleanerThreads))
  }
  ...
  def testLogCleanerConfig(quorum: String): Unit = {
    ...
    props.put(LogCleanerConfig.LOG_CLEANER_THREADS_PROP, "2")
    props.put(LogCleanerConfig.LOG_CLEANER_DEDUPE_BUFFER_SIZE_PROP, "20000000")
    props.put(LogCleanerConfig.LOG_CLEANER_DEDUPE_BUFFER_LOAD_FACTOR_PROP, "0.8")
    props.put(LogCleanerConfig.LOG_CLEANER_IO_BUFFER_SIZE_PROP, "300000")
    props.put(KafkaConfig.MessageMaxBytesProp, "40000")
    props.put(LogCleanerConfig.LOG_CLEANER_IO_MAX_BYTES_PER_SECOND_PROP, "50000000")
    props.put(LogCleanerConfig.LOG_CLEANER_BACKOFF_MS_PROP, "6000")
    ...
    TestUtils.waitUntilTrue(() => {
      reconfigureServers(props, perBrokerConfig = false, (LogCleanerConfig.LOG_CLEANER_THREADS_PROP, "2"))
    ...
    props.put(LogCleanerConfig.LOG_CLEANER_BACKOFF_MS_PROP, "8000")
    reconfigureServers(props, perBrokerConfig = false, (LogCleanerConfig.LOG_CLEANER_BACKOFF_MS_PROP, "8000"))
    verifyThreads("kafka-log-cleaner-thread-", countPerBroker = 2)
  ...
  def testDefaultTopicConfig(): Unit = {
    ...
    props.put(LogCleanerConfig.LOG_CLEANER_DELETE_RETENTION_MS_PROP, TimeUnit.DAYS.toMillis(1).toString)
    props.put(LogCleanerConfig.LOG_CLEANER_MIN_COMPACTION_LAG_MS_PROP, "60000")
    props.put(KafkaConfig.LogDeleteDelayMsProp, "60000")
    props.put(LogCleanerConfig.LOG_CLEANER_MIN_CLEAN_RATIO_PROP, "0.3")
