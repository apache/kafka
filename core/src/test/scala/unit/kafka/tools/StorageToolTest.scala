...
Found problem:
  The kafka configuration file appears to be for a legacy cluster, but the directories are formatted for a cluster in Raft mode.
...
Found problem:
  The kafka configuration file appears to be for a cluster in Raft mode, but the directories are formatted for legacy mode.
