    ...
    if (remoteFetchInfos.isEmpty && (params.maxWaitMs <= 0 || fetchInfos.isEmpty || bytesReadable >= params.minBytes || errorReadingData ||
      ...
      if (!remoteFetchInfos.isEmpty) {
        ...
        val delayedFetch = new DelayedFetch(
          params = params,
          fetchPartitionStatus = fetchPartitionStatus.asScala.toSeq,
          replicaManager = this,
          quota = quota,
        ...
        // create a list of (topic, partition) pairs to use as keys for this delayed fetch operation
        val delayedFetchKeys = fetchPartitionStatus.keySet().stream().map(tp => new TopicPartitionOperationKey(tp)).toList
        // try to complete the request immediately, otherwise put it into the purgatory;
