...
public class ConnectWorkerIntegrationTest {
    ...
    @Test
    public void testFailedSourceConnectorDoesNotLeakResources() throws Exception {
        connect = connectBuilder.build();
        // start the clusters
        connect.start();
        ...
        Map<String, String> props = defaultSourceConnectorProps(TOPIC_NAME);
        // Create a resource and fail the connector on startup
        props.put("connector.create.resource", "true");
        props.put("connector.start.inject.error", "true");
        ...
        ConnectorHandle connector = RuntimeHandles.get().connectorHandle(CONNECTOR_NAME);
        // Ensure that the connector's resources are cleaned up
        // after the connector fails to start
        assertFalse(connector.getResourceStatus());
    ...
    @Test
    public void testFailedSinkConnectorDoesNotLeakResources() throws Exception {
        connect = connectBuilder.build();
        // start the clusters
        connect.start();
