...
class AbstractFetcherThreadTest {
  ...
  def testMetricsRemovedOnShutdown(): Unit = {
    ...
    fetcher.addPartitions(Map(partition -> initialFetchState(Some(topicIds.get(partition.topic())),0L, leaderEpoch = 0)))
  ...
  def testUnknownLeaderEpochInTruncation(): Unit = {
    ...
    fetcher.setReplicaState(partition, replicaState)
    fetcher.addPartitions(Map(partition -> initialFetchState(Some(topicIds.get(partition.topic())), 0L, leaderEpoch = 1)), forceTruncation = true)
  ...
  def testUnknownLeaderEpochWhileFetching(): Unit = {
    ...
    val replicaState = MockFetcherThread.PartitionState(leaderEpoch = 1)
    fetcher.setReplicaState(partition, replicaState)
    fetcher.addPartitions(Map(partition -> initialFetchState(Some(topicIds.get(partition.topic())), 0L, leaderEpoch = 1)))
  ...
  def testTruncation(): Unit = {
    ...
    fetcher.setReplicaState(partition, replicaState)
    fetcher.addPartitions(Map(partition -> initialFetchState(Some(topicIds.get(partition.topic())), 3L, leaderEpoch = 5)))
  ...
  def testTruncateToHighWatermarkIfLeaderEpochRequestNotSupported(): Unit = {
    ...
    fetcher.setReplicaState(partition, replicaState)
    fetcher.addPartitions(Map(partition -> initialFetchState(Some(topicIds.get(partition.topic())), highWatermark, leaderEpoch = 5)))
  ...
  def testTruncationSkippedIfNoEpochChange(): Unit = {
    ...
    fetcher.setReplicaState(partition, replicaState)
    fetcher.addPartitions(Map(partition -> initialFetchState(Some(topicIds.get(partition.topic())), 0L, leaderEpoch = 5)), forceTruncation = true)
  ...
  def testFollowerFetchOutOfRangeHigh(): Unit = {
    ...
    fetcher.setReplicaState(partition, replicaState)
    fetcher.addPartitions(Map(partition -> initialFetchState(Some(topicIds.get(partition.topic())), 3L, leaderEpoch = 4)))
  ...
  def testFencedOffsetResetAfterOutOfRange(): Unit = {
    ...
    val replicaState = MockFetcherThread.PartitionState(replicaLog, leaderEpoch = 4, highWatermark = 0L)
    fetcher.setReplicaState(partition, replicaState)
    fetcher.addPartitions(Map(partition -> initialFetchState(Some(topicIds.get(partition.topic())), 0L, leaderEpoch = 4)))
  ...
  def testFollowerFetchOutOfRangeLow(): Unit = {
    ...
    fetcher.setReplicaState(partition, replicaState)
    fetcher.addPartitions(Map(partition -> initialFetchState(Some(topicIds.get(partition.topic())), 3L, leaderEpoch = 0)))
  ...
  private def testLeaderEpochChangeDuringFetchEpochsFromLeader(leaderEpochOnLeader: Int): Unit = {
    ...
    val fetcher = new MockFetcherThread {
      ...
      override def fetchEpochEndOffsets(partitions: Map[TopicPartition, EpochData]): Map[TopicPartition, EpochEndOffset] = {
        ...
        if (!fetchEpochsFromLeaderOnce) {
          ...
          addPartitions(Map(partition -> initialFetchState(Some(topicIds.get(partition.topic())), 0L, leaderEpoch = nextLeaderEpochOnFollower)), forceTruncation = true)
    ...
    fetcher.addPartitions(Map(partition -> initialFetchState(Some(topicIds.get(partition.topic())), 0L, leaderEpoch = initialLeaderEpochOnFollower)), forceTruncation = true)
  ...
  def testTruncateToEpochEndOffsetsDuringRemovePartitions(): Unit = {
    ...
    fetcher.setReplicaState(partition, MockFetcherThread.PartitionState(leaderEpoch = initialLeaderEpochOnFollower))
    fetcher.addPartitions(Map(partition -> initialFetchState(Some(topicIds.get(partition.topic())), 0L, leaderEpoch = initialLeaderEpochOnFollower)))
  ...
  def testTruncationThrowsExceptionIfLeaderReturnsPartitionsNotRequestedInFetchEpochs(): Unit = {
    ...
    fetcher.setReplicaState(partition, MockFetcherThread.PartitionState(leaderEpoch = 0))
    fetcher.addPartitions(Map(partition -> initialFetchState(Some(topicIds.get(partition.topic())), 0L, leaderEpoch = 0)), forceTruncation = true)
    fetcher.setLeaderState(partition, MockFetcherThread.PartitionState(leaderEpoch = 0))
  ...
  private def verifyFetcherThreadHandlingPartitionFailure(fetcher: MockFetcherThread): Unit = {
    ...
    fetcher.addPartitions(Map(partition1 -> initialFetchState(Some(topicIds.get(partition1.topic())), 0L, leaderEpoch = 0)), forceTruncation = true)
    ...
    fetcher.addPartitions(Map(partition2 -> initialFetchState(Some(topicIds.get(partition2.topic())), 0L, leaderEpoch = 0)), forceTruncation = true)
    ...
    fetcher.addPartitions(Map(partition1 -> initialFetchState(Some(topicIds.get(partition1.topic())), 0L, leaderEpoch = 1)), forceTruncation = true)
