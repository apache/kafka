...
public class StreamsHeartbeatRequestManager implements RequestManager {
    ...
    public StreamsHeartbeatRequestManager(
        ...
        final StreamsAssignmentMetadata streamsAssignmentMetadata
    ) {
        ...
        this.heartbeatState = new StreamsHeartbeatRequestManager.HeartbeatState(subscriptions, membershipManager, maxPollIntervalMs);
        this.heartbeatState.setStreamsGroupMetadata(streamsAssignmentMetadata);
    ...
    StreamsHeartbeatRequestManager(
        ...
        final CoordinatorRequestManager coordinatorRequestManager,
        final MembershipManager membershipManager,
        final StreamsHeartbeatRequestManager.HeartbeatState heartbeatState,
        final StreamsHeartbeatRequestManager.HeartbeatRequestState heartbeatRequestState,
        final BackgroundEventHandler backgroundEventHandler,
        final Metrics metrics) {
    ...
    private void onResponse(final StreamsHeartbeatResponse response, long currentTimeMs) {
        if (Errors.forCode(response.data().errorCode()) == Errors.NONE) {
            ...
            data.setMemberId(response.data().memberId());
            data.setMemberEpoch(response.data().memberEpoch());
            data.setErrorCode(response.data().errorCode());
            data.setErrorMessage(response.data().errorMessage());
            data.setThrottleTimeMs(response.data().throttleTimeMs());
            data.setHeartbeatIntervalMs(response.data().heartbeatIntervalMs());

            // TODO: Use topology to construct assignment
            // data.setAssignment(response.data().assignment());

            membershipManager.onHeartbeatSuccess(data);
            return;
        }
    ...
    static class HeartbeatState {
         ...
         */
        private StreamsAssignmentMetadata streamsAssignmentMetadata;
        ...
        public HeartbeatState(
            ...
            this.streamsAssignmentMetadata = null;
        }

        public void setStreamsGroupMetadata(StreamsAssignmentMetadata streamsAssignmentMetadata) {
            this.streamsAssignmentMetadata = streamsAssignmentMetadata;
        }
        ...
        public StreamsHeartbeatRequestData buildRequestData() {
            ...
            if (sendAllFields || !local.equals(sentFields.localAssignment)) {
                ...
                // data.setActiveTasks(streamsInstanceMetadata.toTasks(local.partitions));
