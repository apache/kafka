...
public class VerifiableSinkTask extends SinkTask {
    ...
    public void start(Properties props) {
        try {
            name = props.getProperty(NAME_CONFIG);
            id = Integer.parseInt(props.getProperty(ID_CONFIG));
        } catch (NumberFormatException e) {
            throw new CopycatException("Invalid VerifiableSourceTask configuration", e);
        }
    }
