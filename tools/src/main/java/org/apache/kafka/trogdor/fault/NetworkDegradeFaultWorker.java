...
public class NetworkDegradeFaultWorker implements TaskWorker {
    ...
    public void start(Platform platform, WorkerStatusTracker status, KafkaFutureImpl<String> haltFuture) throws Exception {
        ...
        for (Map.Entry<String, NetworkDegradeFaultSpec.NodeDegradeSpec> entry : nodeSpecs.entrySet()) {
            if (entry.getKey().equals(curNode.name())) {
                enableTrafficControl(platform, entry.getValue().getNetworkDevice(), entry.getValue().getLatency());
            }
        }
    ...
    public void stop(Platform platform) throws Exception {
        ...
        for (Map.Entry<String, NetworkDegradeFaultSpec.NodeDegradeSpec> entry : nodeSpecs.entrySet()) {
            if (entry.getKey().equals(curNode.name())) {
                disableTrafficControl(platform, entry.getValue().getNetworkDevice());
            }
        }
