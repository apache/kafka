...
class Partition(val topicPartition: TopicPartition,
  ...
  /**
    ...
    */
  def maybeCreateFutureReplica(logDir: String): Option[Replica] = {
      ...
      if (currentLog.dir.getParent == logDir)
        ...
        futureLocalReplica match {
          case Some(replica) =>
            ...
            None
          case None =>
            val futureLog = logManager.getOrCreateLog(topicPartition, currentLog.config,
              isNew = false, isFuture = true)
            val futureReplica = new Replica(Request.FutureLocalReplicaId, topicPartition, time,
              initialHighWatermarkValue = 0L, Some(futureLog))
            allReplicasMap.put(Request.FutureLocalReplicaId, futureReplica)
            Some(futureReplica)
        }
      }
    }
  ...
  def maybeLoadFutureReplica(highWatermarkCheckpoint: LazyOffsetCheckpoints): Option[Replica] = {
    logManager.getLog(topicPartition, isFuture = true).map { log =>
      allReplicasMap.getAndMaybePut(Request.FutureLocalReplicaId, {
        val initialHw = initialHighWatermark(log, highWatermarkCheckpoint)
        new Replica(Request.FutureLocalReplicaId, topicPartition, time, initialHw, Some(log))
      })
    }
  }

  private def initialHighWatermark(log: Log, highWatermarkCheckpoint: LazyOffsetCheckpoints): Long = {
    highWatermarkCheckpoint.fetch(log.dir.getParent, topicPartition) match {
      case Some(hw) => math.min(hw, log.logEndOffset)
      case None =>
  ...
  def getOrCreateReplica(replicaId: Int,
                         isNew: Boolean,
                         highWatermarkCheckpoint: LazyOffsetCheckpoints): Replica = {
    allReplicasMap.getAndMaybePut(replicaId, {
      if (isReplicaLocal(replicaId)) {
        ...
        val initialHw = initialHighWatermark(log, highWatermarkCheckpoint)
        new Replica(replicaId, topicPartition, time, initialHw, Some(log))
  ...
  def makeLeader(controllerId: Int,
    ...
    val (leaderHWIncremented, isNewLeader) = inWriteLock(leaderIsrUpdateLock) {
      ...
      val newInSyncReplicas = partitionStateInfo.basePartitionState.isr.asScala.map {
        id => getOrCreateReplica(id, partitionStateInfo.isNew, highWatermarkCheckpoint)
      }.toSet
      // remove assigned replicas that have been removed by the controller
      (assignedReplicas.map(_.brokerId) -- newAssignedReplicas).foreach(removeReplica)
      inSyncReplicas = newInSyncReplicas
      newAssignedReplicas.foreach(id => getOrCreateReplica(id, partitionStateInfo.isNew, highWatermarkCheckpoint))
