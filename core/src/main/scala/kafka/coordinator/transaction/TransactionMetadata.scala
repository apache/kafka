...
sealed private[transaction] trait TransactionState { def byte: Byte }
    ...
    byte match {
      ...
      case unknown =>
        throw new IllegalStateException(
          "Unknown transaction state byte " + unknown + " from the transaction status message"
        )
    }
...
private[transaction] class TransactionMetadata(val transactionalId: String,
  ...
  def removePartition(topicPartition: TopicPartition): Unit = {
    if (state != PrepareCommit && state != PrepareAbort)
      throw new IllegalStateException(
        s"Transaction metadata's current state is $state, and its pending state is $pendingState " +
  ...
  def prepareFenceProducerEpoch(): TxnTransitMetadata = {
    if (producerEpoch == Short.MaxValue)
      throw new IllegalStateException(
        s"Cannot fence producer with epoch equal to Short.MaxValue since this would overflow"
      )

    prepareTransitionTo(PrepareEpochFence,
                        producerId,
  ...
  def prepareIncrementProducerEpoch(newTxnTimeoutMs: Int, updateTimestamp: Long): TxnTransitMetadata = {
    ...
    prepareTransitionTo(Empty,
                        producerId,
                        nextEpoch.toShort,
                        newTxnTimeoutMs,
                        immutable.Set.empty[TopicPartition],
                        -1,
                        updateTimestamp)
  }
  ...
  private def prepareTransitionTo(newState: TransactionState,
    ...
    if (pendingState.isDefined)
      throw new IllegalStateException(
        s"Preparing transaction state transition to $newState " +
    ...
    if (TransactionMetadata.validPreviousStates(newState).contains(state)) {
      val transitMetadata = TxnTransitMetadata(newProducerId,
                                               newEpoch,
                                               newTxnTimeoutMs,
                                               newState,
                                               newTopicPartitions,
      ...
      throw new IllegalStateException(
        s"Preparing transaction state transition to $newState failed since the target state" +
    ...
    if (toState != transitMetadata.txnState) {
      ...
      toState match {
        ...
        case Dead =>
          ...
          throw new IllegalStateException(
            s"TransactionalId $transactionalId is trying to complete a transition to " +
  ...
  private def throwStateTransitionFailure(txnTransitMetadata: TxnTransitMetadata): Unit = {
    ...
    throw new IllegalStateException(
      s"TransactionalId $transactionalId failed transition to state $txnTransitMetadata " +
  ...
  override def hashCode(): Int = {
    val fields = Seq(transactionalId,
                     producerId,
                     producerEpoch,
                     txnTimeoutMs,
                     state,
