...
class AlterReplicaDirsRequestTest extends BaseRequestTest {
  ...
  @Test
  def testAlterReplicaDirsRequestBeforeTopicCreation() {
    val partitionNum = 5
    ...
    val alterReplicaDirResponse = sendAlterReplicaDirsRequest(partitionDirs)
  ...
  @Test
  def testAlterReplicaDirsRequestErrorCode(): Unit = {
    ...
    val alterReplicaDirResponse = sendAlterReplicaDirsRequest(partitionDirs.toMap)
  ...
  private def sendAlterReplicaDirsRequest(partitionDirs: Map[TopicPartition, String], socketServer: SocketServer = controllerSocketServer): AlterReplicaDirsResponse = {
    val request = new AlterReplicaDirsRequest.Builder(partitionDirs.asJava).build()
    val response = connectAndSend(request, ApiKeys.ALTER_REPLICA_DIRS, socketServer)
    AlterReplicaDirsResponse.parse(response, request.version)
  }
