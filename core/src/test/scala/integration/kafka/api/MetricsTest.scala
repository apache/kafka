...
class MetricsTest extends IntegrationTestHarness with SaslSetup {
  ...
  override def setUp(testInfo: TestInfo): Unit = {
    if (testInfo.getDisplayName.contains("testMetrics") && testInfo.getDisplayName.endsWith("true")) {
      ...
      this.serverConfig.setProperty(RemoteLogManagerConfig.REMOTE_LOG_STORAGE_SYSTEM_ENABLE_CONFIG, "true")
      this.serverConfig.setProperty(RemoteLogManagerConfig.REMOTE_STORAGE_MANAGER_CLASS_NAME_CONFIG, classOf[NoOpRemoteStorageManager].getName)
      this.serverConfig.setProperty(RemoteLogManagerConfig.REMOTE_LOG_METADATA_MANAGER_CLASS_NAME_CONFIG, classOf[NoOpRemoteLogMetadataManager].getName)
    }
