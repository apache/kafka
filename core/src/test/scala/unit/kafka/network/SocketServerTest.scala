...
class SocketServerTest extends JUnitSuite {
  ...
  def testGracefulClose() {
    ...
    for (_ <- 0 until 10) {
      ...
      server.dataRequestChannel.sendResponse(new RequestChannel.NoOpResponse(request))
    }
  }
  ...
  def testIdleConnection() {
    ...
    try {
      ...
      processRequest(overrideServer.dataRequestChannel, request2) // this triggers a failed send since channel has been closed
      TestUtils.waitUntilTrue(() => openOrClosingChannel(request2).isEmpty, "Failed to remove channel with failed sends")
      assertNull("Received request after failed send", overrideServer.dataRequestChannel.receiveRequest(200))
  ...
  def testConnectionIdReuse() {
    ...
    val overrideServer = new SocketServer(KafkaConfig.fromProps(props), serverMetrics, time, credentialProvider) {
      override def newProcessor(id: Int, requestChannel: RequestChannel, connectionQuotas: ConnectionQuotas, listenerName: ListenerName,
                                protocol: SecurityProtocol, memoryPool: MemoryPool, isControlPlane: Boolean): Processor = {
        new Processor(id, isControlPlane, time, config.socketRequestMaxBytes, dataRequestChannel, connectionQuotas,
  ...
  def testSessionPrincipal() {
    ...
    assertEquals(KafkaPrincipal.ANONYMOUS, receiveRequest(server.dataRequestChannel).session.principal)
  }
  ...
  def testRequestMetricsAfterStop(): Unit = {
    ...
    for (_ <- 0 to 1) server.dataRequestChannel.metrics(ApiKeys.PRODUCE.name).requestRate(version).mark()
    server.dataRequestChannel.metrics(ApiKeys.PRODUCE.name).requestRate(version2).mark()
    assertEquals(2, server.dataRequestChannel.metrics(ApiKeys.PRODUCE.name).requestRate(version).count())
    server.dataRequestChannel.updateErrorMetrics(ApiKeys.PRODUCE, Map(Errors.NONE -> 1))
  ...
  def closingChannelException(): Unit = {
    withTestableServer { testableServer =>
      ...
      processRequest(testableServer.dataRequestChannel) // Also process request from other channel
      testableSelector.waitForOperations(SelectorOperation.Send, 2)
  ...
  class TestableSocketServer extends SocketServer(KafkaConfig.fromProps(props),
    ...
    override def newProcessor(id: Int, requestChannel: RequestChannel,connectionQuotas: ConnectionQuotas, listenerName: ListenerName,
                                protocol: SecurityProtocol, memoryPool: MemoryPool, isControlPlane: Boolean): Processor = {
      new Processor(id, isControlPlane, time, config.socketRequestMaxBytes, dataRequestChannel, connectionQuotas, config.connectionsMaxIdleMs,
