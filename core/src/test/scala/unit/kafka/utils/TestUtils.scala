...
object TestUtils extends Logging {
  ...
  def createBrokerConfig(nodeId: Int, zkConnect: String,
    ...
    val listeners = {
      ...
      if (enableSasl)
        protocolAndPorts += "SASL_PLAIN" -> saslPort
  ...
  def createNewProducer(brokerList: String,
    ...
    if (enableSSL) {
      ...
      producerProps.put(CommonClientConfigs.SECURITY_PROTOCOL_CONFIG, "SASL_PLAIN")
    }
  ...
  def createNewConsumer(brokerList: String,
    ...
    if (enableSSL) {
      ...
      consumerProps.put(CommonClientConfigs.SECURITY_PROTOCOL_CONFIG, "SASL_PLAIN")
    }
