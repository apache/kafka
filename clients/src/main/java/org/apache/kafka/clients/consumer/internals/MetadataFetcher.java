...
public class MetadataFetcher {
    ...
    private Long offsetResetStrategyTimestamp(final SubscriptionState subscriptions, final TopicPartition partition) {
     ...
     */
    public Map<String, List<PartitionInfo>> getAllTopicMetadata(final Timer timer) {
     ...
     *
     * @param topic The topic for which to retrieve metadata
     * @param allowAutoTopicCreation Allows topic creation
     * @param timer Timer bounding how long this method can block
     * @return The map of topics with their partition information
     */
    public Map<String, List<PartitionInfo>> getTopicMetadata(final String topic,
    ...
    private Map<String, List<PartitionInfo>> getTopicMetadata(MetadataRequest.Builder request, Timer timer) {
        ...
        do {
            ...
            if (node == null)
                future = RequestFuture.noBrokersAvailable();
            else
                future = client.send(node, request);
     ...
     */
    public void resetOffsetsIfNeeded(final Metadata metadata,
        ...
        for (final TopicPartition partition : partitions) {
            Long timestamp = offsetResetStrategyTimestamp(subscriptions, partition);
            if (timestamp != null)
                offsetResetTimestamps.put(partition, timestamp);
        }

        resetOffsetsAsync(metadata, subscriptions, offsetResetTimestamps);
    }

    /**
     * Validate offsets for all assigned partitions for which a leader change has been detected.
     */
    public void validateOffsetsIfNeeded(final Metadata metadata,
                                        final SubscriptionState subscriptions) {
        ...
        validateOffsetsAsync(metadata, subscriptions, partitionsToValidate);
    }

    public Map<TopicPartition, OffsetAndTimestamp> offsetsForTimes(final ConsumerMetadata metadata,
                                                                   final SubscriptionState subscriptions,
        ...
        try {
            Map<TopicPartition, ListOffsetData> fetchedOffsets = fetchOffsetsByTimes(metadata,
                                                                                     subscriptions,
                                                                                     timestampsToSearch,
                                                                                     timer,
                                                                                     true).fetchedOffsets;
    ...
    private ListOffsetResult fetchOffsetsByTimes(final Metadata metadata,
                                                 final SubscriptionState subscriptions,
                                                 final Map<TopicPartition, Long> timestampsToSearch,
                                                 final Timer timer,
        ...
        do {
            RequestFuture<ListOffsetResult> future = sendListOffsetsRequests(metadata, remainingToSearch, requireTimestamps);
    ...
    public Map<TopicPartition, Long> beginningOffsets(final ConsumerMetadata metadata,
                                                      final SubscriptionState subscriptions,
                                                      final Collection<TopicPartition> partitions,
                                                      final Timer timer) {
        return beginningOrEndOffset(metadata,
                                    subscriptions,
                                    partitions,
    ...
    public Map<TopicPartition, Long> endOffsets(final ConsumerMetadata metadata,
                                                final SubscriptionState subscriptions,
                                                final Collection<TopicPartition> partitions,
                                                final Timer timer) {
        return beginningOrEndOffset(metadata,
    ...
    private Map<TopicPartition, Long> beginningOrEndOffset(final ConsumerMetadata metadata,
                                                           final SubscriptionState subscriptions,
                                                           final Collection<TopicPartition> partitions,
        ...
        try {
            ...
            ListOffsetResult result = fetchOffsetsByTimes(metadata, subscriptions, timestampsToSearch, timer, false);
    ...
    // Visible for testing
    void resetOffsetIfNeeded(final Metadata metadata,
        final SubscriptionState subscriptions, TopicPartition partition, OffsetResetStrategy requestedResetStrategy, ListOffsetData offsetData) {
    ...
    private void resetOffsetsAsync(final Metadata metadata,
                                   final SubscriptionState subscriptions,
        ...
        Map<Node, Map<TopicPartition, ListOffsetsPartition>> timestampsToSearchByNode =
                groupListOffsetRequests(metadata, partitionResetTimestamps, new HashSet<>());
     ...
     */
    private void validateOffsetsAsync(final Metadata metadata,
        final SubscriptionState subscriptions, Map<TopicPartition, FetchPosition> partitionsToValidate) {
     ...
     */
    private RequestFuture<ListOffsetResult> sendListOffsetsRequests(final Metadata metadata,
                                                                    final Map<TopicPartition, Long> timestampsToSearch,
                                                                    final boolean requireTimestamps) {
        final Set<TopicPartition> partitionsToRetry = new HashSet<>();
        Map<Node, Map<TopicPartition, ListOffsetsPartition>> timestampsToSearchByNode = groupListOffsetRequests(metadata,
                                                                                                                timestampsToSearch,
                                                                                                                partitionsToRetry);
     ...
     */
    private Map<Node, Map<TopicPartition, ListOffsetsPartition>> groupListOffsetRequests(final Metadata metadata,
                                                                                         final Map<TopicPartition, Long> timestampsToSearch,
        ...
        return regroupPartitionMapByNode(metadata, partitionDataMap);
    }
     ...
     */
    private void validatePositionsOnMetadataChange(final Metadata metadata,
                                                   final SubscriptionState subscriptions) {
    ...
    private <T> Map<Node, Map<TopicPartition, T>> regroupPartitionMapByNode(final Metadata metadata,
                                                                            final Map<TopicPartition, T> partitionMap) {
        return partitionMap.entrySet()
                .stream()
