...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  def handleTxnOffsetCommitRequest(request: RequestChannel.Request): Unit = {
    ...
    if (!authorize(request.session, Write, Resource(TransactionalId, txnOffsetCommitRequest.data.transactionalId, LITERAL)))
      sendErrorResponseMaybeThrottle(request, Errors.TRANSACTIONAL_ID_AUTHORIZATION_FAILED.exception)
    else if (!authorize(request.session, Read, Resource(Group, txnOffsetCommitRequest.data.groupId, LITERAL)))
      sendErrorResponseMaybeThrottle(request, Errors.GROUP_AUTHORIZATION_FAILED.exception)
    else {
      val unauthorizedTopicErrors = mutable.Map[TopicPartition, Errors]()
      val nonExistingTopicErrors = mutable.Map[TopicPartition, Errors]()
