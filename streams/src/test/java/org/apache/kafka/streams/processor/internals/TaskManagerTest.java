...
public class TaskManagerTest {
    ...
    public void shouldClassifyExistingTasksWithoutStateUpdater() {
        ...
        handleAssignment(runningActiveTasks, standbyTasks, restoringActiveTasks, false);
    ...
    public void shouldReleaseLockForUnassignedTasksAfterRebalanceWithStateUpdater() throws Exception {
        ...
        when(consumer.assignment()).thenReturn(assigned);
        doNothing().when(consumer).pause(mkSet(t1p1, t1p2));
    ...
    public void shouldReviveCorruptTasks() {
        ...
        verifyResumeWasCalledWith(consumer, taskId00Partitions);
    }
    ...
    public void shouldCommitNonCorruptedTasksOnTaskCorruptedException() {
        ...
        Mockito.verify(consumer, times(0)).commitSync(emptyMap());
    }
    ...
    public void shouldNotAttemptToCommitInHandleCorruptedDuringARebalance() {
        ...
        final Set<TopicPartition> partitions = union(HashSet::new, taskId00Partitions, taskId01Partitions);
    ...
    public void shouldCloseAndReviveUncorruptedTasksWhenTimeoutExceptionThrownFromCommitDuringRevocationWithALOS() {
        ...
        final Set<TopicPartition> partition = union(HashSet::new, taskId00Partitions, taskId01Partitions, taskId02Partitions);
        when(consumer.assignment()).thenReturn(partition);
    ...
    public void shouldCloseAndReviveUncorruptedTasksWhenTimeoutExceptionThrownFromCommitDuringRevocationWithEOS() {
        ...
        final Set<TopicPartition> partitions = union(HashSet::new, taskId00Partitions, taskId01Partitions, taskId02Partitions);
        when(consumer.assignment()).thenReturn(partitions);
    ...
    private Map<TaskId, StateMachineTask> handleAssignment(final Map<TaskId, Set<TopicPartition>> runningActiveAssignment,
        ...
        return handleAssignment(runningActiveAssignment, standbyAssignment, restoringActiveAssignment, true);
    }

    private Map<TaskId, StateMachineTask> handleAssignment(final Map<TaskId, Set<TopicPartition>> runningActiveAssignment,
                                                           final Map<TaskId, Set<TopicPartition>> standbyAssignment,
                                                           final Map<TaskId, Set<TopicPartition>> restoringActiveAssignment,
                                                           final boolean shouldSetupConsumer) {
    ...
    public void shouldNotFailForTimeoutExceptionOnConsumerCommit() {
        ...
        doThrow(new TimeoutException("KABOOM!")).doNothing().when(consumer).commitSync(Mockito.any(Map.class));
    ...
    public void shouldListNotPausedTasks() {
        handleAssignment(taskId00Assignment, taskId01Assignment, emptyMap(), false);
