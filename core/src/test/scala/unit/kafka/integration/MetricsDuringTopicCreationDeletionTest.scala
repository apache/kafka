...
class MetricsDuringTopicCreationDeletionTest extends KafkaServerTestHarness with Logging {
  ...
  def testMetricsDuringTopicCreateDelete() {
    ...
    @volatile var offlinePartitionsCount = offlinePartitionsCountGauge.getValue()
    assert(offlinePartitionsCount == 0)

    val preferredReplicaImbalanceCountGauge = getGauge("PreferredReplicaImbalanceCount")
    @volatile var preferredReplicaImbalanceCount = preferredReplicaImbalanceCountGauge.getValue()
    ...
    val thread = new Thread(new Runnable {
      def run() {
        while (running) {
          ...
          preferredReplicaImbalanceCount = preferredReplicaImbalanceCountGauge.getValue()
          if (preferredReplicaImbalanceCount > 0) {
             running = false
          }

          offlinePartitionsCount = offlinePartitionsCountGauge.getValue()
  ...
  private def getGauge(metricName: String) = {
    kafka.metrics.getKafkaMetrics()
           .filterKeys(k => k.endsWith(metricName))
           .headOption
