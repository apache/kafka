...
class DynamicBrokerConfigTest {
  ...
  def testImproperConfigsAreRemoved(): Unit = {
    ...
    assertEquals(JKafkaConfig.MAX_CONNECTIONS_DEFAULT, config.maxConnections)
...
class TestDynamicThreadPool() extends BrokerReconfigurable {
  ...
  override def reconfigure(oldConfig: KafkaConfig, newConfig: KafkaConfig): Unit = {
    assertEquals(JKafkaConfig.NUM_IO_THREADS_DEFAULT, oldConfig.numIoThreads)
    assertEquals(JKafkaConfig.BACKGROUND_THREADS_DEFAULT, oldConfig.backgroundThreads)
