...
class RequestChannelTest {
  ...
  def testAlterRequests(): Unit = {
    ...
    def verifyConfig(resource: ConfigResource, entries: java.util.List[ConfigEntry], expectedValues: Map[String, String]): Unit = {
      val alterConfigs = request(new AlterConfigsRequest.Builder(
          java.util.Map.of(resource, new Config(entries)), true).build())
    ...
    verifyConfig(brokerResource, java.util.List.of(keystorePassword), Map(SslConfigs.SSL_KEYSTORE_PASSWORD_CONFIG -> Password.HIDDEN))

    val keystoreLocation = new ConfigEntry(SslConfigs.SSL_KEYSTORE_LOCATION_CONFIG, "/path/to/keystore")
    verifyConfig(brokerResource, java.util.List.of(keystoreLocation), Map(SslConfigs.SSL_KEYSTORE_LOCATION_CONFIG -> "/path/to/keystore"))
    verifyConfig(brokerResource, java.util.List.of(keystoreLocation, keystorePassword),
    ...
    verifyConfig(brokerResource, java.util.List.of(listenerKeyPassword), Map(listenerKeyPassword.name -> Password.HIDDEN))

    val listenerKeystore = new ConfigEntry(s"listener.name.internal.${SslConfigs.SSL_KEYSTORE_LOCATION_CONFIG}", "/path/to/keystore")
    verifyConfig(brokerResource, java.util.List.of(listenerKeystore), Map(listenerKeystore.name -> "/path/to/keystore"))

    val plainJaasConfig = new ConfigEntry(s"listener.name.internal.plain.${SaslConfigs.SASL_JAAS_CONFIG}", sensitiveValue)
    verifyConfig(brokerResource, java.util.List.of(plainJaasConfig), Map(plainJaasConfig.name -> Password.HIDDEN))
    ...
    verifyConfig(brokerResource, java.util.List.of(plainLoginCallback), Map(plainLoginCallback.name -> plainLoginCallback.value))

    val customConfig = new ConfigEntry("custom.config", sensitiveValue)
    verifyConfig(brokerResource, java.util.List.of(customConfig), Map(customConfig.name -> Password.HIDDEN))
    ...
    verifyConfig(topicResource, java.util.List.of(compressionType), Map(TopicConfig.COMPRESSION_TYPE_CONFIG -> "lz4"))
    verifyConfig(topicResource, java.util.List.of(customConfig), Map(customConfig.name -> Password.HIDDEN))
  ...
  def testNonAlterRequestsNotTransformed(): Unit = {
    val metadataRequest = request(new MetadataRequest.Builder(java.util.List.of("topic"), true).build())
  ...
  def testJsonRequests(): Unit = {
    ...
    val keystorePassword = new ConfigEntry(SslConfigs.SSL_KEYSTORE_PASSWORD_CONFIG, sensitiveValue)
    val entries = java.util.List.of(keystorePassword)
