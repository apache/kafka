...
import org.apache.kafka.server.config.DynamicBrokerConfigManager.JDynamicLogConfig
import org.apache.kafka.server.config.{DynamicBrokerConfigBaseManager, KafkaConfig}
...
class DynamicBrokerReconfigurationTest extends QuorumTestHarness with SaslSetup {
  ...
  override def setUp(testInfo: TestInfo): Unit = {
    ...
    (0 until numServers).foreach { brokerId =>
      ...
      val props = if (isKRaftTest()) {
        ...
        properties.put(KafkaConfig.ADVERTISED_LISTENERS_PROP, s"$SecureInternal://localhost:0, $SecureExternal://localhost:0")
        properties
      } else {
        val properties = TestUtils.createBrokerConfig(brokerId, zkConnect)
        properties.put(KafkaConfig.ZK_ENABLE_SECURE_ACLS_PROP, "true")
        properties
      }
      props ++= securityProps(sslProperties1, TRUSTSTORE_PROPS)
      // Ensure that we can support multiple listeners per security protocol and multiple security protocols
      props.put(KafkaConfig.LISTENERS_PROP, s"$SecureInternal://localhost:0, $SecureExternal://localhost:0")
      props.put(KafkaConfig.LISTENER_SECURITY_PROTOCOL_MAP_PROP, s"PLAINTEXT:PLAINTEXT, $SecureInternal:SSL, $SecureExternal:SASL_SSL, CONTROLLER:$controllerListenerSecurityProtocol")
      props.put(KafkaConfig.INTER_BROKER_LISTENER_NAME_PROP, SecureInternal)
      props.put(KafkaConfig.SSL_CLIENT_AUTH_PROP, "requested")
      props.put(KafkaConfig.SASL_MECHANISM_INTER_BROKER_PROTOCOL_PROP, "PLAIN")
      props.put(KafkaConfig.SASL_ENABLED_MECHANISMS_PROP, kafkaServerSaslMechanisms.mkString(","))
      props.put(KafkaConfig.LOG_SEGMENT_BYTES_PROP, "2000") // low value to test log rolling on config update
      props.put(KafkaConfig.NUM_REPLICA_FETCHERS_PROP, "2") // greater than one to test reducing threads
      props.put(KafkaConfig.PASSWORD_ENCODER_SECRET_PROP, "dynamic-config-secret")
      props.put(KafkaConfig.LOG_RETENTION_TIME_MILLIS_PROP, 1680000000.toString)
      props.put(KafkaConfig.LOG_RETENTION_TIME_HOURS_PROP, 168.toString)
  ...
  def testConfigDescribeUsingAdminClient(quorum: String): Unit = {
    ...
    expectedProps.setProperty(KafkaConfig.LOG_RETENTION_TIME_MILLIS_PROP, "1680000000")
    expectedProps.setProperty(KafkaConfig.LOG_RETENTION_TIME_HOURS_PROP, "168")
    expectedProps.setProperty(KafkaConfig.LOG_ROLL_TIME_HOURS_PROP, "168")
    expectedProps.setProperty(KafkaConfig.LOG_CLEANER_DELETE_RETENTION_MS_PROP, "1")
    val logRetentionMs = configEntry(configDesc, KafkaConfig.LOG_RETENTION_TIME_MILLIS_PROP)
    verifyConfig(KafkaConfig.LOG_RETENTION_TIME_MILLIS_PROP, logRetentionMs,
      isSensitive = false, isReadOnly = false, expectedProps)
    val logRetentionHours = configEntry(configDesc, KafkaConfig.LOG_RETENTION_TIME_HOURS_PROP)
    verifyConfig(KafkaConfig.LOG_RETENTION_TIME_HOURS_PROP, logRetentionHours,
      isSensitive = false, isReadOnly = true, expectedProps)
    val logRollHours = configEntry(configDesc, KafkaConfig.LOG_ROLL_TIME_HOURS_PROP)
    verifyConfig(KafkaConfig.LOG_ROLL_TIME_HOURS_PROP, logRollHours,
      isSensitive = false, isReadOnly = true, expectedProps)
    val logCleanerThreads = configEntry(configDesc, KafkaConfig.LOG_CLEANER_DELETE_RETENTION_MS_PROP)
    verifyConfig(KafkaConfig.LOG_CLEANER_DELETE_RETENTION_MS_PROP, logCleanerThreads,
      isSensitive = false, isReadOnly = false, expectedProps)
    ...
    assertEquals(List((KafkaConfig.LOG_RETENTION_TIME_MILLIS_PROP, ConfigSource.STATIC_BROKER_CONFIG),
      (KafkaConfig.LOG_RETENTION_TIME_HOURS_PROP, ConfigSource.STATIC_BROKER_CONFIG),
      (KafkaConfig.LOG_RETENTION_TIME_HOURS_PROP, ConfigSource.DEFAULT_CONFIG)),
      synonymsList(logRetentionMs))
    assertEquals(List((KafkaConfig.LOG_RETENTION_TIME_HOURS_PROP, ConfigSource.STATIC_BROKER_CONFIG),
      (KafkaConfig.LOG_RETENTION_TIME_HOURS_PROP, ConfigSource.DEFAULT_CONFIG)),
      synonymsList(logRetentionHours))
    assertEquals(List((KafkaConfig.LOG_ROLL_TIME_HOURS_PROP, ConfigSource.DEFAULT_CONFIG)), synonymsList(logRollHours))
    assertEquals(List((KafkaConfig.LOG_CLEANER_DELETE_RETENTION_MS_PROP, ConfigSource.DEFAULT_CONFIG)), synonymsList(logCleanerThreads))
  }
  ...
  def testUpdatesUsingConfigProvider(quorum: String): Unit = {
    ...
    assertFalse(brokerConfigs.exists(_.name == configPrefix + KafkaConfig.SSL_TRUSTSTORE_TYPE_PROP), "Initial value of ssl truststore type")
    assertNull(brokerConfigs.find(_.name == configPrefix+KafkaConfig.SSL_KEYSTORE_PASSWORD_PROP).get.value, "Initial value of ssl keystore password")
    ...
    updatedProps.put(KafkaConfig.METRIC_REPORTER_CLASSES_PROP, classOf[TestMetricsReporter].getName)
    ...
    updatedProps.put(configPrefix+KafkaConfig.SSL_TRUSTSTORE_TYPE_PROP, SslTruststoreTypeVal)

    // merge two properties
    updatedProps ++= secProps

    // 3. update password property using config provider
    updatedProps.put(configPrefix+KafkaConfig.SSL_KEYSTORE_PASSWORD_PROP, SslKeystorePasswordVal)

    alterConfigsUsingConfigCommand(updatedProps)
    waitForConfig(TestMetricsReporter.PollingIntervalProp, "1000")
    waitForConfig(configPrefix+KafkaConfig.SSL_TRUSTSTORE_TYPE_PROP, "JKS")
    waitForConfig(configPrefix+KafkaConfig.SSL_KEYSTORE_PASSWORD_PROP, "ServerPassword")
    ...
    if (!isKRaftTest()) {
      ...
      assertTrue(props.getProperty(configPrefix + KafkaConfig.SSL_TRUSTSTORE_TYPE_PROP) == SslTruststoreTypeVal, "store type is not updated in ZK")
      assertTrue(props.getProperty(configPrefix + KafkaConfig.SSL_KEYSTORE_PASSWORD_PROP) == SslKeystorePasswordVal, "keystore password is not updated in ZK")
    }
  ...
  def testTrustStoreAlter(quorum: String): Unit = {
    ...
    servers.head.config.dynamicConfig.currentDynamicBrokerConfigs.asScala.foreach { case (k, v) =>
  ...
  def testLogCleanerConfig(quorum: String): Unit = {
    ...
    props.put(KafkaConfig.LOG_CLEANER_DELETE_RETENTION_MS_PROP, "2")
    props.put(KafkaConfig.LOG_CLEANER_DEDUPE_BUFFER_SIZE_PROP, "20000000")
    props.put(KafkaConfig.LOG_CLEANER_DEDUPE_BUFFER_LOAD_FACTOR_PROP, "0.8")
    props.put(KafkaConfig.LOG_CLEANER_IO_BUFFER_SIZE_PROP, "300000")
    props.put(KafkaConfig.MESSAGE_MAX_BYTES_PROP, "40000")
    props.put(KafkaConfig.LOG_CLEANER_IO_MAX_BYTES_PER_SECOND_PROP, "50000000")
    props.put(KafkaConfig.LOG_CLEANER_BACKOFF_MS_PROP, "6000")
    ...
    TestUtils.waitUntilTrue(() => {
      reconfigureServers(props, perBrokerConfig = false, (KafkaConfig.LOG_CLEANER_DELETE_RETENTION_MS_PROP, "2"))
    ...
    props.put(KafkaConfig.LOG_CLEANER_BACKOFF_MS_PROP, "8000")
    reconfigureServers(props, perBrokerConfig = false, (KafkaConfig.LOG_CLEANER_BACKOFF_MS_PROP, "8000"))
    verifyThreads("kafka-log-cleaner-thread-", countPerBroker = 2)
  ...
  def testConsecutiveConfigChange(quorum: String): Unit = {
    ...
    topicProps.put(KafkaConfig.MIN_IN_SYNC_REPLICAS_PROP, "2")
    ...
    assertTrue(log.config.overriddenConfigs.contains(KafkaConfig.MIN_IN_SYNC_REPLICAS_PROP))
    assertEquals("2", log.config.originals().get(KafkaConfig.MIN_IN_SYNC_REPLICAS_PROP).toString)

    val props = new Properties
    props.put(KafkaConfig.MIN_IN_SYNC_REPLICAS_PROP, "3")
    // Make a broker-default config
    reconfigureServers(props, perBrokerConfig = false, (KafkaConfig.MIN_IN_SYNC_REPLICAS_PROP, "3"))
    ...
    assertTrue(log.config.overriddenConfigs.contains(KafkaConfig.MIN_IN_SYNC_REPLICAS_PROP))
    assertEquals("2", log.config.originals().get(KafkaConfig.MIN_IN_SYNC_REPLICAS_PROP).toString) // Verify topic-level config survives

    // Make a second broker-default change
    props.clear()
    props.put(KafkaConfig.LOG_RETENTION_TIME_MILLIS_PROP, "604800000")
    reconfigureServers(props, perBrokerConfig = false, (KafkaConfig.LOG_RETENTION_TIME_MILLIS_PROP, "604800000"))
    log = getLogOrThrow(new TopicPartition(topic2, 0))
    assertTrue(log.config.overriddenConfigs.contains(KafkaConfig.MIN_IN_SYNC_REPLICAS_PROP))
    assertEquals("2", log.config.originals().get(KafkaConfig.MIN_IN_SYNC_REPLICAS_PROP).toString) // Verify topic-level config still survives
  }
  ...
  def testDefaultTopicConfig(): Unit = {
    ...
    val props = new Properties
    props.put(KafkaConfig.LOG_SEGMENT_BYTES_PROP, "4000")
    props.put(KafkaConfig.LOG_ROLL_TIME_MILLIS_PROP, TimeUnit.HOURS.toMillis(2).toString)
    props.put(KafkaConfig.LOG_ROLL_TIME_JITTER_MILLIS_PROP, TimeUnit.HOURS.toMillis(1).toString)
    props.put(KafkaConfig.LOG_INDEX_SIZE_MAX_BYTES_PROP, "100000")
    props.put(KafkaConfig.LOG_FLUSH_INTERVAL_MESSAGES_PROP, "1000")
    props.put(KafkaConfig.LOG_FLUSH_INTERVAL_MS_PROP, "60000")
    props.put(KafkaConfig.LOG_RETENTION_BYTES_PROP, "10000000")
    props.put(KafkaConfig.LOG_RETENTION_TIME_MILLIS_PROP, TimeUnit.DAYS.toMillis(1).toString)
    props.put(KafkaConfig.MESSAGE_MAX_BYTES_PROP, "100000")
    props.put(KafkaConfig.LOG_INDEX_INTERVAL_BYTES_PROP, "10000")
    props.put(KafkaConfig.LOG_CLEANER_DELETE_RETENTION_MS_PROP, TimeUnit.DAYS.toMillis(1).toString)
    props.put(KafkaConfig.LOG_CLEANER_MIN_COMPACTION_LAG_MS_PROP, "60000")
    props.put(KafkaConfig.LOG_DELETE_DELAY_MS_PROP, "60000")
    props.put(KafkaConfig.LOG_CLEANER_MIN_CLEAN_RATIO_PROP, "0.3")
    props.put(KafkaConfig.LOG_CLEANUP_POLICY_PROP, "delete")
    props.put(KafkaConfig.UNCLEAN_LEADER_ELECTION_ENABLE_PROP, "false")
    props.put(KafkaConfig.MIN_IN_SYNC_REPLICAS_PROP, "2")
    props.put(KafkaConfig.COMPRESSION_TYPE_PROP, "gzip")
    props.put(KafkaConfig.LOG_PRE_ALLOCATE_PROP, true.toString)
    props.put(KafkaConfig.LOG_MESSAGE_TIMESTAMP_TYPE_PROP, TimestampType.LOG_APPEND_TIME.toString)
    props.put(KafkaConfig.LOG_MESSAGE_TIMESTAMP_DIFFERENCE_MAX_MS_PROP, "1000")
    props.put(KafkaConfig.LOG_MESSAGE_TIMESTAMP_BEFORE_MAX_MS_PROP, "1000")
    props.put(KafkaConfig.LOG_MESSAGE_TIMESTAMP_AFTER_MAX_MS_PROP, "1000")
    props.put(KafkaConfig.LOG_MESSAGE_DOWN_CONVERSION_ENABLE_PROP, "false")
    reconfigureServers(props, perBrokerConfig = false, (KafkaConfig.LOG_SEGMENT_BYTES_PROP, "4000"))
    ...
    props.asScala.foreach { case (k, v) =>
      val logConfigName = JDynamicLogConfig.KAFKA_CONFIG_TO_LOG_CONFIG_NAME.get(k)
      val expectedValue = if (k == KafkaConfig.LOG_CLEANUP_POLICY_PROP) s"[$v]" else v
    ...
    props.clear()
    props.put(KafkaConfig.LOG_MESSAGE_TIMESTAMP_TYPE_PROP, TimestampType.CREATE_TIME.toString)
    props.put(KafkaConfig.LOG_MESSAGE_TIMESTAMP_DIFFERENCE_MAX_MS_PROP, "1000")
    props.put(KafkaConfig.LOG_MESSAGE_TIMESTAMP_BEFORE_MAX_MS_PROP, "1000")
    props.put(KafkaConfig.LOG_MESSAGE_TIMESTAMP_AFTER_MAX_MS_PROP, "1000")
    reconfigureServers(props, perBrokerConfig = false, (KafkaConfig.LOG_MESSAGE_TIMESTAMP_TYPE_PROP, TimestampType.CREATE_TIME.toString))
    ...
    val invalidProps = Map(
      KafkaConfig.LOG_MESSAGE_TIMESTAMP_DIFFERENCE_MAX_MS_PROP -> "abc", // Invalid type
      KafkaConfig.LOG_MESSAGE_TIMESTAMP_BEFORE_MAX_MS_PROP -> "abc", // Invalid type
      KafkaConfig.LOG_MESSAGE_TIMESTAMP_AFTER_MAX_MS_PROP -> "abc", // Invalid type
      KafkaConfig.LOG_MESSAGE_TIMESTAMP_TYPE_PROP -> "invalid", // Invalid value
      KafkaConfig.LOG_ROLL_TIME_MILLIS_PROP -> "0" // Fails KafkaConfig validation
    )
    ...
    props.clear()
    props.put(KafkaConfig.LOG_INDEX_SIZE_MAX_BYTES_PROP, "500000")
    props.put(KafkaConfig.LOG_RETENTION_TIME_MILLIS_PROP, TimeUnit.DAYS.toMillis(2).toString)
    alterConfigsOnServer(servers.head, props)
    assertEquals(500000, servers.head.config.values.get(KafkaConfig.LOG_INDEX_SIZE_MAX_BYTES_PROP))
    assertEquals(TimeUnit.DAYS.toMillis(2), servers.head.config.values.get(KafkaConfig.LOG_RETENTION_TIME_MILLIS_PROP))
    servers.tail.foreach { server =>
      assertEquals(LogConfig.DEFAULT_SEGMENT_INDEX_BYTES, server.config.values.get(KafkaConfig.LOG_INDEX_SIZE_MAX_BYTES_PROP))
      assertEquals(1680000000L, server.config.values.get(KafkaConfig.LOG_RETENTION_TIME_MILLIS_PROP))
    }
    ...
    props.clear()
    props.put(KafkaConfig.LOG_RETENTION_TIME_MILLIS_PROP, "")
    props.put(KafkaConfig.LOG_INDEX_SIZE_MAX_BYTES_PROP, "")
    ...
    servers.foreach { server =>
      waitForConfigOnServer(server, KafkaConfig.LOG_RETENTION_TIME_MILLIS_PROP, 1680000000.toString)
    }
    servers.foreach { server =>
  ...
  def testUncleanLeaderElectionEnable(): Unit = {
    ...
    waitForConfigOnServer(controller, KafkaConfig.UNCLEAN_LEADER_ELECTION_ENABLE_PROP, "true")
  ...
  def testThreadPoolResize(): Unit = {
    ...
    verifyThreadPoolResize(KafkaConfig.NUM_IO_THREADS_PROP, config.numIoThreads,
      requestHandlerPrefix, mayReceiveDuplicates = false)
    verifyThreadPoolResize(KafkaConfig.NUM_REPLICA_FETCHERS_PROP, config.numReplicaFetchers,
      fetcherThreadPrefix, mayReceiveDuplicates = false)
    verifyThreadPoolResize(KafkaConfig.BACKGROUND_THREADS_PROP, config.backgroundThreads,
      "kafka-scheduler-", mayReceiveDuplicates = false)
    verifyThreadPoolResize(KafkaConfig.NUM_RECOVERY_THREADS_PER_DATA_DIR_PROP, config.numRecoveryThreadsPerDataDir,
      "", mayReceiveDuplicates = false)
    verifyThreadPoolResize(KafkaConfig.NUM_NETWORK_THREADS_PROP, config.numNetworkThreads,
  ...
  def testMetricsReporterUpdate(): Unit = {
    ...
    newProps.put(KafkaConfig.METRIC_REPORTER_CLASSES_PROP, "unknownMetricsReporter")
    reconfigureServers(newProps, perBrokerConfig = false, (TestMetricsReporter.PollingIntervalProp, "2000"), expectFailure = true)
    servers.foreach { server =>
      assertEquals(classOf[TestMetricsReporter].getName, server.config.originals.get(KafkaConfig.METRIC_REPORTER_CLASSES_PROP))
    }
    newReporters.foreach(_.verifyState(reconfigureCount = 0, deleteCount = 0, pollingInterval = 2000))
    ...
    // configuration across brokers, they can also be defined at per-broker level for testing
    newProps.put(KafkaConfig.METRIC_REPORTER_CLASSES_PROP, classOf[TestMetricsReporter].getName)
    ...
    servers.tail.foreach { server => assertEquals("", server.config.originals.get(KafkaConfig.METRIC_REPORTER_CLASSES_PROP)) }
  ...
  def testAdvertisedListenerUpdate(): Unit = {
    ...
    val props = new Properties
    props.put(KafkaConfig.INTER_BROKER_LISTENER_NAME_PROP, SecureExternal)
    val e = assertThrows(classOf[ExecutionException], () => reconfigureServers(props, perBrokerConfig = true, (KafkaConfig.INTER_BROKER_LISTENER_NAME_PROP, SecureExternal)))
  ...
  def testAddRemoveSslListener(): Unit = {
    ...
    servers.foreach { server =>
      ...
      config.dynamicConfig.staticBrokerConfigs.put(KafkaConfig.PASSWORD_ENCODER_OLD_SECRET_PROP, oldSecret)
      val passwordConfigs = props.asScala.filter { case (k, _) => DynamicBrokerConfigBaseManager.isPasswordConfig(k) }
      assertTrue(passwordConfigs.nonEmpty, "Password configs not found")
      val passwordDecoder = createPasswordEncoder(config, config.passwordEncoderSecret.asScala)
  ...
  def testReconfigureRemovedListener(quorum: String): Unit = {
    ...
    client.incrementalAlterConfigs(Map(broker0Resource ->
      Seq(new AlterConfigOp(new ConfigEntry(KafkaConfig.LISTENERS_PROP,
  ...
  private def addListener(servers: Seq[KafkaBroker], listenerName: String, securityProtocol: SecurityProtocol,
    ...
    val listeners = config.listeners.asScala
      .map(e => {
        ...
        s"${endpoint.listenerName.value}://${endpoint.host}:${endpoint.port}"
      })
    ...
    props.put(KafkaConfig.LISTENER_SECURITY_PROTOCOL_MAP_PROP, listenerMap)
    ...
    props.asScala.foreach { case (name, value) =>
      ...
      if (DynamicBrokerConfigBaseManager.isPasswordConfig(name) || name == unknownConfig)
  ...
  def testTransactionVerificationEnable(quorum: String): Unit = {
    ...
    updatedProps.put(KafkaConfig.TRANSACTION_PARTITION_VERIFICATION_ENABLE_PROP, "false")
    alterConfigsUsingConfigCommand(updatedProps)
    ...
    updatedProps.put(KafkaConfig.TRANSACTION_PARTITION_VERIFICATION_ENABLE_PROP, "true")
    alterConfigsUsingConfigCommand(updatedProps)
    verifyConfiguration(true)
  }
  ...
  private def fetchBrokerConfigsFromZooKeeper(server: KafkaBroker): Properties = {
    val props = adminZkClient.fetchEntityConfig(ConfigType.BROKER, server.config.brokerId.toString)
    server.config.dynamicConfig.fromPersistentProps(props, true)
  }
    ...
    val configs = servers.map { server =>
      ...
      val newListeners = server.config.effectiveAdvertisedListeners.asScala.map { e =>
        ...
        if (endpoint.listenerName.value == SecureExternal)
          ...
          s"${endpoint.listenerName.value}://${endpoint.host}:${server.boundPort(endpoint.listenerName)}"
      }.mkString(",")
      val configEntry = new ConfigEntry(KafkaConfig.ADVERTISED_LISTENERS_PROP, newListeners)
      (resource, new Config(Collections.singleton(configEntry)))
    }.toMap.asJava
    adminClient.alterConfigs(configs).all.get
    servers.foreach { server =>
      TestUtils.retry(10000) {
        val externalListener = server.config.effectiveAdvertisedListeners.asScala.map(EndPoint.fromJava).find(_.listenerName.value() == SecureExternal)
  ...
  private def configureDynamicKeystoreInZooKeeper(kafkaConfig: KafkaConfig, sslProperties: Properties): Unit = {
    ...
    sslStoreProps.put(KafkaConfig.PASSWORD_ENCODER_SECRET_PROP, kafkaConfig.passwordEncoderSecret.asScala.map(_.value).orNull)
    zkClient.makeSurePersistentPathExists(ConfigEntityChangeNotificationZNode.path)

    val entityType = ConfigType.BROKER
    val entityName = kafkaConfig.brokerId.toString

    val passwordConfigs = sslStoreProps.asScala.keySet.filter(DynamicBrokerConfigBaseManager.isPasswordConfig)
    val passwordEncoder = createPasswordEncoder(kafkaConfig, kafkaConfig.passwordEncoderSecret.asScala)
    ...
    sslStoreProps.remove(KafkaConfig.PASSWORD_ENCODER_SECRET_PROP)
  ...
  private def configureMetricsReporters(reporters: Seq[Class[_]], props: Properties,
    ...
    props.put(KafkaConfig.METRIC_REPORTER_CLASSES_PROP, reporterStr)
    reconfigureServers(props, perBrokerConfig, (KafkaConfig.METRIC_REPORTER_CLASSES_PROP, reporterStr))
  }
  ...
  private def addListenerPropsSasl(listener: String, mechanisms: Seq[String], props: Properties): Unit = {
    ...
    props.put(prefix + KafkaConfig.SASL_ENABLED_MECHANISMS_PROP, mechanisms.mkString(","))
    props.put(prefix + KafkaConfig.SASL_KERBEROS_SERVICE_NAME_PROP, "kafka")
    mechanisms.foreach { mechanism =>
      val jaasSection = jaasSections(Seq(mechanism), None, KafkaSasl, "").head
      val jaasConfig = jaasSection.modules.head.toString
      props.put(listenerName.saslMechanismConfigPrefix(mechanism) + KafkaConfig.SASL_JAAS_CONFIG_PROP, jaasConfig)
    }
  }
...
class TestMetricsReporter extends MetricsReporter with Reconfigurable with Closeable with ClusterResourceListener {
  ...
  override def configure(configs: util.Map[String, _]): Unit = {
    configuredBrokers += configs.get(KafkaConfig.BROKER_ID_PROP).toString.toInt
    configureCount += 1
