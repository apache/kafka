...
import org.apache.kafka.server.config.{KRaftConfigs, KafkaServerConfigs, ServerLogConfigs}
...
""", stream.toString())
  ...
  def testFormattingUnstableMetadataVersionBlocked(enableUnstable: Boolean): Unit = {
    ...
    try {
      properties.setProperty(ServerLogConfigs.LOG_DIRS_CONFIG, TestUtils.tempDir().toString)
      properties.setProperty(KafkaServerConfigs.UNSTABLE_METADATA_VERSIONS_ENABLE_CONFIG, enableUnstable.toString)
      properties.store(propsStream, "config.props")
    } finally {
      propsStream.close()
    }
