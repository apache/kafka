...
class KafkaApisTest extends Logging {
  ...
  def testDescribeConfigsGroupSynonyms(): Unit = {
    ...
    // Override broker config for consumer session timeout.
    val customSessionTimeoutMs = "55000"
    val overrideProperties = Map(
      GroupCoordinatorConfig.CONSUMER_GROUP_SESSION_TIMEOUT_MS_CONFIG -> customSessionTimeoutMs
    )
    ...
    createKafkaApis(authorizer = Some(authorizer), configRepository = configRepository,
      overrideProperties = overrideProperties).handleDescribeConfigsRequest(request)
    ...
    // Not in cgConfigs, has broker override.
    val sessionTimeoutConfig = configMap(CONSUMER_SESSION_TIMEOUT_MS_CONFIG)
    assertEquals(customSessionTimeoutMs, sessionTimeoutConfig.value)
    assertEquals(DescribeConfigsResponse.ConfigSource.STATIC_BROKER_CONFIG.id, sessionTimeoutConfig.configSource)
    val sessionTimeoutSynonyms = sessionTimeoutConfig.synonyms.asScala
    // Synonyms: STATIC_BROKER_CONFIG -> DEFAULT_CONFIG.
    assertEquals(2, sessionTimeoutSynonyms.size)
    assertEquals(GroupCoordinatorConfig.CONSUMER_GROUP_SESSION_TIMEOUT_MS_CONFIG, sessionTimeoutSynonyms.head.name)
    assertEquals(customSessionTimeoutMs, sessionTimeoutSynonyms.head.value)
    assertEquals(DescribeConfigsResponse.ConfigSource.STATIC_BROKER_CONFIG.id, sessionTimeoutSynonyms.head.source)
    assertEquals(GroupCoordinatorConfig.CONSUMER_GROUP_SESSION_TIMEOUT_MS_CONFIG, sessionTimeoutSynonyms(1).name)
    assertEquals(GroupCoordinatorConfig.CONSUMER_GROUP_SESSION_TIMEOUT_MS_DEFAULT.toString, sessionTimeoutSynonyms(1).value)
    assertEquals(DescribeConfigsResponse.ConfigSource.DEFAULT_CONFIG.id, sessionTimeoutSynonyms(1).source)

    // No broker synonym, set in cgConfigs.
    val shareAutoOffsetResetConfig = configMap(SHARE_AUTO_OFFSET_RESET_CONFIG)
    assertEquals(GroupConfig.SHARE_AUTO_OFFSET_RESET_DEFAULT, shareAutoOffsetResetConfig.value)
    val shareAutoOffsetResetSynonyms = shareAutoOffsetResetConfig.synonyms.asScala
    // Synonyms: GROUP_CONFIG -> DEFAULT_CONFIG.
    assertEquals(2, shareAutoOffsetResetSynonyms.size)
    assertEquals(SHARE_AUTO_OFFSET_RESET_CONFIG, shareAutoOffsetResetSynonyms.head.name)
    assertEquals(DescribeConfigsResponse.ConfigSource.GROUP_CONFIG.id, shareAutoOffsetResetSynonyms.head.source)
    assertEquals(SHARE_AUTO_OFFSET_RESET_CONFIG, shareAutoOffsetResetSynonyms(1).name)
    assertEquals(GroupConfig.SHARE_AUTO_OFFSET_RESET_DEFAULT, shareAutoOffsetResetSynonyms(1).value)
    assertEquals(DescribeConfigsResponse.ConfigSource.DEFAULT_CONFIG.id, shareAutoOffsetResetSynonyms(1).source)

    // Has broker synonym, set in cgConfigs, no broker override.
    val heartbeatConfig = configMap(CONSUMER_HEARTBEAT_INTERVAL_MS_CONFIG)
    val heartbeatSynonyms = heartbeatConfig.synonyms.asScala
    // Synonyms: GROUP_CONFIG -> DEFAULT_CONFIG.
    assertEquals(2, heartbeatSynonyms.size)
    assertEquals(CONSUMER_HEARTBEAT_INTERVAL_MS_CONFIG, heartbeatSynonyms.head.name)
    assertEquals(DescribeConfigsResponse.ConfigSource.GROUP_CONFIG.id, heartbeatSynonyms.head.source)
    assertEquals(GroupCoordinatorConfig.CONSUMER_GROUP_HEARTBEAT_INTERVAL_MS_CONFIG, heartbeatSynonyms(1).name)
    assertEquals(GroupCoordinatorConfig.CONSUMER_GROUP_HEARTBEAT_INTERVAL_MS_DEFAULT.toString, heartbeatSynonyms(1).value)
    assertEquals(DescribeConfigsResponse.ConfigSource.DEFAULT_CONFIG.id, heartbeatSynonyms(1).source)
  }
