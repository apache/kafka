...
import org.apache.kafka.server.config.{KafkaSecurityConfigs, KafkaServerConfigs, QuotaConfigs}
...
class RequestQuotaTest extends BaseRequestTest {
  ...
  override def brokerPropertyOverrides(properties: Properties): Unit = {
    properties.put(KafkaServerConfigs.CONTROLLED_SHUTDOWN_ENABLE_CONFIG, "false")
    ...
    if (isKRaftTest()) {
      properties.put(KafkaServerConfigs.AUTHORIZER_CLASS_NAME_CONFIG, classOf[RequestQuotaTest.KraftTestAuthorizer].getName)
    } else {
      properties.put(KafkaServerConfigs.AUTHORIZER_CLASS_NAME_CONFIG, classOf[RequestQuotaTest.ZkTestAuthorizer].getName)
    }
  }
