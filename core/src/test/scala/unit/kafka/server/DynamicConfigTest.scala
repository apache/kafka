...
class DynamicConfigTest extends ZooKeeperTestHarness {
  final private val nonExistentConfig: String = "some.config.that.does.not.exist"
  final private val someValue: String = "some interesting value"
  ...
  def shouldFailLeaderConfigsWithInvalidValues() {
    adminZkClient.changeBrokerConfig(Seq(0), propsWith(DynamicConfig.Broker.LeaderReplicationThrottledRateProp, "-100"))
  }
