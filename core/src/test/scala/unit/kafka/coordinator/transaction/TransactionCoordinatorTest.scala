...
class TransactionCoordinatorTest {
  ...
  private def mockPidManager(): Unit =
    EasyMock
      .expect(pidManager.generateProducerId())
  ...
  private def initPidGenericMocks(transactionalId: String): Unit = {
    mockPidManager()
    EasyMock
      .expect(transactionManager.validateTransactionTimeoutMs(EasyMock.anyInt()))
      .andReturn(true)
      .anyTimes()
  }
  ...
  def shouldInitPidWithEpochZeroForNewTransactionalId(): Unit = {
    ...
    EasyMock
      .expect(transactionManager.getTransactionState(EasyMock.eq(transactionalId)))
      .andReturn(Right(None))
      .once()

    EasyMock
      .expect(
        transactionManager.putTransactionStateIfNotExists(EasyMock.eq(transactionalId), EasyMock.capture(capturedTxn))
      )
  ...
  def shouldGenerateNewProducerIdIfEpochsExhausted(): Unit = {
    ...
    val txnMetadata = new TransactionMetadata(transactionalId,
                                              producerId,
        ...
        Right(
          Some(
            CoordinatorEpochAndTxnMetadata(
              coordinatorEpoch,
              new TransactionMetadata(transactionalId, 0, 0, 0, state, mutable.Set.empty, 0, 0)
            )
          )
        )
      )
      ...
      .andReturn(
        Right(
          Some(
            CoordinatorEpochAndTxnMetadata(
              coordinatorEpoch,
              new TransactionMetadata(transactionalId, 0, 10, 0, PrepareCommit, mutable.Set.empty, 0, 0)
            )
          )
        )
      )
    ...
    EasyMock
      ...
      .andReturn(
        Right(
          Some(
            CoordinatorEpochAndTxnMetadata(coordinatorEpoch,
                                           new TransactionMetadata(transactionalId,
                                                                   10,
                                                                   0,
                                                                   0,
                                                                   Ongoing,
  ...
  def shouldAppendPrepareCommitToLogOnEndTxnWhenStatusIsOngoingAndResultIsCommit(): Unit = {
    ...
    coordinator.handleEndTransaction(transactionalId,
                                     producerId,
                                     producerEpoch,
  ...
  def shouldAbortTransactionOnHandleInitPidWhenExistingTransactionInOngoingState(): Unit = {
    ...
    EasyMock
      .expect(
        transactionManager.putTransactionStateIfNotExists(EasyMock.eq(transactionalId),
                                                          EasyMock.anyObject[TransactionMetadata]())
      )
  ...
  def shouldAbortExpiredTransactionsInOngoingStateAndBumpEpoch(): Unit = {
    ...
    val txnMetadata =
      new TransactionMetadata(transactionalId, producerId, producerEpoch, txnTimeoutMs, Ongoing, partitions, now, now)

    EasyMock
      .expect(transactionManager.timedOutTransactions())
    ...
    val expectedTransition = TxnTransitMetadata(
      producerId,
      bumpedEpoch,
      txnTimeoutMs,
      PrepareAbort,
      partitions.toSet,
      now,
      now + TransactionStateManager.DefaultAbortTimedOutTransactionsIntervalMs
    )
  ...
  def shouldNotAbortExpiredTransactionsThatHaveAPendingStateTransition(): Unit = {
    val metadata = new TransactionMetadata(transactionalId,
                                           producerId,
                                           producerEpoch,
                                           txnTimeoutMs,
                                           Ongoing,
  ...
  private def validateRespondsWithConcurrentTransactionsOnInitPidWhenInPrepareState(state: TransactionState) = {
    ...
    val metadata = new TransactionMetadata(transactionalId,
                                           ...
                                           0,
                                           state,
                                           mutable.Set[TopicPartition](new TopicPartition("topic", 1)),
                                           0,
                                           0)
    EasyMock
      .expect(transactionManager.getTransactionState(EasyMock.eq(transactionalId)))
  ...
  private def mockPrepare(transactionState: TransactionState, runCallback: Boolean = false): TransactionMetadata = {
    ...
    val transition =
      TxnTransitMetadata(producerId, producerEpoch, txnTimeoutMs, transactionState, partitions.toSet, now, now)
    ...
    EasyMock
      .expect(
        transactionManager.appendTransactionToLog(EasyMock.eq(transactionalId),
                                                  EasyMock.eq(coordinatorEpoch),
