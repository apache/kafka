...
class IsrExpirationTest {
  ...
  @Before
  def setUp() {
    replicaManager = new ReplicaManager(configs.head, metrics, time, jTime, null, null, null, new AtomicBoolean(false), QuotaFactory.instantiate(configs.head, metrics).followerReplication)
  }
