...
class ClientRequestQuotaManager(private val config: ClientQuotaManagerConfig,
  ...
  def maybeRecordAndGetThrottleTimeMs(request: RequestChannel.Request): Int = {
    ...
    if (quotasEnabled) {
      request.recordNetworkThreadTimeCallback = Some(
        timeNanos =>
          recordNoThrottle(getOrCreateQuotaSensors(request.session, request.header.clientId),
                           nanosToPercentage(timeNanos))
      )
      recordAndGetThrottleTimeMs(request.session,
