...
import org.apache.kafka.server.config.{KRaftConfigs, KafkaSecurityConfigs, KafkaServerConfigs, ReplicationConfigs, ServerLogConfigs, ZkConfigs}
...
class DynamicBrokerConfigTest {
  ...
  def testUpdateDynamicThreadPool(): Unit = {
    val origProps = TestUtils.createBrokerConfig(0, TestUtils.MockZkConnect, port = 8181)
    origProps.put(KafkaServerConfigs.NUM_IO_THREADS_CONFIG, "4")
    origProps.put(KafkaServerConfigs.NUM_NETWORK_THREADS_CONFIG, "2")
    ...
    origProps.put(KafkaServerConfigs.BACKGROUND_THREADS_CONFIG, "3")
    ...
    props.put(KafkaServerConfigs.NUM_IO_THREADS_CONFIG, "8")
    config.dynamicConfig.updateDefaultConfig(props)
    assertEquals(8, config.numIoThreads)
    Mockito.verify(handlerPoolMock).resizeThreadPool(newSize = 8)

    props.put(KafkaServerConfigs.NUM_NETWORK_THREADS_CONFIG, "4")
    config.dynamicConfig.updateDefaultConfig(props)
    ...
    assertTrue(captor.getValue.containsKey(KafkaServerConfigs.NUM_NETWORK_THREADS_CONFIG))
    assertEquals(4, captor.getValue.get(KafkaServerConfigs.NUM_NETWORK_THREADS_CONFIG))
    ...
    props.put(KafkaServerConfigs.BACKGROUND_THREADS_CONFIG, "6")
    config.dynamicConfig.updateDefaultConfig(props)
  ...
  def testReconfigurableValidation(): Unit = {
    val origProps = TestUtils.createBrokerConfig(0, TestUtils.MockZkConnect, port = 8181)
    val config = KafkaConfig(origProps)
    val invalidReconfigurableProps = Set(CleanerConfig.LOG_CLEANER_THREADS_PROP, KafkaServerConfigs.BROKER_ID_CONFIG, "some.prop")
  ...
  def testDynamicConfigInitializationWithoutConfigsInZK(): Unit = {
    ...
    initialProps.remove(KafkaServerConfigs.BACKGROUND_THREADS_CONFIG)
    ...
    newprops.put(KafkaServerConfigs.NUM_IO_THREADS_CONFIG, "10")
    newprops.put(KafkaServerConfigs.BACKGROUND_THREADS_CONFIG, "100")
  ...
  def testImproperConfigsAreRemoved(): Unit = {
    ...
    newProps.put(KafkaServerConfigs.MESSAGE_MAX_BYTES_CONFIG, "2222")
    ...
    newProps.put(KafkaServerConfigs.MESSAGE_MAX_BYTES_CONFIG, "1111")
...
class TestDynamicThreadPool() extends BrokerReconfigurable {
  ...
  override def reconfigure(oldConfig: KafkaConfig, newConfig: KafkaConfig): Unit = {
    assertEquals(KafkaServerConfigs.NUM_IO_THREADS_DEFAULT, oldConfig.numIoThreads)
    assertEquals(KafkaServerConfigs.BACKGROUND_THREADS_DEFAULT, oldConfig.backgroundThreads)
