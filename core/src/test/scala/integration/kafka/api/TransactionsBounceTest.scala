...
class TransactionsBounceTest extends IntegrationTestHarness {
  ...
  overridingProps.put(KafkaLogConfigs.AUTO_CREATE_TOPICS_ENABLE_CONFIG, false.toString)
  ...
  private def createTopics() =  {
    ...
    topicConfig.put(KafkaLogConfigs.MIN_IN_SYNC_REPLICAS_CONFIG, 2.toString)
