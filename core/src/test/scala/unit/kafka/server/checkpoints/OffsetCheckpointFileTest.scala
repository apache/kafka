...
class OffsetCheckpointFileTest extends JUnitSuite with Logging {
  ...
  def shouldHandleMultipleLines(): Unit = {
    ...
    //Given
    val offsets = Map(
      new TopicPartition("foo", 1) -> 5L,
      new TopicPartition("bar", 6) -> 10L,
      new TopicPartition("foo", 2) -> 5L,
      new TopicPartition("bar", 7) -> 10L,
      new TopicPartition("foo", 3) -> 5L,
      new TopicPartition("bar", 8) -> 10L,
      new TopicPartition("foo", 4) -> 5L,
      new TopicPartition("bar", 9) -> 10L,
      new TopicPartition("foo", 5) -> 5L,
      new TopicPartition("bar", 10) -> 10L
    )
  ...
  def shouldThrowIfVersionIsNotRecognised(): Unit = {
    ...
    val checkpointFile = new CheckpointFile(file,
