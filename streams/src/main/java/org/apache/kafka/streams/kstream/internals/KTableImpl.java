...
public class KTableImpl<K, S, V> extends AbstractStream<K, V> implements KTable<K, V> {
    ...
    private KTable<K, V> doFilter(final Predicate<?, ? super V> predicate,
        ...
        if (materializedInternal != null) {
            ...
            storeBuilder = queryableStoreName != null ? getStoreBuilder(materializedInternal) : null;
        } else {
        ...
        final ProcessorParameters<V> processorParameters = unsafeCastProcessorParametersToCompletelyDifferentType(
    ...
    public KTable<K, V> filter(final Predicate<?, ? super V> predicate,
        ...
        materialized.containsValidStoreSupplier(stateStoreType);
        final MaterializedInternal<K, V, StateStore> materializedInternal = new MaterializedInternal<>(materialized);
    ...
    private <VR> KTable<K, VR> doMapValues(final ValueMapperWithKey<? super K, ? super V, ? extends VR> mapper,
                                           final MaterializedInternal<K, VR, StateStore> materializedInternal) throws IllegalArgumentException {
        final Serde<K> keySerde;
        ...
        final ProcessorParameters<VR> processorParameters = unsafeCastProcessorParametersToCompletelyDifferentType(
            new ProcessorParameters<>(processorSupplier, name)
        );
        final StreamsGraphNode tableNode = new TableProcessorNode<>(
            name,
    ...
    public <VR> KTable<K, VR> mapValues(final ValueMapper<? super V, ? extends VR> mapper,
        ...
        final MaterializedInternal<K, VR, StateStore> materializedInternal = new MaterializedInternal<>(materialized);
    ...
    private <VR> KTable<K, VR> doTransformValues(final ValueTransformerWithKeySupplier<? super K, ? super V, ? extends VR> transformerSupplier,
                                                 final MaterializedInternal<K, VR, StateStore> materializedInternal,
                                                 final String... stateStoreNames) throws IllegalArgumentException {
    ...
    public KStream<K, V> toStream() {
        ...
        final ProcessorSupplier<Change<V>> kStreamMapValues = new KStreamMapValues<>((key, change) -> change.newValue);
        final ProcessorParameters<V> processorParameters = unsafeCastProcessorParametersToCompletelyDifferentType(
            new ProcessorParameters<>(kStreamMapValues, name)
        );

        final ProcessorGraphNode<V> toStreamNode = new ProcessorGraphNode<>(
            name,
            processorParameters
        );
    ...
    public KTable<K, V> suppress(final Suppressed<? super K> suppressed) {
        ...
        final ProcessorSupplier<Change<V>> suppressionSupplier = new KTableSuppressProcessorSupplier<>(
            suppressedInternal,
            storeName,
            this
        );

        final ProcessorGraphNode<Change<V>> node = new StatefulProcessorNode<>(
            name,
    ...
    private <VO, VR> KTable<K, VR> doJoin(final KTable<K, VO> other,
        ...
        final ProcessorParameters<Change<V>> joinThisProcessorParameters = new ProcessorParameters<>(joinThis, joinThisName);
        final ProcessorParameters<Change<VO>> joinOtherProcessorParameters = new ProcessorParameters<>(joinOther, joinOtherName);
        ...
        if (materializedInternal != null) {
            ...
            storeBuilder = getStoreBuilder(materializedInternal);
        } else {
            keySerde = this.keySerde;
            valueSerde = null;
            queryableStoreName = null;
        ...
        final KTableKTableJoinNode<K, V, VO, VR> kTableKTableJoinNode =
            KTableKTableJoinNode.<K, V, VO, VR>kTableKTableJoinNodeBuilder()
                ...
                .withStoreBuilder((StoreBuilder<StateStore>) storeBuilder)
                .build();
    ...
    private <VR> StoreBuilder<?> getStoreBuilder(MaterializedInternal<K, VR, StateStore> materializedInternal) {
        ...
        if (stateStoreType == StateStoreType.KEY_VALUE_STORE) {
            ...
            storeBuilder = new TimestampedWindowStoreMaterializer(materializedInternal, timeWindow.get()).materialize();
        } else {
    ...
    public <K1, V1> KGroupedTable<K1, V1> groupBy(final KeyValueMapper<? super K, ? super V, KeyValue<K1, V1>> selector,
        ...
        final ProcessorParameters<Change<V>> processorParameters = new ProcessorParameters<>(selectSupplier, selectName);

        // select the aggregate key and values (old and new), it would require parent to send old values
        final ProcessorGraphNode<Change<V>> groupByMapNode = new ProcessorGraphNode<>(selectName, processorParameters);
     ...
     */
    @SuppressWarnings("unchecked")
    private <VR> ProcessorParameters<VR> unsafeCastProcessorParametersToCompletelyDifferentType(final ProcessorParameters<Change<V>> kObjectProcessorParameters) {
        return (ProcessorParameters<VR>) kObjectProcessorParameters;
    }

    @Override
    public StateStoreType getStateStoreType() {
        return this.stateStoreType;
    }
