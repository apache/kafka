...
import org.apache.kafka.coordinator.group.{GroupCoordinator, OffsetConfig}
...
class AutoTopicCreationManagerTest {
  ...
  def setup(): Unit = {
    ...
    props.setProperty(OffsetConfig.OFFSETS_TOPIC_REPLICATION_FACTOR_CONFIG, internalTopicPartitions.toString)
    props.setProperty(KafkaConfig.TransactionsTopicReplicationFactorProp, internalTopicPartitions.toString)

    props.setProperty(OffsetConfig.OFFSETS_TOPIC_PARTITIONS_CONFIG, internalTopicReplicationFactor.toString)
