 ...
 * combines a key with a boolean value that specifies if the key is found in the left side of a join or on the right
 * side. This {@link LeftOrRightValue} object contains either the V1 value, which is found in the left topic, or V2
 * value if it is found in the right topic.
 */
public class LeftOrRightValue<V1, V2> {
    private final V1 leftValue;
    private final V2 rightValue;

    LeftOrRightValue(final V1 leftValue, final V2 rightValue) {
    ...
    public String toString() {
        return "<"
                + ((leftValue != null) ? JoinSide.LEFT + "," + leftValue : JoinSide.RIGHT + "," + rightValue)
                + ">";
    }
