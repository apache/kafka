...
class PlaintextAdminIntegrationTest extends BaseAdminIntegrationTest {
  ...
  def testDescribeAndAlterConfigs(quorum: String): Unit = {
    ...
    assertEquals(brokers(2).config.logCleanerThreads.toString,
      configs.get(brokerResource2).get(LogCleanerConfig.LOG_CLEANER_THREADS_PROP).value)
  ...
  def testCreateTopicsReturnsConfigs(quorum: String): Unit = {
    ...
    newLogCleanerDeleteRetention.put(LogCleanerConfig.LOG_CLEANER_DELETE_RETENTION_MS_PROP, "34")
    ...
    if (isKRaftTest()) {
      ...
      controllerServer.controller.incrementalAlterConfigs(ANONYMOUS_CONTEXT,
        Collections.singletonMap(controllerNodeResource,
          Collections.singletonMap(LogCleanerConfig.LOG_CLEANER_DELETE_RETENTION_MS_PROP,
    ...
    waitUntilTrue(() => brokers.forall(_.config.originals.getOrDefault(
      LogCleanerConfig.LOG_CLEANER_DELETE_RETENTION_MS_PROP, "").toString.equals("34")),
      s"Timed out waiting for change to ${LogCleanerConfig.LOG_CLEANER_DELETE_RETENTION_MS_PROP}",
      waitTimeMs = 60000L)
