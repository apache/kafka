...
class ReassignPartitionsCommandArgsTest extends JUnitSuite {
  ...
  def shouldCorrectlyParseValidMinimumExecuteOptions(): Unit = {
    val args = Array("--zookeeper", "localhost:1234", "--execute", "--reassignment-json-file", "myfile.json")
    ReassignPartitionsCommand.validateAndParseArgs(args)
  }

  @Test
  def shouldCorrectlyParseValidMinimumVerifyOptions(): Unit = {
    val args = Array("--zookeeper", "localhost:1234", "--verify", "--reassignment-json-file", "myfile.json")
    ReassignPartitionsCommand.validateAndParseArgs(args)
  }
  ...
  def shouldNotAllowExecuteWithoutReassignmentOption(): Unit = {
    val args = Array("--zookeeper", "localhost:1234", "--execute")
    shouldFailWith(
  ...
  def shouldNotAllowGenerateWithoutBrokersAndTopicsOptions(): Unit = {
    val args = Array("--zookeeper", "localhost:1234", "--generate")
    shouldFailWith(
  ...
  def shouldNotAllowGenerateWithoutBrokersOption(): Unit = {
    val args = Array("--zookeeper", "localhost:1234", "--topics-to-move-json-file", "myfile.json", "--generate")
    shouldFailWith(
      "If --generate option is used, command must include both --topics-to-move-json-file and --broker-list options",
      args
    )
  ...
  def shouldNotAllowGenerateWithoutTopicsOption(): Unit = {
    val args = Array("--zookeeper", "localhost:1234", "--broker-list", "101,102", "--generate")
    shouldFailWith(
      "If --generate option is used, command must include both --topics-to-move-json-file and --broker-list options",
      args
    )
  ...
  def shouldNotAllowVerifyWithoutReassignmentOption(): Unit = {
    val args = Array("--zookeeper", "localhost:1234", "--verify")
    shouldFailWith(
  ...
  def shouldNotAllowThrottleWithVerifyOption(): Unit = {
    val args =
      Array("--zookeeper", "localhost:1234", "--verify", "--throttle", "100", "--reassignment-json-file", "myfile.json")
  ...
  def shouldFailWith(msg: String, args: Array[String]): Unit =
    try {
      ...
      case e: Exception =>
        assertTrue(s"Expected exception with message:\n[$msg]\nbut was\n[${e.getMessage}]",
