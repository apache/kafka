...
object ThrottledEndToEndLatency {
  ...
  def main(args: Array[String]) {
    ...
    val producerThreads = args(6).toInt
    val consumerThreads = args(7).toInt
    val warmupTime = args(8).toInt
    val rateOpsPerSec = args(9).toInt
    val propsFile = if (args.length > 10) Some(args(10)).filter(_.nonEmpty) else None

    val rateOpsPerSecPerThread = (rateOpsPerSec / producerThreads)
    ...
    val producer_threads = (1 to producerThreads).map{e => new Thread(new ThrottledKafkaProducer(producerProps, numMessages, topic, maxTime, messageLen, rateOpsPerSecPerThread)) }
    producer_threads.map{e => e.start() }

    val consumer_threads = (1 to consumerThreads).map{e => new Thread(new ThrottledKafkaConsumer(consumerProps, histogram, timeout, numMessages, topic, warmupTime))}
  ...
  class ThrottledKafkaProducer(producerProps: Properties, numMessages: Int, topic: String, maxTime: Int, messageLen: Int, rateOpsPerSecPerThread: Int) extends Runnable {
    ...
    def run() {
      ...
      var pacer = new Pacer(rateOpsPerSecPerThread, 3.0)
      ...
      while(timeDiff){
        ...
        val unitsCompleted = pacer.getUnitsCompleted
        pacer.acquire(1)
      ...
      println(pacer.getUnitsCompleted + " messages produced")
      producer.close()
    }
  }
