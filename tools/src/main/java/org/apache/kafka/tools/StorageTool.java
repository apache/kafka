...
public class StorageTool {
    public static void main(String... args) {
        try {
            ...
            Optional<RaftConfig> raftConfig = Optional.ofNullable(namespace.getString("config")).map(p -> {
                try {
                    return new RaftConfig(Utils.loadProps(p));
                } catch (IOException e) {
                    throw new RuntimeException(e);
                }
            });

            executeCommand(namespace, command, logConfig, raftConfig);
            Exit.exit(0);
    ...
    private static void executeCommand(Namespace namespace,
                                       ...
                                       Optional<RaftConfig> raftConfig) throws Exception {
        switch (command) {
            case "info": {
                List<String> directories = configToLogDirectories(logConfig.get());
                boolean selfManagedMode = configToSelfManagedMode(raftConfig.get());
            ...
            case "format": {
                ...
                MetadataVersion metadataVersion = getMetadataVersion(namespace, logConfig.get().interBrokerProtocolVersion());
                if (!metadataVersion.isKRaftSupported()) {
                    throw new TerseException("Must specify a valid KRaft metadata version of at least 3.0.");
                }
                if (!metadataVersion.isProduction()) {
                    if (logConfig.get().unstableMetadataVersionsEnabled()) {
                        System.out.println("WARNING: using pre-production metadata version " + metadataVersion + ".");
                    } else {
                        throw new TerseException("Metadata version " + metadataVersion + " is not ready for production use yet.");
                    }
                }
                MetaProperties metaProperties = new MetaProperties.Builder()
                    .setVersion(MetaPropertiesVersion.V1)
                    .setClusterId(clusterId)
                    .setNodeId(raftConfig.get().nodeId())
                    .build();
                ...
                if (!configToSelfManagedMode(raftConfig.get())) {
    ...
    public static Namespace parseArguments(String[] args) {
        ...
        formatParser.addArgument("--release-version", "-r").action(store()).help(
            "A KRaft release version to use for the initial metadata version. The minimum is 3.0, the default is " +
    ...
    static List<String> configToLogDirectories(LogConfig logConfig) {
        List<String> logDirs = logConfig.logDirs();
    ...
    static boolean configToSelfManagedMode(RaftConfig raftConfig) {
        return !raftConfig.processRoles().isEmpty();
    }
    ...
    public static UserScramCredentialRecord getUserScramCredentialRecord(String mechanism, String config) 
    ...
    public static int formatCommand(PrintStream stream,
        ...
        if (!copier.errorLogDirs().isEmpty()) {
            String firstLogDir = copier.errorLogDirs().iterator().next();
            throw new TerseException("I/O error trying to read log directory " + firstLogDir + ".");
        }
