...
class AdminClientWithPoliciesIntegrationTest extends KafkaServerTestHarness with Logging {
  ...
  def testInvalidAlterConfigsDueToPolicy(quorum: String): Unit = {
    ...
    val brokerConfigEntries = Seq(new ConfigEntry(SSL_TRUSTSTORE_PASSWORD_CONFIG, "12313")).asJava
    ...
    assertNull(configs.get(brokerResource).get(SSL_TRUSTSTORE_PASSWORD_CONFIG).value)
