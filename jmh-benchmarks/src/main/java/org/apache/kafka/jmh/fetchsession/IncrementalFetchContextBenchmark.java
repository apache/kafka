...
public class IncrementalFetchContextBenchmark {
    ...
    private final Map<TopicPartition, FetchResponseData.PartitionData> partitionData = partitionData();

    private final IncrementalFetchContext fetchContext = incrementalFetchContext();

    private LinkedHashMap<TopicPartition, FetchResponseData.PartitionData> topicResponses;
    ...
    public void setUp() {
        topicResponses = new LinkedHashMap<>(partitionData);
    }

    private static Map<TopicPartition, FetchResponseData.PartitionData> partitionData() {
        return Collections.unmodifiableMap(IntStream.range(0, 2000).boxed()
            .collect(Collectors.toMap(
                i -> new TopicPartition("foo", i),
                i -> new FetchResponseData.PartitionData()
                    .setPartitionIndex(i)
    ...
    private static IncrementalFetchContext incrementalFetchContext() {
        ...
        return new IncrementalFetchContext(Time.SYSTEM, metadata, session) {
            @Override
            public CachedPartition cachedPartition(TopicPartition topicPartition) {
                return cp;
            }
