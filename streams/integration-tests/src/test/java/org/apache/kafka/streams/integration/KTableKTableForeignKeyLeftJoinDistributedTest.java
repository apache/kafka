...
@Timeout(600)
@Tag("integration")
public class KTableKTableForeignKeyLeftJoinDistributedTest {
    ...
    private void configureBuilder(final StreamsBuilder builder) {
        ...
        final KTable<String, String> fkJoin = left.leftJoin(right, extractor, joiner);
        fkJoin
                .toStream()
                .to(OUTPUT);
    }
    ...
    public void shouldBeInitializedWithDefaultSerde(final TestInfo testInfo) throws Exception {
        ...
        IntegrationTestUtils.produceKeyValuesSynchronously(LEFT_TABLE, List.of(
            new KeyValue<>("lhsValue1", "lhsValue1|rhs2")
        ), producerConfig, CLUSTER.time);

        final Set<KeyValue<String, String>> resultAfterFkChange = new HashSet<>(IntegrationTestUtils.waitUntilMinKeyValueRecordsReceived(
            CONSUMER_CONFIG,
            OUTPUT,
            expectedResult.size()));
