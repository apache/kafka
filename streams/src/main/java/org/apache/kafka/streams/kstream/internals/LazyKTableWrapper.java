...
public abstract class LazyKTableWrapper<K, S, V> extends AbstractStream<K> implements KTable<K, V> {
    ...
    public KTable<K, V> returns(Type keyType, Type valueType) {
        ...
        if (newKeyType != null && newValueType != null) {
            ...
            final LazyKTableWrapper<K, S, V> parent = this;
            return new LazyKTableWrapper<K, S, V>(topology, newKeyType, newValueType) {
    ...
    @Override
    public KTable<K, V> through(String topic, Serializer<K> keySerializer, Serializer<V> valSerializer, Deserializer<K> keyDeserializer, Deserializer<V> valDeserializer) {
        throw new InsufficientTypeInfoException();
    }
    ...
    @Override
    public <K1, V1, T> KTable<K1, T> aggregate(AggregatorSupplier<K1, V1, T> aggregatorSupplier,
