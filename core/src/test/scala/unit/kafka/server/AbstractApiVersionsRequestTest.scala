...
abstract class AbstractApiVersionsRequestTest(cluster: ClusterInstance) {
  ...
  def validateApiVersionsResponse(
    ...
    if (cluster.isKRaftTest && apiVersion >= 3) {
      ...
      assertEquals(MetadataVersion.latest(false).featureLevel(), apiVersionsResponse.data().finalizedFeatures().find(MetadataVersion.FEATURE_NAME).minVersionLevel())
      assertEquals(MetadataVersion.latest(false).featureLevel(), apiVersionsResponse.data().finalizedFeatures().find(MetadataVersion.FEATURE_NAME).maxVersionLevel())
      ...
      assertEquals(MetadataVersion.latest(false).featureLevel(), apiVersionsResponse.data().supportedFeatures().find(MetadataVersion.FEATURE_NAME).maxVersion())
    }
