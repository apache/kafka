...
/**
 * Full topology description of a Kafka Streams application, as stored by the broker's
 * topology description plugin and retrieved via
 * {@link Admin#describeStreamsGroups(Collection, DescribeStreamsGroupsOptions)} with
 * {@link DescribeStreamsGroupsOptions#includeTopologyDescription(boolean)}.
 *
 * <p>Mirrors {@link org.apache.kafka.streams.TopologyDescription} but lives in the
 * {@code clients} module so admin callers do not need a dependency on {@code streams}.
 */
...
public class StreamsGroupTopologyDescription {
     ...
     * and {@link Sink}.
     *
     * <p>Predecessors are reconstructed by the admin client from the wire-format successor
     * edges; they are not carried on the wire and are not present on the broker-side plugin
     * POJO. Both {@link #predecessors()} and {@link #successors()} return immediate
     * neighbours only — transitive ancestors and descendants are not included, matching
     * the behaviour of {@code o.a.k.streams.TopologyDescription.Node}.
     */
    public interface Node {
        String name();

        /**
         * The direct predecessor nodes — immediate parents only, not the full ancestor chain.
         */
        Set<String> predecessors();

        /**
         * The direct successor nodes — immediate children only, not the full descendant chain.
         */
    ...
    public static final class Source implements Node {
        ...
        public Source(
            ...
            this.predecessors = Collections.unmodifiableSet(Objects.requireNonNull(predecessors));
        ...
        @Override
        public int hashCode() {
            return Objects.hash(name, topics, predecessors, successors);
        }
    ...
    public static final class Processor implements Node {
        ...
        @Override
        public int hashCode() {
            return Objects.hash(name, stores, predecessors, successors);
        }
    ...
    public static final class Sink implements Node {
        ...
        @Override
        public int hashCode() {
            return Objects.hash(name, topic, predecessors, successors);
        }
