...
class AuthorizerIntegrationTest extends BaseRequestTest {
  ...
  val requestKeyToResponseDeserializer: Map[ApiKeys, Class[_ <: Any]] =
    Map(ApiKeys.METADATA -> classOf[requests.MetadataResponse],
      ...
      ApiKeys.ALTER_REPLICA_DIRS -> classOf[AlterReplicaDirsResponse],
  ...
  val requestKeyToError = Map[ApiKeys, Nothing => Errors](
    ...
    ApiKeys.ALTER_REPLICA_DIRS -> ((resp: AlterReplicaDirsResponse) => resp.responses.get(tp)),
  ...
  val requestKeysToAcls = Map[ApiKeys, Map[Resource, Set[Acl]]](
    ...
    ApiKeys.ALTER_REPLICA_DIRS -> clusterAlterAcl,
  ...
  private def alterReplicaDirsRequest = new AlterReplicaDirsRequest.Builder(Collections.singletonMap(tp, logDir)).build()
  ...
  def testAuthorizationWithTopicExisting() {
    val requestKeyToRequest = mutable.LinkedHashMap[ApiKeys, AbstractRequest](
      ...
      ApiKeys.ALTER_REPLICA_DIRS -> alterReplicaDirsRequest,
