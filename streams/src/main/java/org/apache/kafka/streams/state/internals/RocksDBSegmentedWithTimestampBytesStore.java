...
public class RocksDBSegmentedWithTimestampBytesStore implements SegmentedBytesStore {
    private static final Logger LOG = LoggerFactory.getLogger(RocksDBSegmentedWithTimestampBytesStore.class);
    private final String name;
    private final WithTimestampSegments segments;
    private final String metricScope;
    private final KeySchema<WithTimestampSegment> keySchema;
    private InternalProcessorContext context;
    private volatile boolean open;
    private Set<WithTimestampSegment> bulkLoadSegments;
    private Sensor expiredRecordSensor;

    RocksDBSegmentedWithTimestampBytesStore(final String name,
                                            final String metricScope,
                                            final long retention,
                                            final long segmentInterval,
                                            final KeySchema<WithTimestampSegment> keySchema) {
        this.name = name;
        this.metricScope = metricScope;
        this.keySchema = keySchema;
        this.segments = new WithTimestampSegments(name, retention, segmentInterval);
    }

    @Override
    public KeyValueIterator<Bytes, byte[]> fetch(final Bytes key, final long from, final long to) {
        final List<WithTimestampSegment> searchSpace = keySchema.segmentsToSearch(segments, from, to);
    ...
    public KeyValueIterator<Bytes, byte[]> all() {
        final List<WithTimestampSegment> searchSpace = segments.allSegments();
    ...
    public KeyValueIterator<Bytes, byte[]> fetchAll(final long timeFrom, final long timeTo) {
        final List<WithTimestampSegment> searchSpace = segments.segments(timeFrom, timeTo);
    ...
    public void remove(final Bytes key) {
        final WithTimestampSegment segment = segments.getSegmentForTimestamp(keySchema.segmentTimestamp(key));
        if (segment == null) {
            return;
        }
        segment.delete(key);
    ...
    public void put(final Bytes key, final byte[] value) {
        ...
        final WithTimestampSegment segment = segments.getOrCreateSegmentIfLive(segmentId, context);
        if (segment == null) {
    ...
    // Visible for testing
    List<WithTimestampSegment> getSegments() {
    ...
    void restoreAllInternal(final Collection<KeyValue<byte[], byte[]>> records) {
        try {
            final Map<WithTimestampSegment, WriteBatch> writeBatchMap = getWriteBatches(records);
            for (final Map.Entry<WithTimestampSegment, WriteBatch> entry : writeBatchMap.entrySet()) {
                final WithTimestampSegment segment = entry.getKey();
    ...
    // Visible for testing
    Map<WithTimestampSegment, WriteBatch> getWriteBatches(final Collection<KeyValue<byte[], byte[]>> records) {
        final Map<WithTimestampSegment, WriteBatch> writeBatchMap = new HashMap<>();
    ...
    private void toggleForBulkLoading(final boolean prepareForBulkload) {
        for (final WithTimestampSegment segment: segments.allSegments()) {
