...
public class ThreadCacheTest {
    ...
    public void setUp() {
        ...
        Mockito.when(mockStreamsMetrics.taskLevelSensor(Mockito.anyString(), Mockito.anyString(), Mockito.anyString(),
    ...
    public void shouldDelete() {
        final ThreadCache cache = new ThreadCache(logContext, 10000L, mockStreamsMetrics);
    ...
    private ThreadCache setupThreadCache(final int first, final int last, final long entrySize, final boolean reverse) {
        final ThreadCache cache = new ThreadCache(logContext, entrySize, mockStreamsMetrics);
        cache.addDirtyEntryFlushListener(namespace, dirty -> { });
        int index = first;
    ...
    public void shouldFlushDirtyEntriesForNamespace() {
        final ThreadCache cache = new ThreadCache(logContext, 100000, mockStreamsMetrics);
    ...
    public void shouldEvictImmediatelyIfCacheSizeIsVerySmall() {
        final ThreadCache cache = new ThreadCache(logContext, 1, mockStreamsMetrics);
        shouldEvictImmediatelyIfCacheSizeIsZeroOrVerySmall(cache);
    }

    @Test
    public void shouldEvictImmediatelyIfCacheSizeIsZero() {
        final ThreadCache cache = new ThreadCache(logContext, 0, mockStreamsMetrics);
        shouldEvictImmediatelyIfCacheSizeIsZeroOrVerySmall(cache);
    }
    ...
    public void shouldNotLoopForEverWhenEvictingAndCurrentCacheIsEmpty() {
        ...
        final ThreadCache threadCache = new ThreadCache(logContext, maxCacheSizeInBytes, mockStreamsMetrics);
    ...
    public void shouldReturnNullIfKeyIsNull() {
        final ThreadCache threadCache = new ThreadCache(logContext, 10, mockStreamsMetrics);
    ...
    public void shouldResizeAndShrink() {
        final ThreadCache cache = new ThreadCache(logContext, 10000, mockStreamsMetrics);
        cache.put(namespace, Bytes.wrap(new byte[]{1}), cleanEntry(new byte[]{0}));
