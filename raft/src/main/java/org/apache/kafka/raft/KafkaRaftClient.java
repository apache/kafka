...
public final class KafkaRaftClient<T> implements RaftClient<T> {
    ...
    public Optional<SnapshotWriter<T>> createSnapshot(
        ...
        LogFetchInfo fetchInfo = log.read(snapshotId.offset(), Isolation.COMMITTED);

        if (fetchInfo.startOffsetMetadata.offset() != snapshotId.offset()) {
            logger.error("{}", fetchInfo.startOffsetMetadata.offset());
