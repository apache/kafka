...
class ConnectionQuotasTest {
  ...
  def brokerPropsWithDefaultConnectionLimits: Properties = {
    ...
    props.put(LISTENERS_PROP, "EXTERNAL://localhost:0,REPLICATION://localhost:1,ADMIN://localhost:2")
    // ConnectionQuotas does not limit inter-broker listener even when broker-wide connection limit is reached
    props.put(INTER_BROKER_LISTENER_NAME_PROP, "REPLICATION")
    props.put(LISTENER_SECURITY_PROTOCOL_MAP_PROP, "EXTERNAL:PLAINTEXT,REPLICATION:PLAINTEXT,ADMIN:PLAINTEXT")
    props.put(NUM_QUOTA_SAMPLES_PROP, numQuotaSamples.toString)
    props.put(QUOTA_WINDOW_SIZE_SECONDS_PROP, quotaWindowSizeSeconds.toString)
    props
  }
  ...
  def testMaxConnectionsPerIp(): Unit = {
    ...
    props.put(MAX_CONNECTIONS_PER_IP_PROP, maxConnectionsPerIp.toString)
  ...
  def testMaxBrokerWideConnectionLimit(): Unit = {
    ...
    val props = brokerPropsWithDefaultConnectionLimits
    props.put(MAX_CONNECTIONS_PROP, maxConnections.toString)
    val config = KafkaConfig.fromProps(props)
    connectionQuotas = new ConnectionQuotas(config, time, metrics)
  ...
  def testMaxListenerConnectionLimits(): Unit = {
    ...
    val listenerConfig = Map(MAX_CONNECTIONS_PROP -> listenerMaxConnections.toString).asJava
    listeners.values.foreach { listener =>
  ...
  def testBrokerConnectionRateLimitWhenActualRateBelowLimit(): Unit = {
    ...
    val props = brokerPropsWithDefaultConnectionLimits
    props.put(MAX_CONNECTION_CREATION_RATE_PROP, brokerRateLimit.toString)
    val config = KafkaConfig.fromProps(props)
    connectionQuotas = new ConnectionQuotas(config, time, metrics)
  ...
  def testListenerConnectionRateLimitWhenActualRateBelowLimit(): Unit = {
    ...
    val listenerConfig = Map(MAX_CONNECTION_CREATION_RATE_PROP -> listenerRateLimit.toString).asJava
  ...
  def testMaxListenerConnectionListenerMustBeAboveZero(): Unit = {
    ...
    val listenerConfig = Map(MAX_CONNECTION_CREATION_RATE_PROP -> maxListenerConnectionRate.toString).asJava
  ...
  def testMaxListenerConnectionRateReconfiguration(): Unit = {
    ...
    val newListenerConfig = Map(MAX_CONNECTION_CREATION_RATE_PROP -> newMaxListenerConnectionRate.toString).asJava
