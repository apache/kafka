...
object KafkaConfig {
  ...
  val AutoLeaderRebalanceEnableDoc = "Enables auto leader balancing. A background thread checks the distribution of partition leaders at regular intervals, configurable by `leader.imbalance.check.interval.seconds`. If required, it triggers a leader rebalance to the preferred leader for partitions"
