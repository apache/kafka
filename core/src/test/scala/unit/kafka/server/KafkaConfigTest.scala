...
import org.apache.kafka.server.config.{ServerTopicConfigSynonyms, ZkConfig}
...
class KafkaConfigTest {
  ...
  def testAdvertiseDefaults(): Unit = {
    ...
    props.setProperty(ZkConfig.ZK_CONNECT_PROP, "localhost:2181")
  ...
  def testFromPropsInvalid(): Unit = {
    def baseProperties: Properties = {
      ...
      validRequiredProperties.setProperty(ZkConfig.ZK_CONNECT_PROP, "127.0.0.1:2181")
    ...
    KafkaConfig.configNames.foreach { name =>
      name match {
        case ZkConfig.ZK_CONNECT_PROP => // ignore string
        case ZkConfig.ZK_SESSION_TIMEOUT_MS_PROP => assertPropertyInvalid(baseProperties, name, "not_a_number")
        case ZkConfig.ZK_CONNECTION_TIMEOUT_MS_PROP => assertPropertyInvalid(baseProperties, name, "not_a_number")
        case ZkConfig.ZK_ENABLE_SECURE_ACLS_PROP => assertPropertyInvalid(baseProperties, name, "not_a_boolean")
        case ZkConfig.ZK_MAX_IN_FLIGHT_REQUESTS_PROP => assertPropertyInvalid(baseProperties, name, "not_a_number", "0")
        case ZkConfig.ZK_SSL_CLIENT_ENABLE_PROP => assertPropertyInvalid(baseProperties, name, "not_a_boolean")
        case ZkConfig.ZK_CLIENT_CNXN_SOCKET_PROP =>  //ignore string
        case ZkConfig.ZK_SSL_KEY_STORE_LOCATION_PROP =>  //ignore string
        case ZkConfig.ZK_SSL_KEY_STORE_PASSWORD_PROP =>  //ignore string
        case ZkConfig.ZK_SSL_KEY_STORE_TYPE_PROP =>  //ignore string
        case ZkConfig.ZK_SSL_TRUST_STORE_LOCATION_PROP =>  //ignore string
        case ZkConfig.ZK_SSL_TRUST_STORE_PASSWORD_PROP =>  //ignore string
        case ZkConfig.ZK_SSL_TRUST_STORE_TYPE_PROP =>  //ignore string
        case ZkConfig.ZK_SSL_PROTOCOL_PROP =>  //ignore string
        case ZkConfig.ZK_SSL_ENABLED_PROTOCOLS_PROP =>  //ignore string
        case ZkConfig.ZK_SSL_CIPHER_SUITES_PROP =>  //ignore string
        case ZkConfig.ZK_SSL_ENDPOINT_IDENTIFICATION_ALGORITHM_PROP => //ignore string
        case ZkConfig.ZK_SSL_CRL_ENABLE_PROP => assertPropertyInvalid(baseProperties, name, "not_a_boolean")
        case ZkConfig.ZK_SSL_OCSP_ENABLE_PROP => assertPropertyInvalid(baseProperties, name, "not_a_boolean")
  ...
  def testSpecificProperties(): Unit = {
    ...
    defaults.setProperty(ZkConfig.ZK_CONNECT_PROP, "127.0.0.1:2181")
    // For ZkConnectionTimeoutMs
    defaults.setProperty(ZkConfig.ZK_SESSION_TIMEOUT_MS_PROP, "1234")
  ...
  def testNonroutableAdvertisedListeners(): Unit = {
    val props = new Properties()
    props.setProperty(ZkConfig.ZK_CONNECT_PROP, "127.0.0.1:2181")
