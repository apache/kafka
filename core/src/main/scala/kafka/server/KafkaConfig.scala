...
object KafkaConfig {
  ...
  val ProcessRolesDoc = "The roles that this process plays: 'broker', 'controller', or 'broker,controller' if it is both. " +
    "This configuration is only applicable for clusters in Raft mode (instead of ZooKeeper). Leave this config undefined or empty for Zookeeper clusters."
  ...
  val BrokerHeartbeatIntervalMsDoc = "The length of time in milliseconds between broker heartbeats. Used when running in Raft mode."
  val BrokerSessionTimeoutMsDoc = "The length of time in milliseconds that a broker lease lasts if no heartbeats are made. Used when running in Raft mode."
  val NodeIdDoc = "The node ID associated with the roles this process is playing when `process.roles` is non-empty. " +
    "This is required configuration when running in Raft mode."
  val MetadataLogDirDoc = "This configuration determines where we put the metadata log for clusters in Raft mode. " +
  ...
  val ControllerListenerNamesDoc = "A comma-separated list of the names of the listeners used by the controller. This is required " +
    "if running in Raft mode. The ZK-based controller will not use this configuration."
  ...
  private[server] val configDef = {
    ...
    new ConfigDef()
      ...
      /*
       * Raft mode configs. Note that these configs are defined as internal. We will make them public in the 3.0.0 release.
       */
    ...
    if (requiresZookeeper) {
      ...
      if (nodeId < 0) {
        throw new ConfigException(s"Missing configuration `${KafkaConfig.NodeIdProp}` which is required " +
          s"when `process.roles` is defined (i.e. when running in Raft mode).")
      }
    }
