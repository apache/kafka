    ...
    override def build(context: AuthenticationContext): KafkaPrincipal = {
      context match {
        case ctx: SaslAuthenticationContext =>
          if (ctx.server.getAuthorizationID == JaasUtils.KAFKA_SCRAM_USER)
...
class GroupEndToEndAuthorizationTest extends SaslScramSslEndToEndAuthorizationTest {
  ...
  override val kafkaPrincipal = new KafkaPrincipal(GroupPrincipalType, JaasUtils.KAFKA_SCRAM_ADMIN)
