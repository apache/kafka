...
class ReplicationQuotasTest extends ZooKeeperTestHarness {
  ...
  def shouldMatchQuotaReplicatingThroughAnAsymmetricTopology(leaderThrottle: Boolean): Unit = {
    ...
    if (leaderThrottle)
      adminZkClient.changeTopicConfig(
        topic,
        propsWith(LeaderReplicationThrottledReplicasProp, "0:100,1:101,2:102,3:103,4:104,5:105")
      )
    else
      adminZkClient.changeTopicConfig(
        topic,
        propsWith(FollowerReplicationThrottledReplicasProp, "0:106,1:106,2:106,3:107,4:107,5:107")
      )
    ...
    (0 to 5).foreach { id =>
      waitForOffsetsToMatch(msgCount, id, 100 + id)
    }
    ...
    (0 to 2).foreach { id =>
      waitForOffsetsToMatch(msgCount, id, 106)
    }
    (3 to 5).foreach { id =>
      waitForOffsetsToMatch(msgCount, id, 107)
    }
  ...
  def shouldThrottleOldSegments(): Unit = {
    ...
    adminZkClient.changeBrokerConfig(
      Seq(100),
      propsWith(DynamicConfig.Broker.LeaderReplicationThrottledRateProp, throttle.toString)
    )
  ...
  private def waitForOffsetsToMatch(offset: Int, partitionId: Int, brokerId: Int): Unit =
    waitUntilTrue(
      () => {
        offset == brokerFor(brokerId).getLogManager
          .getLog(new TopicPartition(topic, partitionId))
          .map(_.logEndOffset)
          .getOrElse(0)
      },
      s"Offsets did not match for partition $partitionId on broker $brokerId",
      60000
    )
