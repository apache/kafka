...
public class RestServerTest {
    ...
    public void testWadlDisabled() {
        checkWadlRequest(true, MediaType.TEXT_PLAIN);
    }

    @Test
    public void testWadlDefault() {
        checkWadlRequest(null, "application/vnd.sun.wadl+xml");
    }

    @Test
    public void testWadlEnabled() {
        checkWadlRequest(false, "application/vnd.sun.wadl+xml");
    }

    private void checkWadlRequest(Boolean wadlEnabled, String expectedMediaType) {
        Map<String, String> configMap = new HashMap<>(baseWorkerProps());
        if (wadlEnabled != null) {
            configMap.put(WorkerConfig.REST_ENABLE_WADL_CONFIG, String.valueOf(wadlEnabled));
        }
        ...
        Response response = request("/connectors")
            ...
            .options();
        expectedMediaType.equals(response.getMediaType());
