...
public class StoreQueryUtilsTest {
    ...
    @Test
    public void shouldReturnErrorOnNullContextForMultiVersionedKeyQueries() {
        @SuppressWarnings("unchecked") final MultiVersionedKeyQuery<String, Integer> query =
            Mockito.mock(MultiVersionedKeyQuery.class);
        @SuppressWarnings("unchecked") final VersionedKeyValueStore<String, Integer> store =
            Mockito.mock(VersionedKeyValueStore.class);
        final Position position = Position.emptyPosition().withComponent("topic", 0, 1);
        final QueryResult<ValueIterator<VersionedRecord<Integer>>> queryResult = StoreQueryUtils.handleBasicQueries(
            query,
            PositionBound.at(position),
            new QueryConfig(false),
            store,
