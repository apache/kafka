...
class KafkaConfig private(doLog: Boolean, val props: util.Map[_, _], enforceProviderAllowlist: Boolean)
  ...
  val leaderImbalanceElectionAlgorithm: String = getString(ReplicationConfigs.LEADER_IMBALANCE_ELECTION_ALGORITHM_CONFIG)
  val leaderImbalanceElectionWaitForSyncThresholdPercent: Int = getInt(ReplicationConfigs.LEADER_IMBALANCE_ELECTION_WAIT_FOR_SYNC_THRESHOLD_PERCENT_CONFIG)
  val leaderImbalanceElectionWaitForSyncMaxWaitMs: Long = getLong(ReplicationConfigs.LEADER_IMBALANCE_ELECTION_WAIT_FOR_SYNC_MAX_WAIT_MS_CONFIG)
  val leaderImbalanceElectionMaxPerRun: Int = getInt(ReplicationConfigs.LEADER_IMBALANCE_ELECTION_MAX_PER_RUN_CONFIG)
  val leaderImbalanceElectionThrottleIntervalMs: Long = getLong(ReplicationConfigs.LEADER_IMBALANCE_ELECTION_THROTTLE_INTERVAL_MS_CONFIG)
