...
class DynamicConfigTest extends ZooKeeperTestHarness {
  ...
  def shouldFailIpConfigsWithInvalidValues(): Unit = {
    adminZkClient.changeIpConfig("1.2.3.4", propsWith(QuotaConfigs.IP_CONNECTION_RATE_OVERRIDE_CONFIG, "-1"))
  }

  @Test(expected = classOf[AdminOperationException])
  def shouldFailIpConfigsWithInvalidIpv4Entity(): Unit = {
    adminZkClient.changeIpConfig("1,1.1.1", propsWith(QuotaConfigs.IP_CONNECTION_RATE_OVERRIDE_CONFIG, "2"));
  }

  @Test(expected = classOf[AdminOperationException])
  def shouldFailIpConfigsWithBadHost(): Unit = {
    adminZkClient.changeIpConfig("ip", propsWith(QuotaConfigs.IP_CONNECTION_RATE_OVERRIDE_CONFIG, "2"));
  }
}
