...
import org.apache.kafka.common.errors.{ControllerMovedException, BrokerEpochMismatchException}
...
class ControllerIntegrationTest extends ZooKeeperTestHarness {
  ...
  def testControlledShutdown() {
    ...
    controller.controlledShutdown(2, servers.find(_.config.brokerId == 2).get.kafkaController.curBrokerEpoch, controlledShutdownCallback)
    ...
    controller.controlledShutdown(1, servers.find(_.config.brokerId == 1).get.kafkaController.curBrokerEpoch, controlledShutdownCallback)
    ...
    controller.controlledShutdown(0, servers.find(_.config.brokerId == 0).get.kafkaController.curBrokerEpoch, controlledShutdownCallback)
    partitionsRemaining = resultQueue.take().get
  ...
  def testControllerRejectControlledShutdownRequestWithStaleBrokerEpoch(): Unit = {
    ...
    controller.controlledShutdown(otherBroker.config.brokerId, otherBroker.kafkaController.curBrokerEpoch - 1, {
      case scala.util.Failure(exception) if exception.isInstanceOf[BrokerEpochMismatchException] => staleBrokerEpochDetected = true
      case _ =>
    })
