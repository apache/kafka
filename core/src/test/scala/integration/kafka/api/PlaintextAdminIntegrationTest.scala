...
import org.apache.kafka.server.config.{KafkaSecurityConfigs, KafkaServerConfigs, QuotaConfigs, ServerLogConfigs, ZkConfigs}
...
class PlaintextAdminIntegrationTest extends BaseAdminIntegrationTest {
  ...
  def testDescribeAndAlterConfigs(quorum: String): Unit = {
    ...
    assertEquals(brokers(1).config.brokerId.toString, configs.get(brokerResource1).get(KafkaServerConfigs.BROKER_ID_CONFIG).value)
    ...
    val compressionType = configs.get(brokerResource1).get(KafkaServerConfigs.COMPRESSION_TYPE_CONFIG)
    assertEquals(brokers(1).config.compressionType, compressionType.value)
    assertEquals(KafkaServerConfigs.COMPRESSION_TYPE_CONFIG, compressionType.name)
    ...
    assertEquals(brokers(2).config.brokerId.toString, configs.get(brokerResource2).get(KafkaServerConfigs.BROKER_ID_CONFIG).value)
...
object PlaintextAdminIntegrationTest {
  ...
  def checkInvalidAlterConfigs(
    ...
    assertEquals(LogConfig.DEFAULT_COMPRESSION_TYPE, configs.get(brokerResource).get(KafkaServerConfigs.COMPRESSION_TYPE_CONFIG).value)
