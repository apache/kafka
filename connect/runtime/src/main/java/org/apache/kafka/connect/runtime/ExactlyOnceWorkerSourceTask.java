...
class ExactlyOnceWorkerSourceTask extends AbstractWorkerSourceTask {
    ...
    public ExactlyOnceWorkerSourceTask(ConnectorTaskId id,
        ...
        super(id, task, statusListener, initialState, keyConverter, valueConverter, headerConverter, transformationChain,
                ...
                offsetBackingStore::producer, admin, topicGroups, offsetReader, offsetWriter, workerConfig, connectMetrics, loader,
    ...
    protected void close() {
        Utils.closeQuietly(offsetBackingStore::stop, "Task-local offsets backing store");
        super.close();
    }
    ...
    private void commitTransaction() {
        ...
        if (offsetWriter.beginFlush()) {
            ...
            // No need to hold onto the returned future; the flush is guaranteed to complete once the
            // producer transaction commit completes
        ...
        if (error != null) {
            ...
            if (isPossibleTransactionTimeoutError(error)) {
                throw wrapTransactionTimeoutError(error);
            } else {
                throw new ConnectException("Failed to flush offsets and/or records for task " + id, error);
            }
        }
    ...
    protected RuntimeException interceptProducerSendException(Throwable error) {
        if (isPossibleTransactionTimeoutError(error)) {
            ...
            return super.interceptProducerSendException(error);
        }
    }
