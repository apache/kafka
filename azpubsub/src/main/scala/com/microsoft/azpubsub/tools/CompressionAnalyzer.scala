package com.microsoft.azpubsub.tools
...
object CompressionAnalyzer {
    ...
    for (arg <- opts.logFiles) {
      ...
      suffix match {
        case Log.LogFileSuffix =>
          println(s"\nAnalyzing $file")
          analyzeLog(file, opts.verbose, opts.maxBatchCount)
        case _ =>
  ...
  private class DumpLogSegmentsOptions(args: Array[String]) extends CommandDefaultOptions(args) {
    ...
    val maxbatchCount = parser.accepts("maxBatch", "Max number of batches to sample. If not specified, all batches are sampled")
      .withOptionalArg()
    ...
    lazy val maxBatchCount = options.valueOf(maxbatchCount).intValue
