...
object DynamicBrokerReconfigurationTest3 {
  val SecureInternal = "INTERNAL"
  val SecureExternal = "EXTERNAL"
}

class DynamicBrokerReconfigurationTest3 extends QuorumTestHarness with SaslSetup {

  import DynamicBrokerReconfigurationTest3._
  ...
  override def setUp(testInfo: TestInfo): Unit = {
    ...
    TestMetricsReporter2.testReporters.clear()
  }
  ...
  def testUpdatesUsingConfigProvider(): Unit = {
    ...
    assertFalse(brokerConfigs.exists(_.name == TestMetricsReporter2.PollingIntervalProp), "Initial value of polling interval")
    ...
    updatedProps.setProperty("config.providers.file.class", "kafka.server.MockFileConfigProvider2")
    updatedProps.put(KafkaConfig.MetricReporterClassesProp, classOf[TestMetricsReporter2].getName)

    // 1. update Integer property using config provider
    updatedProps.put(TestMetricsReporter2.PollingIntervalProp, PollingIntervalVal)
    ...
    waitForConfig(TestMetricsReporter2.PollingIntervalProp, "1000")
    ...
    val reporters = TestMetricsReporter2.waitForReporters(servers.size)
    ...
    assertTrue(props.getProperty(TestMetricsReporter2.PollingIntervalProp) == PollingIntervalVal, "polling interval is not updated in ZK")
    ...
    updatedProps.put(TestMetricsReporter2.PollingIntervalProp, PollingIntervalUpdateVal)
    alterConfigsUsingConfigCommand(updatedProps)
    waitForConfig(TestMetricsReporter2.PollingIntervalProp, "2000")
    reporters.foreach { reporter =>
  ...
  def testThreadPoolResize(): Unit = {
    System.err.println("threadP start")
    ...
    def verifyThreadPoolResize(propName: String, currentSize: => Int, threadPrefix: String, mayReceiveDuplicates: Boolean): Unit = {
      System.err.print(" prop:" + propName)
      System.err.flush()
  ...
  @Test
  def TestMetricsReporter2Update(): Unit = {
    // Add a new metrics reporter
    val newProps = new Properties
    newProps.put(TestMetricsReporter2.PollingIntervalProp, "100")
    configureMetricsReporters(Seq(classOf[TestMetricsReporter2]), newProps)
    ...
    assertEquals(servers.map(_.config.brokerId).toSet, TestMetricsReporter2.configuredBrokers.toSet)
    ...
    reconfigureServers(newProps, perBrokerConfig = false, (TestMetricsReporter2.PollingIntervalProp, "100"))
    ...
    newProps.put(TestMetricsReporter2.PollingIntervalProp, "1000")
    reconfigureServers(newProps, perBrokerConfig = false, (TestMetricsReporter2.PollingIntervalProp, "1000"))
    ...
    newProps.put(TestMetricsReporter2.PollingIntervalProp, "2000")
    configureMetricsReporters(Seq(classOf[TestMetricsReporter2]), newProps)
    val newReporters = TestMetricsReporter2.waitForReporters(servers.size)
    ...
    reconfigureServers(newProps, perBrokerConfig = false, (TestMetricsReporter2.PollingIntervalProp, "2000"), expectFailure = true)
    servers.foreach { server =>
      assertEquals(classOf[TestMetricsReporter2].getName, server.config.originals.get(KafkaConfig.MetricReporterClassesProp))
    }
    newReporters.foreach(_.verifyState(reconfigureCount = 0, deleteCount = 0, pollingInterval = 2000))

    // Verify that validation failure of custom config fails reconfiguration and leaves config unchanged
    newProps.put(TestMetricsReporter2.PollingIntervalProp, "invalid")
    reconfigureServers(newProps, perBrokerConfig = false, (TestMetricsReporter2.PollingIntervalProp, "2000"), expectFailure = true)
    newReporters.foreach(_.verifyState(reconfigureCount = 0, deleteCount = 0, pollingInterval = 2000))
    ...
    // configuration across brokers, they can also be defined at per-broker level for testing
    newProps.put(KafkaConfig.MetricReporterClassesProp, classOf[TestMetricsReporter2].getName)
    newProps.put(TestMetricsReporter2.PollingIntervalProp, "4000")
    alterConfigsOnServer(servers.head, newProps)
    TestUtils.waitUntilTrue(() => !TestMetricsReporter2.testReporters.isEmpty, "Metrics reporter not created")
    val perBrokerReporter = TestMetricsReporter2.waitForReporters(1).head
    perBrokerReporter.verifyState(reconfigureCount = 0, deleteCount = 0, pollingInterval = 4000)

    // update TestMetricsReporter2.PollingIntervalProp to 3000
    newProps.put(TestMetricsReporter2.PollingIntervalProp, "3000")
    alterConfigsOnServer(servers.head, newProps)
  ...
  private case class ConsumerBuilder(group: String) extends ClientBuilder[KafkaConsumer[String, String]] {
    ...
    private var _topic = DynamicBrokerReconfigurationTest3.this.topic
...
object TestMetricsReporter3 {
  val PollingIntervalProp = "polling.interval"
  val testReporters = new ConcurrentLinkedQueue[TestMetricsReporter3]()
  val configuredBrokers = mutable.Set[Int]()

  def waitForReporters(count: Int): List[TestMetricsReporter3] = {
...
class TestMetricsReporter3 extends MetricsReporter with Reconfigurable with Closeable with ClusterResourceListener {
  import TestMetricsReporter3._
...
class MockFileConfigProvider3 extends FileConfigProvider {
