...
class AdminClientWithPoliciesIntegrationTest extends KafkaServerTestHarness with Logging {
  ...
  override def generateConfigs: collection.Seq[KafkaConfig] = {
    ...
    configs.map(KafkaConfigProvider.fromProps)
  }
  ...
  private def overrideNodeConfigs(props: Properties): Unit = {
    props.put(KafkaConfig.ALTER_CONFIG_POLICY_CLASS_NAME_PROP, classOf[Policy])
  }
  ...
  def testInvalidAlterConfigsDueToPolicy(quorum: String): Unit = {
    ...
    val brokerConfigs = Seq(new ConfigEntry(KafkaConfig.MESSAGE_MAX_BYTES_PROP, "50000")).asJava
    val alterResult1 = client.alterConfigs(Map(brokerResource -> new Config(brokerConfigs)).asJava)
    alterResult1.all.get
    assertEquals(Set(KafkaConfig.MESSAGE_MAX_BYTES_PROP), validationsForResource(brokerResource).head.configs().keySet().asScala)
    validations.clear()
    ...
    val brokerConfigEntries = Seq(new ConfigEntry(KafkaConfig.SSL_TRUSTSTORE_PASSWORD_PROP, "12313")).asJava
    ...
    assertNull(configs.get(brokerResource).get(KafkaConfig.SSL_TRUSTSTORE_PASSWORD_PROP).value)
    ...
    alterResult = client.incrementalAlterConfigs(Map(
      brokerResource ->
        Seq(new AlterConfigOp(
          new ConfigEntry(KafkaConfig.MAX_CONNECTIONS_PROP, "9999"), OpType.SET)
        ).asJavaCollection
    ).asJava)
    alterResult.all.get
    assertEquals(Set(KafkaConfig.MAX_CONNECTIONS_PROP), validationsForResource(brokerResource).head.configs().keySet().asScala)
  }
