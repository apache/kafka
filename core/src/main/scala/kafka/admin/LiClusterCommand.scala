...
object LiClusterCommand extends Logging {
  ...
  class ControllerCommandOptions(args: Array[String]) extends CommandDefaultOptions(args) {
    ...
    def checkArgs(): Unit = {
      if (args.length == 0)
        CommandLineUtils.printUsageAndExit(parser, "move a controller")

      CommandLineUtils.maybePrintHelpOrVersion(this, "This tool helps to move a controller.")

      // should have exactly one action
      val actions = Seq(moveControllerOpt).count(options.has)
      if (actions != 1)
        CommandLineUtils.printUsageAndExit(parser, "Command must include exactly one action: --move-controller")
