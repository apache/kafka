...
public class SenderTest {
    ...
    public void testSequenceNumberIncrement() throws InterruptedException {
        final long producerId = 343434L;
        TransactionManager transactionManager = new TransactionManager();
        transactionManager.setPidAndEpoch(new TransactionManager.PidAndEpoch(producerId, (short) 0));
        setupWithTransactionState(transactionManager);
        client.setNode(new Node(1, "localhost", 33343));
    ...
    public void testAbortRetryWhenPidChanges() throws InterruptedException {
        ...
        transactionManager.setPidAndEpoch(new TransactionManager.PidAndEpoch(producerId + 1, (short) 0));
        sender.run(time.milliseconds()); // receive error
        sender.run(time.milliseconds()); // reconnect
