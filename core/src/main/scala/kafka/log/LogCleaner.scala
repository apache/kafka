...
class LogCleaner(val config: CleanerConfig,
  ...
  newGauge("max-buffer-utilization-percent", 
           new Gauge[Int] {
             override def getValue: Int= cleaners.map(_.lastStats).map(100 * _.bufferUtilization).max.toInt
           })
  ...
  newGauge("max-clean-time-secs",
           new Gauge[Int] {
             override def getValue: Int = cleaners.map(_.lastStats).map(_.elapsedSecs).max.toInt
           })
