...
abstract class MultipleListenersWithSameSecurityProtocolBaseTest extends QuorumTestHarness with SaslSetup {
  ...
  override def setUp(testInfo: TestInfo): Unit = {
    ...
    (0 until numServers).foreach { brokerId =>
      ...
      props.put(SASL_MECHANISM_INTER_BROKER_PROTOCOL_CONFIG, kafkaClientSaslMechanism)
      props.put(s"${new ListenerName(SecureInternal).configPrefix}${SASL_ENABLED_MECHANISMS_CONFIG}",
        kafkaServerSaslMechanisms(SecureInternal).mkString(","))
      props.put(s"${new ListenerName(SecureExternal).configPrefix}${SASL_ENABLED_MECHANISMS_CONFIG}",
        kafkaServerSaslMechanisms(SecureExternal).mkString(","))
      props.put(SASL_KERBEROS_SERVICE_NAME_CONFIG, "kafka")
  ...
  protected def addDynamicJaasSection(props: Properties, listener: String, mechanism: String, jaasSection: JaasSection): Unit = {
    ...
    props.put(s"${prefix}${SASL_JAAS_CONFIG}", jaasConfig)
  }
