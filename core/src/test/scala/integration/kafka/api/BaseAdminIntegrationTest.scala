...
import org.apache.kafka.server.config.{KafkaSecurityConfigs, KafkaServerConfigs, ReplicationConfigs, ServerLogConfigs}
...
abstract class BaseAdminIntegrationTest extends IntegrationTestHarness with Logging {
  ...
  override def modifyConfigs(configs: Seq[Properties]): Unit = {
    ...
    configs.foreach { config =>
      config.setProperty(KafkaServerConfigs.DELETE_TOPIC_ENABLE_CONFIG, "true")
      ...
      config.setProperty(KafkaServerConfigs.CONTROLLED_SHUTDOWN_ENABLE_CONFIG, "false")
