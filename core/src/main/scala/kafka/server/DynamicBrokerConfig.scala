...
object DynamicBrokerConfig {
  ...
  def brokerConfigSynonyms(name: String, matchListenerOverride: Boolean): List[String] =
    name match {
      ...
      case KafkaConfig.LogRetentionTimeMillisProp | KafkaConfig.LogRetentionTimeMinutesProp |
          KafkaConfig.LogRetentionTimeHoursProp =>
        List(KafkaConfig.LogRetentionTimeMillisProp,
             KafkaConfig.LogRetentionTimeMinutesProp,
             KafkaConfig.LogRetentionTimeHoursProp)
...
class DynamicBrokerConfig(private val kafkaConfig: KafkaConfig) extends Logging {
  ...
  private[server] val staticBrokerConfigs =
    ConfigDef.convertToStringMapWithPasswordValues(kafkaConfig.originalsFromThisConfig).asScala
  ...
  private[server] def updateBrokerConfig(brokerId: Int, persistentProps: Properties): Unit =
  ...
  private[server] def fromPersistentProps(persistentProps: Properties, perBrokerConfig: Boolean): Properties = {
    ...
    removeInvalidProps(
      securityConfigsWithoutListenerPrefix(props),
  ...
  private def maybeReEncodePasswords(persistentProps: Properties, adminZkClient: AdminZkClient): Properties = {
    ...
    if (props.asScala.keySet.exists(isPasswordConfig)) {
      maybeCreatePasswordEncoder(kafkaConfig.passwordEncoderOldSecret).foreach { passwordDecoder =>
        persistentProps.asScala.filterKeys(isPasswordConfig).foreach {
          case (configName, value) =>
            if (value != null) {
              val decoded = try {
                ...
                case _: Exception =>
                  debug(
                    s"Dynamic password config $configName could not be decoded using old secret, new secret will be used."
                  )
                  None
              }
              decoded.foreach { value =>
                props.put(configName, passwordEncoder.encode(new Password(value)))
              }
            }
        }
  ...
  private def removeInvalidConfigs(props: Properties, perBrokerConfig: Boolean): Unit =
    try {
      ...
      case e: Exception =>
        val invalidProps = props.asScala.filter {
          case (k, v) =>
  ...
  private def updatedConfigs(newProps: java.util.Map[String, _],
                             currentProps: java.util.Map[_, _]): mutable.Map[String, _] =
          ...
          case reconfigurable =>
            if (needsReconfiguration(reconfigurable.reconfigurableConfigs, updatedMap.keySet))
              processReconfigurable(reconfigurable,
                                    updatedMap.keySet,
                                    newConfig.valuesFromThisConfig,
                                    customConfigs,
                                    validateOnly)
        }
    ...
    try {
      ...
      case _: Exception =>
        throw new ConfigException(
          s"Validation of dynamic config update of $updatedConfigNames failed with class ${reconfigurable.getClass}"
        )
    }
...
class DynamicMetricsReporters(brokerId: Int, server: KafkaServer) extends Reconfigurable {
  ...
  override def reconfigure(configs: util.Map[String, _]): Unit = {
    ...
    currentReporters.values.foreach {
      case reporter: Reconfigurable =>
        dynamicConfig.maybeReconfigure(reporter, dynamicConfig.currentKafkaConfig, configs)
      case _ =>
    }
  ...
  private def createReporters(reporterClasses: util.List[String], updatedConfigs: util.Map[String, _]): Unit = {
    ...
    val reporters =
      dynamicConfig.currentKafkaConfig.getConfiguredInstances(reporterClasses, classOf[MetricsReporter], props)
...
class DynamicListenerConfig(server: KafkaServer) extends BrokerReconfigurable with Logging {
  ...
  def validateReconfiguration(newConfig: KafkaConfig): Unit = {
    ...
    if (!newAdvertisedListeners.keySet.subsetOf(newListeners.keySet))
      throw new ConfigException(
        s"Advertised listeners '$newAdvertisedListeners' must be a subset of listeners '$newListeners'"
      )
    if (!newListeners.keySet.subsetOf(newConfig.listenerSecurityProtocolMap.keySet))
      throw new ConfigException(
        s"Listeners '$newListeners' must be subset of listener map '${newConfig.listenerSecurityProtocolMap}'"
      )
    newListeners.keySet.intersect(oldListeners.keySet).foreach { listenerName =>
      ...
      if (newListenerProps != oldListenerProps)
        throw new ConfigException(
          s"Configs cannot be updated dynamically for existing listener $listenerName, " +
    ...
    if (!newAdvertisedListeners.contains(newConfig.interBrokerListenerName))
      throw new ConfigException(
        s"Advertised listener must be specified for inter-broker listener ${newConfig.interBrokerListenerName}"
      )
  }
