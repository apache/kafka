...
import org.apache.kafka.common.config.TopicConfig.{REMOTE_LOG_STORAGE_ENABLE_CONFIG, SEGMENT_BYTES_CONFIG, SEGMENT_MS_CONFIG}
...
class DefaultSupportedConfigCheckerTest {
  val checker = new DefaultSupportedConfigChecker()

  @Test
  def testIsSupported(): Unit = {
    // Test valid topic configs
    assertTrue(checker.isSupported(TOPIC, SEGMENT_BYTES_CONFIG))
    assertTrue(checker.isSupported(TOPIC, SEGMENT_MS_CONFIG))
    assertTrue(checker.isSupported(TOPIC, REMOTE_LOG_STORAGE_ENABLE_CONFIG))
