    ...
    try {
      ...
      if (logger.isTraceEnabled)
        trace("Follower %d has replica log end offset %d for partition %s. Received %d messages and leader hw %d"
          .format(replica.brokerId, replica.logEndOffset.messageOffset, topicPartition, entries.sizeInBytes, partitionData.highWatermark))
      replica.log.get.append(entries, assignOffsets = false)
      if (logger.isTraceEnabled)
        trace("Follower %d has replica log end offset %d after appending %d bytes of messages for partition %s"
          .format(replica.brokerId, replica.logEndOffset.messageOffset, entries.sizeInBytes, topicPartition))
      ...
      if (quota.isThrottled(TopicAndPartition(topic, partitionId)))
        quota.record(entries.sizeInBytes)
    } catch {
