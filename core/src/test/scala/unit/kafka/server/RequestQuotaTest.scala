...
class RequestQuotaTest extends BaseRequestTest {
  ...
  private def requestBuilder(apiKey: ApiKeys): AbstractRequest.Builder[_ <: AbstractRequest] = {
    apiKey match {
        ...
        case ApiKeys.ALTER_REPLICA_DIRS =>
          new AlterReplicaDirsRequest.Builder(Collections.singletonMap(tp, logDir))
  ...
  private def responseThrottleTime(apiKey: ApiKeys, response: Struct): Int = {
    apiKey match {
      ...
      case ApiKeys.ALTER_REPLICA_DIRS => new AlterReplicaDirsResponse(response).throttleTimeMs
