...
public class KeyValueStoreFacade<K, V> extends ReadOnlyKeyValueStoreFacade<K, V> implements KeyValueStore<K, V> {
    ...
    public KeyValueStoreFacade(final KeyValueWithTimestampStore<K, V> inner) {
        super(inner);
    }
    ...
    public void put(final K key,
                    final V value) {
        inner.put(key, value, -1L);
    }

    @Override
    public V putIfAbsent(final K key,
                         final V value) {
        final ValueAndTimestamp<V> old = inner.putIfAbsent(key, value, -1L);
    ...
    public void putAll(final List<KeyValue<K, V>> entries) {
        for (final KeyValue<K, V> entry : entries) {
            inner.put(entry.key, entry.value, -1L);
        }
    }
