...
class DeleteGroupsRequestTest(cluster: ClusterInstance) extends GroupCoordinatorBaseRequestTest(cluster) {
  ...
  private def testDeleteGroups(useNewProtocol: Boolean): Unit = {
    ...
    val (memberIdNonEmptyGroup, memberEpochNonEmptyGroup) = joinConsumerGroup("grp-non-empty", useNewProtocol)
    ...
    for (version <- 1 to ApiKeys.DELETE_GROUPS.latestVersion(isUnstableApiEnabled)) {

      // Join the consumer group. Note that we don't heartbeat here so we must use
      // a session long enough for the duration of the test.
      val (memberId, _) = joinConsumerGroup("grp", useNewProtocol)
      leaveGroup("grp", memberId, useNewProtocol)

      deleteGroups(
        List("grp-non-empty", "grp"),
        List(Errors.NON_EMPTY_GROUP, Errors.NONE),
        version.toShort
      )
