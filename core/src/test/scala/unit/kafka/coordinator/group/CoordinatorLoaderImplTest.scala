...
class CoordinatorLoaderImplTest {
  ...
  def testLoading(): Unit = {
    ...
    TestUtils.resource(new CoordinatorLoaderImpl[(String, String)](
      ...
      verify(coordinator).replayTransactionEndMarker(100L, 5, TransactionResult.COMMIT)
      verify(coordinator).replayTransactionEndMarker(500L, 10, TransactionResult.ABORT)
