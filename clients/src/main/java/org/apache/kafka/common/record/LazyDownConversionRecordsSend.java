    ...
    public long writeTo(GatheringByteChannel channel, long previouslyWritten, int remaining) throws IOException {
        if (convertedRecordsWriter == null || convertedRecordsWriter.completed()) {
            ...
            if (convertedRecordsIterator.hasNext()) {
                ...
                log.debug("Got lazy converted records for topic-partition {} with length={}", topicPartition(), convertedRecords.sizeInBytes());
            } else {
                ...
                //      Length => Int32
                //      ...
                log.debug("Constructing fake message batch for topic-partition {} for remaining length={}", topicPartition(), remaining);
