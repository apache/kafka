...
Flushing also affects **state restoration**, which writes to the store in bulk. KIP-1035 requires `atomic_flush`, so that the offsets column family stays consistent with the data it describes. As a result, memtables are flushed more often and at a smaller size than in earlier releases, and each flush produces another level-0 (L0) SST file. Restore throughput itself is unchanged. However, for a store with large state, the additional L0 files make RocksDB write stalls more likely, either on the L0 file count (`level0_slowdown_writes_trigger` / `level0_stop_writes_trigger`) or on the flush queue (`max_write_buffer_number`). A stalled restore takes longer, and its duration is less predictable.
