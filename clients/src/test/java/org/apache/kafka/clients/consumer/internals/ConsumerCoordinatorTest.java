...
public class ConsumerCoordinatorTest {
    ...
    public void setup() {
        ...
        client.setNode(node);
        this.coordinator = buildCoordinator(metrics, assignors, true);
    }
    ...
    public void testExcludeInternalTopicsConfigOption() { 
        ...
        metadata.update(TestUtils.singletonCluster(Topics.GROUP_METADATA_TOPIC_NAME, 2), time.milliseconds());
    ...
    public void testProtocolMetadataOrder() {
        ...
        try (Metrics metrics = new Metrics(time)) {
            ConsumerCoordinator coordinator = buildCoordinator(metrics, Arrays.<PartitionAssignor>asList(roundRobin, range), true);
        ...
        try (Metrics metrics = new Metrics(time)) {
            ConsumerCoordinator coordinator = buildCoordinator(metrics, Arrays.<PartitionAssignor>asList(range, roundRobin), true);
            List<ProtocolMetadata> metadata = coordinator.metadata();
            assertEquals(2, metadata.size());
