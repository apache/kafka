...
class AdminZkClient(zkClient: KafkaZkClient) extends Logging {
  ...
  def createTopicWithAssignment(topic: String,
    ...
    writeTopicPartitionAssignment(topic, partitionReplicaAssignment.mapValues(PartitionReplicaAssignment(_, List(), List())),
      isUpdate = false)
  }
  ...
  private def writeTopicPartitionAssignment(topic: String, replicaAssignment: Map[Int, PartitionReplicaAssignment], isUpdate: Boolean) {
    try {
  ...
  */
  def addPartitions(topic: String,
    ...
    proposedAssignment.mapValues(_.replicas)
  }
