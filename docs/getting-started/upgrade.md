...
### Notable changes in 4.4.0
  ...
  * Brokers can now be configured with custom task assignors for streams groups. The new broker configuration `group.streams.assignors` (default `sticky`) takes a list of entries, each of which is either the name of a built-in assignor or the fully qualified class name of a custom `TaskAssignor` implementation; the first entry is the default assignor for groups that do not select one. The new group configuration `streams.assignor.name` selects one of the registered assignors for an individual streams group by name, and when unset defaults to the first entry of `group.streams.assignors`. Setting `streams.assignor.name` to a name that is not registered on the broker is rejected with `INVALID_CONFIG`. Note that `TaskAssignor` implementations must be thread-safe, since a single instance is shared across all groups on a broker. For further details, please refer to [KIP-1357](https://cwiki.apache.org/confluence/display/KAFKA/KIP-1357%3A+Broker-side+custom+task+assignors+for+Streams+groups).
