...
class MetricsDuringTopicCreationDeletionTest extends KafkaServerTestHarness with Logging {
  ...
  def testMetricsDuringTopicCreateDelete(): Unit = {
    ...
    // For OfflinePartitionsCount and PreferredReplicaImbalanceCount even with KAFKA-4605
    ...
    assert(preferredReplicaImbalanceCount==0, "Expect PreferredReplicaImbalanceCount to be 0, but got: " + preferredReplicaImbalanceCount)
    assert(underReplicatedPartitionCount==0, "Expect UnderReplicatedPartitionCount to be 0, but got: " + underReplicatedPartitionCount)
  }
