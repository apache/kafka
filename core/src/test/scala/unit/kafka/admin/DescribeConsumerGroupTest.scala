...
class DescribeConsumerGroupTest extends ConsumerGroupCommandTest {
    ...
    TestUtils.waitUntilTrue(
      () => {
        ...
        assignments.get
          ...
          .head
          .consumerId
          .exists(_.trim != ConsumerGroupCommand.MISSING_COLUMN_VALUE) &&
        assignments.get
          .filter(_.group == group)
          .head
          .clientId
  ...
  def testDescribeExistingGroupWithNoMembers() {
    ...
    for (describeType <- describeTypes) {
      ...
      TestUtils.waitUntilTrue(
        () => {
          TestUtils
            .grabConsoleError(service.describeGroup())
            .contains(s"Consumer group '$group' has no active members.")
        },
  ...
  def testDescribeOffsetsWithConsumersWithoutAssignedPartitions() {
    ...
    TestUtils.waitUntilTrue(
      () => {
        ...
        assignments.isDefined &&
        assignments.get.count(_.group == group) == 1 &&
        assignments.get.count { x =>
          x.group == group && x.partition.isDefined
        } == 1
      },
  ...
  def testDescribeMembersWithConsumersWithoutAssignedPartitions() {
    ...
    TestUtils.waitUntilTrue(
      () => {
        ...
        assignments.get.count { x =>
          x.group == group && x.numPartitions == 1
        } == 1 &&
        assignments.get.count { x =>
          x.group == group && x.numPartitions == 0
        } == 1 &&
  ...
  def testDescribeOffsetsWithMultiPartitionTopicAndMultipleConsumers() {
    ...
    TestUtils.waitUntilTrue(
      () => {
        ...
        assignments.get.count { x =>
          x.group == group && x.partition.isEmpty
        } == 0
      },
  ...
  def testDescribeGroupWithShortInitializationTimeout() {
    ...
    try {
      ...
      fail(
        s"The consumer group command should have failed due to low initialization timeout (describe type: ${describeType
          .mkString(" ")})"
      )
    } catch {
  ...
  def testDescribeGroupMembersWithShortInitializationTimeout() {
    ...
    try {
      ...
      case e: ExecutionException =>
        assert(e.getCause.isInstanceOf[TimeoutException]) // OK
        try {
