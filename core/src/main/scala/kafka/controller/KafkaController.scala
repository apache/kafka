...
class KafkaController(val config: KafkaConfig, zkUtils: ZkUtils, val brokerState: BrokerState, time: Time, metrics: Metrics, threadNamePrefix: Option[String] = None) extends Logging with KafkaMetricsGroup {
  ...
  def onControllerFailover() {
    ...
    topicDeletionManager.start(topicsToBeDeleted, topicsIneligibleForDeletion)
  }
  ...
  private def fetchPendingPreferredReplicaElections(): Set[TopicAndPartition] = {
    ...
    val pendingPreferredReplicaElections = partitionsUndergoingPreferredReplicaElection -- partitionsThatCompletedPreferredReplicaElection
  ...
  private def fetchTopicDeletionsInProgress(pendingPreferredReplicaElections: Set[TopicAndPartition]): (Set[String], Set[String]) = {
