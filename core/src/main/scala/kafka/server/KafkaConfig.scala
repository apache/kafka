...
object KafkaConfig {
  ...
  val InterBrokerSecurityProtocolDoc = "Security protocol used to communicate between brokers. Valid values are: " +
    s"${SecurityProtocol.nonTestingValues.asScala.toSeq.map(_.name).mkString(", ")} and the default is ${SecurityProtocol.PLAINTEXT}."
  ...
  val InterBrokerListenerNameDoc = s"Name of listener used for communication between brokers. If this is unset, the listener name is defined by $InterBrokerSecurityProtocolProp. " +
    s"It is an error to set this property and $InterBrokerSecurityProtocolProp at the same time."
  /** ********* Controlled shutdown configuration ***********/
  ...
  private val configDef = {
    ...
    new ConfigDef()
      ...
      .define(InterBrokerSecurityProtocolProp, STRING, null, MEDIUM, InterBrokerSecurityProtocolDoc)
...
class KafkaConfig(val props: java.util.Map[_, _], doLog: Boolean) extends AbstractConfig(KafkaConfig.configDef, props, doLog) {
  ...
  private def getInterBrokerListenerNameAndSecurityProtocol: (ListenerName, SecurityProtocol) = {
    (Option(getString(KafkaConfig.InterBrokerListenerNameProp)), Option(getString(KafkaConfig.InterBrokerSecurityProtocolProp))) match {
      case (None, None) =>
      ...
      case (None, Some(securityProtocolName)) =>
        ...
        (ListenerName.forSecurityProtocol(securityProtocol), securityProtocol)
      case (Some(_), Some(_)) =>
        throw new ConfigException(s"Only one of ${KafkaConfig.InterBrokerListenerNameProp} and ${KafkaConfig.InterBrokerSecurityProtocolProp} should be set.")
    }
  }
