...
import org.apache.kafka.server.config.{KafkaSecurityConfigs, KafkaServerConfigs, ServerLogConfigs}
...
class AdminClientWithPoliciesIntegrationTest extends KafkaServerTestHarness with Logging {
  ...
  def testInvalidAlterConfigsDueToPolicy(quorum: String): Unit = {
    ...
    val brokerConfigs = Seq(new ConfigEntry(KafkaServerConfigs.MESSAGE_MAX_BYTES_CONFIG, "50000")).asJava
    val alterResult1 = client.alterConfigs(Map(brokerResource -> new Config(brokerConfigs)).asJava)
    alterResult1.all.get
    assertEquals(Set(KafkaServerConfigs.MESSAGE_MAX_BYTES_CONFIG), validationsForResource(brokerResource).head.configs().keySet().asScala)
    validations.clear()
