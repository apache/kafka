...
import org.apache.kafka.common.requests.{AbstractRequest, EpochEndOffset, FetchResponse, OffsetsForLeaderEpochResponse, WriteableFetchResponse, FetchMetadata => JFetchMetadata}
...
class ReplicaFetcherMockBlockingSend(offsets: java.util.Map[TopicPartition, EpochEndOffset], destination: BrokerEndPoint, time: Time) extends BlockingSend {
  ...
  override def sendRequest(requestBuilder: Builder[_ <: AbstractRequest]): ClientResponse = {
    ...
    val response = requestBuilder.apiKey() match {
      ...
      case ApiKeys.FETCH =>
        fetchCount += 1
        new WriteableFetchResponse(Errors.NONE, new java.util.LinkedHashMap[TopicPartition, WriteableFetchResponse.PartitionData], 0,
