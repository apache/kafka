...
import kafka.zk.{ZkData, ZooKeeperTestHarness}
...
class ZkNodeChangeNotificationListenerTest extends ZooKeeperTestHarness {
  ...
  def testProcessNotification() {
    ...
    val notificationListener = new ZkNodeChangeNotificationListener(zkClient,  ZkData.literalAclStore.aclChangesZNode.path,
      ZkData.literalAclStore.aclChangeNotificationSequenceZNode.SequenceNumberPrefix, notificationHandler, changeExpirationMs)
    notificationListener.init()

    zkClient.createAclChangeNotification(ZkData.literalAclStore, notificationMessage1)
    ...
    zkClient.createAclChangeNotification(ZkData.literalAclStore, notificationMessage2)
    TestUtils.waitUntilTrue(() => invocationCount == 2 && notification == notificationMessage2,
      "Failed to send/process notification message in the timeout period.")

    (3 to 10).foreach(i => zkClient.createAclChangeNotification(ZkData.literalAclStore, "message" + i))
