...
object ReassignPartitionsCommand extends Logging {
  ...
  case class Throttle(interBrokerLimit: Long,
                      replicaAlterLogDirsLimit: Long = -1,
                      postUpdateAction: () => Unit = () => ())
  ...
  def verifyAssignment(zkClient: KafkaZkClient,
  ...
  def verifyAssignment(zkClient: KafkaZkClient, adminClientOpt: Option[JAdminClient], jsonString: String): Unit = {
    ...
    reassignedPartitionsStatus.foreach {
    ...
    replicasReassignmentStatus.foreach {
      case (replica, status) =>
        status match {
          case ReassignmentCompleted =>
  ...
  private[admin] def removeThrottle(zkClient: KafkaZkClient,
    ...
    if (reassignedPartitionsStatus.forall { case (_, status) => status == ReassignmentCompleted } &&
      ...
      val topics = (reassignedPartitionsStatus.keySet.map(tp => tp.topic) ++ replicasReassignmentStatus.keySet.map(
  ...
  def generateAssignment(zkClient: KafkaZkClient, opts: ReassignPartitionsCommandOptions) {
    ...
    if (duplicateReassignments.nonEmpty)
      throw new AdminCommandFailedException(
        "Broker list contains duplicate entries: %s".format(duplicateReassignments.mkString(","))
      )
    val topicsToMoveJsonString = Utils.readFileAsString(topicsToMoveJsonFile)
    val disableRackAware = opts.options.has(opts.disableRackAware)
    val (proposedAssignments, currentAssignments) =
      generateAssignment(zkClient, brokerListToReassign, topicsToMoveJsonString, disableRackAware)
    println(
  ...
  def generateAssignment(zkClient: KafkaZkClient,
    ...
    if (duplicateTopicsToReassign.nonEmpty)
      throw new AdminCommandFailedException(
        "List of topics to reassign contains duplicate entries: %s".format(duplicateTopicsToReassign.mkString(","))
      )
  ...
  def executeAssignment(zkClient: KafkaZkClient,
    ...
    executeAssignment(zkClient,
                      ...
                      Throttle(interBrokerThrottle, replicaAlterLogDirsThrottle),
                      timeoutMs)
  }
          ...
          case (tp, replicas) =>
            Map(
              ...
              "log_dirs" -> replicas
                .map(
                  r =>
                    replicaLogDirAssignment.getOrElse(new TopicPartitionReplica(tp.topic, tp.partition, r), AnyLogDir)
                )
                .asJava
            ).asJava
        }.asJava
      ...
      case 1 =>
        ...
        for {
          ...
          if (newReplicas.size != newLogDirs.size)
            throw new AdminCommandFailedException(
              s"Size of replicas list $newReplicas is different from " +
  ...
  def parseAndValidate(
      ...
      val duplicatesMsg = duplicateEntries
        .map {
          case (tp, duplicateReplicas) =>
            "%s contains multiple entries for %s".format(tp, duplicateReplicas.mkString(","))
        }
        .mkString(". ")
      throw new AdminCommandFailedException(
        "Partition replica lists may not contain duplicate entries: %s".format(duplicatesMsg)
      )
    }
    ...
    val nonExistentPartitions =
      partitionsToBeReassigned.map { case (tp, _) => tp }.filterNot(existingAssignment.contains)
    if (nonExistentPartitions.nonEmpty)
      throw new AdminCommandFailedException(
        "The proposed assignment contains non-existent partitions: " +
    ...
    if (nonExistingBrokerIDs.nonEmpty)
      throw new AdminCommandFailedException(
        "The proposed assignment contains non-existent brokerIDs: " + nonExistingBrokerIDs.mkString(",")
      )

    (partitionsToBeReassigned, replicaAssignment)
  }

  private def checkIfPartitionReassignmentSucceeded(
    zkClient: KafkaZkClient,
    ...
    partitionsToBeReassigned.keys.map { topicAndPartition =>
      (topicAndPartition,
       checkIfPartitionReassignmentSucceeded(zkClient,
                                             topicAndPartition,
  ...
  private def checkIfReplicaReassignmentSucceeded(
    adminClientOpt: Option[JAdminClient],
    ...
    val replicaLogDirInfos = {
      if (replicaAssignment.nonEmpty) {
        val adminClient = adminClientOpt.getOrElse(
          throw new AdminCommandFailedException(
            "bootstrap-server needs to be provided in order to reassign replica to the specified log directory"
          )
        )
        ...
        val status: ReassignmentStatus = replicaLogDirInfos.get(replica) match {
          case Some(replicaLogDirInfo) =>
            if (replicaLogDirInfo.getCurrentReplicaLogDir == null) {
              println(
                s"Partition ${replica.topic()}-${replica.partition()} is not found in any live log dir on " +
              ...
              println(
                s"Partition ${replica.topic()}-${replica.partition()} on broker ${replica.brokerId()} " +
          ...
          case None =>
            println(
              s"Partition ${replica.topic()}-${replica.partition()} is not found in any live log dir on broker ${replica.brokerId()}."
            )
            ReassignmentFailed
        }
        (replica, status)
  ...
  def checkIfPartitionReassignmentSucceeded(
    ...
    partitionsToBeReassigned: Map[TopicPartition, Seq[Int]],
    partitionsBeingReassigned: Map[TopicPartition, Seq[Int]]
  ): ReassignmentStatus = {
    ...
    partitionsBeingReassigned.get(topicAndPartition) match {
      ...
      case None    =>
        ...
        val assignedReplicas =
          zkClient.getReplicasForPartition(new TopicPartition(topicAndPartition.topic, topicAndPartition.partition))
  ...
  def validateAndParseArgs(args: Array[String]): ReassignPartitionsCommandOptions = {
    ...
    if (opts.options.has(opts.verifyOpt)) {
      ...
      CommandLineUtils.checkInvalidArgs(
        ...
        opts.options,
        opts.verifyOpt,
        Set(opts.interBrokerThrottleOpt,
      ...
      CommandLineUtils.checkInvalidArgs(
        ...
        opts.options,
        opts.generateOpt,
        Set(opts.interBrokerThrottleOpt, opts.replicaAlterLogDirsThrottleOpt, opts.reassignmentJsonFileOpt)
      )
    } else if (opts.options.has(opts.executeOpt)) {
      if (!opts.options.has(opts.reassignmentJsonFileOpt))
        CommandLineUtils.printUsageAndDie(
      ...
      CommandLineUtils.checkInvalidArgs(opts.parser,
                                        opts.options,
                                        opts.executeOpt,
                                        Set(opts.topicsToMoveJsonFileOpt, opts.disableRackAware, opts.brokerListOpt))
    }
    opts
  }
  ...
  class ReassignPartitionsCommandOptions(args: Array[String]) {
    ...
    val bootstrapServerOpt = parser
      .accepts(
        "bootstrap-server",
        "the server(s) to use for bootstrapping. REQUIRED if " +
    ...
    val executeOpt =
      parser.accepts("execute", "Kick off the reassignment as specified by the --reassignment-json-file option.")
    ...
    val interBrokerThrottleOpt = parser
      .accepts(
        "throttle",
        "The movement of partitions between brokers will be throttled to this value (bytes/sec). Rerunning with this option, whilst a rebalance is in progress, will alter the throttle value. The throttle rate should be at least 1 KB/s."
      )
      .withRequiredArg()
      .describedAs("throttle")
      .ofType(classOf[Long])
      .defaultsTo(-1)
    val replicaAlterLogDirsThrottleOpt = parser
      .accepts(
        "replica-alter-log-dirs-throttle",
        "The movement of replicas between log directories on the same broker will be throttled to this value (bytes/sec). Rerunning with this option, whilst a rebalance is in progress, will alter the throttle value. The throttle rate should be at least 1 KB/s."
      )
      .withRequiredArg()
...
class ReassignPartitionsCommand(zkClient: KafkaZkClient,
  ...
  def maybeLimit(throttle: Throttle) {
    if (throttle.interBrokerLimit >= 0 || throttle.replicaAlterLogDirsLimit >= 0) {
      ...
      val proposedBrokers = proposedPartitionAssignment.values.flatten.toSeq ++ proposedReplicaAssignment.keys.toSeq
        .map(_.brokerId())
      ...
      for (id <- brokers) {
        ...
        if (throttle.replicaAlterLogDirsLimit >= 0)
          configs.put(DynamicConfig.Broker.ReplicaAlterLogDirsIoMaxBytesPerSecondProp,
                      throttle.replicaAlterLogDirsLimit.toString)
  ...
  private[admin] def assignThrottledReplicas(existingPartitionAssignment: Map[TopicPartition, Seq[Int]],
    ...
    for (topic <- proposedPartitionAssignment.keySet.map(_.topic).toSeq) {
      ...
      val leader = format(
        preRebalanceReplicaForMovingPartitions(existingPartitionAssignmentForTopic, proposedPartitionAssignmentForTopic)
      )

      //Apply follower throttle to all "move destinations".
      val follower = format(
        postRebalanceReplicasThatMoved(existingPartitionAssignmentForTopic, proposedPartitionAssignmentForTopic)
      )
  ...
  private def postRebalanceReplicasThatMoved(existing: Map[TopicPartition, Seq[Int]],
                                             proposed: Map[TopicPartition, Seq[Int]]): Map[TopicPartition, Seq[Int]] =
    //For each partition in the proposed list, filter out any replicas that exist now, and hence aren't being moved.
    proposed.map {
      case (tp, proposedReplicas) =>
        tp -> (proposedReplicas.toSet -- existing(tp)).toSeq
    }

  private def preRebalanceReplicaForMovingPartitions(
    existing: Map[TopicPartition, Seq[Int]],
    proposed: Map[TopicPartition, Seq[Int]]
  ...
  private def alterReplicaLogDirsIgnoreReplicaNotAvailable(replicaAssignment: Map[TopicPartitionReplica, String],
    ...
    val alterReplicaLogDirsResult = adminClient.alterReplicaLogDirs(
      replicaAssignment.asJava,
      new AlterReplicaLogDirsOptions().timeoutMs(timeoutMs.toInt)
    )
    val replicasAssignedToFutureDir = alterReplicaLogDirsResult.values().asScala.flatMap {
      case (replica, future) => {
        try {
          future.get()
          Some(replica)
  ...
  def reassignPartitions(throttle: Throttle = NoThrottle, timeoutMs: Long = 10000L): Boolean = {
    ...
    try {
      val validPartitions = proposedPartitionAssignment.filter {
        case (p, _) => validatePartition(zkClient, p.topic, p.partition)
      }
      if (validPartitions.isEmpty) false
        ...
        zkClient.createPartitionReassignment(
          validPartitions.map({ case (key, value) => (new TopicPartition(key.topic, key.partition), value) }).toMap
        )
        ...
        while (remainingTimeMs > 0 && replicasAssignedToFutureDir.size < proposedReplicaAssignment.size) {
          replicasAssignedToFutureDir ++= alterReplicaLogDirsIgnoreReplicaNotAvailable(
            proposedReplicaAssignment.filterKeys(replica => !replicasAssignedToFutureDir.contains(replica)),
            adminClientOpt.get,
            remainingTimeMs
          )
          Thread.sleep(100)
    ...
    partitionsOpt match {
      case Some(partitions) =>
        if (partitions.contains(partition)) {
          ...
          error(
            "Skipping reassignment of partition [%s,%d] ".format(topic, partition) +
