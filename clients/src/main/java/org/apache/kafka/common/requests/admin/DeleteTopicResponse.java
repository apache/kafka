...
public class DeleteTopicResponse extends AbstractRequestResponse {
    private static final Schema CURRENT_SCHEMA = ProtoUtils.currentResponseSchema(ApiKeys.DELETE_TOPIC.id);
    ...
    public DeleteTopicResponse(Map<String, Errors> errors) {
    ...
    public DeleteTopicResponse(Struct struct) {
        super(struct);

        Object[] topicErrorCodesStructs = struct.getArray(TOPIC_ERROR_CODES_KEY_NAME);
        errors = new HashMap<String, Errors>();
    ...
    public static DeleteTopicResponse parse(ByteBuffer buffer) {
        return new DeleteTopicResponse((Struct) CURRENT_SCHEMA.read(buffer));
    }
}
