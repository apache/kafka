...
import org.apache.kafka.server.config.{KafkaServerConfigs, ZkConfigs}
...
class DescribeAuthorizedOperationsTest extends IntegrationTestHarness with SaslSetup {
  ...
  this.serverConfig.setProperty(KafkaServerConfigs.AUTHORIZER_CLASS_NAME_CONFIG, classOf[AclAuthorizer].getName)
