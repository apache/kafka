...
public class TopicAdminTest {
    ...
    private ListOffsetResponse listOffsetsResult(TopicPartition tp1, Long offset1) {
        return listOffsetResponse(Collections.singletonMap(tp1, offset1), Errors.NONE, 1L, ListOffsetResponse.UNKNOWN_EPOCH);
    }
    ...
    private ListOffsetResponse listOffsetResponse(Map<TopicPartition, Long> offsets, Errors error, long timestamp, int leaderEpoch) {
        Map<String, List<ListOffsetResponseData.ListOffsetPartitionResponse>> responses = new HashMap<>();
        for (Map.Entry<TopicPartition, Long> entry : offsets.entrySet()) {
            TopicPartition tp = entry.getKey();
            responses.putIfAbsent(tp.topic(), new ArrayList<>());
            responses.get(tp.topic()).add(new ListOffsetResponseData.ListOffsetPartitionResponse()
        ...
        List<ListOffsetResponseData.ListOffsetTopicResponse> topics = new ArrayList<>();
        for (Map.Entry<String, List<ListOffsetResponseData.ListOffsetPartitionResponse>> response : responses.entrySet()) {
            topics.add(new ListOffsetResponseData.ListOffsetTopicResponse()
