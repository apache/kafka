...
object TestLinearWriteSpeed {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val messageSizeOpt = parser
      .accepts("message-size", "REQUIRED: The size of each message in the message set.")
      .withRequiredArg
      .describedAs("num_bytes")
      .ofType(classOf[java.lang.Integer])
      .defaultsTo(1024)
    ...
    for (i <- 0 until numFiles) {
      if (options.has(mmapOpt)) {
        ...
        writables(i) =
          new LogWritable(new File(dir, "kafka-test-" + i), new LogConfig(logProperties), scheduler, messageSet)
      } else {
  ...
  class LogWritable(val dir: File, config: LogConfig, scheduler: Scheduler, val messages: MemoryRecords)
      extends Writable {
    Utils.delete(dir)
    val log = Log(dir,
                  config,
