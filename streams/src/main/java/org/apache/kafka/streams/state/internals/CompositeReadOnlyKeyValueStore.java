...
public class CompositeReadOnlyKeyValueStore<K, V> implements ReadOnlyKeyValueStore<K, V> {
    ...
    public CompositeReadOnlyKeyValueStore(final KafkaStreams streams, final StateStoreProvider storeProvider,
    ...
    public KeyValueIterator<K, V> range(final K from, final K to) {
        ...
        return newKeyValueIterator(nextIteratorFunction);
    }
    ...
    private KeyValueIterator<K, V> newKeyValueIterator(final NextIteratorFunction<K, V, ReadOnlyKeyValueStore<K, V>> nextIteratorFunction) {
        final List<ReadOnlyKeyValueStore<K, V>> stores = getStores();
        final KeyValueIterator<K, V> iterator = new DelegatingPeekingKeyValueIterator<>(storeName,
