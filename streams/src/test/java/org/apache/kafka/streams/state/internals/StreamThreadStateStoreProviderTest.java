...
public class StreamThreadStateStoreProviderTest {
    ...
    public void before() {
        ...
        builder.addStateStore(new KeyValueStoreBuilder<>(
                Stores.inMemoryKeyValueStore("kv-store"),
                Serdes.String(),
                Serdes.String(),
                new SystemTime()), "the-processor");

        builder.addStateStore(new WindowStoreBuilder<>(
                Stores.persistentWindowStore("window-store", 10, 10, 2, false),
                Serdes.String(),
                Serdes.String(),
                new SystemTime()), "the-processor");
        ...
        final ProcessorTopology topology = InternalTopologyAccessor.getInternalTopologyBuilder(builder).build();
