...
public interface AdminApiHandler<K, V> {
    ...
    abstract class Unbatched<K, V> implements AdminApiHandler<K, V> {
        ...
        abstract ApiResult<K, V> handleSingleResponse(int brokerId, K key, AbstractResponse response);
        ...
        @Override
        public final ApiResult<K, V> handleResponse(int brokerId, Set<K> keys, AbstractResponse response) {
            if (keys.size() != 1) {
            ...
            return handleSingleResponse(brokerId, key, response);
        }
    }
}
