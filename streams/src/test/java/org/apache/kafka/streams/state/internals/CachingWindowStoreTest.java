...
public class CachingWindowStoreTest {
    ...
    public void shouldPutFetchFromCache() {
        ...
        final WindowStoreIterator<byte[]> a = cachingStore.fetch(bytesKey("a"), ofEpochMilli(10), ofMillis(0));
        final WindowStoreIterator<byte[]> b = cachingStore.fetch(bytesKey("b"), ofEpochMilli(10), ofMillis(0));
    ...
    public void shouldPutFetchRangeFromCache() {
        ...
        final KeyValueIterator<Windowed<Bytes>, byte[]> iterator = cachingStore.fetch(bytesKey("a"), bytesKey("b"), ofEpochMilli(10), ofMillis(0));
    ...
    public void shouldFetchAllWithinTimestampRange() {
        ...
        final KeyValueIterator<Windowed<Bytes>, byte[]> iterator = cachingStore.fetchAll(ofEpochMilli(0), ofMillis(7));
        for (int i = 0; i < array.length; i++) {
        ...
        final KeyValueIterator<Windowed<Bytes>, byte[]> iterator1 = cachingStore.fetchAll(ofEpochMilli(2), ofMillis(2));
        for (int i = 2; i <= 4; i++) {
            final String str = array[i];
        ...
        final KeyValueIterator<Windowed<Bytes>, byte[]> iterator2 = cachingStore.fetchAll(ofEpochMilli(5), ofMillis(2));
        for (int i = 5; i <= 7; i++) {
            final String str = array[i];
    ...
    public void shouldTakeValueFromCacheIfSameTimestampFlushedToRocks() {
        ...
        final WindowStoreIterator<byte[]> fetch = cachingStore.fetch(bytesKey("1"), ofEpochMilli(DEFAULT_TIMESTAMP), ofMillis(0));
    ...
    public void shouldIterateAcrossWindows() {
        ...
        final WindowStoreIterator<byte[]> fetch = cachingStore.fetch(bytesKey("1"), ofEpochMilli(DEFAULT_TIMESTAMP), ofMillis(WINDOW_SIZE));
    ...
    public void shouldIterateCacheAndStoreKeyRange() {
        ...
        final KeyValueIterator<Windowed<Bytes>, byte[]> fetchRange =
            cachingStore.fetch(key, bytesKey("2"), ofEpochMilli(DEFAULT_TIMESTAMP), ofMillis(WINDOW_SIZE));
    ...
    public void shouldThrowIfTryingToFetchFromClosedCachingStore() {
        cachingStore.close();
        cachingStore.fetch(bytesKey("a"), ofEpochMilli(0), ofMillis(10));
    }

    @Test(expected = InvalidStateStoreException.class)
    public void shouldThrowIfTryingToFetchRangeFromClosedCachingStore() {
        cachingStore.close();
        cachingStore.fetch(bytesKey("a"), bytesKey("b"), ofEpochMilli(0), ofMillis(10));
    }
    ...
    public void shouldFetchAndIterateOverExactKeys() {
        ...
        final List<KeyValue<Long, byte[]>> actual = toList(cachingStore.fetch(bytesKey("a"), ofEpochMilli(0), ofMillis(Long.MAX_VALUE)));
    ...
    public void shouldFetchAndIterateOverKeyRange() {
        ...
        verifyKeyValueList(
            ...
            toList(cachingStore.fetch(bytesKey("a"), bytesKey("a"), ofEpochMilli(0), ofMillis(Long.MAX_VALUE)))
        );

        verifyKeyValueList(
            mkList(windowedPair("aa", "0002", 0), windowedPair("aa", "0004", 1)),
            toList(cachingStore.fetch(bytesKey("aa"), bytesKey("aa"), ofEpochMilli(0), ofMillis(Long.MAX_VALUE)))
        );
        ...
        verifyKeyValueList(
            ...
            toList(cachingStore.fetch(bytesKey("a"), bytesKey("aa"), ofEpochMilli(0), ofMillis(Long.MAX_VALUE)))
        );
    }
    ...
    public void shouldThrowNullPointerExceptionOnFetchNullKey() {
        cachingStore.fetch(null, ofEpochMilli(1L), ofMillis(1L));
    }

    @Test(expected = NullPointerException.class)
    public void shouldThrowNullPointerExceptionOnRangeNullFromKey() {
        cachingStore.fetch(null, bytesKey("anyTo"), ofEpochMilli(1L), ofMillis(1L));
    }

    @Test(expected = NullPointerException.class)
    public void shouldThrowNullPointerExceptionOnRangeNullToKey() {
        cachingStore.fetch(bytesKey("anyFrom"), null, ofEpochMilli(1L), ofMillis(1L));
    }
