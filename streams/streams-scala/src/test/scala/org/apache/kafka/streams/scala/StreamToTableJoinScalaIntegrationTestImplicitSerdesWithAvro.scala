...
class StreamToTableJoinScalaIntegrationTestImplicitSerdesWithAvro
  extends JUnitSuite with StreamToTableJoinTestData with EmbeddedKafkaStreamsAllInOne {
  ...
  @Test def testShouldCountClicksPerRegion(): Unit = {
    ...
    runStreams(
      ...
      implicit val bs = byteArraySerde.serializer()
      ...
      userClicks
        ...
        .foreach(r => publishToKafka(userClicksTopic, r.key, r.value))
