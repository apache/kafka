...
""", stream.toString())
  ...
  def testFormatWithStandaloneFlagOnBrokerFails(): Unit = {
    ...
    assertEquals("--standalone can only be used with controllers.",
      assertThrows(classOf[TerseFailure],
  ...
  def testFormatWithoutStaticQuorumFailsWithoutInitialControllersOnController(processRoles: String): Unit = {
    ...
    properties.setProperty("log.dirs", availableDirs.mkString(","))
    assertEquals("Because controller.quorum.voters is not set, you must use the --standalone or " +
      "--initial-controllers flag to set the initial controllers on each controller.",
        assertThrows(classOf[TerseFailure],
