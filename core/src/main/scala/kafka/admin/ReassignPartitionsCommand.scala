...
import org.apache.kafka.clients.admin.{AdminClientConfig, AlterReplicaDirsOptions, DescribeReplicaLogDirsResult, AdminClient => JAdminClient}
import LogConfig._
    ...
    try {
      ...
      if (validPartitions.isEmpty) false
        ...
        if (proposedReplicaAssignment.nonEmpty) {
          // Send AlterReplicaDirsRequest to allow broker to create replica in the right log dir later if the replica
          ...
          val adminClient = adminClientOpt.getOrElse(
            throw new AdminCommandFailedException("bootstrap-server needs to be provided in order to reassign replica to the specified log directory"))
          val alterReplicaDirResult = adminClient.alterReplicaDirs(
            proposedReplicaAssignment.asJava, new AlterReplicaDirsOptions().timeoutMs(timeoutMs.toInt))
          alterReplicaDirResult.values().asScala.foreach { case (replica, future) => {
              try {
                 ...
                 * in this step. And after the reassignment znode is created, we will need to re-send AlterReplicaDirsRequest to broker
