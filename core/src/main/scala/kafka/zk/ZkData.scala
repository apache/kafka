...
import scala.collection.{breakOut, Seq}
...
object BrokerIdZNode {
  ...
  def encode(version: Int,
    ...
    if (version >= 4) {
      jsonMap += (ListenerSecurityProtocolMapKey -> advertisedEndpoints
        .map { endPoint =>
  ...
  def encode(brokerInfo: BrokerInfo): Array[Byte] = {
    ...
    val plaintextEndpoint = broker.endPoints
      .find(_.securityProtocol == SecurityProtocol.PLAINTEXT)
      .getOrElse(new EndPoint(null, -1, null, null))
    encode(brokerInfo.version,
        ...
        val endpoints =
          if (version < 1)
            throw new KafkaException(
              "Unsupported version of broker registration: " +
            ...
            val endPoint =
              new EndPoint(host, port, ListenerName.forSecurityProtocol(securityProtocol), securityProtocol)
            Seq(endPoint)
          } else {
            val securityProtocolMap = brokerInfo
              .get(ListenerSecurityProtocolMapKey)
              .map(_.to[Map[String, String]].map {
...
object ConfigEntityChangeNotificationSequenceZNode {
  ...
  def encode(sanitizedEntityPath: String): Array[Byte] =
    Json.encodeAsBytes(Map("version" -> 2, "entity_path" -> sanitizedEntityPath).asJava)
}
...
object IsrChangeNotificationSequenceZNode {
  ...
  def encode(partitions: collection.Set[TopicPartition]): Array[Byte] = {
    val partitionsJson =
      partitions.map(partition => Map("topic" -> partition.topic, "partition" -> partition.partition).asJava)
    Json.encodeAsBytes(
      Map("version" -> IsrChangeNotificationHandler.Version, "partitions" -> partitionsJson.asJava).asJava
    )
  }
  ...
  def sequenceNumber(path: String) =
    path.substring(path.lastIndexOf(SequenceNumberPrefix) + SequenceNumberPrefix.length)
}
...
object ReassignPartitionsZNode {
  ...
  case class PartitionAssignment(
    @BeanProperty @JsonProperty("version") version: Int,
  ...
  def encode(reassignmentMap: collection.Map[TopicPartition, Seq[Int]]): Array[Byte] = {
    val reassignment = PartitionAssignment(1, reassignmentMap.toSeq.map {
      case (tp, replicas) =>
...
sealed trait ZkAclChangeStore {
  ...
  def createListener(handler: AclChangeNotificationHandler, zkClient: KafkaZkClient): AclChangeSubscription = {
    ...
    val aclChangeListener =
      new ZkNodeChangeNotificationListener(zkClient, aclChangePath, ZkAclChangeStore.SequenceNumberPrefix, rawHandler)
...
case object LiteralAclChangeStore extends ZkAclChangeStore {
  ...
  def decode(bytes: Array[Byte]): Resource = {
    ...
    string.split(Separator, 2) match {
      case Array(resourceType, resourceName, _*) =>
        new Resource(ResourceType.fromString(resourceType), resourceName, PatternType.LITERAL)
      case _ =>
        throw new IllegalArgumentException("expected a string in format ResourceType:ResourceName but got " + string)
    }
  }
}
...
case object ExtendedAclChangeStore extends ZkAclChangeStore {
  ...
  def encode(resource: Resource): Array[Byte] = {
    ...
    Json.encodeAsBytes(
      ExtendedAclChangeEvent(ExtendedAclChangeEvent.currentVersion,
...
object DelegationTokenInfoZNode {
  ...
  def encode(token: DelegationToken): Array[Byte] =
    Json.encodeAsBytes(DelegationTokenManager.toJsonCompatibleMap(token).asJava)
