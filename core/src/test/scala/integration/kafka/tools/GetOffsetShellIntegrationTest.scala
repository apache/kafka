...
class GetOffsetShellIntegrationTest extends IntegrationTestHarness with Logging {
  ...
  def oneTopicOnePartitionOneMessage: Unit = {
    ...
    val offsets = getOffsets(Set(new TopicPartition(topic1, 0)))
    assertEquals(s"Must have 1 offset entry: $offsets", 1, offsets.size)
    val actualOffset = offsets(new TopicPartition(topic1, 0)).right.get.head
  ...
  def oneTopicManyPartitions: Unit = {
    ...
    val actualP2Offset = offsets(new TopicPartition(topic1, 2)).right.get.head
  ...
  def manyTopicsManyPartitions: Unit = {
    ...
    val actualT1P0Offset = offsets(new TopicPartition(topic1, 0)).right.get.head
    ...
    val actualT1P1Offset = offsets(new TopicPartition(topic1, 1)).right.get.head
    ...
    val actualT2P0Offset = offsets(new TopicPartition(topic2, 0)).right.get.head
    ...
    val actualT2P1Offset = offsets(new TopicPartition(topic2, 1)).right.get.head
  ...
  def nonExistingTopic: Unit = {
    ...
    val offsets = getOffsets(Set(new TopicPartition("topic999", 0)))
    assertEquals(s"Must have 1 offset entry: $offsets", 1, offsets.size)
  ...
  def nonExistingPartition: Unit = {
    sendRecords(topic1, 0, 1)
    val offsets = getOffsets(Set(new TopicPartition(topic1, 9999)))
    assertEquals(s"Must have 1 offset entry: $offsets", 1, offsets.size)
