...
class KStreamWindowAggregate<K, V, T, W extends Window> implements KStreamAggProcessorSupplier<K, Windowed<K>, V, T> {
    ...
    KStreamWindowAggregate(final Windows<W> windows, final String storeName, final Initializer<T> initializer, final Aggregator<K, V, T> aggregator) {
    ...
    private class KStreamWindowAggregateProcessor extends AbstractProcessor<K, V> {
        ...
        public void process(final K key, final V value) {
            ...
            for (final long windowStartMs : matchedWindows.keySet()) {
            ...
            try (WindowStoreIterator<T> iter = windowStore.fetch(key, timeFrom, timeTo)) {
                ...
                while (iter.hasNext()) {
                    final KeyValue<Long, T> entry = iter.next();
                    final W window = matchedWindows.get(entry.key);
            ...
            for (final long windowStartMs : matchedWindows.keySet()) {
                final T oldAgg = initializer.apply();
                final T newAgg = aggregator.apply(key, value, oldAgg);
