...
class SaslConsumerTest extends SaslTestHarness with Logging {
  ...
  override def setUp() {
    ...
    producerConfig.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, TestUtils.getBrokerListStrFromServers(servers, SecurityProtocol.SASL_PLAIN))
    ...
    consumerConfig.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, TestUtils.getBrokerListStrFromServers(servers, SecurityProtocol.SASL_PLAIN))
    ...
    for (i <- 0 until producerCount)
      producers += TestUtils.createNewProducer(TestUtils.getBrokerListStrFromServers(servers, SecurityProtocol.SASL_PLAIN),
        acks = 1,
        enableSasl=true)
    for (i <- 0 until consumerCount)
      consumers += TestUtils.createNewConsumer(TestUtils.getBrokerListStrFromServers(servers, SecurityProtocol.SASL_PLAIN),
        groupId = "my-test",
