...
object ConsumerPerformance extends LazyLogging {
  ...
  class ConsumerPerfConfig(args: Array[String]) extends PerfConfig(args) {
    ...
    if (props.getProperty(ConsumerConfig.CLIENT_ID_CONFIG) == null)
      props.put(ConsumerConfig.CLIENT_ID_CONFIG, "comsumer-per-client")
    if (props.getProperty(ConsumerConfig.RECEIVE_BUFFER_CONFIG) == null)
      props.put(ConsumerConfig.RECEIVE_BUFFER_CONFIG, options.valueOf(socketBufferSizeOpt).toString)
    if (props.getProperty(ConsumerConfig.MAX_PARTITION_FETCH_BYTES_CONFIG) == null)
    ...
    if (props.getProperty(ConsumerConfig.CHECK_CRCS_CONFIG) == null)
