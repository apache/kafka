...
class DynamicBrokerReconfigurationTest extends QuorumTestHarness with SaslSetup {
  ...
  override def setUp(testInfo: TestInfo): Unit = {
    ...
    (0 until numServers).foreach { brokerId =>
      ...
      props.put(INTER_BROKER_LISTENER_NAME_CONFIG, SecureInternal)
      ...
      props.put(NUM_REPLICA_FETCHERS_CONFIG, "2") // greater than one to test reducing threads
  ...
  def testDefaultTopicConfig(): Unit = {
    ...
    props.put(UNCLEAN_LEADER_ELECTION_ENABLE_CONFIG, "false")
  ...
  def testUncleanLeaderElectionEnable(): Unit = {
    ...
    val newProps = new Properties
    newProps.put(UNCLEAN_LEADER_ELECTION_ENABLE_CONFIG, "true")
    TestUtils.incrementalAlterConfigs(servers, adminClients.head, newProps, perBrokerConfig = false).all.get
    waitForConfigOnServer(controller, UNCLEAN_LEADER_ELECTION_ENABLE_CONFIG, "true")
  ...
  def testThreadPoolResize(): Unit = {
    ...
    verifyThreadPoolResize(NUM_REPLICA_FETCHERS_CONFIG, config.numReplicaFetchers,
  ...
  def testAdvertisedListenerUpdate(): Unit = {
    ...
    val props = new Properties
    props.put(INTER_BROKER_LISTENER_NAME_CONFIG, SecureExternal)
    val e = assertThrows(classOf[ExecutionException], () => reconfigureServers(props, perBrokerConfig = true, (INTER_BROKER_LISTENER_NAME_CONFIG, SecureExternal)))
