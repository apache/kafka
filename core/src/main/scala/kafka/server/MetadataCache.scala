...
class MetadataCache(brokerId: Int) extends Logging {
  ...
  private def getEndpoints(brokers: Iterable[Int],
      ...
      partitions.map {
        case (partitionId, partitionState) =>
          ...
          val offlineReplicaInfo =
            getEndpoints(partitionState.offlineReplicas.asScala.map(_.toInt), listenerName, errorUnavailableEndpoints)
          ...
          maybeLeader match {
            case None =>
              val error = if (!aliveBrokers.contains(brokerId)) { // we are already holding the read lock
                ...
                debug(
                  s"Error while fetching metadata for $topicPartition: listener $listenerName not found on leader $leaderBrokerId"
                )
            ...
            case Some(leader) =>
              ...
              if (replicaInfo.size < replicas.size) {
                debug(
                  s"Error while fetching metadata for $topicPartition: replica information not available for " +
                    s"following brokers ${replicas.filterNot(replicaInfo.map(_.id).contains).mkString(",")}"
                )

                new MetadataResponse.PartitionMetadata(Errors.REPLICA_NOT_AVAILABLE,
                                                       partitionId,
                                                       leader,
                                                       replicaInfo.asJava,
                                                       isrInfo.asJava,
                ...
                debug(
                  s"Error while fetching metadata for $topicPartition: in sync replica information not available for " +
  ...
  def getTopicMetadata(topics: Set[String],
    ...
    inReadLock(partitionMetadataLock) {
      topics.toSeq.flatMap { topic =>
        getPartitionMetadata(topic, listenerName, errorUnavailableEndpoints, errorUnavailableListeners).map {
          partitionMetadata =>
  ...
  def getClusterMetadata(clusterId: String, listenerName: ListenerName): Cluster =
    inReadLock(partitionMetadataLock) {
      ...
      new Cluster(
        clusterId,
        nodes.values.filter(_ != null).toList.asJava,
        ...
        case (tp, info) =>
          ...
          if (info.basePartitionState.leader == LeaderAndIsr.LeaderDuringDelete) {
            ...
            stateChangeLogger.trace(
              s"Deleted partition $tp from metadata cache in response to UpdateMetadata " +
            ...
            stateChangeLogger.trace(
              s"Cached leader info $info for partition $tp in response to " +
