...
public class ProducerConfig extends AbstractConfig {
    ...
    private static final String METADATA_MAX_IDLE_DOC = "Controls how long the producer will cache metadata for a "
            + "topic that's idle. If the elapsed time since a topic was last produced to exceeds the metadata idle "
            + "duration, then the topic's metadata is forgotten and the next access to it will force a metadata "
            + "fetch request.";

    /** <code>batch.size</code> */
    public static final String BATCH_SIZE_CONFIG = "batch.size";
    private static final String BATCH_SIZE_DOC = "The producer will attempt to batch records together into fewer "
            + "requests whenever multiple records are being sent to the same partition. This helps performance on "
            ...
            + "will disable batching entirely). A very large batch size may use memory a bit more wastefully as we "
            + "will always allocate a buffer of the specified batch size in anticipation of additional records.";

    /** <code>acks</code> */
    public static final String ACKS_CONFIG = "acks";
    private static final String ACKS_DOC = "The number of acknowledgments the producer requires the leader to have "
            + "received before considering a request complete. This controls the  durability of records that are sent. "
            ...
            + "<code>retries</code> configuration will not take effect (as the client won't generally know of "
            ...
            + "respond  without awaiting full acknowledgement from all followers. In this case should the leader fail "
            + "immediately after acknowledging the record but before the followers have replicated it then the record "
            + "will be lost. <li><code>acks=all</code> This means the leader will wait for the full set of in-sync "
            + "replicas to acknowledge the record. This guarantees that the record will not be lost as long as at "
    ...
    private static final String LINGER_MS_DOC = "The producer groups together any records that arrive in between "
            + "request transmissions into a single batched request. Normally this occurs only under load when records "
            + "arrive faster than they can be sent out. However in some circumstances the client may want to "
            + "reduce the number of requests even under moderate load. This setting accomplishes this by adding a "
            + "small amount of artificial delay&mdash;that is, rather than immediately sending out a record the "
            + "producer will wait for up to the given delay to allow other records to be sent so that the sends can be "
            + "batched together. This can be thought of as analogous to Nagle's algorithm in TCP. This setting gives "
            + "the upper bound on the delay for batching: once we get <code>" + BATCH_SIZE_CONFIG + "</code> worth of "
            + "records for a partition it will be sent immediately regardless of this setting, however if we have "
            + "fewer than this many bytes accumulated for this partition we will 'linger' for the specified time "
            + "waiting for more records to show up. This setting defaults to 0 (i.e. no delay). Setting <code>"
            + LINGER_MS_CONFIG + "=5</code>, for example, would have the effect of reducing the number of requests "
    ...
    private static final String DELIVERY_TIMEOUT_MS_DOC = "An upper bound on the time to report success or failure "
            ...
            + "for retriable send failures. The producer may report failure to send a record earlier than this config "
            + "if either an unrecoverable error is encountered, the retries have been exhausted, "
            + "or the record is added to a batch which reached an earlier delivery expiration deadline. "
            + "The value of this config should be greater than or equal to the sum of <code>"
            + REQUEST_TIMEOUT_MS_CONFIG + "</code> and <code>" + LINGER_MS_CONFIG + "</code>.";
    ...
    private static final String MAX_REQUEST_SIZE_DOC = "The maximum size of a request in bytes. This setting will "
            + "limit the number of record batches the producer will send in a single request to avoid "
            + "sending huge requests. This is also effectively a cap on the maximum uncompressed record batch size. "
            + "Note that the server has its own cap on the record batch size (after compression if compression "
            + "is enabled) which may be different from this.";
    ...
    private static final String MAX_BLOCK_MS_DOC = "The configuration controls how long the "
            + "<code>KafkaProducer</code>'s <code>send()</code>, <code>partitionsFor()</code>, "
            ...
            + "For <code>send()</code> this timeout bounds the total time waiting for both metadata fetch and buffer "
            + "allocation (blocking in the user-supplied serializers or partitioner is not counted against this "
            + "timeout). For <code>partitionsFor()</code> this timeout bounds the time spent waiting for metadata if "
            + "it is unavailable. The transaction-related methods always block, but may timeout if "
            + "the transaction coordinator could not be discovered or did not respond within the timeout.";

    /** <code>buffer.memory</code> */
    public static final String BUFFER_MEMORY_CONFIG = "buffer.memory";
    private static final String BUFFER_MEMORY_DOC = "The total bytes of memory the producer can use to buffer records "
            + "waiting to be sent to the server. If records are sent faster than they can be delivered to the server "
            + "the producer will block for "
    ...
    private static final String COMPRESSION_TYPE_DOC = "The compression type for all data generated by the producer. "
            ...
            + "<code>snappy</code>, <code>lz4</code>, or <code>zstd</code>. Compression is of full batches of data, so "
    ...
    private static final String MAX_IN_FLIGHT_REQUESTS_PER_CONNECTION_DOC = "The maximum number of unacknowledged "
            + "requests the client will send on a single connection before blocking. Note that if this config is set "
            + "to be greater than 1 and <code>enable.idempotence</code> is set to false, there is a risk of"
    ...
    private static final String RETRIES_DOC = "Setting a value greater than zero will cause the client to resend any "
            + "record whose send fails with a potentially transient error. Note that this retry is no different than "
            ...
            + "</code> to 1 will potentially change the ordering of records because if two batches are sent to "
            + "a single partition, and the first fails and is retried but the second succeeds, then the records in the "
            + "second batch may appear first. Note additionally that produce requests will be failed before the number "
            + "of retries has been exhausted if the timeout configured by <code>" + DELIVERY_TIMEOUT_MS_CONFIG
            + "</code> expires first before successful acknowledgement. Users should generally prefer to leave this "
            + "config unset and instead use <code>" + DELIVERY_TIMEOUT_MS_CONFIG + "</code> to control retry behavior.";
    ...
    private static final String PARTITIONER_CLASS_DOC = "A class to use to determine which partition to be send to "
            + "when produce the records. Available options are:"
            + "<ul>"
            +     "<li>"
            +          "<code>org.apache.kafka.clients.producer.internals.DefaultPartitioner</code>: "
            +          "The default partitioner. "
            +          "This strategy will try sticking to a partition until the batch is full, or "
            +          "<code>linger.ms</code> is up. It works with the strategy:"
            +          "<ul>"
            +              "<li>"
            +                    "If no partition is specified but a key is present, choose a partition based on "
            +                    "a hash of the key"
            +              "</li>"
            +              "<li>"
            +                    "If no partition or key is present, choose the sticky partition that changes when the "
            +                    "batch is full, or <code>linger.ms</code> is up."
            +              "</li>"
            +          "</ul>"
            +     "</li>"
            +     "<li>"
            +          "<code>org.apache.kafka.clients.producer.RoundRobinPartitioner</code>: "
            +          "This partitioning strategy is that each record in a series of consecutive records "
            +          "will be sent to a different partition(no matter if the 'key' is provided or not), "
            +          "until we run out of partitions and start over again. Note: There's a known issue "
            +          "that will cause uneven distribution when new batch is created. "
            +          "Please check KAFKA-9965 for more detail."
            +     "</li>"
            +     "<li>"
            +          "<code>org.apache.kafka.clients.producer.UniformStickyPartitioner</code>: "
            +          "This partitioning strategy will try sticking to a partition(no matter if the 'key' is "
            +          "provided or not) until the batch is full, or <code>linger.ms</code> is up."
            +      "</li>"
            +  "</ul>"
            +  "<p>Implementing the <code>org.apache.kafka.clients.producer.Partitioner</code> interface "
    ...
    public static final String INTERCEPTOR_CLASSES_DOC = "A list of classes to use as interceptors. "
            ...
            + "you to intercept (and possibly mutate) the records received by the producer before they are published "
            + "to the Kafka cluster. By default, there are no interceptors.";
    ...
    public static final String ENABLE_IDEMPOTENCE_DOC = "When set to 'true', the producer will ensure that exactly "
            ...
            + "</code> to be less than or equal to 5 "
            + "(with message ordering preserved for any allowable value), <code>" + RETRIES_CONFIG
            + "</code> to be greater than 0, and <code>"
            + ACKS_CONFIG + "</code> must be 'all'. If these values are not explicitly set by the user, suitable "
            + "values will be chosen. If incompatible values are set, a <code>ConfigException</code> will be thrown.";
    ...
    public static final String TRANSACTION_TIMEOUT_DOC = "The maximum amount of time in ms that the transaction "
            + "coordinator will wait for a transaction status update from the producer before proactively aborting the "
            + "ongoing transaction. If this value is larger than the transaction.max.timeout.ms setting in the broker, "
            + "the request will fail with a <code>InvalidTxnTimeoutException</code> error.";
    ...
    public static final String TRANSACTIONAL_ID_DOC = "The TransactionalId to use for transactional delivery. "
            ...
            + "to guarantee that transactions using the same TransactionalId have been completed prior to starting any "
            + "new transactions. If no TransactionalId is provided, then the producer is limited to idempotent "
            + "delivery. If a TransactionalId is configured, <code>enable.idempotence</code> is implied. "
    ...
    static {
        CONFIG = new ConfigDef()
                ...
                .define(LINGER_MS_CONFIG,
                        ...
                        Importance.MEDIUM,
                        LINGER_MS_DOC)
                .define(DELIVERY_TIMEOUT_MS_CONFIG,
                        Type.INT,
                        120 * 1000, atLeast(0),
                        Importance.MEDIUM,
