...
Processors that are linked to the same state store by returning the same `StoreBuilder` instance from `ConnectedStoreProvider#stores()` are now placed in a single subtopology, as the `ConnectedStoreProvider` Javadoc describes ([KAFKA-20464](https://issues.apache.org/jira/browse/KAFKA-20464)). Previously they were split across separate subtopologies, so each of the resulting tasks built its own instance of the store, backed by the same `<application.id>-<store name>-changelog` topic — the processors did not in fact share a store. Linking processors to a store by store name was never affected and behaves as before. Applications that hit the old behavior will see their subtopologies regrouped after upgrading: task IDs from the previously separate subtopology no longer exist, and state for the regrouped tasks is restored from the changelog topic on first startup.
