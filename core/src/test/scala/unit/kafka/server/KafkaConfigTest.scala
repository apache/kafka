...
class KafkaConfigTest {
  ...
  def testCaseInsensitiveListenerProtocol(): Unit = {
    ...
    val config = KafkaConfig.fromProps(props)
    assertEquals(Some("SSL://localhost:9092"), config.listeners.find(_.listenerName == "SSL").map(JTestUtils.endpointToString))
    assertEquals(Some("PLAINTEXT://localhost:9091"), config.listeners.find(_.listenerName == "PLAINTEXT").map(JTestUtils.endpointToString))
  }
