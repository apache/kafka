...
public final class WaitForEarliestLocalOffsetAction implements TieredStorageTestAction {
    ...
    public void doExecute(TieredStorageTestContext context) throws ExecutionException, InterruptedException {
        ...
        for (BrokerLocalStorage localStorage : localStorages) {
            ...
            if (isAssignedReplica && context.isActive(localStorage.getBrokerId())) {
    ...
    public void describe(PrintStream output) {
        output.println("wait-for-earliest-local-offset topic-partition: " + topicPartition
                + ", earliest-local-offset: " + earliestLocalOffset);
    }
}
