...
import org.apache.kafka.image.{AclsImage, ClientQuotasImage, ClusterImageTestFixtures, ConfigurationsImage, DelegationTokenImage, FeaturesImage, MetadataDelta, MetadataImage, MetadataImageTestFixtures, MetadataProvenance, ProducerIdsImage, ScramImage, TopicsImage}
...
class BrokerMetadataPublisherTest {
  ...
  def testGetTopicDelta(): Unit = {
    assert(BrokerMetadataPublisher.getTopicDelta(
      "not-a-topic",
      MetadataImageTestFixtures.IMAGE1,
      MetadataImageTestFixtures.DELTA1).isEmpty, "Expected no delta for unknown topic")

    assert(BrokerMetadataPublisher.getTopicDelta(
      "foo",
      MetadataImageTestFixtures.IMAGE1,
      MetadataImageTestFixtures.DELTA1).isEmpty, "Expected no delta for deleted topic")

    assert(BrokerMetadataPublisher.getTopicDelta(
      "bar",
      MetadataImageTestFixtures.IMAGE1,
      MetadataImageTestFixtures.DELTA1).isDefined, "Expected to see delta for changed topic")
  }
