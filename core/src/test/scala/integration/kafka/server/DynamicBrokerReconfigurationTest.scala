...
import org.apache.kafka.server.config.{ConfigType, KafkaSecurityConfigs, KafkaServerConfigs, ReplicationConfigs, ServerLogConfigs, ZkConfigs}
...
class DynamicBrokerReconfigurationTest extends QuorumTestHarness with SaslSetup {
  ...
  def testLogCleanerConfig(quorum: String): Unit = {
    ...
    props.put(KafkaServerConfigs.MESSAGE_MAX_BYTES_CONFIG, "40000")
  ...
  def testDefaultTopicConfig(): Unit = {
    ...
    props.put(KafkaServerConfigs.MESSAGE_MAX_BYTES_CONFIG, "100000")
    ...
    props.put(KafkaServerConfigs.COMPRESSION_TYPE_CONFIG, "gzip")
  ...
  def testThreadPoolResize(): Unit = {
    ...
    verifyThreadPoolResize(KafkaServerConfigs.NUM_IO_THREADS_CONFIG, config.numIoThreads,
    ...
    verifyThreadPoolResize(KafkaServerConfigs.BACKGROUND_THREADS_CONFIG, config.backgroundThreads,
    ...
    verifyThreadPoolResize(KafkaServerConfigs.NUM_NETWORK_THREADS_CONFIG, config.numNetworkThreads,
...
class TestMetricsReporter extends MetricsReporter with Reconfigurable with Closeable with ClusterResourceListener {
  ...
  override def configure(configs: util.Map[String, _]): Unit = {
    configuredBrokers += configs.get(KafkaServerConfigs.BROKER_ID_CONFIG).toString.toInt
    configureCount += 1
