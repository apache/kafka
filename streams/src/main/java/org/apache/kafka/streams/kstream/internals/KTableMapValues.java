...
class KTableMapValues<K, V, V1> implements KTableProcessorSupplier<K, V, V1> {
    ...
    private class KTableMapValuesProcessor extends AbstractProcessor<K, Change<V>> {
        ...
        public void init(final ProcessorContext context) {
            super.init(context);
            if (queryableName != null) {
                store = (KeyValueWithTimestampStore) context.getStateStore(queryableName);
    ...
    private class KTableMapValuesValueGetter implements KTableValueGetter<K, V1> {
        ...
        public ValueAndTimestamp<V1> get(final K key) {
            ...
            return resultValue == null ? null : new ValueAndTimestampImpl<>(resultValue, parentValueAndTimestamp.timestamp());  // this is a potential NPE -- if `parentValueAndTimestamp == null` we could also directly return `null`, but this would be a semantic change
        }
