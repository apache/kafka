...
import kafka.server.share.{SharePartitionManager, ShareSessionContext}
...
import org.apache.kafka.common.message.ShareFetchRequestData.AcknowledgementBatch
...
class KafkaApisTest extends Logging {
  ...
  private val sharePartitionManager = mock(classOf[SharePartitionManager])
  ...
  def testHandleShareFetchRequestSuccessWithAcknowledgements(): Unit = {
    ...
    ).thenReturn(new ShareSessionContext(new ShareFetchMetadata(memberId, 1), new ShareSession(
  ...
  def testProcessShareAcknowledgeResponse(): Unit = {
    ...
    kafkaApis = createKafkaApis(
      overrideProperties = Map(
        ...
        ShareGroupConfig.SHARE_GROUP_ENABLE_CONFIG -> "true"),
      raftSupport = true)
    val response = kafkaApis.processShareAcknowledgeResponse(responseAcknowledgeData, request, topicNames)
    val responseData = response.data()
    val topicResponses = responseData.responses()
