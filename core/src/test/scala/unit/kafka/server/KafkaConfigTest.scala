...
import org.apache.kafka.server.config.{KafkaSecurityConfigs, ReplicationConfigs, ServerLogConfigs, ServerQuotaConfigs, ServerTopicConfigSynonyms, ZkConfigs}
  ...
  def testFromPropsInvalid(): Unit = {
    ...
    KafkaConfig.configNames.foreach { name =>
      name match {
        ...
        case ServerQuotaConfigs.QUOTA_WINDOW_SIZE_SECONDS_CONFIG => assertPropertyInvalid(baseProperties, name, "not_a_number", "0")
  ...
  def testDynamicLogConfigs(): Unit = {
    ...
    ServerTopicConfigSynonyms.TOPIC_CONFIG_SYNONYMS.forEach { case (logConfig, kafkaConfigProp) =>
      logConfig match {
        ...
        case ServerQuotaConfigs.FOLLOWER_REPLICATION_THROTTLED_REPLICAS_CONFIG =>
        // topic only config
        case ServerQuotaConfigs.LEADER_REPLICATION_THROTTLED_REPLICAS_CONFIG =>
        // topic only config
        case prop =>
