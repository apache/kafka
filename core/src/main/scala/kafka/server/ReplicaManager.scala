  ...
  def getResponseFor(replicaManager: ReplicaManager, requestedEpochInfo: JMap[TopicPartition, Integer]): JMap[TopicPartition, EpochEndOffset] = {
    ...
    requestedEpochInfo.asScala.map { case (tp, epoch) =>
      val offset = try {
        ...
        case e: NotLeaderForPartitionException => new EpochEndOffset(NOT_LEADER_FOR_PARTITION, UNDEFINED_OFFSET)
        case e: UnknownTopicOrPartitionException => new EpochEndOffset(UNKNOWN_TOPIC_OR_PARTITION, UNDEFINED_OFFSET)
      }
      (tp, offset)
    }.toMap.asJava
  }
