...
public interface HeadersBytesStore {
    ...
    static byte[] convertToHeaderFormat(final byte[] key, final byte[] value) {
        ...
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream();
            ...
            // Write format: [headersSize(varint)][headersBytes][payload]
            ByteUtils.writeVarint(emptyHeadersBytes.length, out);  // headersSize
            out.write(emptyHeadersBytes);                          // headersBytes: [count=0]
