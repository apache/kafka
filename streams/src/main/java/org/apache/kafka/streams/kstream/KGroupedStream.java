...
public interface KGroupedStream<K, V> {
    ...
    /**
     * Combine the values of records in this stream by the grouped key with read-only keys.
     * Note that provided key in {@link ReducerWithKey} is read-only and should not be modified.
     ...
     * Furthermore, updates to the store are sent downstream into a {@link KTable} changelog stream.
     * <p>
     * The specified {@link ReducerWithKey} is applied for each input record and computes a new aggregate using the current
     * aggregate and the record's value.
     * If there is no current aggregate the {@link ReducerWithKey} is not applied and the new aggregate will be the
     * record's value as-is.
     * Thus, {@code reduce(ReducerWithKey, String)} can be used to compute aggregate functions like sum, min, or max.
     * <p>
     * Not all updates might get sent downstream, as an internal cache is used to deduplicate consecutive updates to
     * the same key.
     * The rate of propagated updates depends on your input data rate, the number of distinct keys, the number of
     ...
     * You can retrieve all generated internal topic names via {@link KafkaStreams#toString()}.
     *
     * @param reducerWithKey a {@link ReducerWithKey} that computes a new aggregate result given read-only key
     * @return a {@link KTable} that contains "update" records with unmodified keys, and values that represent the
     * latest (rolling) aggregate for each key
     */
     ...
     * }</pre>
     * <p>
     * If there is no current aggregate the {@link ReducerWithKey} is not applied and the new aggregate will be the record's
     * value as-is.
     * Thus, {@code reduce(ReducerWithKey, String)} can be used to compute aggregate functions like sum, min, or max.
     * <p>
     * Not all updates might get sent downstream, as an internal cache is used to deduplicate consecutive updates to
     ...
     * You can retrieve all generated internal topic names via {@link KafkaStreams#toString()}.
     *
     * @param reducerWithKey     a {@link ReducerWithKey} that computes a new aggregate result with read-only key.
     * @param queryableStoreName the name of the underlying {@link KTable} state store; valid characters are ASCII
     *                           alphanumerics, '.', '_' and '-'. If {@code null} then this will be equivalent to
     *                           {@link KGroupedStream#reduce(Reducer)} ()}.
     * @return a {@link KTable} that contains "update" records with unmodified keys, and values that represent the
     * latest (rolling) aggregate for each key
     */
    KTable<K, V> reduce(final ReducerWithKey<K, V> reducerWithKey,
     ...
     * If there is no current aggregate the {@link ReducerWithKey} is not applied and the new aggregate will be the record's
     * value as-is.
     * Thus, {@code reduce(ReducerWithKey, StateStoreSupplier)} can be used to compute aggregate functions like sum, min, or
     * max.
     * <p>
     * Not all updates might get sent downstream, as an internal cache is used to deduplicate consecutive updates to
     * the same key.
     ...
     * If there is no current aggregate the {@link ReducerWithKey} is not applied and the new aggregate will be the
     * record's value as-is.
     * Thus, {@code reduce(ReducerWithKey, Windows, String)} can be used to compute aggregate functions like sum, min,
     * or max.
     * <p>
     * Not all updates might get sent downstream, as an internal cache is used to deduplicate consecutive updates to
     * the same window and key.
     ...
     * If there is no current aggregate the {@link ReducerWithKey} is not applied and the new aggregate will be the record's
     * value as-is.
     * Thus, {@code reduce(ReducerWithKey, Windows, String)} can be used to compute aggregate functions like sum, min, or max.
     * <p>
     * Not all updates might get sent downstream, as an internal cache is used to deduplicate consecutive updates to
     * the same window and key.
     * The rate of propagated updates depends on your input data rate, the number of distinct keys, the number of
     ...
     * If there is no current aggregate the {@link ReducerWithKey} is not applied and the new aggregate will be the record's
     * value as-is.
     * Thus, {@code reduce(ReducerWithKey, Windows, StateStoreSupplier)} can be used to compute aggregate functions like sum,
     * min, or max.
     * <p>
     * Not all updates might get sent downstream, as an internal cache is used to deduplicate consecutive updates to
     * the same window and key.
     ...
     * If there is no current aggregate the {@link ReducerWithKey} is not applied and the new aggregate will be the record's
     * value as-is.
     * Thus, {@code reduce(ReducerWithKey, SessionWindows, String)} can be used to compute aggregate functions like sum, min,
     * or max.
     * <p>
     * Not all updates might get sent downstream, as an internal cache is used to deduplicate consecutive updates to
     * the same window and key.
     ...
     * provide {@code queryableStoreName}, and "-changelog" is a fixed suffix.
     * You can retrieve all generated internal topic names via {@link KafkaStreams#toString()}.
     * @param reducerWithKey         the instance of {@link ReducerWithKey}
     * @param sessionWindows         the specification of the aggregation {@link SessionWindows}
     * @param queryableStoreName     the name of the state store created from this operation; valid characters are ASCII
     * alphanumerics, '.', '_' and '-'. If {@code null} then this will be equivalent to {@link KGroupedStream#reduce(Reducer, SessionWindows)} ()}.
     * @return a windowed {@link KTable} that contains "update" records with unmodified keys, and values that represent
     ...
     * If there is no current aggregate the {@link ReducerWithKey} is not applied and the new aggregate will be the
     * record's value as-is.
     * Thus, {@code reduce(ReducerWithKey, SessionWindows, String)} can be used to compute aggregate functions like sum,
     * min, or max.
     * <p>
     * Not all updates might get sent downstream, as an internal cache is used to deduplicate consecutive updates to
     * the same window and key.
     ...
     * If there is no current aggregate the {@link ReducerWithKey} is not applied and the new aggregate will be the record's
     * value as-is.
     * Thus, {@code reduce(ReducerWithKey, SessionWindows, StateStoreSupplier)} can be used to compute aggregate
     ...
     * Furthermore, updates to the store are sent downstream into a {@link KTable} changelog stream.
     * <p>
     * The specified {@link InitializerWithKey} is applied once directly before the first input record is processed to
     * provide an initial intermediate aggregation result that is used to process the first record.
     * Note that provided key in {@link InitializerWithKey} is read-only and should not be modified.
     * Any key modification can result in corrupt partitioning.
     * The specified {@link Aggregator} is applied for each input record and computes a new aggregate using the current
     * aggregate (or for the very first record using the intermediate aggregation result provided via the
     * {@link Initializer}) and the record's value.
     * Thus, {@code aggregate(InitializerWithKey, Aggregator, Serde, String)} can be used to compute aggregate functions like
     * count (c.f. {@link #count(String)}).
     * <p>
     * Not all updates might get sent downstream, as an internal cache is used to deduplicate consecutive updates to
     * the same key.
     ...
     * You can retrieve all generated internal topic names via {@link KafkaStreams#toString()}.
     *
     * @param initializerWithKey an {@link InitializerWithKey} that computes an initial intermediate aggregation result
     *                           given read-only key
     * @param aggregator         an {@link Aggregator} that computes a new aggregate result
     * @param aggValueSerde      aggregate value serdes for materializing the aggregated table,
     *                           if not specified the default serdes defined in the configs will be used
     * @param queryableStoreName the name of the state store created from this operation; valid characters are ASCII
     *                           alphanumerics, '.', '_' and '-'. If {@code null} then this will be equivalent to
     *                           {@link KGroupedStream#aggregate(Initializer, Aggregator, Serde)} ()} ()}.
     * @param <VR>          the value type of the resulting {@link KTable}
     * @return a {@link KTable} that contains "update" records with unmodified keys, and values that represent the
     * latest (rolling) aggregate for each key
     */
     ...
     * The specified {@link Aggregator} is applied for each input record and computes a new aggregate using the current
     * aggregate (or for the very first record using the intermediate aggregation result provided via the
     * {@link InitializerWithKey}) and the record's value.
     * Thus, {@code aggregate(InitializerWithKey, Aggregator, Serde, String)} can be used to compute aggregate
     ...
     * aggregate (or for the very first record using the intermediate aggregation result provided via the
     * {@link InitializerWithKey}) and the record's value.
     * Thus, {@code aggregate(InitializerWithKey, Aggregator, StateStoreSupplier)} can be used to compute aggregate
     * functions like count (c.f. {@link #count(String)}).
     * <p>
     * Not all updates might get sent downstream, as an internal cache is used to deduplicate consecutive updates to
     * the same key.
     ...
     * "windowed" implies that the {@link KTable} key is a combined key of the original record key and a window ID.
     * <p>
     * The specified {@link InitializerWithKey} is applied once per window directly before the first input record is
     * processed to provide an initial intermediate aggregation result that is used to process the first record.
     * Note that provided key in {@link InitializerWithKey} is read-only and should not be modified.
     * Any key modification can result in corrupt partitioning.
     * The specified {@link Aggregator} is applied for each input record and computes a new aggregate using the current
     * aggregate (or for the very first record using the intermediate aggregation result provided via the
     * {@link InitializerWithKey}) and the record's value.
     * Thus, {@code aggregate(InitializerWithKey, Aggregator, Windows, Serde, String)} can be used to compute aggregate
     * functions like count (c.f. {@link #count(String)}).
     * <p>
     * Not all updates might get sent downstream, as an internal cache is used to deduplicate consecutive updates to
     * the same window and key.
     ...
     * aggregate (or for the very first record using the intermediate aggregation result provided via the
     * {@link InitializerWithKey}) and the record's value.
     * Thus, {@code aggregate(InitializerWithKey, Aggregator, Windows, StateStoreSupplier)} can be used to compute
     ...
     * "windowed" implies that the {@link KTable} key is a combined key of the original record key and a window ID.
     * <p>
     * The specified {@link InitializerWithKey} is applied once per session directly before the first input record is
     * processed to provide an initial intermediate aggregation result that is used to process the first record.
     * Note that provided key in {@link InitializerWithKey} is read-only and should not be modified.
     * Any key modification can result in corrupt partitioning.
     * The specified {@link Aggregator} is applied for each input record and computes a new aggregate using the current
     * aggregate (or for the very first record using the intermediate aggregation result provided via the
     * {@link InitializerWithKey}) and the record's value.
     * Thus, {@code aggregate(InitializerWithKey, Aggregator, Merger, SessionWindows, Serde, String)} can be used to
     * compute aggregate functions like count (c.f. {@link #count(String)})
     * <p>
     * Not all updates might get sent downstream, as an internal cache is used to deduplicate consecutive updates to
     * the same window and key.
     * The rate of propagated updates depends on your input data rate, the number of distinct keys, the number of
     ...
     * query the value of the key on a parallel running instance of your Kafka Streams application.
     *
     * @param initializerWithKey the instance of {@link InitializerWithKey}
     * @param aggregator         the instance of {@link Aggregator}
     * @param sessionMerger      the instance of {@link Merger}
     * @param sessionWindows     the specification of the aggregation {@link SessionWindows}
     * @param aggValueSerde      aggregate value serdes for materializing the aggregated table,
     ...
     * the latest (rolling) aggregate for each key within a window
     */
    <T> KTable<Windowed<K>, T> aggregate(final InitializerWithKey<K, T> initializerWithKey,
                                         final Aggregator<? super K, ? super V, T> aggregator,
                                         final Merger<? super K, T> sessionMerger,
                                         final SessionWindows sessionWindows,
                                         final Serde<T> aggValueSerde,
     ...
     * aggregate (or for the very first record using the intermediate aggregation result provided via the
     * {@link InitializerWithKey}) and the record's value.
     * Thus, {@code #aggregate(InitializerWithKey, Aggregator, Merger, SessionWindows, Serde, StateStoreSupplier)}
     * can be used to compute aggregate functions like count (c.f. {@link #count(String)}).
     * <p>
     * Not all updates might get sent downstream, as an internal cache is used to deduplicate consecutive updates to
     * the same window and key.
     * The rate of propagated updates depends on your input data rate, the number of distinct keys, the number of
