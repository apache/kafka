...
class FetchRequestTest extends BaseRequestTest {
  ...
  private def createFetchRequest(maxResponseBytes: Int,
                                 maxPartitionBytes: Int,
                                 topicPartitions: Seq[TopicPartition],
                                 offsetMap: Map[TopicPartition, Long]): FetchRequest =
    FetchRequest.Builder
      .forConsumer(Int.MaxValue, 0, createPartitionMap(maxPartitionBytes, topicPartitions, offsetMap))
  ...
  def testBrokerRespectsPartitionsOrderAndSizeLimits(): Unit = {
    ...
    producer
      .send(
        new ProducerRecord(partitionWithLargeMessage1.topic,
                           partitionWithLargeMessage1.partition,
                           "larger than partition limit",
                           new String(new Array[Byte](maxPartitionBytes + 1)))
      )
      .get
    producer
      .send(
        new ProducerRecord(partitionWithLargeMessage2.topic,
                           partitionWithLargeMessage2.partition,
                           "larger than response limit",
                           new String(new Array[Byte](maxResponseBytes + 1)))
      )
      .get
    ...
    checkFetchResponse(shuffledTopicPartitions1,
                       fetchResponse1,
                       maxPartitionBytes,
                       maxResponseBytes,
    ...
    checkFetchResponse(shuffledTopicPartitions2,
                       fetchResponse2,
                       maxPartitionBytes,
                       maxResponseBytes,
                       messagesPerPartition)
    ...
    val fetchRequest3 =
      createFetchRequest(shuffledTopicPartitions3, Map(partitionWithLargeMessage1 -> messagesPerPartition))
    ...
    val shuffledTopicPartitions4 = Seq(partitionWithLargeMessage2, partitionWithLargeMessage1) ++
      random.shuffle(partitionsWithoutLargeMessages)
    val fetchRequest4 =
      createFetchRequest(shuffledTopicPartitions4, Map(partitionWithLargeMessage2 -> messagesPerPartition))
  ...
  def testFetchRequestV2WithOversizedMessage(): Unit = {
    ...
    producer
      .send(
        new ProducerRecord(topicPartition.topic,
    ...
    val fetchRequest = FetchRequest.Builder
      .forConsumer(Int.MaxValue, 0, createPartitionMap(maxPartitionBytes, Seq(topicPartition)))
      .build(2)
  ...
  def testFetchRequestToNonReplica(): Unit = {
    ...
    val fetchRequest =
      FetchRequest.Builder.forConsumer(Int.MaxValue, 0, createPartitionMap(1024, Seq(topicPartition))).build()
  ...
  def testDownConversionWithConnectionFailure(): Unit = {
    ...
    def fetch(version: Short,
              maxPartitionBytes: Int,
              closeAfterPartialResponse: Boolean): Option[FetchResponse[MemoryRecords]] = {
      val fetchRequest = FetchRequest.Builder
        .forConsumer(Int.MaxValue, 0, createPartitionMap(maxPartitionBytes, Seq(topicPartition)))
        .build(version)
  ...
  def testDownConversionFromBatchedToUnbatchedRespectsOffset(): Unit = {
    ...
    def check(fetchOffset: Long,
              requestVersion: Short,
              expectedOffset: Long,
              expectedNumBatches: Int,
              expectedMagic: Byte): Unit = {
  ...
  def testCreateIncrementalFetchWithPartitionsInError(): Unit = {
    def createFetchRequest(topicPartitions: Seq[TopicPartition],
      ...
      FetchRequest.Builder
        .forConsumer(Int.MaxValue, 0, createPartitionMap(Integer.MAX_VALUE, topicPartitions, Map.empty))
    ...
    assertEquals("Expected the broker to continue the incremental fetch session", resp1.sessionId(), resp2.sessionId())
  ...
  private def checkFetchResponse(expectedPartitions: Seq[TopicPartition],
                                 fetchResponse: FetchResponse[MemoryRecords],
                                 maxPartitionBytes: Int,
                                 maxResponseBytes: Int,
                                 numMessagesPerPartition: Int): Unit = {
  ...
  private def createTopics(numTopics: Int,
                           numPartitions: Int,
                           configs: Map[String, String] = Map.empty): Map[TopicPartition, Int] = {
