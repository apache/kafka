...
public class LeaderState<T> implements EpochState {
    ...
    private OptionalLong currentVoterOffset = OptionalLong.empty();
    ...
    public void updateCheckQuorumForFollowingVoter(ReplicaKey replicaKey, long currentTimeMs) {
        ...
        if (currentVoterStates.containsKey(localVoterNode.voterKey().id())) {
            majority = majority - 1;
        }
    ...
    public ElectionState election() {
        return ElectionState.withElectedLeader(
                epoch, localVoterNode.voterKey().id(), Optional.empty(), currentVoterStates.keySet());
    }
    ...
    private void updateVoterAndObserverStates(VoterSet lastVoterSet, long lastVoterSetOffset) {
        ...
        currentVoterOffset = OptionalLong.of(lastVoterSetOffset);
