...
/**
 * {@code KGroupedStream} is an abstraction of a <i>grouped</i> record stream of {@link KeyValue}
 * pairs. It is an intermediate representation of a {@link KStream} in order to apply an aggregation
 * operation on the original {@link KStream} records.
 * <p>
 * It is an intermediate representation after a grouping of a {@link KStream} before an aggregation
 * is applied to the new partitions resulting in a {@link KTable}.
 * <p>
 * A {@code KGroupedStream} must be obtained from a {@link KStream} via {@link KStream#groupByKey()
 * groupByKey()} or {@link KStream#groupBy(KeyValueMapper) groupBy(...)}.
 *
...
public interface KGroupedStream<K, V> {
    ...
    /**
     * Count the number of records in this stream by the grouped key. Records with {@code null} key
     * or value are ignored. The result is written into a local {@link KeyValueStore} (which is
     * basically an ever-updating materialized view). Furthermore, updates to the store are sent
     * downstream into a {@link KTable} changelog stream.
     * <p>
     ...
     * created in Kafka. The changelog topic will be named "${applicationId}-${internalStoreName}-changelog",
     * where "applicationId" is user-specified in {@link StreamsConfig} via parameter {@link
     * StreamsConfig#APPLICATION_ID_CONFIG APPLICATION_ID_CONFIG}, "internalStoreName" is an
     * internal name and "-changelog" is a fixed suffix. Note that the internal store name may not
     * be queriable through Interactive Queries.
     * <p>
     * You can retrieve all generated internal topic names via {@link Topology#describe()}.
     *
     * @return a {@link KTable} that contains "update" records with unmodified keys and {@link Long}
     ...
     * Count the number of records in this stream by the grouped key. Records with {@code null} key
     * or value are ignored. The result is written into a local {@link KeyValueStore} (which is
     * basically an ever-updating materialized view) provided by the given store name in {@code
     ...
     *                     Cannot be {@code null}. Note: the valueSerde will be automatically set to
     *                     {@link org.apache.kafka.common.serialization.Serdes#Long() Serdes#Long()}
    ...
    /**
     * Combine the values of records in this stream by the grouped key. Records with {@code null}
     * key or value are ignored. Combining implies that the type of the aggregate result is the same
     * as the type of the input value (c.f. {@link #aggregate(Initializer, Aggregator)}).
     * <p>
     * The specified {@link Reducer} is applied for each input record and computes a new aggregate
     * using the current aggregate and the record's value. If there is no current aggregate the
     * {@link Reducer} is not applied and the new aggregate will be the record's value as-is. Thus,
     * {@code reduce(Reducer)} can be used to compute aggregate functions like sum, min, or max.
     * <p>
     * Not all updates might get sent downstream, as an internal cache is used to deduplicate
     * consecutive updates to the same key. The rate of propagated updates depends on your input
     * data rate, the number of distinct keys, the number of parallel running Kafka Streams
     ...
     * You can retrieve all generated internal topic names via {@link Topology#describe()}.
     *
     * @param reducer a {@link Reducer} that computes a new aggregate result. Cannot be {@code
     *                null}.
     * @return a {@link KTable} that contains "update" records with unmodified keys, and values that
     * represent the latest (rolling) aggregate for each key. If the reduce function returns {@code
     * null}, it is then interpreted as deletion for the key, and future messages of the same key
     * coming from upstream operators will be handled as newly initialized value.
     */
    KTable<K, V> reduce(final Reducer<V> reducer);

    /**
     * Combine the value of records in this stream by the grouped key. Records with {@code null} key
     * or value are ignored. Combining implies that the type of the aggregate result is the same as
     * the type of the input value (c.f. {@link #aggregate(Initializer, Aggregator, Materialized)}).
     * The result is written into a local {@link KeyValueStore} (which is basically an ever-updating
     * materialized view) provided by the given store name in {@code materialized}. Furthermore,
     * updates to the store are sent downstream into a {@link KTable} changelog stream.
     * <p>
     * The specified {@link Reducer} is applied for each input record and computes a new aggregate
     * using the current aggregate (first argument) and the record's value (second argument):
     * <pre>{@code
     ...
     * will be the record's value as-is. Thus, {@code reduce(Reducer, Materialized)} can be used to
     ...
     * key or value are ignored. Aggregating is a generalization of {@link #reduce(Reducer)
     * combining via reduce(...)} as it, for example, allows the result to have a different type
     ...
     * Thus, {@code aggregate(Initializer, Aggregator)} can be used to compute aggregate functions
     * like count (c.f. {@link #count()}).
     * <p>
     * The default value serde from config will be used for serializing the result. If a different
     * serde is required then you should use {@link #aggregate(Initializer, Aggregator,
     * Materialized)}.
     * <p>
     * Not all updates might get sent downstream, as an internal cache is used to deduplicate
     * consecutive updates to the same key. The rate of propagated updates depends on your input
     ...
     * @return a {@link KTable} that contains "update" records with unmodified keys, and values that
     * represent the latest (rolling) aggregate for each key. If the aggregate function returns
     * {@code null}, it is then interpreted as deletion for the key, and future messages of the same
     * key coming from upstream operators will be handled as newly initialized value.
     */
     ...
     * key or value are ignored. Aggregating is a generalization of {@link #reduce(Reducer)
     * combining via reduce(...)} as it, for example, allows the result to have a different type
     * than the input values. The result is written into a local {@link KeyValueStore} (which is
     * basically an ever-updating materialized view) that can be queried by the given store name in
     * {@code materialized}. Furthermore, updates to the store are sent downstream into a {@link
    ...
    /**
     * Create a new {@link TimeWindowedKStream} instance that can be used to perform windowed
     * aggregations.
     *
    ...
    /**
     * Create a new {@link SessionWindowedKStream} instance that can be used to perform session
