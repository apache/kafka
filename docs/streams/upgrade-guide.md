...
Kafka Streams now supports static membership with the Streams Rebalance Protocol. Applications using `group.protocol=streams` may configure `group.instance.id`. Kafka Streams appends the 1-based stream thread index to the configured value; for example, `my-app` produces `my-app-1`, `my-app-2`, and so on. For topologies without persistent state stores, a restart generates a new process ID. The static instance IDs remain stable, but the new process ID can cause the broker to recompute the group assignment.
