...
class ZkOffsetCommitTest extends ZooKeeperTestHarness {
  ...
  def testCommitAndFetchOffsetsFromZk() {
    ...
    assertEquals(44L, fetchResponse.requestInfo(TopicAndPartition(topic3, 0)).offset)
    assertEquals(45L, fetchResponse.requestInfo(TopicAndPartition(topic2, 1)).offset)
  }
