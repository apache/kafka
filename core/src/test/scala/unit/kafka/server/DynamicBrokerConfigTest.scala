...
import org.apache.kafka.server.config.{Defaults, ZkConfig}
...
class DynamicBrokerConfigTest {
  ...
  def testConfigUpdateWithSomeInvalidConfigs(): Unit = {
    ...
    val nonDynamicProps = Map(ZkConfig.ZK_CONNECT_PROP -> "somehost:2181")
  ...
  def testNonInternalValuesDoesNotExposeInternalConfigs(): Unit = {
    val props = new Properties()
    props.put(ZkConfig.ZK_CONNECT_PROP, "localhost:2181")
