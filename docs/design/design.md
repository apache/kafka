...
Note that this compaction deadline is not a hard guarantee since it is still subjected to the availability of log cleaner threads and the actual compaction time. You will want to monitor the `uncleanable-partitions-count`, `uncleanable-bytes`, `max-clean-time-secs`, and `max-compaction-delay-secs` metrics. If an unexpected error occurs while cleaning a partition, the log cleaner marks that partition as uncleanable so that cleaning can continue for other partitions. After correcting the cause reported in the broker log, update any configuration for the affected topic to retry cleaning its partitions.
