...
public class MetadataResponse extends AbstractResponse {
    ...
    public MetadataResponse(Struct struct) {
        ...
        for (Object topicInformation : topicInfos) {
            Struct topicInfo = (Struct) topicInformation;
            ...
            for (Object partitionInformation : partitionInfos) {
                Struct partitionInfo = (Struct) partitionInformation;
                Errors partitionError =
                        Errors.forCode(partitionInfo.getShort(PARTITION_ERROR_CODE_KEY_NAME));
                ...
                for (Object replicaNodeId : replicas) {
                    ...
                    else replicaNodes.add(new Node((int) replicaNodeId, "", -1));
                }

                Object[] isr = (Object[]) partitionInfo.get(ISR_KEY_NAME);
                List<Node> isrNodes = new ArrayList<>(isr.length);
                for (Object isrNode : isr) {
                    if (brokers.containsKey(isrNode)) isrNodes.add(brokers.get(isrNode));
                    else isrNodes.add(new Node((int) isrNode, "", -1));
                }

                partitionMetadata.add(
                        new PartitionMetadata(partitionError, partition, leaderNode, replicaNodes,
                                isrNodes));
            }
