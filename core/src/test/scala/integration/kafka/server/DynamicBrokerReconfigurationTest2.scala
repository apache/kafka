...
class DynamicBrokerReconfigurationTest2 extends QuorumTestHarness with SaslSetup {
...
object TestMetricsReporter2 {
  val PollingIntervalProp = "polling.interval"
  val testReporters = new ConcurrentLinkedQueue[TestMetricsReporter2]()
  val configuredBrokers = mutable.Set[Int]()

  def waitForReporters(count: Int): List[TestMetricsReporter2] = {
...
class TestMetricsReporter2 extends MetricsReporter with Reconfigurable with Closeable with ClusterResourceListener {
  import TestMetricsReporter2._
...
class MockFileConfigProvider2 extends FileConfigProvider {
