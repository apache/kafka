...
object ConsumerGroupCommand extends Logging {
  ...
  def convertTimestamp(timeString: String): java.lang.Long = {
    val datetime: String = timeString match {
      case ts if ts.split("T")(1).contains("+") || ts.split("T")(1).contains("-") || ts.split("T")(1).contains("Z") =>
        ts.toString
      case ts => s"${ts}Z"
    }
    val date = try {
  ...
  def printOffsetsToReset(groupAssignmentsToReset: Map[TopicPartition, OffsetAndMetadata]): Unit = {
    ...
    groupAssignmentsToReset.foreach {
      case (consumerAssignment, offsetAndMetadata) =>
        println(
          "%-30s %-10s %-15s".format(consumerAssignment.topic, consumerAssignment.partition, offsetAndMetadata.offset)
        )
    }
  }

  private[admin] case class PartitionAssignmentState(group: String,
  ...
  private[admin] case class MemberAssignmentState(group: String,
                                                  consumerId: String,
                                                  host: String,
                                                  clientId: String,
                                                  numPartitions: Int,
  ...
  private[admin] case class GroupState(group: String,
                                       coordinator: Node,
  ...
  class ConsumerGroupService(val opts: ConsumerGroupCommandOptions) {
          ...
          state match {
            ...
            case other          =>
              ...
              throw new KafkaException(
                s"Expected a valid consumer group state, but found '${other.getOrElse("NONE")}'."
              )
          }
    ...
    private def printOffsets(group: String,
                             state: Option[String],
      ...
      if (shouldPrintMemberState(group, state, size(assignments))) {
        ...
        assignments match {
          ...
          case Some(consumerAssignments) =>
            consumerAssignments.foreach { consumerAssignment =>
              ...
              maxConsumerIdLen =
                Math.max(maxConsumerIdLen, consumerAssignment.consumerId.getOrElse(MISSING_COLUMN_VALUE).length)
        ...
        assignments match {
          ...
          case Some(consumerAssignments) =>
            consumerAssignments.foreach { consumerAssignment =>
              println(
                s"%-${maxTopicLen}s %-10s %-15s %-15s %-15s %-${maxConsumerIdLen}s %-${maxHostLen}s %s".format(
                  ...
                  consumerAssignment.host.getOrElse(MISSING_COLUMN_VALUE),
    ...
    private def printMembers(group: String,
                             state: Option[String],
                             assignments: Option[Seq[MemberAssignmentState]],
      ...
      if (shouldPrintMemberState(group, state, size(assignments))) {
        ...
        assignments match {
          ...
          case Some(memberAssignments) =>
            memberAssignments.foreach { memberAssignment =>
              print(
                s"%${-maxConsumerIdLen}s %${-maxHostLen}s %${-maxClientIdLen}s %-15s ".format(
                  memberAssignment.consumerId,
                  memberAssignment.host,
                  memberAssignment.clientId,
                ...
                val partitions = memberAssignment.assignment match {
                  ...
                  case assignment =>
                    assignment
                      .groupBy(_.topic)
                      .map {
                        case (topic, partitionList) =>
                          topic + partitionList.map(_.partition).sorted.mkString("(", ",", ")")
                      }
                      .toList
                      .sorted
                      .mkString(", ")
    ...
    private def printState(group: String, state: GroupState): Unit =
      if (shouldPrintMemberState(group, Some(state.state), Some(1))) {
        ...
        print(
          s"\n%${-coordinatorColLen}s %-25s %-20s %s"
            .format("COORDINATOR (ID)", "ASSIGNMENT-STRATEGY", "STATE", "#MEMBERS")
        )
        print(
          s"\n%${-coordinatorColLen}s %-25s %-20s %s"
            .format(coordinator, state.assignmentStrategy, state.state, state.numMembers)
        )
        println()
      }
    ...
    private def collectConsumerAssignment(group: String,
      ...
      if (topicPartitions.isEmpty) {
        Array[PartitionAssignmentState](
          PartitionAssignmentState(group,
                                   coordinator,
                                   None,
                                   None,
                                   None,
        ...
        describePartitions(group,
                           coordinator,
                           topicPartitions.sortBy(_.partition),
                           getPartitionOffset,
                           consumerIdOpt,
                           hostOpt,
                           clientIdOpt)
    ...
    private def describePartitions(group: String,
      ...
      def getDescribePartitionResult(topicPartition: TopicPartition,
        ...
        PartitionAssignmentState(
          group,
          coordinator,
          Option(topicPartition.topic),
          Option(topicPartition.partition),
          offset,
      ...
      getLogEndOffsets(topicPartitions).map { logEndOffsetResult =>
    ...
    def resetOffsets(): Map[TopicPartition, OffsetAndMetadata] = {
      ...
      group.state.toString match {
        ...
        case currentState =>
          printError(
            s"Assignments can only be reset if the group '$groupId' is inactive, but the current state is $currentState."
          )
          Map.empty
      }
    }
    ...
    def collectGroupOffsets(): (Option[String], Option[Seq[PartitionAssignmentState]]) = {
      ...
      val rowsWithConsumer = consumerGroup.members.asScala
        .filter(!_.assignment.topicPartitions.isEmpty)
        .toSeq
        ...
        .flatMap { consumerSummary =>
          ...
          val partitionOffsets = consumerSummary.assignment.topicPartitions.asScala.map { topicPartition =>
         ...
         group
           ...
           .map { consumer =>
             MemberAssignmentState(
               ...
               if (verbose) consumer.assignment.topicPartitions.asScala.toList
               else List()
             )
           }
           .toList
    ...
    private[admin] def collectGroupState(): GroupState = {
      ...
      val group = consumerGroups.get(groupId).get
      GroupState(groupId, group.coordinator, group.partitionAssignor(), group.state.toString, group.members().size)
    }
    ...
    private def getLogTimestampOffsets(topicPartitions: Seq[TopicPartition],
      ...
      val successfulLogTimestampOffsets = successfulOffsetsForTimes.map {
        case (topicPartition, offsetAndTimestamp) =>
          topicPartition -> LogOffsetResult.LogOffset(offsetAndTimestamp.offset)
      }.toMap
    ...
    private def createAdminClient(): admin.AdminClient = {
      val props =
        if (opts.options.has(opts.commandConfigOpt)) Utils.loadProps(opts.options.valueOf(opts.commandConfigOpt))
        else new Properties()
    ...
    private def prepareOffsetsToReset(groupId: String,
      ...
      if (opts.options.has(opts.resetToOffsetOpt)) {
        ...
        val requestedOffsets = partitionsToReset.map { topicPartition =>
          ...
          val currentOffset = currentCommittedOffsets.asScala
            .getOrElse(topicPartition,
                       throw new IllegalArgumentException(
                         s"Cannot shift offset for partition $topicPartition since there is no current committed offset"
                       ))
            .offset
        ...
        val preparedOffsetsForPartitionsWithCommittedOffset = partitionsToResetWithCommittedOffset.map {
          topicPartition =>
            (topicPartition, new OffsetAndMetadata(currentCommittedOffsets.get(topicPartition) match {
              ...
              case _ =>
                throw new IllegalStateException(s"Expected a valid current offset for topic partition: $topicPartition")
            }))
        }.toMap
        ...
        CommandLineUtils.printUsageAndDie(
          opts.parser,
          "Option '%s' requires one of the following scenarios: %s".format(opts.resetOffsetsOpt,
    ...
    private def checkOffsetsRange(requestedOffsets: Map[TopicPartition, Long]) = {
      ...
      requestedOffsets.map {
        case (topicPartition, offset) =>
          (topicPartition, logEndOffsets.get(topicPartition) match {
            case Some(LogOffsetResult.LogOffset(endOffset)) if offset > endOffset =>
              warn(
                s"New offset ($offset) is higher than latest offset for topic partition $topicPartition. Value will be set to $endOffset"
              )
              endOffset
            ...
            case Some(_) =>
              logStartOffsets.get(topicPartition) match {
                case Some(LogOffsetResult.LogOffset(startOffset)) if offset < startOffset =>
                  warn(
                    s"New offset ($offset) is lower than earliest offset for topic partition $topicPartition. Value will be set to $startOffset"
                  )
                  startOffset
            ...
            case None => // the control should not reach here
              throw new IllegalStateException(
                s"Unexpected non-existing offset value for topic partition $topicPartition"
              )
          })
      }
    }
    ...
    def exportOffsetsToReset(assignmentsToReset: Map[TopicPartition, OffsetAndMetadata]): String = {
      val rows = assignmentsToReset.map { case (k, v) => s"${k.topic},${k.partition},${v.offset}" }(collection.breakOut): List[
        String
      ]
      rows.foldRight("")(_ + "\n" + _)
    }
  ...
  class ConsumerGroupCommandOptions(args: Array[String]) {
    ...
    val DryRunDoc =
      "Only show results without executing changes on Consumer Groups. Supported operations: reset-offsets."
    ...
    val groupOpt = parser
      .accepts("group", GroupDoc)
      .withRequiredArg
      .describedAs("consumer group")
      .ofType(classOf[String])
    val topicOpt = parser
      .accepts("topic", TopicDoc)
      .withRequiredArg
      .describedAs("topic")
      .ofType(classOf[String])
    ...
    val resetToOffsetOpt = parser
      .accepts("to-offset", ResetToOffsetDoc)
      .withRequiredArg()
      .describedAs("offset")
      .ofType(classOf[Long])
    val resetFromFileOpt = parser
      .accepts("from-file", ResetFromFileDoc)
      .withRequiredArg()
      .describedAs("path to CSV file")
      .ofType(classOf[String])
    val resetToDatetimeOpt = parser
      .accepts("to-datetime", ResetToDatetimeDoc)
      .withRequiredArg()
      .describedAs("datetime")
      .ofType(classOf[String])
    val resetByDurationOpt = parser
      .accepts("by-duration", ResetByDurationDoc)
      .withRequiredArg()
    ...
    val resetShiftByOpt = parser
      .accepts("shift-by", ResetShiftByDoc)
      .withRequiredArg()
      .describedAs("number-of-offsets")
      .ofType(classOf[Long])
    val membersOpt = parser
      .accepts("members", MembersDoc)
      .availableIf(describeOpt)
    val verboseOpt = parser
      .accepts("verbose", VerboseDoc)
      .availableIf(describeOpt)
    val offsetsOpt = parser
      .accepts("offsets", OffsetsDoc)
      .availableIf(describeOpt)
    val stateOpt = parser
      .accepts("state", StateDoc)
      .availableIf(describeOpt)
    ...
    val allResetOffsetScenarioOpts: Set[OptionSpec[_]] = Set(resetToOffsetOpt,
                                                             resetShiftByOpt,
                                                             resetToDatetimeOpt,
                                                             resetByDurationOpt,
                                                             resetToEarliestOpt,
    ...
    def checkArgs() {
      ...
      if (options.has(deleteOpt) && options.has(topicOpt))
        CommandLineUtils.printUsageAndDie(parser,
                                          s"The consumer does not support topic-specific offset " +
      ...
      if (options.has(resetOffsetsOpt)) {
        if (options.has(dryRunOpt) && options.has(executeOpt))
          CommandLineUtils.printUsageAndDie(parser,
                                            s"Option $resetOffsetsOpt only accepts one of $executeOpt and $dryRunOpt")
