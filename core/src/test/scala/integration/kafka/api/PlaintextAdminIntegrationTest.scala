...
import org.apache.kafka.server.config.{KRaftConfigs, QuotaConfig, ServerConfigs, ServerLogConfigs}
...
object PlaintextAdminIntegrationTest {
  ...
  def checkInvalidAlterConfigs(
    ...
    alterConfigs.put(brokerResource, util.Arrays.asList(new AlterConfigOp(new ConfigEntry(KRaftConfigs.NODE_ID_CONFIG, "456"), OpType.SET)))
    var alterResult = admin.incrementalAlterConfigs(alterConfigs)
