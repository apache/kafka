...
class ReplicaFetcherThreadTest {
  ...
  private var metadataCache = new ZkMetadataCache(0, MetadataVersion.latest(false), BrokerFeatures.createEmpty())
  ...
  def shouldNotFetchLeaderEpochOnFirstFetchWithTruncateOnFetch(): Unit = {
    verifyFetchLeaderEpochOnFirstFetch(MetadataVersion.latest(false), epochFetchCount = 0)
  }
