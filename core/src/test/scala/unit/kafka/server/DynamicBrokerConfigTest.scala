...
import org.apache.kafka.storage.internals.log.{LogCleanerConfig, LogConfig, ProducerStateManagerConfig}
...
class DynamicBrokerConfigTest {
  ...
  def testConfigUpdateWithSomeInvalidConfigs(): Unit = {
    ...
    val invalidProps = Map(LogCleanerConfig.LOG_CLEANER_THREADS_PROP -> "invalid")
  ...
  def testConfigUpdateWithReconfigurableValidationFailure(): Unit = {
    val origProps = TestUtils.createBrokerConfig(0, TestUtils.MockZkConnect, port = 8181)
    origProps.put(LogCleanerConfig.LOG_CLEANER_DEDUPE_BUFFER_SIZE_PROP, "100000000")
    val config = KafkaConfig(origProps)
    config.dynamicConfig.initialize(None, None)

    val validProps = Map.empty[String, String]
    val invalidProps = Map(LogCleanerConfig.LOG_CLEANER_THREADS_PROP -> "20")

    def validateLogCleanerConfig(configs: util.Map[String, _]): Unit = {
      val cleanerThreads = configs.get(LogCleanerConfig.LOG_CLEANER_THREADS_PROP).toString.toInt
    ...
    val reconfigurable = new Reconfigurable {
      ...
      override def reconfigurableConfigs(): util.Set[String] = Set(LogCleanerConfig.LOG_CLEANER_THREADS_PROP).asJava
    ...
    val brokerReconfigurable = new BrokerReconfigurable {
      override def reconfigurableConfigs: collection.Set[String] = Set(LogCleanerConfig.LOG_CLEANER_THREADS_PROP)
  ...
  def testReconfigurableValidation(): Unit = {
    val origProps = TestUtils.createBrokerConfig(0, TestUtils.MockZkConnect, port = 8181)
    val config = KafkaConfig(origProps)
    val invalidReconfigurableProps = Set(LogCleanerConfig.LOG_CLEANER_THREADS_PROP, KafkaConfig.BrokerIdProp, "some.prop")
    val validReconfigurableProps = Set(LogCleanerConfig.LOG_CLEANER_THREADS_PROP, LogCleanerConfig.LOG_CLEANER_DEDUPE_BUFFER_SIZE_PROP, "some.prop")
