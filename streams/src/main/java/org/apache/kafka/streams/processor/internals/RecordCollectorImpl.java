    ...
    public <K, V> void send(final String topic,
        ...
        try {
            ...
            if (response == ProductionExceptionHandler.ProductionExceptionHandlerResponse.CONTINUE) {
                log.info("Error serializing the record {} (key=[{}] value=[{}] timestamp=[{}]) to topic=[{}] and partition=[{}]; " +  "The exception handler chose to CONTINUE processing in spite of this error.",
