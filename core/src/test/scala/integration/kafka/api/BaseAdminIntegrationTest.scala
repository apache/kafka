...
import org.apache.kafka.server.config.{ControlledShutdownConfigs, KafkaSecurityConfigs, ReplicationConfigs, ServerLogConfigs}
...
abstract class BaseAdminIntegrationTest extends IntegrationTestHarness with Logging {
  ...
  override def modifyConfigs(configs: Seq[Properties]): Unit = {
    ...
    configs.foreach { config =>
      ...
      config.setProperty(ControlledShutdownConfigs.CONTROLLED_SHUTDOWN_ENABLE_CONFIG, "false")
