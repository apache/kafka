...
object ReplicaVerificationTool extends Logging {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val topicWhiteListOpt = parser
      .accepts("topic-white-list", "White list of topics to verify replica consistency. Defaults to all topics.")
      .withRequiredArg
    ...
    if (filteredTopicMetadata.isEmpty) {
      error(
        s"No topics found. $topicWhiteListOpt if specified, is either filtering out all topics or there is no topic."
      )
      Exit.exit(1)
    }
    ...
    val topicPartitionReplicas = filteredTopicMetadata.flatMap { topicMetadata =>
      topicMetadata.partitions.asScala.flatMap { partitionMetadata =>
        partitionMetadata.replicas.asScala.map { node =>
          TopicPartitionReplica(topic = topicMetadata.name,
                                partitionId = partitionMetadata.partition,
                                replicaId = node.id)
        }
      }
    }
    debug(s"Selected topic partitions: $topicPartitionReplicas")
    val brokerToTopicPartitions = topicPartitionReplicas.groupBy(_.replicaId).map {
    ...
    val expectedReplicasPerTopicPartition = topicPartitionReplicas
      .groupBy { replica =>
    ...
    val fetcherThreads: Iterable[ReplicaFetcher] = brokerToTopicPartitions.map {
      case (brokerId, topicPartitions) =>
        new ReplicaFetcher(
          name = s"ReplicaFetcher-$brokerId",
  ...
  private def initialOffsets(topicPartitions: Seq[TopicPartition],
...
private class ReplicaBuffer(expectedReplicasPerTopicPartition: Map[TopicPartition, Int],
  ...
  def addFetchedData(topicAndPartition: TopicPartition,
                     replicaId: Int,
                     partitionData: FetchResponse.PartitionData[MemoryRecords]) {
  ...
  def verifyCheckSum(println: String => Unit) {
    ...
    for ((topicPartition, fetchResponsePerReplica) <- recordsCache) {
      ...
      assert(
        fetchResponsePerReplica.size == expectedReplicasPerTopicPartition(topicPartition),
      ...
      while (isMessageInAllReplicas) {
          ...
          try {
                ...
                messageInfoFromFirstReplicaOpt match {
                  ...
                  case Some(messageInfoFromFirstReplica) =>
                    if (messageInfoFromFirstReplica.offset != batch.lastOffset) {
                      println(
                        ReplicaVerificationTool.getCurrentTimeString + ": partition " + topicPartition
            ...
            case t: Throwable =>
              throw new RuntimeException("Error in processing replica %d in partition %s at offset %d."
                                           .format(replicaId, topicPartition, fetchOffsetMap.get(topicPartition)),
                                         t)
          }
        }
        if (isMessageInAllReplicas) {
          ...
          debug(
            expectedReplicasPerTopicPartition(topicPartition) + " replicas match at offset " +
    ...
    if (currentTimeMs - lastReportTime > reportInterval) {
      println(
        ReplicaVerificationTool.dateFormat.format(new Date(currentTimeMs)) + ": max lag is "
...
private class ReplicaFetcher(name: String,
  ...
  private val fetchEndpoint = new ReplicaFetcherBlockingSend(
    sourceBroker,
    new ConsumerConfig(consumerConfig),
    new Metrics(),
    Time.SYSTEM,
    fetcherId,
  ...
  override def doWork() {
    ...
    for (topicPartition <- topicPartitions)
      requestMap.put(topicPartition,
                     new JFetchRequest.PartitionData(replicaBuffer.getOffset(topicPartition), 0L, fetchSize))

    val fetchRequestBuilder = JFetchRequest.Builder.forReplica(ApiKeys.FETCH.latestVersion,
    ...
    if (fetchResponse != null) {
      fetchResponse.responseData.asScala.foreach {
        case (tp, partitionData) =>
          replicaBuffer.addFetchedData(tp, sourceBroker.id, partitionData)
      }
    } else {
      val emptyResponse = new FetchResponse.PartitionData(
        Errors.NONE,
  ...
  def sendRequest(requestBuilder: Builder[_ <: AbstractRequest]): ClientResponse =
    try {
      if (!NetworkClientUtils.awaitReady(networkClient, sourceNode, time, socketTimeout))
        ...
        val clientRequest =
          networkClient.newClientRequest(sourceNode.id.toString, requestBuilder, time.milliseconds(), true)
