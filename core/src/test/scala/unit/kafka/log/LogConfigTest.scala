...
class LogConfigTest {
  ...
  @Test
  def testToHtml(): Unit = {
    val html = LogConfig.configDefCopy.toHtml(4, (key: String) => "prefix_" + key, java.util.Map.of)
  ...
  private def doTestInvalidLocalLogRetentionProps(localRetentionMs: Long,
    ...
    assertThrows(classOf[ConfigException],
      () => LogConfig.validate(java.util.Map.of, props, kafkaConfig.extractLogConfigMap, kafkaConfig.remoteLogManagerConfig.isRemoteStorageSystemEnabled))
  }
  ...
  def testEnableRemoteLogStorageCleanupPolicy(): Unit = {
    ...
    def validateCleanupPolicy(): Unit = {
      LogConfig.validate(java.util.Map.of, logProps, kafkaConfig.extractLogConfigMap, kafkaConfig.remoteLogManagerConfig.isRemoteStorageSystemEnabled)
    }
  ...
  def testDisableRemoteLogStorage(wasRemoteStorageEnabled: Boolean): Unit = {
    ...
    if (wasRemoteStorageEnabled) {
      val message = assertThrows(classOf[InvalidConfigurationException],
        () => LogConfig.validate(java.util.Map.of(TopicConfig.REMOTE_LOG_STORAGE_ENABLE_CONFIG, "true"),
      ...
      LogConfig.validate(java.util.Map.of, logProps, kafkaConfig.extractLogConfigMap, kafkaConfig.remoteLogManagerConfig.isRemoteStorageSystemEnabled)
      LogConfig.validate(java.util.Map.of(TopicConfig.REMOTE_LOG_STORAGE_ENABLE_CONFIG, "false"), logProps,
  ...
  def testTopicCreationWithInvalidRetentionTime(sysRemoteStorageEnabled: Boolean): Unit = {
    ...
    if (sysRemoteStorageEnabled) {
      val message = assertThrows(classOf[ConfigException],
        () => LogConfig.validate(java.util.Map.of, logProps, kafkaConfig.extractLogConfigMap,
