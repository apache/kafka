...
public interface KTable<K, V> {
     ...
     * predicate, with default serializers, deserializers, and internal state storage.
     ...
     * {@link Serde the result table's value serde}, and {@link KeyValueStore internal state storage}.
     * All records that do not satisfy the predicate are dropped.
     * For each {@code KTable} update, the filter is evaluated based on the current update
     * record and then an update record is produced for the result {@code KTable}.
     * This is a stateless record-by-record operation.
     ...
     * given predicate, with default serializers, deserializers, and internal state storage.
     ...
     * (with possibly new type) in the new {@code KTable}, with default serializers, deserializers, and internal state
     * storage.
     ...
     * the {@link Serde key serde}, {@link Serde the result table's value serde}, and {@link KeyValueStore internal state storage}.
     * For each {@code KTable} update the provided {@link ValueMapper} is applied to the value of the updated record and
     * computes a new value for it, resulting in an updated record for the result {@code KTable}.
     * Thus, an input record {@code <K,V>} can be transformed into an output record {@code <K:V'>}.
     * This is a stateless record-by-record operation.
    ...
    /**
     * Create a new {@code KTable} by transforming the value of each record in this {@code KTable} into a new value
     * (with possibly new type), with default serializers, deserializers, and internal state storage.
     ...
     * with default serializers, deserializers, and internal state storage.
     ...
     * non-windowed left equi join, with default serializers, deserializers, and internal state storage.
     * The join is a primary key join with join attribute {@code thisKTable.key == otherKTable.key}.
    ...
    /**
     * Join records of this {@code KTable} (left input) with another {@code KTable}'s (right input) records using
     * non-windowed outer equi join, with default serializers, deserializers, and internal state storage.
     * The join is a primary key join with join attribute {@code thisKTable.key == otherKTable.key}.
