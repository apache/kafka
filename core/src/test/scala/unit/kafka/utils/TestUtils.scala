...
object TestUtils extends Logging {
  ...
  def createBrokerConfig(nodeId: Int,
    ...
    props.put(OffsetConfig.OFFSETS_TOPIC_REPLICATION_FACTOR_CONFIG, "1")
    if (!props.containsKey(OffsetConfig.OFFSETS_TOPIC_PARTITIONS_CONFIG))
      props.put(OffsetConfig.OFFSETS_TOPIC_PARTITIONS_CONFIG, "5")
  ...
  def createOffsetsTopicWithAdmin[B <: KafkaBroker](
    ...
    createTopicWithAdmin(
      ...
      numPartitions = broker.config.getInt(OffsetConfig.OFFSETS_TOPIC_PARTITIONS_CONFIG),
      replicationFactor = broker.config.getShort(OffsetConfig.OFFSETS_TOPIC_REPLICATION_FACTOR_CONFIG).toInt,
      brokers = brokers,
