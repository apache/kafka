...
public class ConsumerNetworkClientTest {
    ...
    public void send() {
        ...
        RequestFuture<ClientResponse> future = consumerClient.send(node, ApiKeys.METADATA, heartbeat());
    ...
    public void multiSend() {
        client.prepareResponse(heartbeatResponse(Errors.NONE.code()));
        client.prepareResponse(heartbeatResponse(Errors.NONE.code()));
        RequestFuture<ClientResponse> future1 = consumerClient.send(node, ApiKeys.METADATA, heartbeat());
        RequestFuture<ClientResponse> future2 = consumerClient.send(node, ApiKeys.METADATA, heartbeat());
    ...
    public void sendExpiry() throws InterruptedException {
        ...
        isReady.set(false);
        RequestFuture<ClientResponse> future3 = consumerClient.send(node, ApiKeys.METADATA, heartbeat());
        assertEquals(1, consumerClient.pendingRequestCount());
        assertEquals(1, consumerClient.pendingRequestCount(node));
