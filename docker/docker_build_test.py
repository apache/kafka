...
def build_image(image, tag, kafka_url, image_type):
    image = f'{image}:{tag}'
    build_image_runner(f"docker build -f $DOCKER_FILE -t {image} --build-arg kafka_url={kafka_url} --build-arg build_date={date.today()} $DOCKER_DIR", image_type)
...
if __name__ == '__main__':
    ...
    if args.build_only or not (args.build_only or args.test_only):
        if args.kafka_url:
            build_image(args.image, args.tag, args.kafka_url, args.image_type)
        else:
