...
object AclCommand extends Logging {
  ...
  val ClusterResourceFilter =
    new ResourcePatternFilter(JResourceType.CLUSTER, JResource.CLUSTER_NAME, PatternType.LITERAL)
  ...
  val ResourceTypeToValidOperations: Map[JResourceType, Set[Operation]] = Map[JResourceType, Set[Operation]](
    ...
    JResourceType.CLUSTER -> Set(Create,
                                 ClusterAction,
                                 DescribeConfigs,
                                 AlterConfigs,
                                 IdempotentWrite,
  ...
  private def addAcl(opts: AclCommandOptions) {
    ...
    if (patternType == PatternType.MATCH || patternType == PatternType.ANY)
      CommandLineUtils.printUsageAndDie(
        opts.parser,
        s"A '--resource-pattern-type' value of '$patternType' is not valid when adding acls."
      )
    ...
    withAuthorizer(opts) { authorizer =>
      ...
      for ((filter, acls) <- filterToAcl) {
        if (acls.isEmpty) {
          ...
          if (confirmAction(
                opts,
                s"Are you sure you want to remove ACLs: $Newline ${acls.map("\t" + _).mkString(Newline)} $Newline from resource filter `$filter`? (y/n)"
              ))
            removeAcls(authorizer, acls, filter)
        }
      }
  ...
  private def removeAcls(authorizer: Authorizer, acls: Set[Acl], filter: ResourcePatternFilter) {
    getAcls(authorizer, filter).keys
      .foreach(
        resource =>
  ...
  private def getResourceFilterToAcls(opts: AclCommandOptions): Map[ResourcePatternFilter, Set[Acl]] = {
    ...
    if (opts.options.has(opts.consumerOpt))
      resourceToAcls ++= getConsumerResourceFilterToAcls(opts).map {
  ...
  private def getAcl(opts: AclCommandOptions): Set[Acl] = {
    val operations =
      opts.options.valuesOf(opts.operationsOpt).asScala.map(operation => Operation.fromString(operation.trim)).toSet
  ...
  private def getHosts(opts: AclCommandOptions,
  ...
  private def getPrincipals(opts: AclCommandOptions,
                            principalOptionSpec: ArgumentAcceptingOptionSpec[String]): Set[KafkaPrincipal] =
  ...
  private def getResourceFilter(opts: AclCommandOptions,
                                dieIfNoResourceFound: Boolean = true): Set[ResourcePatternFilter] = {
    val patternType: PatternType = opts.options.valueOf(opts.resourcePatternType)
    ...
    if (opts.options.has(opts.topicOpt))
      opts.options
        ...
        .asScala
        .foreach(topic => resourceFilters += new ResourcePatternFilter(JResourceType.TOPIC, topic.trim, patternType))

    if (patternType == PatternType.LITERAL && (opts.options.has(opts.clusterOpt) || opts.options.has(
          opts.idempotentOpt
        )))
    ...
    if (opts.options.has(opts.groupOpt))
      opts.options
        ...
        .asScala
        .foreach(group => resourceFilters += new ResourcePatternFilter(JResourceType.GROUP, group.trim, patternType))

    if (opts.options.has(opts.transactionalIdOpt))
      opts.options
        .valuesOf(opts.transactionalIdOpt)
        .asScala
        .foreach(
          transactionalId =>
    ...
    if (opts.options.has(opts.delegationTokenOpt))
      opts.options
        .valuesOf(opts.delegationTokenOpt)
        .asScala
        .foreach(
          token => resourceFilters += new ResourcePatternFilter(JResourceType.DELEGATION_TOKEN, token.trim, patternType)
        )

    if (resourceFilters.isEmpty && dieIfNoResourceFound)
      CommandLineUtils.printUsageAndDie(
        opts.parser,
        "You must provide at least one resource: --topic <topic> or --cluster or --group <group> or --delegation-token <Delegation Token ID>"
      )
  ...
  class AclCommandOptions(args: Array[String]) {
    ...
    val transactionalIdOpt = parser
      .accepts(
        "transactional-id",
        "The transactionalId to which ACLs should " +
    ...
    val idempotentOpt = parser.accepts(
      "idempotent",
    ...
    val delegationTokenOpt = parser
      .accepts("delegation-token",
    ...
    val resourcePatternType = parser
      .accepts(
        "resource-pattern-type",
        "The type of the resource pattern or pattern filter. " +
    ...
    val producerOpt = parser.accepts("producer",
    ...
    def checkArgs() {
      ...
      CommandLineUtils.checkInvalidArgs(
        ...
        options,
        listOpt,
        Set(producerOpt, consumerOpt, allowHostsOpt, allowPrincipalsOpt, denyHostsOpt, denyPrincipalsOpt)
      )
      ...
      if (options.has(consumerOpt) && (!options.has(topicOpt) || !options.has(groupOpt) || (!options.has(producerOpt) && (options
            .has(clusterOpt) || options.has(transactionalIdOpt)))))
        CommandLineUtils.printUsageAndDie(
          parser,
