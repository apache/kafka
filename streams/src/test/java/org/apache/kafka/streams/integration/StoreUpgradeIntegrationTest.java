...
public class StoreUpgradeIntegrationTest {
    ...
    private void runUpgradeTest(final boolean usePlainKeyValueByteStoreSupplier) throws Exception {
        ...
        expectedResult.add(KeyValue.pair(1, ValueAndTimestamp.make(3L, usePlainKeyValueByteStoreSupplier ? -1 : 42L)));
        expectedResult.add(KeyValue.pair(2, ValueAndTimestamp.make(1L, -1)));
        expectedResult.add(KeyValue.pair(3, ValueAndTimestamp.make(1L, -1)));
        expectedResult.add(KeyValue.pair(4, ValueAndTimestamp.make(3L, -1)));
        processKeyValueAndVerifyCountWithTimestamp(1, 42L, expectedResult);

        expectedResult.remove(1);
        expectedResult.add(1, KeyValue.pair(2, ValueAndTimestamp.make(2L, usePlainKeyValueByteStoreSupplier ? -1 : 45)));
        processKeyValueAndVerifyCountWithTimestamp(2, 45L, expectedResult);

        // can process "out of order" record for different key
        expectedResult.remove(3);
        expectedResult.add(3, KeyValue.pair(4, ValueAndTimestamp.make(4L, usePlainKeyValueByteStoreSupplier ? -1 : 21)));
        processKeyValueAndVerifyCountWithTimestamp(4, 21L, expectedResult);

        expectedResult.remove(3);
        expectedResult.add(3, KeyValue.pair(4, ValueAndTimestamp.make(5L, usePlainKeyValueByteStoreSupplier ? -1 : 42)));
        ...
        expectedResult.remove(3);
        expectedResult.add(3, KeyValue.pair(4, ValueAndTimestamp.make(6L, usePlainKeyValueByteStoreSupplier ? -1 : 42)));
