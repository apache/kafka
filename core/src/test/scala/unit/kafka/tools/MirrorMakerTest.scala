...
import org.apache.kafka.common.record.{LogEntry, TimestampType}
...
class MirrorMakerTest {
  ...
  def testDefaultMirrorMakerMessageHandlerWithNoTimestampInSourceMessage() {
    val consumerRecord = BaseConsumerRecord("topic", 0, 1L, LogEntry.NO_TIMESTAMP, TimestampType.CREATE_TIME, "key".getBytes, "value".getBytes)
