...
object ApiVersionsRequestTest {
  def validateApiVersionsResponse(apiVersionsResponse: ApiVersionsResponse) {
    assertEquals("API keys in ApiVersionsResponse must match API keys supported by broker.",
                 ApiKeys.values.length,
                 apiVersionsResponse.apiVersions.size)
    for (expectedApiVersion: ApiVersion <- ApiVersionsResponse.defaultApiVersionsResponse().apiVersions.asScala) {
      ...
      assertNotNull(
        s"API key ${actualApiVersion.apiKey} is supported by broker, but not received in ApiVersionsResponse.",
        actualApiVersion
      )
      assertEquals("API key must be supported by the broker.", expectedApiVersion.apiKey, actualApiVersion.apiKey)
      assertEquals(s"Received unexpected min version for API key ${actualApiVersion.apiKey}.",
                   expectedApiVersion.minVersion,
                   actualApiVersion.minVersion)
      assertEquals(s"Received unexpected max version for API key ${actualApiVersion.apiKey}.",
                   expectedApiVersion.maxVersion,
                   actualApiVersion.maxVersion)
    }
  }
}
...
class ApiVersionsRequestTest extends BaseRequestTest {
  ...
  private def sendApiVersionsRequest(request: ApiVersionsRequest,
                                     ...
                                     responseVersion: Short = 1): ApiVersionsResponse = {
