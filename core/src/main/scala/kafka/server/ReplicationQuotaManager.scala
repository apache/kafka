...
class ReplicationQuotaManager(val config: ReplicationQuotaManagerConfig,
  ...
  private val rateMetricName =
    metrics.metricName("byte-rate", replicationType.toString, s"Tracking byte-rate for ${replicationType}")
  ...
  override def isQuotaExceeded: Boolean = {
    try {
      ...
      case qve: QuotaViolationException =>
        trace(
          "%s: Quota violated for sensor (%s), metric: (%s), metric-value: (%f), bound: (%f)"
            .format(replicationType, sensor().name(), qve.metricName, qve.value, qve.bound)
        )
        return true
    }
    false
  ...
  def record(value: Long) {
    try {
      ...
      case qve: QuotaViolationException =>
        trace(
          s"Record: Quota violated, but ignored, for sensor (${sensor.name}), metric: (${qve.metricName}), value : (${qve.value}), bound: (${qve.bound}), recordedValue ($value)"
        )
    }
  }
