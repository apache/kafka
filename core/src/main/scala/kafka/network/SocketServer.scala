...
private[kafka] abstract class AbstractServerThread(connectionQuotas: ConnectionQuotas) extends Runnable with Logging {
  ...
  def close(channel: SocketChannel): Unit = {
    if (channel != null) {
      ...
      CoreUtils.swallow(channel.socket().close(), logger, Level.ERROR)
      CoreUtils.swallow(channel.close(), logger, Level.ERROR)
    }
  }
}
...
private[kafka] class Acceptor(val endPoint: EndPoint,
  ...
  def run() {
    ...
    try {
      ...
      CoreUtils.swallow(serverChannel.close(), logger, Level.ERROR)
      CoreUtils.swallow(nioSelector.close(), logger, Level.ERROR)
      shutdownComplete()
    }
  }
...
private[kafka] class Processor(val id: Int,
  ...
  override def run() {
    ...
    try {
      ...
      CoreUtils.swallow(closeAll(), logger, Level.ERROR)
      shutdownComplete()
    }
  }
