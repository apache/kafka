...
import org.apache.kafka.server.config.{KRaftConfigs, KafkaServerConfigs}
...
abstract class QuorumTestHarness extends Logging {
  ...
  protected def newKRaftQuorum(overridingProps: Properties): KRaftQuorumImplementation = {
    ...
    props.setProperty(KafkaServerConfigs.UNSTABLE_METADATA_VERSIONS_ENABLE_CONFIG, "true")
