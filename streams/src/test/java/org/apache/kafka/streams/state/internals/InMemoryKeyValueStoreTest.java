...
public class InMemoryKeyValueStoreTest extends AbstractKeyValueStoreTest {
    ...
    protected <K, V> KeyValueStore<K, V> createKeyValueStore(
        ...
        if (useContextSerdes) {
            ...
            supplier = Stores.create("my-store").withKeys(Serializations.serialization(keySer, keyDeser)).withValues(Serializations.serialization(valSer, valDeser)).inMemory().build();
        } else {
