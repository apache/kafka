...
class InternalTopicManagerTest {
    ...
    void testMissingTopics() {
        ...
        Map<String, TopicsInfo> subtopologyMap = makeExpectedConfiguredTopology();
    ...
    void testConfigureTopics() {
        ...
        Map<String, TopicsInfo> configuredSubtopologies =
            InternalTopicManager.configureTopics(new LogContext(), subtopologyList, metadataImage);

        Map<String, TopicsInfo> expectedTopicsInfoMap = makeExpectedConfiguredTopology();
        assertEquals(expectedTopicsInfoMap, configuredSubtopologies);
    }

    private static Map<String, TopicsInfo> makeExpectedConfiguredTopology() {
        return mkMap(
            mkEntry("subtopology1",
                new TopicsInfo()
                    .setSourceTopics(mkSet("source_topic1"))
                    .setStateChangelogTopics(Collections.singletonMap("state_changelog_topic1",
                        new InternalTopicConfig("state_changelog_topic1",
                            Collections.singletonMap("cleanup.policy", "compact"),
                            Optional.empty(),
                            Optional.empty()
                        ).setNumberOfPartitions(2)))
                    .setRepartitionSinkTopics(mkSet("repartition_topic"))
            ),
            mkEntry("subtopology2",
                new TopicsInfo()
                    .setSourceTopics(mkSet("source_topic2"))
                    .setRepartitionSourceTopics(Collections.singletonMap("repartition_topic",
                        new InternalTopicConfig("repartition_topic",
                    ...
                    .setStateChangelogTopics(Collections.singletonMap("state_changelog_topic2",
                        new InternalTopicConfig("state_changelog_topic2",
                            Collections.emptyMap(),
                            Optional.empty(),
                            Optional.empty()
                        ).setNumberOfPartitions(2)))
