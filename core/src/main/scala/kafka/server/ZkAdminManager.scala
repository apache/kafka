...
class ZkAdminManager(val config: KafkaConfig,
  ...
  private val createTopicPolicy =
    Option(config.getConfiguredInstance(KafkaConfig.CREATE_TOPIC_POLICY_CLASS_NAME_PROP, classOf[CreateTopicPolicy]))

  private val alterConfigPolicy =
    Option(config.getConfiguredInstance(KafkaConfig.ALTER_CONFIG_POLICY_CLASS_NAME_PROP, classOf[AlterConfigPolicy]))
