...
class AdminClientWithPoliciesIntegrationTest extends KafkaServerTestHarness with Logging {
  ...
  def testInvalidAlterConfigsDueToPolicy(): Unit = {
    ...
    assertEquals(Set(topicResource1, topicResource2, topicResource3, brokerResource).asJava, alterResult.value.keySet)
    assertTrue(intercept[ExecutionException](alterResult.value.get(topicResource1).get).getCause.isInstanceOf[PolicyViolationException])
    alterResult.value.get(topicResource2).get
    assertTrue(intercept[ExecutionException](alterResult.value.get(topicResource3).get).getCause.isInstanceOf[InvalidRequestException])
