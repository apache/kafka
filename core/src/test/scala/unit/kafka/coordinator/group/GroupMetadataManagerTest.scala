...
import kafka.server.{HostedPartition, KafkaConfigProvider, ReplicaManager, RequestLocal}
...
class GroupMetadataManagerTest {
  ...
  private val offsetConfig = {
    val config = KafkaConfigProvider.fromProps(TestUtils.createBrokerConfig(nodeId = 0, zkConnect = ""))
    new OffsetConfig(
  ...
  @BeforeEach
  def setUp(): Unit = {
    defaultOffsetRetentionMs = offsetConfig.getOffsetsRetentionMs()
