...
public class ConnectorsResourceTest {
    ...
    public void testFenceZombiesNoInternalRequestSignature() throws Throwable {
        final Capture<Callback<Void>> cb = Capture.newInstance();
        herder.fenceZombies(EasyMock.eq(CONNECTOR_NAME), EasyMock.capture(cb), EasyMock.anyObject(InternalRequestSignature.class));
        expectAndCallbackResult(cb, null);
    ...
    public void testFenceZombiesWithInternalRequestSignature() throws Throwable {
        ...
        final Capture<Callback<Void>> cb = Capture.newInstance();
        final Capture<InternalRequestSignature> signatureCapture = Capture.newInstance();
        herder.fenceZombies(EasyMock.eq(CONNECTOR_NAME), EasyMock.capture(cb), EasyMock.capture(signatureCapture));
        expectAndCallbackResult(cb, null);
