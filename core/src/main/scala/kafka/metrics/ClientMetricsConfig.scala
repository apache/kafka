...
object ClientMetricsConfig {
  ...
  object ClientMetrics {
    ...
    val configDef: ConfigDef = new ConfigDef()
      ...
      .define(PushIntervalMs, INT, DEFAULT_INTERVAL_MS, MEDIUM, "Interval that a client can push the metrics")

    def names: util.Set[String] = configDef.names
    ...
    def validateProperties(properties: Properties): Unit = {
      // Make sure that all the properties are valid.
      properties.keySet().forEach(x =>
        if (!names.contains(x))
          throw new InvalidRequestException(s"Unknown client metrics configuration: $x")
      )
      ...
      if (properties.containsKey(PushIntervalMs)) {
        ...
        if (pushIntervalMs < MIN_INTERVAL_MS || pushIntervalMs > MAX_INTERVAL_MS)
          throw new InvalidRequestException(s"Invalid value for $PushIntervalMs." +
            s" Interval must be between 100 and 3600000 (1 hour)")
      }
