...
public class KafkaConsumerClone<K, V> implements Consumer<K, V> {
     ...
     */
    public KafkaConsumerClone(Map<String, Object> configs) {
     ...
     */
    public KafkaConsumerClone(Properties properties) {
    ...
    KafkaConsumerClone(ConsumerConfig config, Deserializer<K> keyDeserializer, Deserializer<V> valueDeserializer) {
        try {
    ...
    // visible for testing
    KafkaConsumerClone(LogContext logContext, ConsumerDelegate<K, V> delegate) {
        this.log = logContext.logger(getClass());
