  ...
  private def doTxnCommitOffsets(group: GroupMetadata,
    ...
    group.inLock {
      ...
      if (validationErrorOpt.isDefined) {
        ...
        val offsets = offsetMetadata.map { case(k,v) => new TopicIdPartition(Uuid.ZERO_UUID, k) -> v }
        def callback(commitStatus: Map[TopicIdPartition, Errors]) = {
          responseCallback(commitStatus.map { case(k,v) => k.topicPartition -> v }.toMap)
        }
        groupManager.storeOffsets(group, memberId, offsets, callback, producerId, producerEpoch, requestLocal)
      }
    }
  }
