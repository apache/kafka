...
public interface AbstractTable<K, V, S extends StateStore> {

    /**
     * Create a new {@code AbstractTable} that consists of all records of this {@code AbstractTable} which satisfy the given
     * predicate, with default serializers, deserializers, and state store.
     * All records that do not satisfy the predicate are dropped.
     * For each {@code AbstractTable} update, the filter is evaluated based on the current update
     * record and then an update record is produced for the result {@code AbstractTable}.
     ...
     * @return a {@code AbstractTable} that contains only those records that satisfy the given predicate
     * @see #filterNot(Predicate)
     */
    AbstractTable<K, V, S> filter(final Predicate<? super K, ? super V> predicate);
     ...
     *
     * @param predicate     a filter {@link Predicate} that is applied to each record
     * @param materialized  a {@link Materialized} that describes how the {@link StateStore} for the resulting {@code AbstractTable}
    ...
    /**
     * Create a new {@code AbstractTable} that consists all records of this {@code AbstractTable} which do <em>not</em> satisfy the
     ...
     *
     * @param predicate a filter {@link Predicate} that is applied to each record
     * @return a {@code AbstractTable} that contains only those records that do <em>not</em> satisfy the given predicate
     * @see #filter(Predicate)
     */
    AbstractTable<K, V, S> filterNot(final Predicate<? super K, ? super V> predicate);
    ...
    /**
     * Create a new {@code AbstractTable} by transforming the value of each record in this {@code AbstractTable} into a new value
     * (with possibly a new type) in the new {@code AbstractTable}, with default serializers, deserializers, and state store.
     * For each {@code AbstractTable} update the provided {@link ValueMapper} is applied to the value of the updated record and
     * computes a new value for it, resulting in an updated record for the result {@code AbstractTable}.
     * Thus, an input record {@code <K,V>} can be transformed into an output record {@code <K:V'>}.
     * This is a stateless record-by-record operation.
     * <p>
     * The example below counts the number of token of the value string.
     * <pre>{@code
     * AbstractTable<String, String> inputTable = builder.table("topic");
     * AbstractTable<String, Integer> outputTable = inputTable.mapValue(new ValueMapper<String, Integer> {
     ...
     * delete the corresponding record in the result {@code AbstractTable}.
     *
     * @param mapper a {@link ValueMapper} that computes a new output value
     * @param <VR>   the value type of the result {@code AbstractTable}
     * @return a {@code AbstractTable} that contains records with unmodified keys and new values (possibly of different type)
     */
    <VR> AbstractTable<K, VR, S> mapValues(final ValueMapper<? super V, ? extends VR> mapper);

    /**
     * Create a new {@code AbstractTable} by transforming the value of each record in this {@code AbstractTable} into a new value
     * (with possibly a new type) in the new {@code AbstractTable}, with default serializers, deserializers, and state store.
     * For each {@code AbstractTable} update the provided {@link ValueMapperWithKey} is applied to the value of the update
     * record and computes a new value for it, resulting in an updated record for the result {@code AbstractTable}.
     * Thus, an input record {@code <K,V>} can be transformed into an output record {@code <K:V'>}.
     * This is a stateless record-by-record operation.
     * <p>
     ...
     * <pre>{@code
     * AbstractTable<String, String> inputTable = builder.table("topic");
     * AbstractTable<String, Integer> outputTable = inputTable.mapValue(new ValueMapperWithKey<String, String, Integer> {
     ...
     * @return a {@code AbstractTable} that contains records with unmodified keys and new values (possibly of different type)
     */
    <VR> AbstractTable<K, VR, S> mapValues(final ValueMapperWithKey<? super K, ? super V, ? extends VR> mapper);

    /**
     * Create a new {@code AbstractTable} by transforming the value of each record in this {@code AbstractTable} into a new value
     * (with possibly a new type) in the new {@code AbstractTable}, with the {@link Serde key serde}, {@link Serde value serde},
     ...
     * this changelog stream is no longer treated as an updated record (cf. {@link KStream} vs {@code AbstractTable}).
     *
     * @return a {@link KStream} that contains the same records as this {@code AbstractTable}
     */
     ...
     * <pre>{@code
     * AbstractTable<String, String> table = builder.table("topic");
     * AbstractTable<Integer, String> keyedStream = table.toStream(new KeyValueMapper<String, String, Integer> {
     ...
     * each record of this changelog stream is no longer treated as an updated record (cf. {@link KStream} vs {@code AbstractTable}).
     *
     ...
     * @return A new AbstractTable with the desired suppression characteristics.
     */
    AbstractTable<K, V, S> suppress(final Suppressed<? super K> suppressed);
     ...
     * In contrast, if the resulting AbstractTable is materialized, (cf. {@link #transformValues(ValueTransformerWithKeySupplier, Materialized, String...)}),
     ...
     *
     * AbstractTable outputTable = inputTable.transformValues(new ValueTransformerWithKeySupplier() { ... }, "myValueTransformState");
     * }</pre>
     * <p>
     ...
     * @return a {@code AbstractTable} that contains records with unmodified key and new values (possibly of different type)
     * @see #mapValues(ValueMapper)
     * @see #mapValues(ValueMapperWithKey)
     */
    <VR> AbstractTable<K, VR, S> transformValues(final ValueTransformerWithKeySupplier<? super K, ? super V, ? extends VR> transformerSupplier,
     ...
     * Furthermore, via {@link org.apache.kafka.streams.processor.Punctuator#punctuate(long)} the processing progress can be observed and additional
     * periodic actions can be performed.
     * The resulting {@code AbstractTable} is materialized into another state store (additional to the provided state store names)
     ...
     * builder.addStateStore(keyValueStoreBuilder);
     *
     * AbstractTable outputTable = inputTable.transformValues(
     *     new ValueTransformerWithKeySupplier() { ... },
    ...
    /**
     * Re-groups the records of this {@code AbstractTable} using the provided {@link KeyValueMapper} and default serializers
     * and deserializers.
     * Each {@link KeyValue} pair of this {@code AbstractTable} is mapped to a new {@link KeyValue} pair by applying the
     * provided {@link KeyValueMapper}.
     * Re-grouping a {@code AbstractTable} is required before an aggregation operator can be applied to the data
     ...
     *
     * <p>
     * All data of this {@code AbstractTable} will be redistributed through the repartitioning topic by writing all update
     ...
     * @return a {@link KGroupedTable} that contains the re-grouped records of the original {@code AbstractTable}
     */
    <KR, VR> KGroupedTable<KR, VR> groupBy(final KeyValueMapper<? super K, ? super V, KeyValue<KR, VR>> selector);

    /**
     * Re-groups the records of this {@code AbstractTable} using the provided {@link KeyValueMapper}
     ...
     * @return a {@link KGroupedTable} that contains the re-grouped records of the original {@code AbstractTable}
     *
     * @deprecated since 2.1. Use {@link org.apache.kafka.streams.kstream.AbstractTable#groupBy(KeyValueMapper, Grouped)} instead
     */
    @Deprecated
    <KR, VR> KGroupedTable<KR, VR> groupBy(final KeyValueMapper<? super K, ? super V, KeyValue<KR, VR>> selector,
    ...
    /**
     * Join records of this {@code AbstractTable} with another {@code AbstractTable}'s records using non-windowed inner equi join,
     * with default serializers, deserializers, and state store.
     * The join is a primary key join with join attribute {@code thisAbstractTable.key == otherAbstractTable.key}.
     * The result is an ever updating {@code AbstractTable} that represents the <em>current</em> (i.e., processing time) result
     * of the join.
     * <p>
     * The join is computed by (1) updating the internal state of one {@code AbstractTable} and (2) performing a lookup for a
     * matching record in the <em>current</em> (i.e., processing time) internal state of the other {@code AbstractTable}.
     * This happens in a symmetric way, i.e., for each update of either {@code this} or the {@code other} input
     * {@code AbstractTable} the result gets updated.
     * <p>
     * For each {@code AbstractTable} record that finds a corresponding record in the other {@code AbstractTable} the provided
     ...
     * directly to delete a record in the result {@code AbstractTable} if required (i.e., if there is anything to be deleted).
     * <p>
     ...
     * <table border='1'>
     * <tr>
     * <th>thisAbstractTable</th>
     * <th>thisState</th>
     * <th>otherAbstractTable</th>
     * <th>otherState</th>
     ...
     * partitions.
     *
     * @param other  the other {@code AbstractTable} to be joined with this {@code AbstractTable}
     * @param joiner a {@link ValueJoiner} that computes the join result for a pair of matching records
     * @param <VO>   the value type of the other {@code AbstractTable}
     * @param <VR>   the value type of the result {@code AbstractTable}
     * @return a {@code AbstractTable} that contains join-records for each key and values computed by the given
     * {@link ValueJoiner}, one for each matched record-pair with the same key
     * @see #leftJoin(AbstractTable, ValueJoiner)
     * @see #outerJoin(AbstractTable, ValueJoiner)
     */
    <VO, VR> AbstractTable<K, VR, S> join(final AbstractTable<K, VO, S> other,
     ...
     * @return a {@code AbstractTable} that contains join-records for each key and values computed by the given
     * {@link ValueJoiner}, one for each matched record-pair with the same key
     * @see #leftJoin(AbstractTable, ValueJoiner, Materialized)
     * @see #outerJoin(AbstractTable, ValueJoiner, Materialized)
     */
    <VO, VR> AbstractTable<K, VR, S> join(final AbstractTable<K, VO, S> other,
                                          final ValueJoiner<? super V, ? super VO, ? extends VR> joiner,
                                          final Materialized<K, VR, S> materialized);

    /**
     * Join records of this {@code AbstractTable} (left input) with another {@code AbstractTable}'s (right input) records using
     * non-windowed left equi join, with default serializers, deserializers, and state store.
     * The join is a primary key join with join attribute {@code thisAbstractTable.key == otherAbstractTable.key}.
     * In contrast to {@link #join(AbstractTable, ValueJoiner) inner-join}, all records from left {@code AbstractTable} will produce
     ...
     * {@code AbstractTable} the result gets updated.
     * <p>
     * For each {@code AbstractTable} record that finds a corresponding record in the other {@code AbstractTable}'s state the
     * provided {@link ValueJoiner} will be called to compute a value (with arbitrary type) for the result record.
     * Additionally, for each record of left {@code AbstractTable} that does not find a corresponding record in the
     * right {@code AbstractTable}'s state the provided {@link ValueJoiner} will be called with {@code rightValue =
     ...
     * forwarded directly to delete a record in the result {@code AbstractTable} if required (i.e., if there is anything to be
     * deleted).
     * <p>
     * Input records with {@code null} key will be dropped and no join computation is performed.
     * <p>
     ...
     * left {@code AbstractTable}
     * @see #join(AbstractTable, ValueJoiner)
     * @see #outerJoin(AbstractTable, ValueJoiner)
     */
    <VO, VR> AbstractTable<K, VR, S> leftJoin(final AbstractTable<K, VO, S> other,
                                              final ValueJoiner<? super V, ? super VO, ? extends VR> joiner);
     ...
     * {@link ValueJoiner}, one for each matched record-pair with the same key plus one for each non-matching record of
     * left {@code AbstractTable}
     * @see #join(AbstractTable, ValueJoiner, Materialized)
     * @see #outerJoin(AbstractTable, ValueJoiner, Materialized)
     */
    <VO, VR> AbstractTable<K, VR, S> leftJoin(final AbstractTable<K, VO, S> other,
                                    final ValueJoiner<? super V, ? super VO, ? extends VR> joiner,
     ...
     * The join is a primary key join with join attribute {@code thisAbstractTable.key == otherAbstractTable.key}.
     * In contrast to {@link #join(AbstractTable, ValueJoiner) inner-join} or {@link #leftJoin(AbstractTable, ValueJoiner) left-join},
     * all records from both input {@code AbstractTable}s will produce an output record (cf. below).
     * The result is an ever updating {@code AbstractTable} that represents the <em>current</em> (i.e., processing time) result
     * of the join.
     * <p>
     * The join is computed by (1) updating the internal state of one {@code AbstractTable} and (2) performing a lookup for a
     ...
     * {@code AbstractTable}'s state the provided {@link ValueJoiner} will be called with {@code null} value for the
     ...
     * to delete a record in the result {@code AbstractTable} if required (i.e., if there is anything to be deleted).
     * <p>
     * Input records with {@code null} key will be dropped and no join computation is performed.
     * <p>
     * Example:
     ...
     * @return a {@code AbstractTable} that contains join-records for each key and values computed by the given
     * {@link ValueJoiner}, one for each matched record-pair with the same key plus one for each non-matching record of
     * both {@code AbstractTable}s
     * @see #join(AbstractTable, ValueJoiner)
     * @see #leftJoin(AbstractTable, ValueJoiner)
     */
    <VO, VR> AbstractTable<K, VR, S> outerJoin(final AbstractTable<K, VO, S> other);
    ...
    /**
     * Get the name of the local state store used that can be used to query this {@code AbstractTable}.
     *
     * @return the underlying state store name, or {@code null} if this {@code AbstractTable} cannot be queried.
    */
