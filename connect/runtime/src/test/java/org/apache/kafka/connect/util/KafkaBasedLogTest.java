...
public class KafkaBasedLogTest {
    ...
    @Test
    public void testSendAndReadToEndWithUnexpectedException() throws Exception {
        expectStart();
        TestFuture<RecordMetadata> tp0Future = new TestFuture<>();
        ProducerRecord<String, String> tp0Record = new ProducerRecord<>(TOPIC, TP0_KEY, TP0_VALUE);
        Capture<org.apache.kafka.clients.producer.Callback> callback0 = EasyMock.newCapture();
        ...
        consumer.schedulePollTask(() -> {
            ...
            consumer.scheduleNopPollTask();
            consumer.schedulePollTask(() -> {
                // Throw an unexpected exception that will handled by outermost catch blockof WorkThread.
                // And would create a new thread
                // time outs (for instance via ConnectRestException)
