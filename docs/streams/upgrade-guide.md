...
Downgrading from 4.3.x or newer version to 4.2.x or older version needs special attention: Since 4.3.0 release, Kafka Streams persists state store changelog offsets inside each state store rather than in a per-task `.checkpoint` file (see [KIP-1035](https://cwiki.apache.org/confluence/display/KAFKA/KIP-1035%3A+StateStore+managed+changelog+offsets)). On startup, the legacy per-task `.checkpoint` file is migrated into the per-store offset metadata and then deleted, so older Kafka Streams versions that still expect the legacy file will not find their offsets after a downgrade and will treat the state stores as uninitialized — forcing a full restore from the changelog topics. To support an in-flight downgrade without restoring state, 4.3.0+ will write the legacy consolidated per-task `.checkpoint` file back out on a clean shutdown when `upgrade.from` is set to a version less than 4.3. Perform the downgrade as two rolling bounces:

  * In the first rolling bounce, on each application instance running 4.3.0+, set `upgrade.from="4.2"` (or earlier, e.g. `"4.1"`, `"4.0"`) and restart the instance. Each instance must be cleanly shut down at least once with this config set so that an up-to-date legacy `.checkpoint` file is written into every task and global state directory alongside the per-store offsets.
  * In the second rolling bounce, swap the binaries to the older version (4.2.x or earlier) and remove the `upgrade.from` config. The older version will read the legacy `.checkpoint` file as it always did and resume from the persisted offsets.

If an instance is downgraded without first being cleanly shut down with `upgrade.from` set (for example, after a crash, or by skipping the first rolling bounce), the legacy `.checkpoint` file will be missing or stale, and the older version will restore the affected stores in full from the changelog topics. See [KAFKA-19710](https://issues.apache.org/jira/browse/KAFKA-19710) for the change that introduced this downgrade path.
...
Kafka Streams now persists state store changelog offsets inside each state store rather than in a single per-task `.checkpoint` file, as described in [KIP-1035](https://cwiki.apache.org/confluence/display/KAFKA/KIP-1035%3A+StateStore+managed+changelog+offsets). For built-in (RocksDB) stores, offsets are written into a dedicated column family alongside store data. Persistent stores that do not manage their own offsets (including custom user-implemented stores) are transparently wrapped to maintain a per-store `.checkpoint_<storeName>` file, preserving the existing behavior. Existing per-task `.checkpoint` files are migrated automatically on first startup and then deleted; no application changes or operator action is required to upgrade. This change is largely internal infrastructure — and importantly, **it does not yet change behavior under the `exactly-once` `processing.guarantee`**: on a crash or processing error, state stores are still wiped and fully restored from the changelog topics, the same as in 4.2 and earlier. KIP-1035 is a precursor and prerequisite for [KIP-892: Transactional Semantics for StateStores](https://cwiki.apache.org/confluence/display/KAFKA/KIP-892%3A+Transactional+Semantics+for+StateStores), which will leverage these per-store offsets to make state store writes transactional with the changelog and avoid full restoration after EOS errors. Until KIP-892 lands, the user-visible impact of KIP-1035 is limited to the on-disk layout change and its downgrade implications — see [Notable compatibility changes in past releases](#notable-compatibility-changes-in-past-releases) for the procedure to downgrade to 4.2.x or earlier. Custom `StateStore` implementations may opt-in to managing their own offsets by overriding `managesOffsets()` to return `true`, implementing `commit(Map<TopicPartition, Long>)` to atomically persist offsets with their data, and implementing `committedOffset(TopicPartition)` to return the most-recently-committed offset for a partition; otherwise the wrapper continues to handle checkpointing on their behalf.
