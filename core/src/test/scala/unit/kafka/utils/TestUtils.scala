...
object TestUtils extends Logging {
  ...
  def createBrokerConfig(nodeId: Int,
    ...
    props.put(KafkaServerConfigs.UNSTABLE_METADATA_VERSIONS_ENABLE_CONFIG, "true")
    if (zkConnect == null) {
      ...
      props.put(KafkaServerConfigs.BROKER_ID_CONFIG, nodeId.toString)
      ...
      if (nodeId >= 0) props.put(KafkaServerConfigs.BROKER_ID_CONFIG, nodeId.toString)
      props.put(SocketServerConfigs.LISTENERS_CONFIG, listeners)
    }
    if (logDirCount > 1) {
    ...
    props.put(KafkaServerConfigs.CONTROLLED_SHUTDOWN_ENABLE_CONFIG, enableControlledShutdown.toString)
    props.put(KafkaServerConfigs.DELETE_TOPIC_ENABLE_CONFIG, enableDeleteTopic.toString)
    props.put(ServerLogConfigs.LOG_DELETE_DELAY_MS_CONFIG, "1000")
    props.put(KafkaServerConfigs.CONTROLLED_SHUTDOWN_RETRY_BACKOFF_MS_CONFIG, "100")
    ...
    rack.foreach(props.put(KafkaServerConfigs.BROKER_RACK_CONFIG, _))
    // Reduce number of threads per broker
    props.put(KafkaServerConfigs.NUM_NETWORK_THREADS_CONFIG, "2")
    props.put(KafkaServerConfigs.BACKGROUND_THREADS_CONFIG, "2")
    ...
    if (enableFetchFromFollower) {
      props.put(KafkaServerConfigs.BROKER_RACK_CONFIG, nodeId.toString)
