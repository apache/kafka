...
class LogTest {
  ...
  def createLogConfig(segmentMs: Long = 24 * 7 * 60 * 60 * 1000L,
                      segmentBytes: Int = 1 * 1024 * 1024 * 1024,
                      retentionMs: Long = 24 * 7 * 60 * 60 * 1000L,
                      retentionBytes: Long = -1L,
                      segmentJitterMs: Long = 0L,
                      cleanupPolicy: String = "delete",
                      maxMessageBytes: Int = 1000000 + MessageSet.LogOverhead,
                      indexIntervalBytes: Int = 4096,
                      segmentIndexBytes: Int = 10 * 1024 * 1024,
                      messageFormatVersion: String = ApiVersion.latestVersion.toString,
                      fileDeleteDelayMs: Long = 60000L): LogConfig = {
    val logProps = new Properties()
