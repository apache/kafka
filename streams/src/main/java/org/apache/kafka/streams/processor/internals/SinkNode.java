...
public class SinkNode<KIn, VIn, KOut, VOut> extends ProcessorNode<KIn, VIn, KOut, VOut> {
    ...
    public void process(final Record<KIn, VIn> record, final Optional<RecordMetadata> recordMetadata) {
        ...
        // and fall back to the context (which is undefined, but extractors may still depend on the current behavior.
        ...
        final ProcessorRecordContext contextForExtraction =
            maybeContext.orElseGet(
                () -> new ProcessorRecordContext(
                    timestamp,
                    context.recordContext() == null ? -1 : context.offset(),
                    context.recordContext() == null ? -1 : context.partition(),
                    context.recordContext() == null ? null : context.topic(),
                    record.headers()
                )
            );
