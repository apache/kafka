...
import org.apache.kafka.common.requests.UpdateMetadataRequest.{Broker, EndPoint, UpdateMetadataRequestPartitionState}
...
class MetadataCacheTest {
  ...
  def getTopicMetadata() {
    ...
    val partitionStates = Map(
      new TopicPartition(topic0, 0) -> new UpdateMetadataRequestPartitionState(controllerEpoch, 0, 0, asList(0, 1, 3), zkVersion, asList(0, 1, 3), asList()),
      new TopicPartition(topic0, 1) -> new UpdateMetadataRequestPartitionState(controllerEpoch, 1, 1, asList(1, 0), zkVersion, asList(1, 2, 0, 4), asList()),
      new TopicPartition(topic1, 0) -> new UpdateMetadataRequestPartitionState(controllerEpoch, 2, 2, asList(2, 1), zkVersion, asList(2, 1, 3), asList()))
  ...
  def getTopicMetadataPartitionLeaderNotAvailable() {
    ...
    val leaderEpoch = 1
    val partitionStates = Map(
      new TopicPartition(topic, 0) -> new UpdateMetadataRequestPartitionState(controllerEpoch, leader, leaderEpoch, asList(0), zkVersion, asList(0), asList()))
  ...
  def getTopicMetadataReplicaNotAvailable() {
    ...
    val partitionStates = Map(
      new TopicPartition(topic, 0) -> new UpdateMetadataRequestPartitionState(controllerEpoch, leader, leaderEpoch, isr, zkVersion, replicas, asList()))
  ...
  def getTopicMetadataWithNonSupportedSecurityProtocol() {
    ...
    val isr = asList[Integer](0, 1)
    val partitionStates = Map(
      new TopicPartition(topic, 0) -> new UpdateMetadataRequestPartitionState(controllerEpoch, leader, leaderEpoch, isr, 3, replicas, asList()))
    val version = ApiKeys.UPDATE_METADATA_KEY.latestVersion
