...
class KafkaApisTest extends Logging {
  ...
  private def testFindCoordinatorWithTopicCreation(coordinatorType: CoordinatorType,
    ...
    topicConfigOverride.put(KafkaServerConfigs.REQUEST_TIMEOUT_MS_CONFIG, requestTimeout.toString)
  ...
  def testHandleWriteTxnMarkersRequestWithNewGroupCoordinator(): Unit = {
    ...
    when(groupCoordinator.completeTransaction(
      ...
      ArgumentMatchers.eq(TransactionResult.COMMIT),
      ArgumentMatchers.eq(Duration.ofMillis(KafkaServerConfigs.REQUEST_TIMEOUT_MS_DEFAULT))
    ...
    when(replicaManager.appendRecords(
      ArgumentMatchers.eq(KafkaServerConfigs.REQUEST_TIMEOUT_MS_DEFAULT.toLong),
