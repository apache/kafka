...
object ZkSecurityMigrator extends Logging {
  val usageMessage = ("ZooKeeper Migration Tool Help. This tool updates the ACLs of "
                      + "nodes as part of the process of setting up ZooKeeper "
                      + "authentication.")
  ...
  private object GetChildrenCallback extends ChildrenCallback {
    def processResult(rc: Int,
      ...
      Code.get(rc) match {
        ...
        case Code.NONODE =>
          warn(s"Node is gone, it could be have been legitimately deleted: $path")
          promise success "done"
        case Code.SESSIONEXPIRED =>
          ...
          println("ZooKeeper session expired while changing ACLs")
          promise failure ZkException.create(KeeperException.create(Code.get(rc)))
        case _ =>
          println(s"Unexpected return code: $rc")
          promise failure ZkException.create(KeeperException.create(Code.get(rc)))
      }
    }
  }
    ...
    def processResult(rc: Int,
      ...
      Code.get(rc) match {
        ...
        case Code.SESSIONEXPIRED =>
          ...
          promise failure ZkException.create(KeeperException.create(Code.get(rc)))
        case _ =>
          println("Unexpected return code: %d".format(rc))
          promise failure ZkException.create(KeeperException.create(Code.get(rc)))
      }
    }
  }
