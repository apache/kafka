 ...
 * If IOException is encountered while accessing a log directory, the corresponding class can add the the log directory name
 * to the LogDirFailureChannel using maybeAddLogFailureEvent(). Each log directory can be added only once. After a log directory
 * is added for the first time, a thread which is blocked waiting for new offline log directories
 * can take the name of the new offline log directory out of the LogDirFailureChannel and handles the log failure properly. An
 * offline log directory will stay offline until a broker restarts.
 *
 */
class LogDirFailureChannel(logDirNum: Int) extends Logging {
   ...
   */
  def takeNextOfflineLogDir(): String = {
    offlineLogDirQueue.take()
  }
