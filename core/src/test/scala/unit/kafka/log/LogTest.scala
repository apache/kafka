...
class LogTest {
  ...
  def testLogRecoversToCorrectOffset() {
    ...
    // test recovery case
    log = new Log(logDir, config, logStartOffset = 0L, recoveryPoint = 0L, scheduler = time.scheduler, time = time, brokerState = brokerState)
  ...
  def testBuildTimeIndexWhenNotAssigningOffsets() {
    ...
    val config = LogConfig(logProps)
    val log = new Log(logDir, config, logStartOffset = 0L, recoveryPoint = 0L, scheduler = time.scheduler, time = time, brokerState = brokerState)
  ...
  def testIndexRebuild() {
    ...
    val config = LogConfig(logProps)
    var log = new Log(logDir, config, logStartOffset = 0L, recoveryPoint = 0L, scheduler = time.scheduler, time = time, brokerState = brokerState)
  ...
  def testCorruptIndexRebuild() {
    ...
    // reopen the log
    log = new Log(logDir, config, logStartOffset = 0L, recoveryPoint = 200L, scheduler = time.scheduler, time = time, brokerState = brokerState)
  ...
  def testTruncateTo() {
    ...
    // create a log
    val log = new Log(logDir, LogConfig(logProps), logStartOffset = 0L, recoveryPoint = 0L, scheduler = time.scheduler, time = time, brokerState = brokerState)
  ...
  def testCorruptLog() {
    ...
    for (_ <- 0 until 10) {
      ...
      // attempt recovery
      log = new Log(logDir, config, 0L, recoveryPoint, time.scheduler, time, brokerState = brokerState)
  ...
  def testCleanShutdownFile() {
    ...
    // Create a clean shutdown file
    val cleanShutdownFile = new File(parentLogDir, ".kafka_cleanshutdown")
    ...
    // check if recovery was attempted. Recovery should be attempted regardless of clean shutdown file
    recoveryPoint = log.logEndOffset
    log = new Log(logDir, config, 0L, 0L, time.scheduler, time, brokerState = brokerState)
  ...
  def shouldApplyEpochToMessageOnAppendIfLeader() {
    ...
    val epoch = 72
    val log = new Log(logDir, LogConfig(), recoveryPoint = 0L, scheduler = time.scheduler, time = time, brokerState = brokerState)
  ...
  def shouldTruncateLeaderEpochFileWhenTruncatingLog() {
    ...
    val log = new Log(logDir, LogConfig( logProps), recoveryPoint = 0L, scheduler = time.scheduler, time = time, brokerState = brokerState)
    val cache = epochCache(log)
  ...
  def testLogRecoversForLeaderEpoch() {
    val log = new Log(logDir, LogConfig(new Properties()), recoveryPoint = 0L, scheduler = time.scheduler, time = time, brokerState = brokerState)
