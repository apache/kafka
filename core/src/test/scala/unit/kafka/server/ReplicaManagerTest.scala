...
class ReplicaManagerTest {
  ...
  def testStopReplicaWithStaleControllerEpoch(enableRemoteStorage: Boolean): Unit = {
    ...
    try {
      ...
      val becomeLeaderRequest = new LeaderAndIsrRequest.Builder(0, 10, brokerEpoch,
  ...
  def testPartitionMetadataFileNotCreated(): Unit = {
    ...
    try {
      ...
      def leaderAndIsrRequest(epoch: Int, name: String, version: Short): LeaderAndIsrRequest =
        new LeaderAndIsrRequest.Builder(0, 0, brokerEpoch,
          Seq(new LeaderAndIsrRequest.PartitionState()
            .setTopicName(name)
            .setPartitionIndex(0)
