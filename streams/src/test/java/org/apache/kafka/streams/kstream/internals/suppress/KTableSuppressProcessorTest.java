...
public class KTableSuppressProcessorTest {
    ...
    private static class Harness<K, V> {
        private final TypedProcessor<K, Change<V>, K, Change<V>> processor;
        ...
        Harness(final Suppressed<K> suppressed,
            ...
            final TypedProcessor<K, Change<V>, K, Change<V>> processor =
    ...
    public void zeroTimeLimitShouldImmediatelyEmit() {
        ...
        harness.context.setRecordMetadata("", 0, 0L, null, timestamp);
        final String key = "hey";
        final Change<Long> value = ARBITRARY_CHANGE;
        harness.processor.process(key, value);

        assertThat(harness.context.forwarded(), hasSize(1));
        final MockProcessorContext.CapturedForward<String, Change<Long>> capturedForward = harness.context.forwarded().get(0);
    ...
    public void windowedZeroTimeLimitShouldImmediatelyEmit() {
        ...
        assertThat(harness.context.forwarded(), hasSize(1));
        final MockProcessorContext.CapturedForward<Windowed<String>, Change<Long>> capturedForward =
            harness.context.forwarded().get(0);
        assertThat(capturedForward.keyValue(), is(new KeyValue<>(key, value)));
        assertThat(capturedForward.timestamp(), is(timestamp));
    }
    ...
    public void intermediateSuppressionShouldBufferAndEmitLater() {
        ...
        harness.context.setRecordMetadata("topic", 0, 0, null, timestamp);
        final String key = "hey";
        final Change<Long> value = new Change<>(null, 1L);
        harness.processor.process(key, value);
        assertThat(harness.context.forwarded(), hasSize(0));

        harness.context.setRecordMetadata("topic", 0, 1, null, 1L);
        harness.processor.process("tick", new Change<>(null, null));

        assertThat(harness.context.forwarded(), hasSize(1));
        final MockProcessorContext.CapturedForward capturedForward = harness.context.forwarded().get(0);
        assertThat(capturedForward.keyValue(), is(new KeyValue<>(key, value)));
        assertThat(capturedForward.timestamp(), is(timestamp));
    }
    ...
    public void finalResultsSuppressionShouldBufferAndEmitAtGraceExpiration() {
        ...
        harness.context.setRecordMetadata("topic", 0, 0, null, recordTime);
        ...
        harness.context.setRecordMetadata("topic", 0, 1, null, recordTime2);
        ...
        harness.context.setRecordMetadata("topic", 0, 1, null, recordTime3);
    ...
    public void finalResultsWithZeroGraceShouldStillBufferUntilTheWindowEnd() {
        ...
        harness.context.setRecordMetadata("", 0, 1L, null, windowEnd);
    ...
    public void suppressShouldEmitWhenOverRecordCapacity() {
        ...
        harness.context.setRecordMetadata("", 0, 1L, null, timestamp + 1);
    ...
    public void suppressShouldShutDownWhenOverRecordCapacity() {
        ...
        final long timestamp = 100L;
        harness.context.setRecordMetadata("", 0, 0L, null, timestamp);
        harness.context.setCurrentNode(new ProcessorNode("testNode"));
        final String key = "hey";
        final Change<Long> value = new Change<>(null, ARBITRARY_LONG);
        harness.processor.process(key, value);

        harness.context.setRecordMetadata("", 0, 1L, null, timestamp);
        try {
            harness.processor.process("dummyKey", value);
