...
public class ConnectorOffsetBackingStore implements OffsetBackingStore {
    ...
    /**
     * If configured to use a connector-specific offset store, {@link OffsetBackingStore#start() stop} that store,
    ...
    public Future<Map<ByteBuffer, ByteBuffer>> get(Collection<ByteBuffer> keys) {
        ...
        return new Future<Map<ByteBuffer, ByteBuffer>>() {
            ...
            public Map<ByteBuffer, ByteBuffer> get(long timeout, TimeUnit unit) throws InterruptedException, ExecutionException, TimeoutException {
                long endTime = time.milliseconds() + unit.toMillis(timeout);
                Map<ByteBuffer, ByteBuffer> result = new HashMap<>(workerGetFuture.get(timeout, unit));
                timeout = Math.max(1, endTime - time.milliseconds());
                result.putAll(connectorGetFuture.get(timeout, TimeUnit.MILLISECONDS));
                return result;
            }
        };
    }
