...
class MetadataCacheTest {
  ...
  def getTopicMetadata() {
    ...
    val partitionStates = Map(
      new TopicPartition(topic0, 0) -> new UpdateMetadataRequest.PartitionState(controllerEpoch,
                                                                                0,
                                                                                0,
                                                                                asList(0, 1, 3),
                                                                                zkVersion,
    ...
    val updateMetadataRequest =
      new UpdateMetadataRequest.Builder(version, controllerId, controllerEpoch, partitionStates.asJava, brokers.asJava)
        .build()
    ...
    for (securityProtocol <- Seq(SecurityProtocol.PLAINTEXT, SecurityProtocol.SSL)) {
      ...
      def checkTopicMetadata(topic: String): Unit = {
        ...
        partitionMetadatas.zipWithIndex.foreach {
          case (partitionMetadata, partitionId) =>
            ...
            assertEquals(partitionState.basePartitionState.replicas,
  ...
  def getTopicMetadataPartitionLeaderNotAvailable(): Unit = {
    ...
    verifyTopicMetadataPartitionLeaderOrEndpointNotAvailable(brokers,
                                                             ...
                                                             errorUnavailableListeners = false)
    verifyTopicMetadataPartitionLeaderOrEndpointNotAvailable(brokers,
                                                             listenerName,
                                                             leader = 1,
                                                             Errors.LEADER_NOT_AVAILABLE,
                                                             errorUnavailableListeners = true)
  }
  ...
  def getTopicMetadataPartitionListenerNotAvailableOnLeader(): Unit = {
    ...
    val broker0Endpoints = Seq(new EndPoint("host0", 9092, SecurityProtocol.PLAINTEXT, plaintextListenerName),
  ...
  private def verifyTopicMetadataPartitionLeaderOrEndpointNotAvailable(brokers: Set[Broker],
    ...
    val topicMetadatas =
      cache.getTopicMetadata(Set(topic), listenerName, errorUnavailableListeners = errorUnavailableListeners)
    assertEquals(1, topicMetadatas.size)
  ...
  def getTopicMetadataWithNonSupportedSecurityProtocol() {
    ...
    val version = ApiKeys.UPDATE_METADATA.latestVersion
    val updateMetadataRequest =
      new UpdateMetadataRequest.Builder(version, 2, controllerEpoch, partitionStates.asJava, brokers.asJava).build()
    cache.updateCache(15, updateMetadataRequest)
