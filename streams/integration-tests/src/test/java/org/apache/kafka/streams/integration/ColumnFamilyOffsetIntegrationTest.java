 ...
 *
 * Companion to {@link ColumnFamilyOffsetRecoveryIntegrationTest} which covers failure/recovery.
 * These tests validate the happy path: clean starts, processing, clean stops, restarts,
 * and verification that column-family-based offset management correctly preserves state
 * across these cycles without unnecessary changelog restoration.
 *
 * <p>These tests confirm correct CF offset behavior indirectly by verifying the observable
 * effects. If offsets were not correctly written to the column family on clean shutdown and
 * read back on restart, Streams would detect an inconsistency between local state and the
 * changelog and fall back to a full changelog restore.
 *
 */
@Tag("integration")
@Timeout(600)
public class ColumnFamilyOffsetIntegrationTest {
    ...
    private void setProcessingGuarantee(final String guarantee) {
        ...
        // ALOS is the default — no config needed
    }
    ...
    public void shouldPreserveStateAcrossCleanRestart(final String processingGuarantee) throws Exception {
        ...
        // Clean stop — no cleanUp on restart
        closeStreams(streams);
        streams = null;
