...
class SimpleAclAuthorizer extends Authorizer with Logging {
  ...
  override def configure(javaConfigs: util.Map[String, _]) {
    ...
    val zkMaxInFlightRequests = configs
      .get(SimpleAclAuthorizer.ZkMaxInFlightRequests)
      .map(_.toString.toInt)
      .getOrElse(kafkaConfig.zkMaxInFlightRequests)

    val time = Time.SYSTEM
    zkClient = KafkaZkClient(zkUrl,
  ...
  private def aclMatch(operation: Operation,
                       resource: Resource,
    ...
    acls
      ...
      .exists { acl =>
        authorizerLogger.debug(
          s"operation = $operation on resource = $resource from host = $host is $permissionType based on acl = $acl"
        )
        true
      }
    ...
    if (acls != null && acls.nonEmpty) {
      if (!extendedAclSupport && resource.patternType == PatternType.PREFIXED) {
        throw new UnsupportedVersionException(
          s"Adding ACLs on prefixed resource patterns requires " +
  ...
  def getMatchingAcls(resourceType: ResourceType, resourceName: String): Set[Acl] =
    inReadLock(lock) {
      val wildcard = aclCache
        .get(Resource(resourceType, Acl.WildCardResource, PatternType.LITERAL))
        .map(_.acls)
        .getOrElse(Set.empty[Acl])

      val literal = aclCache
        .get(Resource(resourceType, resourceName, PatternType.LITERAL))
        .map(_.acls)
        .getOrElse(Set.empty[Acl])
  ...
  private def getAclsFromCache(resource: Resource): VersionedAcls =
    aclCache.getOrElse(resource,
                       throw new IllegalArgumentException(s"ACLs do not exist in the cache for resource $resource"))
