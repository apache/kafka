...
class KafkaController(val config: KafkaConfig, zkClient: KafkaZkClient, time: Time, metrics: Metrics, initialBrokerInfo: BrokerInfo,
  ...
  private def onBrokerStartup(newBrokers: Seq[Int]) {
    ...
    // Don't need to include any partition states for the UpdateMetadataRequest sent to existing brokers
    // since there is not partition state changes
    sendUpdateMetadataRequest(existingBrokers.toSeq)
    // Include full partition states for the UpdateMetadataRequest sent to new brokers for initlization
  ...
  private def onReplicasBecomeOffline(newOfflineReplicas: Set[PartitionAndReplica]): Unit = {
    ...
    if (partitionsWithoutLeader.isEmpty) {
      sendUpdateMetadataRequest(controllerContext.liveOrShuttingDownBrokerIds.toSeq,
        newOfflineReplicas.map(_.topicPartition).toSet)
    }
  }
