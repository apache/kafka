...
class ReplicaFetcherThread(name: String,
  ...
  private val logContext = new LogContext(
    s"[ReplicaFetcher replicaId=$replicaId, leaderId=${sourceBroker.id}, " +
      s"fetcherId=$fetcherId] "
  )
  this.logIdent = logContext.logPrefix

  private val leaderEndpoint = leaderEndpointBlockingSend.getOrElse(
    new ReplicaFetcherBlockingSend(sourceBroker,
                                   brokerConfig,
                                   metrics,
                                   time,
                                   fetcherId,
                                   s"broker-$replicaId-fetcher-$fetcherId",
                                   logContext)
  )
  ...
  def processPartitionData(topicPartition: TopicPartition, fetchOffset: Long, partitionData: PartitionData) {
    ...
    if (isTraceEnabled)
      trace(
        "Follower has replica log end offset %d for partition %s. Received %d messages and leader hw %d"
    ...
    if (isTraceEnabled)
      trace(
        "Follower has replica log end offset %d after appending %d bytes of messages for partition %s"
  ...
  def maybeWarnIfOversizedRecords(records: MemoryRecords, topicPartition: TopicPartition): Unit =
    ...
    if (fetchRequestVersion <= 2 && records.sizeInBytes > 0 && records.validBytes <= 0)
      error(
        s"Replication is failing due to a message that is greater than replica.fetch.max.bytes for partition $topicPartition. " +
  ...
  def handleOffsetOutOfRange(topicPartition: TopicPartition): Long = {
    ...
    if (leaderEndOffset < replica.logEndOffset.messageOffset) {
      ...
      if (!LogConfig
            .fromProps(brokerConfig.originals, adminZkClient.fetchEntityConfig(ConfigType.Topic, topicPartition.topic))
            .uncleanLeaderElectionEnable) {
        // Log a fatal error and shutdown the broker to ensure that data loss does not occur unexpectedly.
        fatal(
          s"Exiting because log truncation is not allowed for partition $topicPartition, current leader's " +
      ...
      warn(
        s"Reset fetch offset for partition $topicPartition from ${replica.logEndOffset.messageOffset} to current " +
  ...
  protected def fetch(fetchRequest: FetchRequest): Seq[(TopicPartition, PartitionData)] =
    try {
      ...
      if (!fetchSessionHandler.handleResponse(fetchResponse)) {
        ...
        fetchResponse.responseData.asScala.toSeq.map {
          case (key, value) =>
  ...
  override def buildFetchRequest(
      ...
      case (tp, leaderEpochOffset) =>
        try {
          ...
          if (leaderEpochOffset.hasError) {
            info(
              s"Retrying leaderEpoch request for partition ${replica.topicPartition} as the leader reported an error: ${leaderEpochOffset.error}"
            )
            ...
            if (offsetTruncationState.offset < replica.highWatermark.messageOffset)
              warn(
                s"Truncating $tp to offset ${offsetTruncationState.offset} below high watermark ${replica.highWatermark.messageOffset}"
              )
