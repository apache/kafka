...
class PlaintextConsumerTest extends BaseConsumerTest {
  ...
  @Test
  def testConsumerFetchesWholeLogAfterCompaction(): Unit = {
    val topic = "test_topic"
    val partition = 0;
    val tp = new TopicPartition(topic, partition)
    val props = new Properties()
    ...
    props.setProperty(LogConfig.CleanupPolicyProp, "compact")
    props.setProperty(LogConfig.MinCleanableDirtyRatioProp, "0.01")
    props.setProperty(LogConfig.SegmentBytesProp, "256")
    ...
    val producer = createProducer()
    // Each segment can contain three records
    producer.send(new ProducerRecord(topic, partition, "k1".getBytes, "0".getBytes)).get()
    producer.send(new ProducerRecord(topic, partition, "k1".getBytes, "1".getBytes)).get()
    producer.send(new ProducerRecord(topic, partition, "k1".getBytes, "2".getBytes)).get()
    
    producer.send(new ProducerRecord(topic, partition, "k1".getBytes, "3".getBytes)).get()
    producer.send(new ProducerRecord(topic, partition, "k2".getBytes, "4".getBytes)).get()
    producer.send(new ProducerRecord(topic, partition, "k2".getBytes, "5".getBytes)).get()

    producer.send(new ProducerRecord(topic, partition, "k2".getBytes, "6".getBytes)).get()
    producer.send(new ProducerRecord(topic, partition, "k2".getBytes, "7".getBytes)).get()
    producer.send(new ProducerRecord(topic, partition, "k2".getBytes, "8".getBytes)).get()

    producer.send(new ProducerRecord(topic, partition, "k3".getBytes, "9".getBytes)).get()
    producer.send(new ProducerRecord(topic, partition, "k4".getBytes, "10".getBytes)).get()
    producer.close()
