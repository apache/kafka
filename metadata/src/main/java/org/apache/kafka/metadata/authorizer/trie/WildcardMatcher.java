...
/**
 * The wildcard matching strategy.
 * <ol>
 ...
 *         <li>a leaf node</li>
 *         </ul></li>
 *     <li>Descent did not match a node with ACLs then unwind the path of descent checking each node
 *     for wildcard matches.  if a match is found return that node.</li>
 * </ol>
 * @param <T> the data type stored in the Trie
 */
public class WildcardMatcher<T> extends AbstractMatcher<T> {
     ...
     */
    public WildcardMatcher(String pattern, Predicate<NodeData<T>> exit) {
        super(pattern, exit);
    }

    private WildcardMatcher(WildcardMatcher<T> existing, int advance) {
        super(existing, advance);
    }

    @Override
    public WildcardMatcher<T> advance(int advance) {
        return new WildcardMatcher<>(this, advance);
    }
    ...
    public SearchResult<T> searchIn(final Node<T> node) {
        ...
        if (!test(node) && node.getChildren() != null) {
            ...
            SearchResult<T> result = WildcardRegistry.processWildcards(node, this);
            if (result.hasContents()) {
                return searchResult(candidate);
            }
        }
