...
class TransactionMarkerRequestCompletionHandler(brokerId: Int,
  ...
  override def onComplete(response: ClientResponse): Unit = {
    ...
    if (response.wasDisconnected) {
      ...
      for (txnIdAndMarker <- txnIdAndMarkerEntries.asScala) {
        ...
        txnStateManager.getTransactionState(transactionalId) match {
          ...
          case Left(Errors.NOT_COORDINATOR) =>
            info(
              s"I am no longer the coordinator for $transactionalId; cancel sending transaction markers $txnMarker to the brokers"
            )
          ...
          case Left(Errors.COORDINATOR_LOAD_IN_PROGRESS) =>
            info(
              s"I am loading the transaction partition that contains $transactionalId which means the current markers have to be obsoleted; " +
          ...
          case Right(None) =>
            throw new IllegalStateException(
              s"The coordinator still owns the transaction partition for $transactionalId, but there is " +
          ...
          case Right(Some(epochAndMetadata)) =>
            if (epochAndMetadata.coordinatorEpoch != txnMarker.coordinatorEpoch) {
              ...
              info(
                s"Transaction coordinator epoch for $transactionalId has changed from ${txnMarker.coordinatorEpoch} to " +
      ...
      debug(
        s"Received WriteTxnMarker response $response from node ${response.destination} with correlation id $correlationId"
      )
      ...
      for (txnIdAndMarker <- txnIdAndMarkerEntries.asScala) {
        ...
        if (errors == null)
          throw new IllegalStateException(
            s"WriteTxnMarkerResponse does not contain expected error map for producer id ${txnMarker.producerId}"
          )
              ...
              txnMetadata.inLock {
                for ((topicPartition, error) <- errors.asScala) {
                  error match {
                    ...
                    case Errors.CORRUPT_MESSAGE | Errors.MESSAGE_TOO_LARGE | Errors.RECORD_LIST_TOO_LARGE |
                        Errors.INVALID_REQUIRED_ACKS => // these are all unexpected and fatal errors

                      throw new IllegalStateException(
                        s"Received fatal error ${error.exceptionName} while sending txn marker for $transactionalId"
                      )
                    ...
                    case Errors.UNKNOWN_TOPIC_OR_PARTITION | Errors.NOT_LEADER_FOR_PARTITION |
                      ...
                      info(
                        s"Sending $transactionalId's transaction marker for partition $topicPartition has failed with error ${error.exceptionName}, retrying " +
                    ...
                    case Errors.INVALID_PRODUCER_EPOCH |
                      ...
                      info(
                        s"Sending $transactionalId's transaction marker for partition $topicPartition has permanently failed with error ${error.exceptionName} " +
                    ...
                    case Errors.UNSUPPORTED_FOR_MESSAGE_FORMAT | Errors.UNSUPPORTED_VERSION =>
                      ...
                      info(
                        s"Sending $transactionalId's transaction marker from partition $topicPartition has failed with " +
                    ...
                    case other =>
                      throw new IllegalStateException(
                        s"Unexpected error ${other.exceptionName} while sending txn marker for $transactionalId"
                      )
                  }
                }
              }
