...
public class CachingPerformanceBenchmarks {
    ...
    private static final byte[][] byteVals = new byte[HALF_MILLION][];
    private static final long[] longVals = new long[HALF_MILLION];
    ...
    private static Map<String, Long> putGetCacheTiming(int numberIteration, int cacheMaxSize, int numberInserts) {
        ...
        MemoryLRUCache.EldestEntryRemovalListener<Integer, String> intStringThrowingRemovalListener = getThrowingRemovalListener();
        MemoryLRUCache.EldestEntryRemovalListener<byte[], byte[]> bytesThrowingRemovalListener = getThrowingRemovalListener();
        ...
        for (int i = 0; i < numberIteration; i++) {
            ...
            memoryPutGetTiming(cacheMaxSize, numberInserts, intStringThrowingRemovalListener, timingResults);
    ...
    private static void serializedPutGetTiming(int cacheMaxSize, int numberInserts, MemoryLRUCache.EldestEntryRemovalListener<byte[], byte[]> bytesThrowingRemovalListener, Map<String, Long> timingResults) {
        long elapsed;
        long current;
        MemoryLRUCache<byte[], byte[]> bytesCache = getBytesCache(cacheMaxSize);
    ...
    private static void memoryPutGetTiming(int cacheMaxSize, int numberInserts, MemoryLRUCache.EldestEntryRemovalListener<Integer, String> intStringThrowingRemovalListener, Map<String, Long> timingResults) {
        long elapsed;
        long current;
        MemoryLRUCache<Integer, String> memoryTrackedCache = getIntKeyStringValueCache(cacheMaxSize * (INT_WRAPPER_BYTES + STRING_MESSAGE_BYTES + 16));
        ...
        current = timingResults.get(OBJ_MEMORY);
        timingResults.put(OBJ_MEMORY, current + elapsed);
    }

    private static void controlPutGetTiming(int cacheMaxSize, int numberInserts, MemoryLRUCache.EldestEntryRemovalListener<Integer, String> intStringThrowingRemovalListener, Map<String, Long> timingResults) {
        long elapsed;
        long current;
        MemoryLRUCache<Integer, String> sizeTrackedCache = getIntKeyStringValueCache(cacheMaxSize);
    ...
    private static long insertIntKeyStringValue(MemoryLRUCache<Integer, String> cache, int count, BlackHole<String> blackHole) {
    ...
    private static long insertBytes(MemoryLRUCache<byte[], byte[]> cache, int count, BlackHole<String> blackHole) {
    ...
    private static long complexMemoryDeepTrackingTiming() {
        ...
        for (int i = 0; i < HALF_MILLION; i++) {
            longVals[i] = memoryMeter.measureDeep(new ComplexObject());
        }
        long end = System.currentTimeMillis();
        return (end - start);
    }
    ...
    private static long simpleMemoryDeepTrackingTiming() {
        long start = System.currentTimeMillis();
        for (int i = 0; i < HALF_MILLION; i++) {
            longVals[i] = memoryMeter.measureDeep(STRING_MESSAGE + i);
        }
        long end = System.currentTimeMillis();
        return (end - start);
    }
    ...
    private static long complexMemoryTrackingTiming() {
        long start = System.currentTimeMillis();
        for (int i = 0; i < HALF_MILLION; i++) {
            longVals[i] = memoryMeter.measure(new ComplexObject());
        }
        long end = System.currentTimeMillis();
        return (end - start);
    }
    ...
    private static long simpleMemoryTrackingTiming() {
        long start = System.currentTimeMillis();
        for (int i = 0; i < HALF_MILLION; i++) {
            longVals[i] = memoryMeter.measure(STRING_MESSAGE + i);
        }
        long end = System.currentTimeMillis();
        return (end - start);
    }
    ...
    private static long complexSerializationTiming() throws Exception {
        long start = System.currentTimeMillis();
        for (int i = 0; i < HALF_MILLION; i++) {
            byte[] bytes = objectMapper.writeValueAsBytes(new ComplexObject());
            byteVals[i] = bytes;
        }
        long end = System.currentTimeMillis();
        return (end - start);
    ...
    private static MemoryLRUCache<byte[], byte[]> getBytesCache(int size) {
        return getCache("bytesCache", size, BYTES_SERDE, BYTES_SERDE);
    }

    private static MemoryLRUCache<Integer, String> getIntKeyStringValueCache(int size) {
        return getCache("objectCache", size, INTEGER_SERDE, STRING_SERDE);
    }

    private static <K, V> MemoryLRUCache<K, V> getCache(String name, int size, Serde<K> keySerde, Serde<V> valueSerde) {
        return new MemoryLRUCache<>(name, size, keySerde, valueSerde);
    }

    private static <K, V> MemoryLRUCache.EldestEntryRemovalListener<K, V> getThrowingRemovalListener() {
        return new MemoryLRUCache.EldestEntryRemovalListener<K, V>() {
            @Override
