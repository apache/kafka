...
object LogDirsCommand {
              ...
              "logDirs" -> logDirInfos.map {
                case (logDir, logDirInfo) =>
                  Map(
                    ...
                    "partitions" -> logDirInfo.replicaInfos.asScala
                      .filter {
  ...
  class LogDirsCommandOptions(args: Array[String]) {
    ...
    val bootstrapServerOpt = parser
      .accepts("bootstrap-server", "REQUIRED: the server(s) to use for bootstrapping")
      .withRequiredArg
    ...
    val brokerListOpt = parser
      .accepts("broker-list",
               "The list of brokers to be queried in the form \"0,1,2\". " +
