...
import org.apache.kafka.server.config.{ClientQuotaManagerConfig, ReplicationQuotaManagerConfig, ServerQuotaConfigs}
...
object QuotaFactory extends Logging {
  ...
  def instantiate(cfg: KafkaConfig, metrics: Metrics, time: Time, threadNamePrefix: String): QuotaManagers = {
    ...
    val clientQuotaCallback = Option(cfg.getConfiguredInstance(ServerQuotaConfigs.CLIENT_QUOTA_CALLBACK_CLASS_CONFIG,
