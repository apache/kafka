...
class AdminTest extends ZooKeeperTestHarness with Logging with RackAwareTest {
  ...
  def testPartitionReassignmentWithLeaderInNewReplicas() {
    ...
    assertFalse("/admin/reassignments/$topic/$partition should be deleted",
      zkClient.getReassignment(new TopicPartition(topicAndPartition.topic, topicAndPartition.partition)).isDefined)
