...
class TransactionMarkerChannelManagerTest {
  ...
  def shouldGenerateRequestPerPartitionPerBroker(): Unit = {
    ...
    val expectedBroker1Request = new WriteTxnMarkersRequest.Builder(
      util.List.of(new WriteTxnMarkersRequest.TxnMarkerEntry(producerId1, producerEpoch, coordinatorEpoch, txnResult, util.List.of(partition1), 0),
        new WriteTxnMarkersRequest.TxnMarkerEntry(producerId2, producerEpoch, coordinatorEpoch, txnResult, util.List.of(partition1), 0))).build()
    val expectedBroker2Request = new WriteTxnMarkersRequest.Builder(
      util.List.of(new WriteTxnMarkersRequest.TxnMarkerEntry(producerId1, producerEpoch, coordinatorEpoch, txnResult, util.List.of(partition2), 0))).build()
