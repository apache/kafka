...
class LogSegmentTest {
  ...
  def records(offset: Long, records: String*): MemoryRecords =
    MemoryRecords.withRecords(RecordBatch.MAGIC_VALUE_V1,
                              offset,
                              CompressionType.NONE,
                              TimestampType.CREATE_TIME,
                              records.map { s =>
                                new SimpleRecord(offset * 10, s.getBytes)
                              }: _*)
  ...
  def testMaxOffset() {
    ...
    def validate(offset: Long) =
      assertEquals(
        ms.records.asScala.filter(_.offset == offset).toList,
  ...
  def testTruncateEmptySegment() {
    ...
    assertFalse(
      reopened.shouldRoll(messagesSize = 1024,
  ...
  def testRecoverTransactionIndex(): Unit = {
    ...
    segment.append(
      ...
      MemoryRecords.withTransactionalRecords(100L,
                                             CompressionType.NONE,
                                             pid1,
                                             producerEpoch,
                                             sequence,
    ...
    segment.append(
      ...
      MemoryRecords.withTransactionalRecords(102L,
                                             CompressionType.NONE,
                                             pid2,
                                             producerEpoch,
                                             sequence,
    ...
    segment.append(
      largestOffset = 105L,
      largestTimestamp = RecordBatch.NO_TIMESTAMP,
      shallowOffsetOfMaxTimestamp = 104L,
      MemoryRecords.withRecords(104L,
                                CompressionType.NONE,
                                partitionLeaderEpoch,
                                new SimpleRecord("a".getBytes),
                                new SimpleRecord("b".getBytes))
    ...
    stateManager.loadProducerEntry(
      new ProducerStateEntry(pid2,
  ...
  private def createSegment(baseOffset: Long,
                            fileAlreadyExists: Boolean,
                            initFileSize: Int,
                            preallocate: Boolean): LogSegment = {
    ...
    val seg = LogSegment.open(tempDir,
                              baseOffset,
                              logConfig,
                              Time.SYSTEM,
  ...
  def testCreateWithInitFileSizeClearShutdown() {
    ...
    val segReopen = LogSegment.open(tempDir,
                                    baseOffset = 40,
                                    logConfig,
                                    Time.SYSTEM,
  ...
  def shouldTruncateEvenIfOffsetPointsToAGapInTheLog() {
    ...
    def records(offset: Long, record: String): MemoryRecords =
      MemoryRecords.withRecords(RecordBatch.MAGIC_VALUE_V2,
                                offset,
                                CompressionType.NONE,
                                TimestampType.CREATE_TIME,
