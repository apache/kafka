...
import org.apache.kafka.server.common.MetadataVersion.IBP_3_0_IV0
...
class ControllerRequestMerger(config: KafkaConfig) extends Logging {
  val liCombinedControlRequestVersion: Short =
    if (config.interBrokerProtocolVersion.isAtLeast(IBP_3_0_IV0)) 1
    else 0
    ...
    topicIds.forEach((topicName, topicId) => {
      ...
      if (existingTopicId != null && !existingTopicId.equals(topicId)) {
        ...
        def clearObsoleteEntriesInMap(map: mutable.Map[TopicIdPartition, _]) = {
          val obsoletePartitions = map.view.filterKeys{
  ...
  private def addLeaderAndIsrRequest(request: LeaderAndIsrRequest.Builder,
    ...
    if (request.build().data().isKRaftController() /* requestType approximation */.equals(LeaderAndIsrRequestType.FULL)) {
    ...
    request.partitionStates().forEach{partitionState => {
      val transformedPartitionState = LiCombinedControlTransformer.transformLeaderAndIsrPartition(partitionState, -1L)
  ...
  def pollLatestRequest(): LiCombinedControlRequest.Builder = {
    if (controllerState == null) {
      ...
      val latestRequest = new LiCombinedControlRequest.Builder(
        ...
        pollLatestStopReplicaPartitions(), java.util.Collections.unmodifiableMap(new java.util.HashMap(aggregateTopicIds))
      )
