...
See the [Processor API state store documentation](developer-guide/processor-api.html#headers-in-state-stores).
...
`TopologyTestDriver` and Interactive Queries support the new store types. The existing `store()` facades continue to return values (or `ValueAndTimestamp`) without exposing record headers. See the [interactive queries guide](developer-guide/interactive-queries.html#header-aware-stores-interactive-queries).
