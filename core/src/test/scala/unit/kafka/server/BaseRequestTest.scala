...
abstract class BaseRequestTest extends KafkaServerTestHarness {
  ...
  def controllerSocketServer =
    servers
      ...
      .map(_.socketServer)
      .getOrElse(throw new IllegalStateException("No controller broker is available"))
  ...
  def notControllerSocketServer =
    servers
      ...
      .map(_.socketServer)
      .getOrElse(throw new IllegalStateException("No non-controller broker is available"))
   ...
   */
  def connectAndSend(request: AbstractRequest,
                     apiKey: ApiKeys,
   ...
   * @return A ByteBuffer containing the response (without the response header).
   */
  def connectAndSendStruct(requestStruct: Struct,
                           apiKey: ApiKeys,
                           apiVersion: Short,
                           destination: SocketServer = anySocketServer,
                           protocol: SecurityProtocol = SecurityProtocol.PLAINTEXT): ByteBuffer = {
   ...
   */
  def requestResponse(socket: Socket,
                      clientId: String,
                      correlationId: Int,
