...
class LogCleanerLagIntegrationTest(compressionCodecName: String)
  ...
  def cleanerTest(): Unit = {
    ...
    assertTrue(
      s"log cleaner should have processed up to offset $firstBlock1SegmentBaseOffset, but lastCleaned=$lastCleaned",
      lastCleaned >= firstBlock1SegmentBaseOffset
    )
    assertTrue(
      s"log should have been compacted: size up to offset of active segment at T0=$sizeUpToActiveSegmentAtT0 compacted size=$compactedSize",
  ...
  private def writeDups(numKeys: Int,
                        ...
                        timestamp: Long): Seq[(Int, Int)] =
    for (_ <- 0 until numDups; key <- 0 until numKeys) yield {
      val count = counter
      log.appendAsLeader(TestUtils.singletonRecords(value = counter.toString.getBytes,
                                                    codec = codec,
