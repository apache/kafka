  ...
  def makeLeaderForPartition(zkUtils: ZkUtils,
    ...
    leaderPerPartitionMap.foreach {
      case (partition, leader) =>
        try{
          val newLeaderAndIsr = zkUtils.getLeaderAndIsrForPartition(topic, partition) match {
            case Some(current) => current.newLeader(leader)
            case None => LeaderAndIsr(leader, List(leader))
          }
