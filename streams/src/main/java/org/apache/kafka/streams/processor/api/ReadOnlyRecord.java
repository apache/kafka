...
public interface ReadOnlyRecord<K, V> {
     ...
     *
     * <p>The returned {@link Headers} should be treated as read-only. Records served as IQv2 results
     * from a state store have their headers frozen via {@code RecordHeaders.setReadOnly()}, so adding
     * or removing a header throws {@link IllegalStateException}. Freezing is not a deep copy, however:
     * the {@code value()} bytes of an existing header can still be mutated in place, so callers must
     * not modify them.
     */
    Headers headers();
}
