...
The checkstyle warnings will be found in `checkstyle/main.html` files inside the subproject directories.
They are also are printed to the console.  The build will fail if Checkstyle fails.
...
The findbugs warnings will be found in `findbugs/main.html` files inside the subproject directories.
Currently, findbugs warnings do not cause the build to fail.
