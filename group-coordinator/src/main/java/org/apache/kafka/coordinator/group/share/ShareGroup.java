 ...
 * A Share Group.
 */
public class ShareGroup implements Group {
   ...
   * The group members.
   */
  private final TimelineHashMap<String, ShareGroupMember> members;
  ...
  public void validateOffsetCommit(
    ...
    final ShareGroupMember member = getOrMaybeCreateMember(memberId, false);
  ...
  public void validateOffsetFetch(
    ...
    final ShareGroupMember member = members.get(memberId, lastCommittedOffset);
    if (member == null) {
      throw new UnknownMemberIdException(String.format("Member %s is not a member of group %s.",
              memberId, groupId));
    }
  ...
  private static void maybeUpdateSubscribedTopicNames(
    ...
    if (oldMember != null) {
      oldMember.subscribedTopicNames().forEach(topicName ->
              subscribedTopicCount.compute(topicName, ShareGroup::decValue)
      );
    }

    if (newMember != null) {
      newMember.subscribedTopicNames().forEach(topicName ->
              subscribedTopicCount.compute(topicName, ShareGroup::incValue)
      );
    }
  }
