...
class LeaderEpochIntegrationTest extends QuorumTestHarness with Logging {
  ...
  def shouldAddCurrentLeaderEpochToMessagesAsTheyAreWrittenToLeader(quorum: String): Unit = {
    brokers ++= (0 to 1).map { id => createBroker(createBrokerProps(id)) }
  ...
  def shouldSendLeaderEpochRequestAndGetAResponse(quorum: String): Unit = {
    ...
    brokers ++= (100 to 102).map { id => createBroker(createBrokerProps(id)) }
  ...
  def shouldIncreaseLeaderEpochBetweenLeaderRestarts(quorum: String): Unit = {
    ...
    brokers += createBroker(createBrokerProps(100))
    if (!isKRaftTest()) {
      assertEquals(100, TestUtils.waitUntilControllerElected(zkClient))
    }

    brokers += createBroker(createBrokerProps(101))
  ...
  def createBrokerProps(id: Int): KafkaConfig = {
    fromProps(createBrokerConfig(id, if (isKRaftTest()) null else zkConnect))
  }

  def createTopic(topic: String, partitionReplicaAssignment: collection.Map[Int, Seq[Int]]): Unit = {
    if (isKRaftTest()) {
      QuorumControllerIntegrationTestUtils.createTopic(
        controllerServer.controller.asInstanceOf[QuorumController],
        topic,
        partitionReplicaAssignment.map {
          case (key, value) => (Integer.valueOf(key), value.map(Integer.valueOf).asJava)
        }.asJava)
    } else {
